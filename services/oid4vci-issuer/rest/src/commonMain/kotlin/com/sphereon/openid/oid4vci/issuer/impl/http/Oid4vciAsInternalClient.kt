...
interface Oid4vciAsInternalClient {
    ...
    suspend fun introspectAccessToken(token: String): IdkResult<JsonObject, IdkError>
}
...
class LegacyBasicAuthOid4vciAsInternalClient(
    ...
    override suspend fun introspectAccessToken(token: String): IdkResult<JsonObject, IdkError> =
        try {
            val credentials = clientCredentials()
    ...
    private suspend fun clientCredentials(): AsBridgeClientCredentials {
        val reference = resolveAsBridgeClientCredentialReference(configService::getPropertyAsString)
        val clientSecret =
...
internal fun resolveAsBridgeClientCredentialReference(read: (String) -> String?): AsBridgeClientCredentialReference {
    val activeServerId = read("oauth2.servers.default-server")?.trim()?.takeIf { it.isNotEmpty() }
