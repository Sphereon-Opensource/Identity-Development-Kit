...
class OAuth2Handlers(
    ...
    suspend fun handleTokenRequest(
        ...
        val result =
            when (val params = tokenRequest.grantParameters) {
                                    ...
                                    ids.map { configId ->
                                        buildJsonObject {
                                            ...
                                            putJsonArray("credential_identifiers") {
                                                add(JsonPrimitive("$configId-${generateCredentialIdentifierSuffix()}"))
                                            }
                                        }
                                    },
                                )
     ...
     */
    suspend fun handleAuthorizationRequest(queryParameters: Map<String, String>): IdkResult<AuthorizationSession, IdkError> {
        // Parse authorization request
        val authRequest =
            commands.parseAuthorizationRequest
                .execute(
        ...
        val verified =
            commands.verifyAuthorizationRequest
                .execute(authRequest)
                .getOrElse { error -> return Err(error) }
    ...
    suspend fun handleAuthorizationApproval(
        ...
        val code =
            commands.createAuthorizationCode
                .execute(
                    CreateAuthorizationCodeArgs(session, userId, consent, userClaims, acr, amr),
                ).getOrElse { error -> return Err(error) }
    ...
    suspend fun handleAuthorizationError(
        ...
        commands.createAuthorizationErrorResponse.execute(
     ...
     */
    suspend fun handleIntrospectionRequest(requestBody: Map<String, List<String>>): IdkResult<TokenIntrospectionResponse, IdkError> {
     ...
     */
    suspend fun handleRevocationRequest(requestBody: Map<String, List<String>>): IdkResult<Unit, IdkError> {
    ...
    suspend fun handleDiscoveryRequest(
        ...
        commands.buildServerMetadata.execute(
     ...
     */
    suspend fun handleUserInfoRequest(accessToken: String): IdkResult<UserInfoResponse, IdkError> =
        commands.getUserInfo.execute(
     ...
     */
    suspend fun handleJwksRequest(): IdkResult<JwksResult, IdkError> = commands.getJwks.execute(GetJwksArgs())
     ...
     * 12 random bytes give 16 base64url characters (96 bits of entropy).
     */
    private fun generateCredentialIdentifierSuffix(): String {
        val bytes = Random.Default.nextBytes(12)
        return bytes
            .fold(StringBuilder()) { sb, b ->
