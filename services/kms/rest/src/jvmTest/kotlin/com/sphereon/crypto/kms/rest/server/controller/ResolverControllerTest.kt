...
class ResolverControllerTest {
        ...
        server =
            embeddedServer(CIO, port = port) {
                install(KotlinInjectPlugin) {
                    this.appGraph = this@ResolverControllerTest.appGraph
    ...
    fun listResolvers() =
        runTest {
            val response =
                client.get("http://localhost:$port/resolvers") {
    ...
    fun getResolver() =
        runTest {
            val response =
                client.get("http://localhost:$port/resolvers/jose_cose_resolver") {
                    header("X-Tenant-ID", TEST_TENANT_ID)
                    header("X-User-ID", TEST_USER_ID)
                    accept(ContentType.Application.Json)
                }
    ...
    fun resolveKey() =
        runTest {
            ...
            val response =
                client.post("http://localhost:$port/resolvers/jose_cose_resolver/resolve") {
                    header("X-Tenant-ID", TEST_TENANT_ID)
                    header("X-User-ID", TEST_USER_ID)
