...
class SignatureControllerTest {
    ...
    fun createRawSignature() =
        runTest {
            ...
            // When
            val response =
                client.post("http://localhost:$port/signatures/raw/create") {
    ...
    fun verifyRawSignature() =
        runTest {
            ...
            // When
            val response =
                client.post("http://localhost:$port/signatures/raw/verify") {
                    header("X-Tenant-ID", TEST_TENANT_ID)
                    header("X-User-ID", TEST_USER_ID)
                    contentType(ContentType.Application.Json)
                    setBody(payload)
    ...
    private fun createSignaturePayload(
        generated: GeneratedKey,
        input: ByteArray,
    ): String {
    ...
    private fun createVerifyPayload(
        generated: GeneratedKey,
        input: ByteArray,
        signature: String,
    ): String {
