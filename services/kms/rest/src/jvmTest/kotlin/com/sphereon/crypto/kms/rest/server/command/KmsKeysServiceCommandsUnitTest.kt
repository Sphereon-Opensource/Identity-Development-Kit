...
class KmsKeysServiceCommandsUnitTest {
    ...
    fun setUp() {
        ...
        // Get the facade and commands from session graph via ContributesTo interface
        val sessionGraph = session.graph as KmsKeysServiceFacadeImpl.Graph
        facade = sessionGraph.kmsKeysServiceFacade

        // Get individual commands via SessionScopedCommandRegistry lookup.
        // KmsCommandDescriptors registers them via @IntoMap @StringKey map multibinding.
    ...
    fun testGenerateKeyViaFacade() =
        runTest {
            ...
            assertTrue(
                result.isOk,
                "Generate key should succeed, but got: ${if (result.isErr) {
                    result.error
                } else {
                    "N/A"
                }}"
    ...
    fun testListKeysViaFacade() =
        runTest {
            ...
            assertTrue(
                listResult.isOk,
                "List keys should succeed, but got: ${if (listResult.isErr) {
                    listResult.error
                } else {
                    "N/A"
                }}"
    ...
    fun testGetKeyViaFacade() =
        runTest {
            ...
            assertTrue(
                getResult.isOk,
                "Get key should succeed, but got: ${if (getResult.isErr) {
                    getResult.error
                } else {
                    "N/A"
                }}"
    ...
    fun testDeleteKeyViaFacade() =
        runTest {
            ...
            assertTrue(
                deleteResult.isOk,
                "Delete key should succeed, but got: ${if (deleteResult.isErr) {
                    deleteResult.error
                } else {
                    "N/A"
                }}"
    ...
    @Test
    fun testListKeysServiceCommandDirectly() =
        runTest {
    ...
    @Test
    fun testGetKeyServiceCommandDirectly() =
        runTest {
    ...
    @Test
    fun testDeleteKeyServiceCommandDirectly() =
        runTest {
            // Generate a key first
