...
// ===========================================
// Auto-tunes workers.max and Kotlin daemon heap based on system RAM and CPU cores.
// Skipped when included by a parent composite build (parent handles tuning).
if (gradle.parent == null) run {
    val totalRamGb: Long = run detect@{
        try {
            ...
            return@detect (clazz.getMethod("getTotalMemorySize").invoke(bean) as Long) / (1024L * 1024 * 1024)
        } catch (_: Exception) { }
        try {
            val os = System.getProperty("os.name").lowercase()
            val process = when {
                "win" in os -> ProcessBuilder("cmd", "/c", "wmic", "ComputerSystem", "get", "TotalPhysicalMemory", "/value")
                "mac" in os -> ProcessBuilder("sysctl", "-n", "hw.memsize")
                else -> ProcessBuilder("bash", "-c", "awk '/MemTotal/ {print \$2 * 1024}' /proc/meminfo")
            }.redirectErrorStream(true).start()
            val output = process.inputStream.bufferedReader().readText().trim()
            process.waitFor(5, java.util.concurrent.TimeUnit.SECONDS)
            val bytes = output.lines().mapNotNull { it.replace(Regex("[^0-9]"), "").toLongOrNull() }.filter { it > 0 }.maxOrNull() ?: return@detect -1L
            return@detect bytes / (1024L * 1024 * 1024)
        } catch (_: Exception) { }
        -1L
    }
    val cpuCores = Runtime.getRuntime().availableProcessors()
    val gradleHeapMb = Runtime.getRuntime().maxMemory() / (1024L * 1024)
    val isCI = System.getenv("CI") != null

    // --- workers.max ---
    val hasExplicitWorkers = System.getProperty("org.gradle.workers.max") != null
        || providers.gradleProperty("org.gradle.workers.max").isPresent
    if (!hasExplicitWorkers && totalRamGb > 0) {
        // CI runners are dedicated — scale more aggressively than dev machines
        val workers = if (isCI) {
            cpuCores
        } else when {
            totalRamGb >= 96  -> minOf(cpuCores, 10)
            totalRamGb >= 64  -> minOf(cpuCores, 6)
            totalRamGb >= 48  -> minOf(cpuCores, 4)
            totalRamGb >= 32  -> minOf(cpuCores, 2)
            totalRamGb >= 24  -> minOf(cpuCores, 2)
            totalRamGb >= 16  -> minOf(cpuCores, 1)
            else              -> 1
        }
        gradle.startParameter.maxWorkerCount = workers
    }
    ...
    val hasExplicitKotlinDaemon = System.getProperty("kotlin.daemon.jvmargs") != null
        || providers.gradleProperty("kotlin.daemon.jvmargs").isPresent
    if (!hasExplicitKotlinDaemon && totalRamGb > 0) {
        val kotlinHeapMb = if (isCI) when {
            totalRamGb >= 64  -> 8192
            totalRamGb >= 32  -> 6144
            totalRamGb >= 16  -> 4096
            else              -> 2048
        ...
        val metaspaceMb = 1024
        val codeCacheMb = 320
        System.setProperty("kotlin.daemon.jvmargs", "-Xmx${kotlinHeapMb}M -XX:+UseZGC -XX:+ZGenerational -XX:MaxMetaspaceSize=${metaspaceMb}m -XX:ReservedCodeCacheSize=${codeCacheMb}m")
    }

    // --- kotlin.native.jvmArgs (Konan out-of-process linker heap) ---
    // Konan runs out-of-process (kotlin.native.toolExecutionInProcess=false) so parallel
    // linkDebug*Test tasks don't share the Gradle daemon heap. Each worker may spawn one
    // Konan JVM, so peak Konan heap = workers.max × this value.
    val hasExplicitKonan = System.getProperty("kotlin.native.jvmArgs") != null
        || providers.gradleProperty("kotlin.native.jvmArgs").isPresent
    if (!hasExplicitKonan && totalRamGb > 0) {
        val konanHeapGb = if (isCI) when {
            totalRamGb >= 64 -> 6
            totalRamGb >= 32 -> 4
            totalRamGb >= 16 -> 3
            else             -> 2
        ...
        System.setProperty("kotlin.native.jvmArgs", "-Xmx${konanHeapGb}G -XX:MaxMetaspaceSize=512m")
    }

    val workers = gradle.startParameter.maxWorkerCount
    val kotlinArgs = System.getProperty("kotlin.daemon.jvmargs") ?: "(from gradle.properties)"
    val konanArgs = System.getProperty("kotlin.native.jvmArgs") ?: "(from gradle.properties)"
    val mode = if (isCI) "CI" else "local"
    println("==> System: ${totalRamGb}GB RAM, ${cpuCores} cores | Gradle heap: ${gradleHeapMb}MB, workers: $workers ($mode)")
    println("==> Kotlin daemon: $kotlinArgs")
    println("==> Konan (native): $konanArgs")

    if (totalRamGb > 0) {
        val recommendedHeapMb = when {
            totalRamGb >= 96  -> 16384L
            totalRamGb >= 64  -> 12288L
            totalRamGb >= 48  -> 10240L
            totalRamGb >= 32  -> 8192L
            else              -> 6144L
        }
        if (gradleHeapMb < recommendedHeapMb) {
            println("==> TIP: Gradle heap is ${gradleHeapMb}MB but ${recommendedHeapMb}MB is recommended for ${totalRamGb}GB RAM.")
            println("    Add to ~/.gradle/gradle.properties:")
            println("    org.gradle.jvmargs=-Xmx${recommendedHeapMb}M -XX:+UseZGC -XX:+ZGenerational -XX:MaxMetaspaceSize=1536m -XX:+HeapDumpOnOutOfMemoryError -Dfile.encoding=UTF-8")
        }
    }
}
