...
internal actual class ComponentServiceResolver {
    ...
    actual fun <T : Any> findService(component: Any, serviceType: KClass<T>): T? {
        return try {
            ...
            val componentDynamic = component.asDynamic()

            // For wrapped instances (UserContextInstance, SessionInstance), access the inner component
            val actualComponent = when (component) {
                is UserContextInstance -> component.component.asDynamic()
                is SessionInstance -> component.component.asDynamic()
            ...
            for (propertyName in propertyNames) {
                try {
                    ...
                    val proto = js("Object.getPrototypeOf(actualComponent)")
                    ...
                    for (getterName in matchingGetters) {
                        try {
                            val getter = actualComponent[getterName]
                            if (getter != js("undefined") && js("typeof getter === 'function'") as Boolean) {
                                // Call the getter function
                                val value = getter.call(actualComponent)
