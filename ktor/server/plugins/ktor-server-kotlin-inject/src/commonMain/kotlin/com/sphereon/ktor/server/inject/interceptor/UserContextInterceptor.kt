 ...
 *
 * @property appComponent The root application component
...
class UserContextInterceptor(
    ...
    private val appLogger: LogService by lazy {
        (appComponent as CoreApiAppExtensionComponent).appLogManager.withTag(LOG_TAG)
    }
    ...
    suspend fun intercept(call: ApplicationCall) {
        try {
            ...
            val sessionInstance = contextInstance.sessionContextManager.createOrGetFromId(
                sessionId = sessionId,
                makeActive = false  // ID-based resolution, no global active state
            )

            // Store context in call attributes
            val requestContext = RequestScopedContext(
            ...
            appLogger.error("Error processing request in UserContextInterceptor: ${e.message}", e)
            throw e
        }
    }
}
