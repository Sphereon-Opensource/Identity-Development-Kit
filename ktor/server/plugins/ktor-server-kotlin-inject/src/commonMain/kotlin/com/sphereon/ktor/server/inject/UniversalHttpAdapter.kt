 ...
 *         appComponent = { createGraph<MyAppComponent>() }
 *         // ... other config
 *     }
 *     
 ...
 */
fun Application.installUniversalHttpAdapters(
    configure: UniversalHttpAdapterConfig.() -> Unit = {}
) {
    ...
    routing {
        ...
        route(config.pathPrefix ?: "{...}") {
            handle {
                try {
                    // Get the dispatcher from SessionScope
                    val dispatcher = call.getSessionService<HttpAdapterDispatcher>()

                    // Convert Ktor call to GenericHttpRequest
                    val genericRequest = call.toGenericHttpRequest()

                    if (config.verboseLogging) {
                        application.log.info("Dispatching: ${genericRequest.method} ${genericRequest.path}")
                    }
                    ...
                    if (handler != null) {
                        ...
                        application.log.error("Error in UniversalHttpAdapter dispatch", e)
                        call.respondText(
                            text = "Internal server error: ${e.message}",
 ...
 * ```
 */
fun Route.installUniversalHttpAdapters(
    configure: UniversalHttpAdapterConfig.() -> Unit = {}
) {
    val config = UniversalHttpAdapterConfig().apply(configure)
...
suspend fun ApplicationCall.respondWithGenericResponse(response: GenericHttpResponse) {
    ...
    val contentType = response.headers["Content-Type"]?.let {
