...
/**
 * Extension property to access the AppComponent from within a route.
 *
 * **Usage:**
 * ```kotlin
 * routing {
 ...
 * ```
 */
val ApplicationCall.appComponent: AppComponent
    get() = application.kotlinInject.appComponent
 ...
 * without manually navigating the component tree.
 *
 * **Usage:**
 * ```kotlin
 * routing {
...
inline fun <reified T : Any> ApplicationCall.getUserService(): T {
    ...
    val service = userInstance.scope.getService<T>(serviceType.simpleName ?: "Unknown")
        ?: findServiceInComponent(userInstance.component, serviceType)
...
inline fun <reified T : Any> ApplicationCall.getSessionService(): T {
    ...
    // Try to get from scope first
    val service = sessionInstance.scope.getService<T>(serviceType.simpleName ?: "Unknown")
        ?: findServiceInComponent(sessionInstance.component, serviceType)
        ?: throw IllegalStateException(
...
inline fun <reified T : Any> ApplicationCall.getAppService(): T {
    val serviceType = T::class
    val appComponent = this.appComponent

    // Get from scope - this is the proper way in GraalVM
    val service = appComponent.rootScopeProvider.rootScope.getService<T>(serviceType.simpleName ?: "Unknown")

    if (service != null) {
        return service
    }

    // Fallback: try component inspection (won't work in GraalVM native image without reflection config)
    return findServiceInComponent(appComponent, serviceType)
        ?: throw IllegalStateException(
...
/**
 * Helper function to find a service in a kotlin-inject component.
 *
 * Uses platform-specific resolution strategy:
 * - JVM: Full Kotlin reflection
 * - JS: Dynamic property access
 * - Native: Scope registry only (component inspection not available)
 *
 * This function searches the component for:
 * 1. Properties that return the service type
 * 2. Zero-parameter functions that return the service type (provider functions)
 */
@PublishedApi
internal fun <T : Any> findServiceInComponent(component: Any, serviceType: KClass<T>): T? {
    val resolver = ComponentServiceResolver()
    return resolver.findService(component, serviceType)
}
