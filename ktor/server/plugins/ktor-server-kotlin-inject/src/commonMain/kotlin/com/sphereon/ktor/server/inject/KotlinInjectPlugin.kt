 ...
 * Services are accessed via component inspection (properties and provider methods) using reflection.
 *
 * **Key Features:**
 * - Request-scoped context resolution (tenant + principal + session)
 * - Service access via extension functions using component inspection
 ...
 *         val appConfig = call.appComponent.appConfigEnvironment
 *
 ...
 */
class KotlinInjectPlugin(configuration: KotlinInjectConfiguration) {

    val appComponent: AppComponent = configuration.appComponent
        ?: throw IllegalStateException("AppComponent is required. Please provide it in the configuration.")
    ...
    // Get the app-scoped logger from the component
    private val logger: LogService by lazy {
        (appComponent as CoreApiAppExtensionComponent).appLogManager.withTag("KotlinInjectPlugin")
    }

    init {
        logger.info("Initializing KotlinInject plugin with AppComponent: ${appComponent::class.simpleName}")
        logger.info("Plugin is multiplatform compatible - services accessed via component inspection")
    }
    ...
    companion object Plugin : BaseApplicationPlugin<Application, KotlinInjectConfiguration, KotlinInjectPlugin> {
        ...
        override fun install(
            ...
            val interceptor = UserContextInterceptor(
                appComponent = plugin.appComponent,
