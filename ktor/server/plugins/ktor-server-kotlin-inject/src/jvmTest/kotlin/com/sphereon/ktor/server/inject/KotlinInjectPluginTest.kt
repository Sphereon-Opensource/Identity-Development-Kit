...
class KotlinInjectPluginTest {
    ...
    @Test
    fun `test plugin installation`() = testApplication {
    ...
    @Test
    fun `test request context resolution`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
            appId = "test-app",
    ...
    @Test
    fun `test custom resolvers`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
            appId = "test-app",
    ...
    @Test
    fun `test app component access`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
            appId = "test-app-123",
    ...
    @Test
    fun `test get app scoped service`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
            appId = "test-app",
    ...
    @Test
    fun `test get user scoped service`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
            appId = "test-app",
        ...
        val response = client.get("/user-log") {
            header("X-Tenant-ID", "test-tenant")
            header("X-User-ID", "test-user")
        }
    ...
    @Test
    fun `test get session scoped service`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
            appId = "test-app",
        ...
        // Make a request with tenant/user headers
        val response = client.get("/session-log") {
            header("X-Tenant-ID", "test-tenant")
            header("X-User-ID", "test-user")
        }
    ...
    @Test
    fun `test multiple scoped services in single request`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
        ...
        // Make a request with tenant/user headers
        val response = client.get("/multi-scope") {
    ...
    @Test
    fun `test service resolution across multiple requests`() = testApplication {
        // Create a test app component
        val appComponent = createTestAppComponent(
            application = Unit,
            appId = "test-app",
        ...
        val response1 = client.get("/check-user-context") {
        ...
        val response2 = client.get("/check-user-context") {
        ...
        val response3 = client.get("/check-user-context") {
            header("X-Tenant-ID", "tenant1")
            header("X-User-ID", "user1")
        }
    ...
    @Test
    fun `test routed http adapter directly`() = testApplication {
        ...
        fun normalizeRequest(request: GenericHttpRequest, serverPrefix: String): GenericHttpRequest {
        ...
        val postResponse = client.post("/api/test/items") {
    ...
    @Test
    fun `test routed http adapter with canHandle routing`() = testApplication {
        // Create an instance of the test HTTP adapter
        val testAdapter = TestHttpAdapter()
...
private class TestHttpAdapter : RoutedHttpAdapter() {
    ...
    override val routes: List<HttpRoute> = httpRoutes {
        get("/items") {
    ...
    private suspend fun handleListItems(request: GenericHttpRequest): GenericHttpResponse {
        return jsonResponse(200, """{"items": ["item1", "item2", "item3"]}""")
    }
