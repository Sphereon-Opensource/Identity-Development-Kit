    ...
    return GenericHttpRequest(
        ...
        queryParameters = LazyMap {
            ...
            request.queryParameters.entries().forEach { (key, values) ->
                queryMap[key] = if (values.size == 1) values[0] else values.joinToString(",")
            }
            queryMap
        },
...
suspend fun ApplicationCall.respondWithGeneric(response: GenericHttpResponse) {
    ...
    when (val bodyContent = response.bodyContent) {
        is GenericHttpBody.Bytes -> this.respondBytes(
        ...
        is GenericHttpBody.Text -> this.respondText(
        ...
        is GenericHttpBody.Empty -> this.respond(status)
    }
}
