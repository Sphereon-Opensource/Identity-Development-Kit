 ...
 * Direct component inspection is limited, so services must be properly registered
 * in the scope or accessed directly from component references.
 *
 * For Native targets, developers should:
 * 1. Ensure services are registered in scopes (AppScope, UserScope, SessionScope)
 * 2. Access services directly from component references when needed
...
internal actual class ComponentServiceResolver {
    ...
    actual fun <T : Any> findService(component: Any, serviceType: KClass<T>): T? {
        ...
        // Services must be accessed through the scope registry or direct component references
        ...
        // 2. Try component inspection (JVM/JS only)
