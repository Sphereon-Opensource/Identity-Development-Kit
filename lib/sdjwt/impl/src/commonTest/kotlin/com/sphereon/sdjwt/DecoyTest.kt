...
class DecoyTest {
    ...
    fun testDecoyModeNone() = runTest {
        ...
        val issueArgs = _root_ide_package_.com.sphereon.sdjwt.IssueSdJwtArgs(
            issuer = issuer,
            payload = payload,
            spec = _root_ide_package_.com.sphereon.sdjwt.SdJwtSpec(
                digestAlg = DigestAlg.SHA256,
                decoyConfig = _root_ide_package_.com.sphereon.sdjwt.DecoyConfig(mode = _root_ide_package_.com.sphereon.sdjwt.DecoyMode.NONE)
            )
        )
        ...
        _root_ide_package_.kotlin.test.assertEquals(3, sdJwtResult.disclosures.size, "Should have 3 disclosures")

        // Parse the JWT to check _sd array size
        val sdJwt = _root_ide_package_.com.sphereon.sdjwt.SdJwtCodec.parse(sdJwtResult.sdJwt)
        val sdArray = sdJwt.value.payload.undisclosedPayload["_sd"]?.jsonArray
     ...
     */
    @Test
    fun testDecoyModeFixed() = runTest {
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val issueArgs = _root_ide_package_.com.sphereon.sdjwt.IssueSdJwtArgs(
            ...
            spec = _root_ide_package_.com.sphereon.sdjwt.SdJwtSpec(
                digestAlg = DigestAlg.SHA256,
                decoyConfig = _root_ide_package_.com.sphereon.sdjwt.DecoyConfig(mode = _root_ide_package_.com.sphereon.sdjwt.DecoyMode.FIXED, count = decoyCount)
            )
        )
        ...
        _root_ide_package_.kotlin.test.assertEquals(2, sdJwtResult.disclosures.size, "Should have 2 actual disclosures")
     ...
     */
    @Test
    fun testDecoyModeMinimum() = runTest {
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val issueArgs1 = _root_ide_package_.com.sphereon.sdjwt.IssueSdJwtArgs(
            issuer = issuer,
            payload = payload1,
            spec = _root_ide_package_.com.sphereon.sdjwt.SdJwtSpec(
                digestAlg = DigestAlg.SHA256,
                decoyConfig = _root_ide_package_.com.sphereon.sdjwt.DecoyConfig(mode = _root_ide_package_.com.sphereon.sdjwt.DecoyMode.MINIMUM, count = minimumDigests)
            )
        )
        ...
        val sdJwt1 = _root_ide_package_.com.sphereon.sdjwt.SdJwtCodec.parse(sdJwtResult1.sdJwt)
        val sdArray1 = sdJwt1.value.payload.undisclosedPayload["_sd"]?.jsonArray
        ...
        val issueArgs2 = _root_ide_package_.com.sphereon.sdjwt.IssueSdJwtArgs(
            issuer = issuer,
            payload = payload2,
            spec = _root_ide_package_.com.sphereon.sdjwt.SdJwtSpec(
                digestAlg = DigestAlg.SHA256,
        ...
        _root_ide_package_.kotlin.test.assertEquals(12, sdJwtResult2.disclosures.size, "Should have 12 disclosures")

        val sdJwt2 = _root_ide_package_.com.sphereon.sdjwt.SdJwtCodec.parse(sdJwtResult2.sdJwt)
        val sdArray2 = sdJwt2.value.payload.undisclosedPayload["_sd"]?.jsonArray
     ...
     */
    @Test
    fun testDecoyModeRandom() = runTest {
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val issueArgs = _root_ide_package_.com.sphereon.sdjwt.IssueSdJwtArgs(
            ...
            spec = _root_ide_package_.com.sphereon.sdjwt.SdJwtSpec(
                digestAlg = DigestAlg.SHA256,
                decoyConfig = _root_ide_package_.com.sphereon.sdjwt.DecoyConfig(mode = _root_ide_package_.com.sphereon.sdjwt.DecoyMode.RANDOM, count = maxDecoys)
            )
        )
     ...
     */
    @Test
    fun testDecoyDigestsAreValid() = runTest {
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val issueArgs = _root_ide_package_.com.sphereon.sdjwt.IssueSdJwtArgs(
            ...
            spec = _root_ide_package_.com.sphereon.sdjwt.SdJwtSpec(
                digestAlg = DigestAlg.SHA256,
                decoyConfig = _root_ide_package_.com.sphereon.sdjwt.DecoyConfig(mode = _root_ide_package_.com.sphereon.sdjwt.DecoyMode.FIXED, count = 5)
            )
        )
        ...
        val sdJwt = _root_ide_package_.com.sphereon.sdjwt.SdJwtCodec.parse(issueResult.value.sdJwt)
        val sdArray = sdJwt.value.payload.undisclosedPayload["_sd"]?.jsonArray
     ...
     */
    @Test
    fun testDecoyDigestsDoNotMatchDisclosures() = runTest {
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val issueArgs = _root_ide_package_.com.sphereon.sdjwt.IssueSdJwtArgs(
            ...
            spec = _root_ide_package_.com.sphereon.sdjwt.SdJwtSpec(
                digestAlg = DigestAlg.SHA256,
                decoyConfig = _root_ide_package_.com.sphereon.sdjwt.DecoyConfig(mode = _root_ide_package_.com.sphereon.sdjwt.DecoyMode.FIXED, count = 3)
            )
        )
        ...
        val actualDigests = sdJwtResult.disclosures.map {
            _root_ide_package_.com.sphereon.sdjwt.DisclosureDigest.Companion.calculate(DigestAlg.SHA256, it).value
        }.toSet()

        // Count digests in _sd array
        val totalDigests = sdArray.size
        val matchingDigests = sdArray.count { digest ->
