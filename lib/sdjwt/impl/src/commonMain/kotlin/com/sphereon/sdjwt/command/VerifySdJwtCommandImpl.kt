...
import com.sphereon.crypto.jose.jws.command.VerifyJwsCommand as JwsVerifyCommand
...
class VerifySdJwtCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        try {
            ...
            log.error("Failed to verify SD-JWT: ${expected.message}", expected)
            return IdkResult.err(
                IdkError.fromString(
                    message = "Failed to verify SD-JWT: ${expected.message}",
                    exception = expected,
                ),
            )
        }
