...
internal class SdJwtVerifier(
    ...
    private fun verifyDisclosures(
        ...
        try {
            ...
            val expectedDigests =
                DisclosureDigestUtil.extractDigests(
            ...
            errorMessages.add("Disclosure verification failed: ${expected.message}")
            return false
        }
    }
    ...
    private suspend fun verifyKeyBinding(
        ...
        val sdJwtWithoutKb =
            fullSdJwtString.substringBeforeLast(SdJwt.SEPARATOR.toString()).let {
                if (it.endsWith(SdJwt.SEPARATOR)) {
                    it
                } else {
                    "$it${SdJwt.SEPARATOR}"
                }
        ...
        val kbSignatureValid =
            verifyKeyBindingJwtSignature(
                sdJwt = sdJwt,
                kbJwt = kbJwt,
                errorMessages = errorMessages,
            )
    ...
    private suspend fun verifyKeyBindingJwtSignature(
        ...
        try {
            ...
            errorMessages.add("Failed to verify KB-JWT signature: ${expected.message}")
            return false
        }
    }
    ...
    private suspend fun resolveHolderKeyFromCnf(
        ...
        val jwk =
            if (jwkElement is JsonObject) {
                try {
                    Jwk.fromJsonObject(jwkElement)
                } catch (expected: Exception) {
                    errorMessages.add("Cannot verify KB-JWT signature: failed to parse 'cnf.jwk': ${expected.message}")
                    null
                }
            } else {
                null
