...
class IssueSdJwtCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        try {
            ...
            // SD-JWT serialization: JWT~disclosure1~disclosure2~...~
            // Trailing ~ indicates no Key Binding JWT is attached (SD-JWT spec Section 5)
            val compactSdJwt =
                buildString {
                    append(signedJwt)
            ...
            log.error("Failed to issue SD-JWT: ${expected.message}", expected)
            return IdkResult.err(
                IdkError.fromString(
                    message = "Failed to issue SD-JWT: ${expected.message}",
                    exception = expected,
                ),
            )
        }
