...
class PresentSdJwtCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        try {
            ...
            val presentationWithoutKb = buildPresentationString(
            ...
            val kbJwt = if (appliedArgs.holderKey != null && appliedArgs.audience != null && appliedArgs.nonce != null) {
            ...
            val finalPresentation = if (kbJwt != null) {
            ...
            log.info("Created SD-JWT presentation with ${selectedDisclosures.size} disclosed claims${if (kbJwt != null) " and Key Binding" else ""}")
            ...
            log.error("Failed to create SD-JWT presentation: ${e.message}", e)
            return IdkResult.err(
                IdkError.fromString(
                    message = "Failed to create SD-JWT presentation: ${e.message}",
                    exception = e
                )
            )
        }
    ...
    private suspend fun createKeyBindingJwt(
        ...
        if (kbResult.isErr) {
            throw IllegalStateException("Failed to create Key Binding JWT: ${kbResult.error.message}")
        }
