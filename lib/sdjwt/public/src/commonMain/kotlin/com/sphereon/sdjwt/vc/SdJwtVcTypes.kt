...
sealed interface TypeMetadataResolutionResult {
    ...
    data class Success(val metadata: SdJwtVcTypeMetadata) : TypeMetadataResolutionResult

    /** Failed to resolve metadata */
    data class Failure(val error: TypeMetadataResolutionError) : TypeMetadataResolutionResult
}
...
sealed interface TypeMetadataResolutionError {
    ...
    data class NetworkError(val message: String, val cause: Throwable? = null) : TypeMetadataResolutionError

    /** Invalid metadata format */
    data class InvalidFormat(val message: String) : TypeMetadataResolutionError

    /** Metadata not found */
    data object NotFound : TypeMetadataResolutionError

    /** Unsupported VCT format */
    data class UnsupportedVct(val vct: String) : TypeMetadataResolutionError

    /** Circular dependency detected in type extends chain (draft-13 §7.2.3) */
    data class CircularDependency(val vct: String, val chain: List<String>) : TypeMetadataResolutionError
}
...
sealed interface IssuerMetadataResolutionResult {
    /** Successfully resolved metadata */
    data class Success(val metadata: SdJwtVcIssuerMetadata) : IssuerMetadataResolutionResult

    /** Failed to resolve metadata */
    data class Failure(val error: IssuerMetadataResolutionError) : IssuerMetadataResolutionResult
}
...
sealed interface IssuerMetadataResolutionError {
    /** Network or fetch error */
    data class NetworkError(val message: String, val cause: Throwable? = null) : IssuerMetadataResolutionError

    /** Invalid metadata format */
    data class InvalidFormat(val message: String) : IssuerMetadataResolutionError

    /** Metadata not found */
    data object NotFound : IssuerMetadataResolutionError

    /** Invalid issuer URL */
    data class InvalidIssuerUrl(val issuer: String) : IssuerMetadataResolutionError
}
