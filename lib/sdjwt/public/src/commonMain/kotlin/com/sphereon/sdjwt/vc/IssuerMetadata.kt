...
class WellKnownIssuerMetadataResolver(
        ...
        return try {
            ...
            if (!response.status.isSuccess()) {
                return when (response.status.value) {
                    404 -> IssuerMetadataResolutionResult.Failure(IssuerMetadataResolutionError.NotFound)
                    else -> IssuerMetadataResolutionResult.Failure(
            ...
            IssuerMetadataResolutionResult.Failure(
                IssuerMetadataResolutionError.NetworkError(
                    message = "Failed to resolve issuer metadata: ${e.message}",
                    cause = e
                )
            )
        }
    ...
    override fun supports(issuer: String): Boolean {
        return issuer.startsWith("https://", ignoreCase = true)
    }
...
class CachedIssuerMetadataResolver(
    ...
    override fun supports(issuer: String): Boolean {
        return delegate.supports(issuer)
    }
...
class CompositeIssuerMetadataResolver(
    ...
    override fun supports(issuer: String): Boolean {
        return resolvers.any { it.supports(issuer) }
    }
}
...
class StaticIssuerMetadataResolver(
    ...
    override fun supports(issuer: String): Boolean {
        return metadata.containsKey(issuer)
    }
}
