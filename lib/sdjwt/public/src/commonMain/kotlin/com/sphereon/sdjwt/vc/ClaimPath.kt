...
sealed interface ClaimPathLookupResult {
    /** Claim found at path */
    data class Found(val value: JsonElement) : ClaimPathLookupResult
    ...
    data class Error(val message: String) : ClaimPathLookupResult
}
...
object ClaimPathUtils {
     ...
     */
    fun lookup(json: JsonObject, path: ClaimPath): ClaimPathLookupResult {
        if (path.isEmpty) {
     ...
     */
    fun lookupAll(json: JsonObject, path: ClaimPath): List<JsonElement> {
        if (path.isEmpty) {
            return listOf(json)
        }
     ...
     */
    fun exists(json: JsonObject, path: ClaimPath): Boolean {
        return lookup(json, path) is ClaimPathLookupResult.Found
    }
     ...
     */
    fun extractPaths(json: JsonObject, includeArrayIndices: Boolean = false): List<ClaimPath> {
    ...
    private fun lookupRecursive(element: JsonElement, path: ClaimPath): ClaimPathLookupResult {
        if (path.isEmpty) {
        ...
        return when (element) {
            ...
            else -> ClaimPathLookupResult.Error("Cannot traverse path through primitive value")
        }
    }

    private fun lookupAllRecursive(element: JsonElement, path: ClaimPath): List<JsonElement> {
        if (path.isEmpty) {
