...
class AndroidUserContextMobileLogService(
    ...
    repository: MobileLogRepository,
) : AbstractAndroidMobileLogService(userContextInstance.toSessionContext(), SERVICE_ID, repository) {
...
abstract class AbstractAndroidMobileLogService(
    ...
    private val logLevelToPriority = mapOf(
    ...
    private fun logToAndroid(
        ...
        val baseMessage = buildString(message.message.length + 100) {
        ...
        if (baseMessage.length <= MAX_LOG_LENGTH) {
            ...
            when (message.level) {
                LogLevel.OFF -> return // No logging
            ...
            chunks.forEachIndexed { index, chunk ->
                val chunkMessage = buildString(chunk.length + 20) {
                    append(chunkPrefix)
                    append(index + 1)
...
object AndroidMobileLogUtils {
     ...
     */
    fun getAndroidLogPriority(level: LogLevel): Int = when (level) {
    ...
    fun isLoggable(tag: String, level: LogLevel): Boolean {
        return Log.isLoggable(tag, getAndroidLogPriority(level))
    }
}
