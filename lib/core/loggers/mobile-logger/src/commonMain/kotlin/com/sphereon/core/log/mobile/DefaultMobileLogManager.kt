...
class MobileLogManagerImpl(
    ...
    override suspend fun getLogsByLevel(level: LogLevel): List<MobileLogEntry> = repository.getLogsByLevel(level)

    override suspend fun getLogsByTag(tag: String): List<MobileLogEntry> = repository.getLogsByTag(tag)

    override suspend fun getLogsByTimeRange(
        from: Instant,
        to: Instant,
    ): List<MobileLogEntry> = repository.getLogsByTimeRange(from, to)
    ...
    override suspend fun getRecentLogs(count: Int): List<MobileLogEntry> = repository.getAllLogs().takeLast(count)

    override suspend fun getErrorLogs(): List<MobileLogEntry> = getLogsByLevel(LogLevel.ERROR)

    override suspend fun getWarningAndErrorLogs(): List<MobileLogEntry> =
        repository.getAllLogs().filter {
            it.level == LogLevel.WARN || it.level == LogLevel.ERROR
        }

    override suspend fun searchLogs(query: String): List<MobileLogEntry> = getFilteredLogs(MobileLogFilter(searchText = query))
        ...
        buildString {
            ...
            logs.forEach { log ->
                ...
                if (options.includeTimestamps) {
                    parts.add(log.timestamp.toString())
                }
                if (options.includeLevel) {
                    parts.add(log.level.toString())
                }
    ...
    private fun exportAsCsv(
        ...
        buildString {
            ...
            logs.forEach { log ->
                ...
                if (options.includeMetadata) {
                    values.add("\"${log.metadata?.entries?.joinToString("; ") { "${it.key}=${it.value}" } ?: ""}\"")
                }
                if (options.includeContext) {
                    values.add("\"${log.sessionContextId ?: ""}\"")
                }
                if (options.includeStackTrace) {
                    values.add("\"${log.throwable?.replace("\"", "\"\"") ?: ""}\"")
                }
