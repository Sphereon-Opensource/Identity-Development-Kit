...
object ExclusiveC14N {
    ...
    fun canonicalizeSubtree(
        ...
        return when (node) {
            is Document -> {
                val docElem = node.getDocumentElement()
                    ?: return ByteArray(0)
                canonicalize(docElem, inclusiveNamespacePrefixes, excludeNode)
            }
            is Element -> canonicalize(node, inclusiveNamespacePrefixes, excludeNode)
            else -> {
    ...
    private fun processElement(
        ...
        for (inclPrefix in inclusivePrefixes) {
            val p = if (inclPrefix == "#default") "" else inclPrefix
        ...
        val qName = if (elemPrefix.isNotEmpty()) "$elemPrefix:${element.getLocalName()}" else element.getLocalName()
        ...
        for ((prefix, uri) in nsToOutput) {
            if (prefix.isEmpty()) {
                ...
                output.append(" xmlns:").append(prefix).append("=\"")
                    .append(escapeAttributeValue(uri)).append('"')
            }
        }
        ...
        for (attr in regularAttrs) {
            val attrPrefix = attr.getPrefix()
            val attrQName = if (!attrPrefix.isNullOrEmpty()) {
                "$attrPrefix:${attr.getLocalName()}"
            } else {
                attr.getLocalName() ?: attr.getName()
            }
            output.append(' ').append(attrQName).append("=\"")
                .append(escapeAttributeValue(attr.getValue())).append('"')
        }
        ...
        for (child in element.getChildNodes()) {
            if (excludeNode != null && isSameNode(child, excludeNode)) continue
            when (child.nodetype) {
                NodeType.ELEMENT_NODE -> {
                    processElement(
                        child as Element, output, childNs, inclusivePrefixes, excludeNode
                    )
                }
     ...
     */
    private fun isSameNode(a: Node, b: Node): Boolean {
        if (a === b) return true
        if (a == b) return true
