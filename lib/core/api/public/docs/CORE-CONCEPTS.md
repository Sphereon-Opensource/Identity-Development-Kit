...
- **Unprefixed keys**: Property keys are used directly without `sphereon.app.` prefix
...
```yaml
# application.yml - Spring Boot style
sphereon:
  app:
    database:
      host: localhost
...
1. First tries `production.sphereon.app.database.host`
2. Falls back to `sphereon.app.database.host`
