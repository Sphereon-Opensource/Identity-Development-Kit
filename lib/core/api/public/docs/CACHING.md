...
Use `CacheService` for high-level cache operations with `IdkResult`-wrapped error handling.  
Use `ScopedCache` directly (via `CacheManager`) for low-level typed cache access with scope partitioning.
