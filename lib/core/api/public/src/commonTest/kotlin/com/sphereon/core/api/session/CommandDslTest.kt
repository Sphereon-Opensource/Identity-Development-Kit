...
class CommandDslTest {
    ...
    @Test
    fun commandDslCreatesBasicCommand() = runTest {
        val cmd = command<String, Int>("test.dsl.string.length") {
    ...
    @Test
    fun commandDslWithCustomSubsystem() = runTest {
        val cmd = command<String, String>("test.dsl.crypto.encrypt") {
    ...
    @Test
    fun commandDslWithCustomSupports() = runTest {
        val cmd = command<String, String>("test.dsl.string.uppercase") {
    ...
    @Test
    fun commandDslWithEnabledFalse() = runTest {
        val cmd = command<String, String>("test.dsl.string.disabled") {
            enabled(false)
            execute { str -> Ok(str) }
        }
    ...
    @Test
    fun commandDslReturnsError() = runTest {
        val cmd = command<Int, Int>("test.dsl.math.divide") {
            execute { n ->
                if (n == 0) {
        ...
        assertTrue(errResult.error.message.defaultMessage.contains("Division by zero"))
    }

    @Test
    fun commandDslWithSessionContextAccess() = runTest {
        val cmd = command<String, String>("test.dsl.session.info") {
            execute { str ->
                Ok("$str from command")
            }
        }
    ...
    @Test
    fun commandDslWithAllOptions() = runTest {
        val cmd = command<String, Int>("test.dsl.string.full") {
    ...
    @Test
    fun commandDslSupportsWithContextDelegatesToContextFreePrimary() = runTest {
        val cmd = command<String, String>("test.dsl.legacy.supports.bridge") {
            supports { args ->
...
class SimpleCommandDslTest {
    ...
    @Test
    fun simpleCommandDslCreatesBasicCommand() = runTest {
        val cmd = simpleCommandDsl<String, Int>("test.simple.dsl.length") {
            execute { str -> Ok(str.length) }
        }
    ...
    @Test
    fun simpleCommandDslWithCustomSupports() = runTest {
        val cmd = simpleCommandDsl<String, String>("test.simple.dsl.upper") {
            supports { args -> args is String && args.isNotBlank() }
            execute { str -> Ok(str.uppercase()) }
        }
    ...
    @Test
    fun simpleCommandDslReturnsError() = runTest {
        val cmd = simpleCommandDsl<String, Int>("test.simple.dsl.parse") {
            execute { str ->
                str.toIntOrNull()
                    ?.let { Ok(it) }
    ...
    @Test
    fun simpleCommandDslConvertsToFullCommand() = runTest {
        val simple = simpleCommandDsl<String, Int>("test.simple.dsl.convert") {
            execute { str -> Ok(str.length) }
        }
    ...
    @Test
    fun simpleCommandAsCommandSupportsWithContextDelegatesToContextFreePrimary() = runTest {
        val simple = simpleCommandDsl<String, String>("test.simple.dsl.supports.delegate") {
    ...
    @Test
    fun simpleCommandAsCommandWithErrorSupportsWithContextDelegatesToContextFreePrimary() = runTest {
        val simple = simpleCommandDsl<String, String>("test.simple.dsl.supports.error.delegate") {
