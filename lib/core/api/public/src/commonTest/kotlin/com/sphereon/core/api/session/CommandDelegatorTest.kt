...
class CommandDelegatorTest {
    ...
    @Test
    fun invokeCallsCommandWithSessionContext() = runTest {
    ...
    @Test
    fun invokeReturnsErrorResult() = runTest {
    ...
    @Test
    fun invokePropagatesCommandNotAuthorizedError() = runTest {
        val error = IdkError.COMMAND_NOT_AUTHORIZED_ERROR(
    ...
    @Test
    fun invokePropagatesNotFoundError() = runTest {
    ...
    @Test
    fun invokeOrThrowReturnsValueOnSuccess() = runTest {
        val command = TestCommand(IdkResult.ok("success-value"))
        val execution = TestSessionExecution()
        val delegator = TestCommandDelegator(execution)

        with(delegator) {
            val value = command.invokeOrThrow("args")
            assertEquals("success-value", value)
        }
    }

    @Test
    fun invokeOrThrowThrowsOnError() = runTest {
        val error = IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Throw test")
        val command = TestCommand(IdkResult.err(error))
        val execution = TestSessionExecution()
        val delegator = TestCommandDelegator(execution)
