...
class HttpAdapterCatalogCollisionTest {
    ...
    fun collisionHasType() {
        val collision = HttpAdapterCatalogCollision(
    ...
    fun collisionsAreEqual() {
        val collision1 = HttpAdapterCatalogCollision(
            type = HttpAdapterCatalogCollisionType.DUPLICATE_ADAPTER_ID,
            message = "test"
        )
        val collision2 = HttpAdapterCatalogCollision(
            type = HttpAdapterCatalogCollisionType.DUPLICATE_ADAPTER_ID,
            message = "test"
        )
...
class HttpAdapterCatalogDiagnosticsTest {
    ...
    fun duplicateAdapterIdsDetected() {
        ...
        val duplicateIdCollisions = diagnostics.collisions.filter {
    ...
    fun overlappingEndpointsDetected() {
        ...
        val desc1 = HttpAdapterDescription(
            id = "adapter-1",
            mount = mount,
            endpoints = listOf(endpoint)
        )
        val desc2 = HttpAdapterDescription(
            id = "adapter-2",
            mount = mount,
            endpoints = listOf(endpoint)
        )
        ...
        val overlappingCollisions = diagnostics.collisions.filter {
    ...
    fun duplicateEndpointInSameAdapterDetected() {
        ...
        val duplicateInAdapterCollisions = diagnostics.collisions.filter {
    ...
    fun differentEndpointsNoCollisions() {
        val endpoint1 = HttpEndpointDescriptor(
            method = HttpMethod.GET,
            pathPattern = "/users/{id}"
        )
        val endpoint2 = HttpEndpointDescriptor(
    ...
    private fun createTestDescription(id: String, basePath: String): HttpAdapterDescription {
        val endpoint = HttpEndpointDescriptor(
            method = HttpMethod.GET,
            pathPattern = "/test"
        )
