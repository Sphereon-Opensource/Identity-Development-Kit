...
class UrlEncodingTest {
    ...
    @Test
    fun encodeUrlComponentEncodesSpace() {
        assertEquals("Hello%20World", "Hello World".encodeUrlComponent())
    }

    @Test
    fun encodeUrlComponentEncodesSpecialChars() {
        assertTrue("Hello!".encodeUrlComponent().contains("%21"))
        assertTrue("a@b".encodeUrlComponent().contains("%40"))
        assertTrue("a#b".encodeUrlComponent().contains("%23"))
    }

    @Test
    fun encodeUrlComponentEncodesPercentFirst() {
        val result = "100%".encodeUrlComponent()
        assertTrue(result.contains("%25"))
        assertTrue(!result.contains("%%"))
    }

    @Test
    fun decodeUrlComponentDecodesSpace() {
        assertEquals("Hello World", "Hello%20World".decodeUrlComponent())
    }

    @Test
    fun decodeUrlComponentDecodesPlusAsSpace() {
        assertEquals("Hello World", "Hello+World".decodeUrlComponent())
    }

    @Test
    fun decodeUrlComponentDecodesSpecialChars() {
        assertEquals("Hello!", "Hello%21".decodeUrlComponent())
        assertEquals("a@b", "a%40b".decodeUrlComponent())
        assertEquals("a#b", "a%23b".decodeUrlComponent())
    }

    @Test
    fun urlEncodingRoundTrips() {
        val original = "Hello World! Test @#\$&*()"
        val encoded = original.encodeUrlComponent()
        val decoded = encoded.decodeUrlComponent()
...
class Base64SerializerTest {
    ...
    fun serializeRoundTrips() {
        ...
        val json = kotlinx.serialization.json.Json.encodeToString(Base64Serializer, original)
        val decoded = kotlinx.serialization.json.Json.decodeFromString(Base64Serializer, json)
        assertEquals(original.toList(), decoded.toList())
    }
    ...
    fun serializeProducesBase64() {
        val bytes = "Test".encodeToByteArray()
        val json = kotlinx.serialization.json.Json.encodeToString(Base64Serializer, bytes)
...
class Base64UrlSerializerTest {
    ...
    fun serializeRoundTrips() {
        val original = "Hello, World!".encodeToByteArray()
        val json = kotlinx.serialization.json.Json.encodeToString(Base64UrlSerializer, original)
        val decoded = kotlinx.serialization.json.Json.decodeFromString(Base64UrlSerializer, json)
        assertEquals(original.toList(), decoded.toList())
    }
    ...
    fun serializeUsesUrlSafeAlphabet() {
        val bytes = byteArrayOf(0xFB.toByte(), 0xFF.toByte(), 0xFE.toByte())
        val json = kotlinx.serialization.json.Json.encodeToString(Base64UrlSerializer, bytes)
...
class Base58BtcSerializerTest {
    ...
    fun serializeRoundTrips() {
        val original = "Hello, World!".encodeToByteArray()
        val json = kotlinx.serialization.json.Json.encodeToString(Base58BtcSerializer, original)
        val decoded = kotlinx.serialization.json.Json.decodeFromString(Base58BtcSerializer, json)
        assertEquals(original.toList(), decoded.toList())
    }
    ...
    fun serializeProducesBase58Btc() {
        val bytes = "Test".encodeToByteArray()
        val json = kotlinx.serialization.json.Json.encodeToString(Base58BtcSerializer, bytes)
...
class InstantIso8601SerializerTest {
    ...
    fun serializeRoundTrips() {
        ...
        val json = kotlinx.serialization.json.Json.encodeToString(InstantIso8601Serializer, original)
        val decoded = kotlinx.serialization.json.Json.decodeFromString(InstantIso8601Serializer, json)
        assertEquals(original, decoded)
    }

    @Test
    fun serializeProducesIso8601() {
        val instant = kotlinx.datetime.Instant.parse("2024-01-15T10:30:00Z")
        val json = kotlinx.serialization.json.Json.encodeToString(InstantIso8601Serializer, instant)
