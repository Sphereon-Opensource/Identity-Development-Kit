...
class ChainedCommandTest {
    ...
    @Test
    fun chainedCommandExecutesBothCommandsInSequence() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.string.parse") { input ->
            Ok(input.length)
        }

        val doubleCommand = testCommand<Int, Int>("test.chain.int.double") { input ->
            Ok(input * 2)
        }

        val chained = ChainedCommand(
    ...
    @Test
    fun chainedCommandReturnsErrorWhenFirstCommandFails() = runTest {
        val failingCommand = testCommand<String, Int>("test.chain.string.fail") { _ ->
        ...
        assertTrue(result.error.message.defaultMessage.contains("First command failed"))
    }

    @Test
    fun chainedCommandReturnsErrorWhenSecondCommandDoesNotSupportIntermediate() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.string.parse") { input ->
            Ok(input.length)
        }

        val selectiveCommand = testCommand<Int, Int>(
    ...
    @Test
    fun chainedCommandPropagatesAuthorizationFailureFromFirstCommand() = runTest {
        var secondExecuted = false
        val unauthorizedCommand = testCommand<String, Int>("test.chain.string.authz") { _ ->
            Err(
        ...
        val secondCommand = testCommand<Int, Int>("test.chain.int.next") { input ->
            secondExecuted = true
            Ok(input * 2)
        }
        val chained = ChainedCommand(
    ...
    @Test
    fun chainedCommandReturnsErrorWhenSecondCommandFails() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.string.parse") { input ->
            Ok(input.length)
        }

        val failingCommand = testCommand<Int, Int>("test.chain.int.fail") { _ ->
        ...
        val result = chained.execute("hello")
        assertTrue(result.isErr)
        assertTrue(result.error.message.defaultMessage.contains("Second command failed"))
    }

    @Test
    fun chainedCommandUsesContextFreePrimaryForSecondSupports() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.legacy.string.parse") { input ->
            Ok(input.length)
        }

        val legacySecond = object : Command<Int, Int, IdkError> {
            ...
            override suspend fun execute(args: Int): IdkResult<Int, IdkError> =
                Ok(args * 2)
        }
    ...
    @Test
    fun chainedCommandSupportsReturnsFirstCommandSupports() = runTest {
        val selectiveFirst = testCommand<String, Int>(
    ...
    @Test
    fun chainExtensionFunctionCreatesChainedCommand() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.string.parse") { input ->
            Ok(input.length)
        }
    ...
    @Test
    fun chainAllWithSingleCommandWrapsIt() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.string.parse") { input ->
            Ok(input.length)
        }

        val chained = chainAll<String, Int, IdkError>(
    ...
    @Test
    fun chainAllWithMultipleCommandsChainsAll() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.string.parse") { input ->
            Ok(input.length)
        }
        ...
        val addTenCommand = testCommand<Int, Int>("test.chain.int.add") { input ->
            Ok(input + 10)
        }
    ...
    @Test
    fun chainAllSupportsForwardsToFirstCommand() = runTest {
        val selectiveFirst = testCommand<String, Int>(
            commandId = "test.chain.string.selective",
    ...
    @Test
    fun chainedCommandCanBeChainedFurther() = runTest {
        val parseCommand = testCommand<String, Int>("test.chain.string.parse") { input ->
            Ok(input.length)
        }
        ...
        val toStringCommand = testCommand<Int, String>("test.chain.int.string") { input ->
            Ok("Result: $input")
        }

        // Chain three commands
        val chained = parseCommand
