...
class PermissivePolicyProviderTest {
    ...
    @Test
    fun permissivePolicyProviderAlwaysAllows() = runTest {
        val context = PolicyContext(
            actorId = "user-123",
    ...
    @Test
    fun permissivePolicyProviderAllowsAnyCommand() = runTest {
        val commands = listOf(
...
class DenyAllPolicyProviderTest {
    ...
    @Test
    fun denyAllPolicyProviderAlwaysDenies() = runTest {
        val context = PolicyContext(
            actorId = "user-123",
            commandId = "any.command.type-action"
        )
    ...
    @Test
    fun denyAllPolicyProviderDeniesAllCommands() = runTest {
        val commands = listOf(
            "core.session.session-create",
            "public.api.api-read",
...
class PolicyAuthorizerTest {
    ...
    @Test
    fun policyAuthorizerAllowsWhenPolicyAllows() = runTest {
        val authorizer = PolicyAuthorizer(PermissivePolicyProvider)
    ...
    @Test
    fun policyAuthorizerDeniesWhenPolicyDenies() = runTest {
        val authorizer = PolicyAuthorizer(DenyAllPolicyProvider)
    ...
    @Test
    fun policyAuthorizerPassesErrorFromPolicy() = runTest {
        val errorPolicy = object : PolicyDecisionProvider {
            override suspend fun isAllowed(context: PolicyContext): IdkResult<Boolean, IdkError> =
                Err(IdkError.UNKNOWN_ERROR(message = "Policy evaluation failed"))
        }

        val authorizer = PolicyAuthorizer(errorPolicy)
        ...
        assertTrue(result.isErr)
        assertTrue(result.error.message.defaultMessage.contains("Policy evaluation failed"))
    }

    @Test
    fun policyAuthorizerUsesCustomContextBuilder() = runTest {
        var capturedContext: PolicyContext? = null
        val capturingPolicy = object : PolicyDecisionProvider {
            override suspend fun isAllowed(context: PolicyContext): IdkResult<Boolean, IdkError> {
        ...
        assertEquals("custom-actor", capturedContext!!.actorId)
        assertEquals("value", capturedContext!!.metadata["custom"])
    }
}

class ToPolicyResourceTest {

    @Test
    fun toPolicyResourceParsesValidCommandId() {
        val context = PolicyContext(
            actorId = "user-123",
            commandId = "core.session.session-create",
            subjectId = "subject-456",
        ...
        val resource = toPolicyResource(context)

        assertEquals("core.session.session-create", resource["commandId"])
        assertEquals("core", resource["module"])
        assertEquals("session", resource["service"])
        assertEquals("session-create", resource["command"])
        assertEquals("user-123", resource["actorId"])
        assertEquals("subject-456", resource["subjectId"])
        assertEquals("resource-789", resource["resourceId"])
        assertEquals("corr-abc", resource["correlationId"])
    }

    @Test
    fun toPolicyResourceHandlesInvalidCommandId() {
        val context = PolicyContext(
            actorId = "user-123",
            commandId = "invalid"
        )
        ...
        assertEquals("invalid", resource["commandId"])
        assertEquals("user-123", resource["actorId"])
        assertNull(resource["module"])
    }

    @Test
    fun toPolicyResourceHandlesNullOptionalFields() {
        val context = PolicyContext(
            actorId = "user-123",
            commandId = "core.session.session-create"
        )
        ...
        assertNull(resource["subjectId"])
        assertNull(resource["resourceId"])
        assertNull(resource["correlationId"])
    }
}
