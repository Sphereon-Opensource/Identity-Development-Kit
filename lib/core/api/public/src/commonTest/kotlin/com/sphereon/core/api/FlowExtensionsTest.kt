...
class OfTypeTest {
    ...
    sealed class TestEvent {
        data class TypeA(val value: String) : TestEvent()
        data class TypeB(val number: Int) : TestEvent()
    }

    @Test
    fun filtersOnlyMatchingType() = runTest {
        val events = flowOf(
    ...
    @Test
    fun returnsEmptyForNoMatches() = runTest {
...
class MapOfTypeTest {
    ...
    @Test
    fun mapsMatchingTypeEvents() = runTest {
        val events = flowOf(
...
class MapOfTypeNotNullTest {
    ...
    @Test
    fun filtersNullResults() = runTest {
        val events = flowOf(
            TestEvent.TypeA("hello"),
...
class OnlyOnChangeTest {
    ...
    @Test
    fun emitsOnlyDistinctConsecutiveValues() = runTest {
...
class CombineEventStreamsTest {
    ...
    @Test
    fun mergesMultipleFlows() = runTest {
...
class WithErrorHandlingTest {
    ...
    @Test
    fun invokesErrorHandlerOnException() = runTest {
        var errorCaught: Throwable? = null
        val errorFlow = kotlinx.coroutines.flow.flow {
            emit(1)
    ...
    @Test
    fun usesDefaultErrorHandler() = runTest {
        // Test flow with error but no custom handler (uses default which prints stack trace)
        val errorFlow = kotlinx.coroutines.flow.flow {
            emit(1)
...
class FilterByTypeTest {
    ...
    @Test
    fun filtersMatchingType() = runTest {
...
class FilterSuccessTest {
    ...
    @Test
    fun filtersSuccessfulResults() = runTest {
        val results = flowOf(
...
class FilterErrorsTest {
    ...
    @Test
    fun filtersErrorResults() = runTest {
...
class SplitTest {
    ...
    @Test
    fun splitsIntoSuccessAndErrorFlows() = runTest {
...
class MapSuccessTest {
    ...
    @Test
    fun transformsSuccessValues() = runTest {
        val results = flowOf(
...
class FilterByTypeAndConditionTest {
    ...
    @Test
    fun filtersTypeAndAppliesCondition() = runTest {
...
class BufferEventsTest {
    ...
    @Test
    fun buffersEventsIntoChunks() = runTest {
    ...
    @Test
    fun buffersEventsWithDefaultSize() = runTest {
        val events = flowOf(1, 2, 3)
...
class AsStateFlowTest {
    ...
    @Test
    fun createsStateFlowWithInitialValue() = runTest {
        val events = flowOf(1, 2, 3)
...
class DebounceEventsTest {
    ...
    @Test
    fun debounceEventsReturnsFlow() = runTest {
        val events = flowOf(1, 2, 3)
...
class ThrottleEventsTest {
    ...
    @Test
    fun throttleEventsReturnsFlow() = runTest {
        val events = flowOf(1, 2, 3)
...
class ShareEventsTest {
    ...
    @Test
    fun shareEventsReturnsSharedFlow() = runTest {
        val events = flowOf(1, 2, 3)
