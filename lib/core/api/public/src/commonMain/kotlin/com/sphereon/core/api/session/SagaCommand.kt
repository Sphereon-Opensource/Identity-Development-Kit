...
class SagaCommandAdapter<Arg : Any, Result : Any, E : IdkErrorType>(
    ...
    override suspend fun supports(args: Any): Boolean = steps.isNotEmpty()
 ...
 */
fun <A : Any, R : Any> sagaStep(command: CompensatableCommand<A, R, IdkError>): SagaStep<IdkError> = TypedSagaStep(command, IdkErrorCommandErrorMapper)
...
class SagaBuilder<A : Any, R : Any, E : IdkErrorType>(
     ...
     */
    fun <StepArg : Any, StepResult : Any> step(command: CompensatableCommand<StepArg, StepResult, E>): SagaBuilder<A, R, E> {
 ...
 */
fun <A : Any, R : Any> sagaBuilder(id: String): SagaBuilder<A, R, IdkError> = SagaBuilder(id, IdkErrorCommandErrorMapper)
...
): CompensatableCommand<A, R, E> =
    object : CompensatableCommand<A, R, E> {
        override val id = id
