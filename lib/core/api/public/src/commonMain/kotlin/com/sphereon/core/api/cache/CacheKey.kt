...
data class ScopedKey<K : Any>(
     ...
     */
    fun toStringKey(keySerializer: CacheSerializer<K>? = null): String = buildString {
        append(namespace)
        append(SEPARATOR)
        append(scope.name)
        append(SEPARATOR)
     ...
     * Supports wildcard (*) matching for each component.
     */
    fun matches(pattern: ScopedKeyPattern): Boolean {
        if (pattern.namespace != null && pattern.namespace != "*" && pattern.namespace != namespace) return false
        if (pattern.scope != null && pattern.scope != scope) return false
        if (pattern.tenantId != null && pattern.tenantId != "*" && pattern.tenantId != tenantId) return false
        if (pattern.principalId != null && pattern.principalId != "*" && pattern.principalId != principalId) return false
        return true
    }
    ...
    companion object {
         ...
         */
        fun <K : Any> app(namespace: String, key: K) = ScopedKey(
            namespace = namespace,
         ...
         */
        fun <K : Any> tenant(namespace: String, tenantId: String, key: K) = ScopedKey(
            namespace = namespace,
         ...
         */
        fun <K : Any> principal(namespace: String, tenantId: String, principalId: String, key: K) = ScopedKey(
            namespace = namespace,
        ...
        fun parseString(stringKey: String): ScopedKey<String>? {
            ...
            val scope = try { CacheScope.valueOf(parts[1]) } catch (e: Exception) { return null }
            val tenantId = parts[2].ifEmpty { null }
            val principalId = parts[3].ifEmpty { null }
            val key = parts[4]
...
data class ScopedKeyPattern(
    ...
    fun toRegexPattern(): Regex {
        val pattern = buildString {
            ...
            append(scope?.name ?: ".*")
            append(ScopedKey.SEPARATOR)
            append(if (tenantId != null && tenantId != "*") Regex.escape(tenantId) else ".*")
            append(ScopedKey.SEPARATOR)
            append(if (principalId != null && principalId != "*") Regex.escape(principalId) else ".*")
            append(ScopedKey.SEPARATOR)
    ...
    companion object {
        ...
        fun principal(tenantId: String, principalId: String) = ScopedKeyPattern(
            tenantId = tenantId,
            principalId = principalId
        )
