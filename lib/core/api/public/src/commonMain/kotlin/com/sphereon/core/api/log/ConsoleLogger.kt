...
) : AbstractLogService(id = id, sessionContext = runtimeSessionContext, config = config), LogService
...
abstract class AbstractConsoleLogService(
    ...
    override suspend fun doExecute(args: LogMessage, applyDuring: (LogMessage) -> LogMessage): IdkResult<Unit, IdkErrorType> {
...
) : AbstractConsoleLogService(userContextInstance.toSessionContext(), SERVICE_ID), LogService {
...
class SessionConsoleLogServiceImpl(runtimeSessionContext: SessionContext) :
