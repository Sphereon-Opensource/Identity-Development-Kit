...
class DefaultPolymorphicConfigBinder<T : Any>(
    ...
    private sealed class BuildConfigOutcome<out T> {
        data class Success<T>(val value: T) : BuildConfigOutcome<T>()
        data class Failure(val failure: PolymorphicBindFailure) : BuildConfigOutcome<Nothing>()
    ...
    override fun getEntryConfig(resolver: PropertyResolver, entryId: String): T? {
        return getEntryConfigResult(resolver, entryId).getOrNull()
    }
    ...
    fun getEntryConfigResult(resolver: PropertyResolver, entryId: String): IdkResult<T, IdkError> {
        ...
        return when (val outcome = buildConfigFromProperties(allSubProps, normalizedEntryId, entryId)) {
            BuildConfigOutcome.NotFound -> Err(ConfigErrors.propertyNotFound("$prefix.$entryId"))
            is BuildConfigOutcome.Success -> Ok(outcome.value)
            is BuildConfigOutcome.Failure -> Err(
    ...
    override fun getEntryConfigs(resolver: PropertyResolver): List<T> {
        return getEntryConfigsAsMapResult(resolver, strict = false).getOrElse { emptyMap() }.values.toList()
    }
    ...
    override fun getEntryConfigsAsMap(resolver: PropertyResolver): Map<String, T> {
        return getEntryConfigsAsMapResult(resolver, strict = false).getOrElse { emptyMap() }
    }
    ...
    fun getEntryConfigsAsMapResult(resolver: PropertyResolver, strict: Boolean = true): IdkResult<Map<String, T>, IdkError> {
        ...
        entryIds.forEach { entryId ->
            ...
            when (val outcome = buildConfigFromProperties(allSubProps, normalizedEntryId, entryId)) {
                ...
                is BuildConfigOutcome.Success -> result[entryId] = outcome.value
                is BuildConfigOutcome.Failure -> if (strict) failures += outcome.failure
            }
        }
    ...
    private fun buildConfigFromProperties(
        ...
        val properties = allSubProps.filter { (key, _) ->
        ...
        if (properties.isEmpty()) return BuildConfigOutcome.NotFound
        ...
        for ((key, value) in properties) {
            val normalizedKey = keyNormalizer.normalize(key.removePrefix("."))
            val nestedAlias = normalizedNestedPrefixAliases.entries.firstOrNull { (prefix, _) ->
                normalizedKey == prefix || normalizedKey.startsWith("$prefix.")
            }

            val outputKey = if (nestedAlias != null) {
    ...
    private fun bindFailureError(
        ...
        val reason = failures.joinToString("; ") { failure ->
    ...
    private fun canonicalizeKey(normalizedKey: String): String =
        normalizedKey.replace(PROPERTY_KEY_DELIMITER, "")
...
): PolymorphicConfigBinder<T> = DefaultPolymorphicConfigBinder(
    prefix = prefix,
    baseClass = baseClass,
    json = json,
...
inline fun <reified T : Any> polymorphicConfigBinder(
    ...
    entryDetection: EntryDetectionStrategy = TypeSuffixEntryDetection(),
    propertyNameAliases: Map<String, String> = emptyMap()
): PolymorphicConfigBinder<T> = polymorphicConfigBinder(
    prefix = prefix,
    baseClass = T::class,
    json = json,
    entryDetection = entryDetection,
...
inline fun <reified T : Any> PropertyResolver.getConfigMapPolymorphic(
    ...
    entryDetection: EntryDetectionStrategy = TypeSuffixEntryDetection(),
    propertyNameAliases: Map<String, String> = emptyMap()
): Map<String, T> = getConfigMapPolymorphic(
    prefix = prefix,
    baseClass = T::class,
    json = json,
    entryDetection = entryDetection,
...
inline fun <reified T : Any> PropertyResolver.getConfigMapPolymorphicResult(
    ...
    propertyNameAliases: Map<String, String> = emptyMap(),
    strict: Boolean = true
): IdkResult<Map<String, T>, IdkError> = getConfigMapPolymorphicResult(
    prefix = prefix,
    baseClass = T::class,
    json = json,
    entryDetection = entryDetection,
