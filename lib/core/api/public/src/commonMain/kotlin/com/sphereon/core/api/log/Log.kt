...
data class LogPolicy(
    ...
    fun isEnabled(
        ...
        if (scope in disabledScopes) return false
        if (disabledServicePatterns.any { wildcardMatch(serviceId, it) }) return false
        if (commandOrTag != null && disabledCommandPatterns.any { wildcardMatch(commandOrTag, it) }) return false
...
data class LoggerConfig(
    ...
    companion object {
        ...
        @JsStatic
        val JsonWithTimestamp = LoggerConfig(
...
interface LogService: Logger {
    ...
    fun isEnabled(level: LogLevel, tag: String? = null, context: SessionContext? = null): Boolean = isEnabled
    ...
    override fun trace(message: String, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.TRACE, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
    ...
    override fun debug(message: String, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.DEBUG, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
    ...
    override fun info(message: String, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.INFO, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(LogMessage(level = LogLevel.INFO, message = message, metadata = metadata)).asVoid()
    }

    override fun warn(message: String, errorResult: IdkResult<Nothing, *>?, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.WARN, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(
            LogMessage(level = LogLevel.WARN, message = message, errorResult = errorResult, metadata = metadata)).asVoid()
    }

    override fun error(message: String, exception: Throwable?, errorResult: IdkResult<Nothing, *>?, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.ERROR, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(
            LogMessage(
    ...
    fun trace(metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.TRACE, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(LogMessage(level = LogLevel.TRACE, message = message(), metadata = metadata)).asVoid()
    }

    fun debug(metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.DEBUG, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(LogMessage(level = LogLevel.DEBUG, message = message(), metadata = metadata)).asVoid()
    }

    fun info(metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.INFO, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(LogMessage(level = LogLevel.INFO, message = message(), metadata = metadata)).asVoid()
    }

    fun warn(errorResult: IdkResult<Nothing, *>? = null, metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.WARN, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(
            LogMessage(level = LogLevel.WARN, message = message(), errorResult = errorResult, metadata = metadata)).asVoid()
    }
    ...
    fun error(
        ...
        if (!isEnabled(level = LogLevel.ERROR, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        executeAsync(
            LogMessage(level = LogLevel.ERROR, message = message(), exception = exception, errorResult = errorResult, metadata = metadata)).asVoid()
    }
...
interface AsyncLogService : Command<LogMessage, Unit, IdkErrorType>, AsyncLogger {
    val sessionContext: SessionContext
    ...
    override suspend fun trace(message: String, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.TRACE, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(LogMessage(level = LogLevel.TRACE, message = message, metadata = metadata)).asVoid()
    }

    override suspend fun debug(message: String, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.DEBUG, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(LogMessage(level = LogLevel.DEBUG, message = message, metadata = metadata)).asVoid()
    }

    override suspend fun info(message: String, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.INFO, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(LogMessage(level = LogLevel.INFO, message = message, metadata = metadata)).asVoid()
    }

    override suspend fun warn(message: String, errorResult: IdkResult<Nothing, *>?, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.WARN, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(
            LogMessage(level = LogLevel.WARN, message = message, errorResult = errorResult, metadata = metadata)).asVoid()
    }

    override suspend fun error(message: String, exception: Throwable?, errorResult: IdkResult<Nothing, *>?, metadata: Map<String, String>?) {
        if (!isEnabled(level = LogLevel.ERROR, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(
            LogMessage(level = LogLevel.ERROR, message = message, errorResult = errorResult, exception = exception, metadata = metadata)).asVoid()
    }

    suspend fun trace(metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.TRACE, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(LogMessage(level = LogLevel.TRACE, message = message(), metadata = metadata)).asVoid()
    }

    suspend fun debug(metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.DEBUG, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(LogMessage(level = LogLevel.DEBUG, message = message(), metadata = metadata)).asVoid()
    }

    suspend fun info(metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.INFO, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(LogMessage(level = LogLevel.INFO, message = message(), metadata = metadata)).asVoid()
    }

    suspend fun warn(errorResult: IdkResult<Nothing, *>? = null, metadata: Map<String, String>? = null, message: () -> String) {
        if (!isEnabled(level = LogLevel.WARN, tag = commandOrTagFromMetadata(metadata), context = sessionContext)) return
        execute(
            LogMessage(level = LogLevel.WARN, message = message(), errorResult = errorResult, metadata = metadata)).asVoid()
    }
...
interface Logger {
    fun trace(message: String, metadata: Map<String, String>? = null)

    fun debug(message: String, metadata: Map<String, String>? = null)

    fun info(message: String, metadata: Map<String, String>? = null)

    fun warn(message: String, errorResult: IdkResult<Nothing, *>? = null, metadata: Map<String, String>? = null)

    fun error(message: String, exception: Throwable? = null, errorResult: IdkResult<Nothing, *>? = null, metadata: Map<String, String>? = null)
}

interface AsyncLogger {
    suspend fun trace(message: String, metadata: Map<String, String>? = null)

    suspend fun debug(message: String, metadata: Map<String, String>? = null)

    suspend fun info(message: String, metadata: Map<String, String>? = null)

    suspend fun warn(message: String, errorResult: IdkResult<Nothing, *>? = null, metadata: Map<String, String>? = null)

    suspend fun error(message: String, exception: Throwable? = null, errorResult: IdkResult<Nothing, *>? = null, metadata: Map<String, String>? = null)
}
...
interface LogManager {
    ...
    fun withTagAsync(tag: String = "sphereon", config: LoggerConfig? = null): AsyncLogService

    fun withTag(tag: String = "sphereon", config: LoggerConfig? = null): LogService
}
...
abstract class AbstractLogService(
    ...
    //    abstract val _sync: SyncLogService = object : SyncLogService()
    // Use lazy initialization to ensure subclass properties (like scope) are available
    val _async: AsyncLogService by lazy {
        object : AsyncLogService {
            ...
            override fun isEnabled(level: LogLevel, tag: String?, context: SessionContext?): Boolean =
                this@AbstractLogService.isEnabled(level = level, tag = tag, context = context)
            ...
            override suspend fun execute(args: LogMessage): IdkResult<Unit, IdkErrorType> {
                return this@AbstractLogService.execute(args)
            }
...
object LogMessageFormatter {
     ...
     */
    fun format(message: LogMessage, config: LoggerConfig): String {
    ...
    private fun formatText(message: LogMessage, config: LoggerConfig): String {
        // Pre-compute capacity for efficient allocation
        val tagLen = message.tag?.length ?: 0
        val metadataLen = message.metadata?.entries?.sumOf { it.key.length + it.value.length + 3 } ?: 0
        val exceptionLen = message.exception?.let { 50 + (it.message?.length ?: 0) } ?: 0
        val capacity = 30 + message.message.length + tagLen + metadataLen + exceptionLen
    ...
    private fun formatJson(message: LogMessage, config: LoggerConfig): String {
        // Pre-compute capacity for efficient allocation
        val metadataLen = message.metadata?.entries?.sumOf { it.key.length + it.value.length + 10 } ?: 0
        val capacity = 100 + message.message.length * 2 + metadataLen
...
sealed class LogError(
    ...
    fun getByCode(code: String): ErrorDefinitionType =
        asList().find { it.code == code } ?: NotFound("The error with code [$code] was not found")

    fun getByI18nKey(i18nKey: String): ErrorDefinitionType =
        asList().find { it.i18nKey == i18nKey } ?: NotFound("The error with i18nKey [$i18nKey] was not found")

    class NotFound(message: String?) : LogError(
        code = "NOT_FOUND",
