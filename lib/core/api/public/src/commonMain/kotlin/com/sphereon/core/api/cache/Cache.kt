...
interface BatchCache<K : Any, V : Any> : Cache<K, V> {
     ...
     */
    suspend fun putMany(entries: Map<K, V>, ttl: Duration? = null)
...
interface ScopedCache<K : Any, V : Any> : BatchCache<ScopedKey<K>, V> {
    ...
    suspend fun putApp(key: K, value: V, ttl: Duration? = null)
    ...
    suspend fun putTenant(tenantId: String, key: K, value: V, ttl: Duration? = null)
    suspend fun removeTenant(tenantId: String, key: K): Boolean
    suspend fun containsTenant(tenantId: String, key: K): Boolean

    // ========== Principal-scoped operations ==========

    suspend fun getPrincipal(tenantId: String, principalId: String, key: K): V?
    suspend fun putPrincipal(tenantId: String, principalId: String, key: K, value: V, ttl: Duration? = null)
    suspend fun removePrincipal(tenantId: String, principalId: String, key: K): Boolean
    suspend fun containsPrincipal(tenantId: String, principalId: String, key: K): Boolean
