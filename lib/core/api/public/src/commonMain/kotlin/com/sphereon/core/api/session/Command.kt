...
private fun <E : IdkErrorType> defaultCommandErrorMapper(): CommandErrorMapper<E> =
    IdkErrorTypeCommandErrorMapper as CommandErrorMapper<E>
...
fun interface BaseCommand<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> {
     ...
     */
    suspend fun supports(args: Any /*Do not change to Arg!*/): Boolean = true
     ...
     */
    fun <SuccessResult : Any, ErrorResult : IdkErrorType>
            IdkResult<SuccessResult, ErrorResult>.getOrElse(default: SuccessResult): SuccessResult = if (isOk) this.value else default


    /**
     * Feed a successful value into the next command.
     */
    suspend fun <Arg : Any, NextSuccess : Any, ErrorResult : IdkErrorType>
            IdkResult<Arg, ErrorResult>.andThenCommand(
     ...
     */
    @Suppress("UNCHECKED_CAST")
    fun <FirstArg : Any, FirstSuccessResult : Any, NextSuccessResult : Any, ErrorResult : IdkErrorType>
            BaseCommand<FirstArg, FirstSuccessResult, ErrorResult>.andThen(
        ...
        object : BaseCommand<FirstArg, NextSuccessResult, ErrorResult> {
            ...
            override suspend fun execute(
                args: FirstArg
            ): IdkResult<NextSuccessResult, ErrorResult> {
     ...
     */
    fun <FirstArg : Any, FirstSuccessResult : Any, NextSuccessResult : Any>
            BaseCommand<FirstArg, FirstSuccessResult, IdkError>.andThen(
        next: BaseCommand<FirstSuccessResult, NextSuccessResult, IdkError>
    ): BaseCommand<FirstArg, NextSuccessResult, IdkError> =
        andThen(next = next, errorMapper = IdkErrorCommandErrorMapper)
...
interface BaseChainCommand<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> : BaseCommand<Arg, SuccessResult, ErrorResult> {
    ...
    val config: IChainCommandConfig
        get() = object : IChainCommandConfig {
            /**
 ...
 */
@JsExportCompat
interface Command<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> : HasId, BaseCommand<Arg, SuccessResult, ErrorResult> {
    /**
 ...
 */
@JsExportCompat
interface ChainCommand<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> : Command<Arg, SuccessResult, ErrorResult>,
 ...
 */
@JsExportCompat
interface PipelineCommand<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> : Command<Arg, SuccessResult, ErrorResult>,
...
interface ICommandExecutionExtension<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> {
    fun beforeExecute(service: Command<Arg, SuccessResult, ErrorResult>, args: Arg) {}
    fun duringExecute(service: Command<Arg, SuccessResult, ErrorResult>, args: Arg): Arg = args
    fun afterExecute(
...
interface ICommandExecutionListener<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> {
    fun onBeforeExecute(serviceId: String, args: Arg) {}
    fun onDuringExecute(serviceId: String, args: Arg): Arg = args
    fun onAfterExecute(serviceId: String, args: Arg, result: IdkResult<SuccessResult, ErrorResult>) {}
}

abstract class MultiCommandAdapter<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType>(
    id: String, isEnabled: Boolean, initExtensions: Array<ICommandInitExtension<Arg, SuccessResult, ErrorResult>> = emptyArray(),
    ...
    override fun getById(id: String): Command<Arg, SuccessResult, ErrorResult>? {
        return commands.firstOrNull { it.id == id }
    }

    @Suppress("UNCHECKED_CAST")
    override fun addCommand(filter: BaseCommand<*, *, *>): BasePipelineCommand<Arg, SuccessResult, ErrorResult> =
        apply { this.commands.add(filter as Command<Arg, SuccessResult, ErrorResult>) }


    @Suppress("UNCHECKED_CAST")
    override fun removeCommand(filter: BaseCommand<*, *, *>): BasePipelineCommand<Arg, SuccessResult, ErrorResult> = apply {
...
abstract class CommandAdapter<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType>(
     ...
     * Downstream layers (EDK/VDX) replace [DefaultInterceptorChainComponent] via DI to inject
    ...
    override suspend fun execute(args: Arg): IdkResult<SuccessResult, ErrorResult> {
        ...
        val context = CommandExecutionContext(
            commandId = id,
            commandIdParts = parseCommandId(id),
        ...
        try {
            ...
            for (interceptor in interceptors.sortedBy { it.order }) {
                try {
                    ...
                    // LD#7: Interceptor failures must not fail command execution
                }
            }

            // ── PHASE 2: command execution (SKIPPED if any Deny) ──
            if (firstDenial != null) {
                commandResult = IdkResult.err(
            ...
            for (ext in enhancedExecutionExtensions) {
                when (val beforeResult = ext.beforeExecute(this, currentArgs)) {
                    is BeforeExecuteResult.Continue -> currentArgs = beforeResult.args
            ...
            val result = doExecute(currentArgs) { argDuring ->
