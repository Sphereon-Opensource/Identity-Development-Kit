...
interface IdkBuilder<T> {
    ...
    fun build(): T {
        ...
        if (errors.isNotEmpty()) {
            throw IllegalStateException("Builder validation failed: ${errors.joinToString(", ")}")
        }
...
sealed class BuilderValidationResult {
    ...
    data class Invalid(val errors: List<String>) : BuilderValidationResult()
...
fun List<String>.toValidationResult(): BuilderValidationResult =
    if (isEmpty()) BuilderValidationResult.Valid
    else BuilderValidationResult.Invalid(this)
...
fun <T> IdkBuilder<T>.buildSafely(): IdkResult<T, IdkErrorType> {
    return try {
        ...
        if (errors.isNotEmpty()) {
            ...
            return IdkResult.err(com.sphereon.core.api.error.IdkError.ILLEGAL_ARGUMENT_ERROR(message = errorMessage))
        }
        IdkResult.ok(buildInternal())
    } catch (e: Exception) {
        IdkResult.err(com.sphereon.core.api.error.IdkError.UNKNOWN_ERROR(exception = e))
    }
}
 ...
 */
inline fun <T> T.applyConfigIf(condition: Boolean, config: BuilderConfig<T>): T =
    if (condition) apply(config) else this
 ...
 * @return The (possibly) configured object
 */
inline fun <T> T.applyConfigIf(predicate: (T) -> Boolean, config: BuilderConfig<T>): T =
    if (predicate(this)) apply(config) else this
