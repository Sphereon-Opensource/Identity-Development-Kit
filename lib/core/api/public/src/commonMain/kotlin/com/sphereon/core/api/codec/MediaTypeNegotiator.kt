...
interface MediaTypeNegotiator {
     ...
     */
    fun selectResponseCodec(accept: List<String>, supported: Set<String>): StreamingCodec
...
class DefaultMediaTypeNegotiator(
    ...
    override fun selectRequestCodec(contentType: String?): StreamingCodec {
        if (contentType == null) return defaultCodec
    ...
    override fun selectResponseCodec(accept: List<String>, supported: Set<String>): StreamingCodec {
        if (accept.isEmpty()) return defaultCodec

        // Parse accept header with quality values
        val preferences = accept.flatMap { parseAcceptHeader(it) }
        ...
        for (preference in preferences) {
            ...
            // Handle wildcard
            if (normalized == "*/*") return defaultCodec
    ...
    private fun parseAcceptHeader(accept: String): List<MediaTypePreference> {
        return accept.split(',').mapNotNull { part ->
            ...
            val quality = trimmed.substringAfter("q=", "1.0")
                .substringBefore(',')
                .substringBefore(';')
                .toDoubleOrNull() ?: 1.0
...
object GrpcMediaTypes {
     ...
     */
    fun toHttpContentType(grpcType: String): String? = when (grpcType.lowercase()) {
     ...
     */
    fun fromHttpContentType(httpType: String): String? = when (httpType.lowercase()) {
