...
interface StreamingCodecRegistry {
     ...
     */
    fun codecForOrDefault(contentType: String?): StreamingCodec = contentType?.let { codecFor(it) } ?: defaultCodec
...
object CodecErrors {
    ...
    ): IdkError = IdkError.UNKNOWN_ERROR(message = "Encoding error: $message", exception = cause)

    fun decodingError(
        message: String,
        cause: Throwable? = null,
    ): IdkError = IdkError.UNKNOWN_ERROR(message = "Decoding error: $message", exception = cause)

    fun unsupportedType(type: String): IdkError = IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unsupported type for codec: $type")

    fun unsupportedContentType(contentType: String): IdkError = IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unsupported content type: $contentType")
}
