...
interface HttpBodyCodec : HasOrder {
     ...
     * @throws CodecException if encoding fails
     */
    fun <T : Any> encodeToString(value: T): String =
        encode(value).asTextOrNull()
...
interface HttpBodyCodecRegistry : HasOrder {
    ...
    fun <T : Any> decode(
        ...
        mediaType: MediaType?,
        targetType: KClass<T>,
    ): T = codecFor(mediaType).decode(body, targetType)
    ...
    ): GenericHttpBody = codecFor(mediaType).encode(value)
    ...
    fun <T : Any> encodeToString(
        value: T,
        mediaType: MediaType? = null,
    ): String = codecFor(mediaType).encodeToString(value)
}

/**
 * Inline reified extension for decoding.
 */
inline fun <reified T : Any> HttpBodyCodecRegistry.decode(
    body: GenericHttpBody,
    mediaType: MediaType? = null,
): T = decode(body, mediaType, T::class)

/**
 * Inline reified extension for codec's decode.
 */
inline fun <reified T : Any> HttpBodyCodec.decode(body: GenericHttpBody): T = decode(body, T::class)
