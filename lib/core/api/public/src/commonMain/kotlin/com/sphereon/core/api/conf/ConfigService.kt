...
class AppConfigServiceImpl(environment: AppConfigEnvironment) : AbstractConfigService(environment), AppConfigService {
}
...
class TenantConfigServiceImpl(environment: TenantConfigEnvironment, override val parent: AppConfigService) : AbstractConfigService(environment), TenantConfigService {
}
...
class PrincipalConfigServiceImpl(environment: PrincipalConfigEnvironment, override val parent: TenantConfigService) :
...
abstract class AbstractConfigService(protected val configEnvironment: ConfigEnvironment) : ConfigService {
    ...
    override fun removePropertySource(source: PropertySource<*>): ConfigService = apply { configEnvironment.getPropertySources(includeParents = false).remove(source); }
    ...
    override fun <T : Any> getProperty(key: String, targetType: KClass<T>, defaultValue: T?): T? = configEnvironment.getProperty(key, targetType, defaultValue)

    override fun getPropertyAsString(key: String, defaultValue: String?): String? = configEnvironment.getPropertyAsString(key, defaultValue)

    override fun <T : Any> getRequiredProperty(key: String, targetType: KClass<T>, defaultValue: T?): T = configEnvironment.getRequiredProperty(key, targetType, defaultValue)

    override fun getRequiredPropertyAsString(key: String, defaultValue: String?): String = configEnvironment.getRequiredPropertyAsString(key, defaultValue)
    ...
    override fun getSubProperties(prefixes: Set<String>, stripPrefix: Boolean): Map<String, Any> = configEnvironment.getSubProperties(prefixes, stripPrefix)

    override fun getSubPropertiesAsString(prefixes: Set<String>, stripPrefix: Boolean, redact: Boolean): Map<String, String> = configEnvironment.getSubPropertiesAsString(prefixes, stripPrefix, redact)

    override fun getNamespace(): String = configEnvironment.getNamespace()
}


inline fun <reified T : Any> ConfigService.getProperty(key: String, defaultValue: T?) = getProperty(key, T::class, defaultValue)
inline fun <reified T : Any> ConfigService.getRequiredProperty(key: String, defaultValue: T?) = getRequiredProperty(key, T::class, defaultValue)
