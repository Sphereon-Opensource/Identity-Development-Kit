...
import com.sphereon.core.api.session.CommandAuthorizer
...
import com.sphereon.core.api.session.PermissiveAuthorizer
...
object CommandTestSupport {
    ...
    /**
     * Permissive authorizer for testing.
     *
     * Always allows command execution. Use this in tests where
     * authorization is not the focus.
     */
    val testAuthorizer: CommandAuthorizer = PermissiveAuthorizer
     ...
     */
    suspend fun <A : Any, R : Any, E : IdkErrorType> BaseCommand<A, R, E>.testExecute(args: A): IdkResult<R, E> =
        execute(args)
     ...
     */
    suspend fun <A : Any, R : Any, E : IdkErrorType> BaseCommand<A, R, E>.testSupports(args: Any): Boolean =
        supports(args)
}
...
): Command<A, R, IdkError> = mockCommand(
    id = id,
