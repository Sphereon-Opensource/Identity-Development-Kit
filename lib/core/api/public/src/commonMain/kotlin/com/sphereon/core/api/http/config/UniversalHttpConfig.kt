...
data class UniversalHttpConfig(
     ...
     */
    fun resolveMount(adapterId: String, declaredMount: HttpAdapterMount): HttpAdapterMount {
        val override = overrides[adapterId]
        
        return HttpAdapterMount(
            serverPrefix = override?.serverPrefix 
                ?: declaredMount.serverPrefix.ifEmpty { defaults.serverPrefix },
            adapterBasePath = override?.adapterBasePath 
                ?: declaredMount.adapterBasePath,
            tenantPathMode = override?.tenantPathMode 
                ?: if (declaredMount.tenantPathMode != TenantPathMode.OFF) declaredMount.tenantPathMode 
                   else defaults.tenantPathMode,
            tenantSegmentPattern = override?.tenantSegmentPattern 
                ?: if (declaredMount.tenantSegmentPattern != HttpAdapterMount.DEFAULT_TENANT_SEGMENT_PATTERN) 
                       declaredMount.tenantSegmentPattern 
                   else defaults.tenantSegmentPattern,
            tenantResolutionPriority = override?.tenantResolutionPriority 
                ?: if (declaredMount.tenantResolutionPriority != TenantResolutionPriority.HEADER_THEN_PATH) 
                   ...
                   else defaults.tenantResolutionPriority
        )
    }
    ...
    fun isAdapterEnabled(adapterId: String): Boolean {
        return overrides[adapterId]?.enabled ?: true
    }
