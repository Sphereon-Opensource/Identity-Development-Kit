...
private fun matchesStructuredPrefix(
    stringKey: String,
    prefix: String,
): Boolean {
...
data class CachedConfigValue(
    ...
    fun <T> toResolvedValue(): ResolvedValue<T>? {
        if (isExpired() || isNegativeCache || value == null) {
            return null
        }
...
object NoOpConfigCache : ScopedConfigCache {
    ...
    override suspend fun getMany(keys: List<ConfigCacheKey>): Map<ConfigCacheKey, CachedConfigValue?> = keys.associateWith { null }
...
object NoOpSnapshotCache : ConfigSnapshotCache {
    ...
    override suspend fun putSnapshot(
        key: SnapshotKey,
        snapshot: ConfigSnapshot,
    ) {
        // No-op
...
class InMemoryConfigCache(
    ...
    override suspend fun getMany(keys: List<ConfigCacheKey>): Map<ConfigCacheKey, CachedConfigValue?> = keys.associateWith { get(it) }
...
data class TtlConfig(
    ...
    fun forScope(scope: ConfigLevel): Duration =
        when (scope) {
            ConfigLevel.APP -> app
...
class DefaultSyncConfigSnapshotCache : SyncConfigSnapshotCache by createSyncConfigSnapshotCache() {
    /**
     * Graph interface for DI access to the sync cache.
...
) : SyncConfigSnapshotCache by syncCache,
    ConfigCacheWarmup {
