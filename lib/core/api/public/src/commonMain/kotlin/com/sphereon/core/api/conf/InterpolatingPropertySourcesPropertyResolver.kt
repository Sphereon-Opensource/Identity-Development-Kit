...
class InterpolatingPropertySourcesPropertyResolver(
    ...
    override fun getAllProperties(): Map<String, Any> {
        return delegate.getAllProperties().mapValues { (_, value) -> interpolateIfString(value) }
    }

    override fun getSubProperties(prefixes: Set<String>, stripPrefix: Boolean): Map<String, Any> {
        return delegate.getSubProperties(prefixes, stripPrefix).mapValues { (_, value) -> interpolateIfString(value) }
    }

    @Suppress("UNCHECKED_CAST")
    private fun <T> interpolateIfString(value: T): T {
        if (value !is String) return value
        if (!interpolator.containsPlaceholders(value)) return value
        val result = runBlockingCompat {
            interpolator.interpolate(value, delegate)
        }
        return if (result.isOk) result.value as T else value
    }
}
