...
class HttpRoutesBuilder internal constructor() {
    ...
    fun get(
        pathPattern: String,
        block: HttpRouteBuilder.() -> Unit,
    ) = add(HttpMethod.GET, pathPattern, block)

    fun post(
        pathPattern: String,
        block: HttpRouteBuilder.() -> Unit,
    ) = add(HttpMethod.POST, pathPattern, block)

    fun put(
        pathPattern: String,
        block: HttpRouteBuilder.() -> Unit,
    ) = add(HttpMethod.PUT, pathPattern, block)

    fun delete(
        pathPattern: String,
        block: HttpRouteBuilder.() -> Unit,
    ) = add(HttpMethod.DELETE, pathPattern, block)

    fun patch(
        pathPattern: String,
        block: HttpRouteBuilder.() -> Unit,
    ) = add(HttpMethod.PATCH, pathPattern, block)
