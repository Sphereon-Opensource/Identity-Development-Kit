...
data class GenericHttpRequest(
    ...
    val accept: List<String>
        get() = (headers["Accept"] ?: headers["accept"])
            ?.split(",")
...
class CompiledPathPattern private constructor(
    ...
    fun extractParams(path: String): Map<String, String> {
        ...
        return pathSegments.zip(segments)
    ...
    companion object {
        ...
        fun compile(pattern: String): CompiledPathPattern {
            return cache.getOrPut(pattern) {
                val segments = splitPath(pattern).map { segment ->
 ...
 */
internal fun matchesPathPattern(path: String, pattern: String): Boolean {
    return CompiledPathPattern.compile(pattern).matches(path)
}
 ...
 */
internal fun extractPathParams(path: String, pattern: String): Map<String, String> {
    return CompiledPathPattern.compile(pattern).extractParams(path)
}
