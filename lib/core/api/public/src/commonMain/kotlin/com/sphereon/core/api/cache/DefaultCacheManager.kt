...
class DefaultCacheManager : CacheManager {
    ...
    override fun hasDistributedBackend(): Boolean =
        backends.values.any { it.capabilities.isDistributed }

    override fun hasLocalBackend(): Boolean =
        backends.values.any { it.capabilities.isLocal }
    ...
    override fun getRequirements(namespace: String): CacheRequirements? =
        registeredRequirements[namespace]

    override fun getNamespaces(): Set<String> =
        registeredRequirements.keys + caches.keys
    ...
    private fun selectBackend(requirements: CacheRequirements): CacheBackend {
        ...
        return when (requirements.locality) {
            ...
            CacheLocality.DISTRIBUTED_ONLY -> {
                distributedBackend ?: throw IllegalStateException(
    ...
    override fun aggregateStats(): Map<String, CacheStatistics> =
        caches.mapValues { (_, cache) -> cache.stats() }
    ...
    override suspend fun isHealthy(): Boolean =
        backends.values.all { it.isHealthy() }
}
