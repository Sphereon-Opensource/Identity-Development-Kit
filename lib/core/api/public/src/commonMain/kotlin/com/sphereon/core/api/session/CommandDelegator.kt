...
interface CommandDelegator {
     ...
     */
    suspend operator fun <A : Any, R : Any, E : IdkErrorType> BaseCommand<A, R, E>.invoke(args: A): IdkResult<R, E> =
        this.execute(args)

    /**
     * Invokes a command and throws if the result is an error.
     *
     * Boundary-only helper. Prefer `invoke()` and explicit `IdkResult` handling in public flows.
     *
     * @param args The command arguments
     * @return The success value
     * @throws RuntimeException if the command returns an error
     */
    @Deprecated(
        message = "Boundary-only API. Prefer invoke(args) and explicit IdkResult handling in public flows."
    )
    @InternalApi
    suspend fun <A : Any, R : Any, E : IdkErrorType> BaseCommand<A, R, E>.invokeOrThrow(args: A): R =
        this.execute(args).getOrThrow()
}
