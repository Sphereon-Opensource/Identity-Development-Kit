 ...
 * boundaries — typically AppScope multi-tenant stores like `TenantIdpRegistry` whose
 * cross-tenant discovery queries (`findByIssuer`) return rows from any tenant and
 * therefore need to decrypt with the tenant id baked into each row, not the calling
 * user's tenant.
 *
