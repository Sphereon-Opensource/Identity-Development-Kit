...
abstract class RoutedHttpAdapter : RoutableHttpAdapter {
    ...
    protected val basePathPrefix: String by lazy {
        mount.adapterBasePath.let { if (it.startsWith("/")) it else "/$it" }.trimEnd('/')
    }
    ...
    protected fun fullPath(routePattern: String): String {
        val pattern = routePattern.let { if (it.startsWith("/")) it else "/$it" }
    ...
    override fun canHandle(request: GenericHttpRequest): Boolean =
        routes.any { request.matches(it.endpoint.method.name, fullPath(it.endpoint.pathPattern)) }
