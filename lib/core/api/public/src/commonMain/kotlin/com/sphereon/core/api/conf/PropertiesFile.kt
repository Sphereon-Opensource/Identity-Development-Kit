...
private const val UNICODE_ESCAPE_TOTAL_LENGTH = 5
private const val UNICODE_ESCAPE_PREFIX_LENGTH = 2
private const val UNICODE_ESCAPE_END_OFFSET = 6
    ...
    fun unescapeProperty(input: String): String {
        ...
        while (i < input.length) {
            ...
            when (next) {
                ...
                'u' -> {
                    if (i + UNICODE_ESCAPE_TOTAL_LENGTH < input.length) {
                        val hex = input.substring(i + UNICODE_ESCAPE_PREFIX_LENGTH, i + UNICODE_ESCAPE_END_OFFSET)
                        val unicode = hex.toIntOrNull(HEX_RADIX)
                        if (unicode != null) {
                            result.append(unicode.toChar())
                            i += UNICODE_ESCAPE_END_OFFSET
                            continue
                        }
                    }
                    result.append(next)
