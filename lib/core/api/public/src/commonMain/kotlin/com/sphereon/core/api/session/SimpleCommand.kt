 ...
 */
fun <A : Any, R : Any> SimpleCommand<A, R>.asCommand(subsystem: EventSubsystem = EventSubsystems.CUSTOM): Command<A, R, IdkError> =
    object : Command<A, R, IdkError> {
        ...
        override suspend fun supports(args: Any): Boolean = this@asCommand.supports(args)

        override suspend fun execute(args: A): IdkResult<R, IdkError> = this@asCommand.execute(args)
    }
...
fun <A : Any, R : Any, E : com.sphereon.core.api.error.IdkErrorType> SimpleCommand<A, R>.asCommandWithError(
    ...
    object : Command<A, R, E> {
        override val id: String = this@asCommandWithError.id
        override val isEnabled: Boolean = true
        override val subsystem: EventSubsystem = subsystem

        override suspend fun supports(args: Any): Boolean = this@asCommandWithError.supports(args)
