...
data class ProfileChain(
    ...
    fun getResolutionChain(): List<String> {
        return if (fallbackToDefault && !profiles.contains(defaultProfile)) {
     ...
     */
    fun contains(profile: String): Boolean = profiles.contains(profile) ||
    ...
    companion object {
         ...
         */
        fun of(profile: String, fallbackToDefault: Boolean = true): ProfileChain =
            ProfileChain(listOf(profile), fallbackToDefault)

        /**
         * Create a profile chain with multiple profiles.
         */
        fun of(vararg profiles: String, fallbackToDefault: Boolean = true): ProfileChain =
            ProfileChain(profiles.toList(), fallbackToDefault)
...
class DefaultProfileResolver(
    ...
    private val profiles: List<String> = activeProfiles.ifEmpty {
    ...
    private fun loadProfilesFromEnvironment(): List<String> {
        ...
        return if (envValue.isNullOrBlank()) {
            ...
            envValue.split(profileSeparator)
                .map { it.trim() }
    ...
    override fun isProfileActive(profile: String): Boolean =
        profiles.contains(profile) || profile == defaultProfile
     ...
     */
    fun getProfileChain(): ProfileChain = ProfileChain(
        profiles = profiles,
 ...
 */
fun ResolutionContext.withProfile(profile: String): ResolutionContext = copy(
 ...
 */
fun ResolutionContext.withProfileChain(chain: ProfileChain): ResolutionContext = copy(
