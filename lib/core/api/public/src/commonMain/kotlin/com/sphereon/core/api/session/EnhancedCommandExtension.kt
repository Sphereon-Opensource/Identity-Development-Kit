...
sealed class BeforeExecuteResult<out A, out R, out E> {
     ...
     */
    data class Continue<A>(val args: A) : BeforeExecuteResult<A, Nothing, Nothing>()
 ...
 *
 * Use cases:
 * - Policy enforcement (authorization checks before execution)
 * - Caching (return cached results, skip execution)
 * - Audit logging (capture before/after state)
 * - Request/response transformation
 * - Rate limiting (reject requests before execution)
 *
 * Example - Authorization Extension:
 * ```kotlin
 ...
 *         val authResult = authorizer.isAuthorized(CommandId(service.id), sessionContext)
 *         return if (authResult.isOk) {
 *             BeforeExecuteResult.Continue(args)
 *         } else {
 *             BeforeExecuteResult.ShortCircuit(
 *                 Err(IdkError.FORBIDDEN_ERROR(message = "Not authorized: ${service.id}"))
 *             )
 *         }
 *     }
 * }
    ...
    override suspend fun beforeExecute(
        ...
        for (ext in extensions) {
            when (val result = ext.beforeExecute(service, currentArgs)) {
                is BeforeExecuteResult.Continue -> currentArgs = result.args
                is BeforeExecuteResult.Skip -> return result
...
fun <Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> compositeExtension(
    ...
    CompositeEnhancedExtension(extensions.toList())
