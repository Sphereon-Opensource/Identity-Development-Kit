...
class CloudConfigProviderRegistry {
     ...
     */
    fun getForLevel(level: ConfigLevel): List<CloudConfigProvider> =
        providers.values.filter { it.configLevel == level }
     ...
     */
    suspend fun refreshAll(): Map<String, IdkResult<RefreshResult, IdkError>> =
        providers.mapValues { (_, provider) -> provider.refresh() }

    /**
     * Health check all providers.
     */
    suspend fun healthCheckAll(): Map<String, IdkResult<CloudProviderHealth, IdkError>> =
        providers.mapValues { (_, provider) -> provider.healthCheck() }
}
...
) : MapPropertySource(name, initialSource, order), CloudConfigProvider {
