 ...
 * @property commandIdParts Parsed command ID parts, or null if not a 3-segment ID
 * @property subsystem The event subsystem this command belongs to
 */
data class CommandExecutionContext(
    val commandId: String,
    val commandIdParts: CommandIdParts?,
...
sealed class InterceptorVerdict {
    ...
    data class Deny(val reason: String, val errorCode: String = "INTERCEPTOR_DENIED") : InterceptorVerdict()
}
 ...
 * of [DefaultInterceptorChainComponent].
 *
...
interface CommandLifecycleInterceptor {
     ...
     */
    suspend fun beforeExecute(context: CommandExecutionContext, args: Any): InterceptorVerdict =
 ...
 *
 * IDK provides [EmptyInterceptorChain] by default via [DefaultInterceptorChainComponent].
 ...
 * Downstream layers replace this component to inject real interceptors:
 * ```kotlin
 * @ContributesTo(SessionScope::class, replaces = [DefaultInterceptorChainComponent::class])
 * interface MyInterceptorChainComponent {
 *     @Provides
...
interface DefaultInterceptorChainComponent {
    @Provides
