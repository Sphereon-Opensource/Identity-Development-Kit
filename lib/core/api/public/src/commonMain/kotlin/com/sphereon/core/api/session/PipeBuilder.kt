...
class PipeBuilder<In : Any, Current : Any, E : IdkErrorType> private constructor(
     ...
     */
    fun <Next : Any> then(next: BaseCommand<Current, Next, E>): PipeBuilder<In, Next, E> =
        PipeBuilder(id, command.chain("$id-step", next, errorMapper), errorMapper)
     ...
     */
    fun <Next : Any> thenMap(
        transform: suspend (Current) -> IdkResult<Next, E>
    ): PipeBuilder<In, Next, E> =
        PipeBuilder(id, command.mapOutput(transform), errorMapper)
     ...
     * @return A new PipeBuilder with the transformation
     */
    fun <Next : Any> thenMapSimple(transform: suspend (Current) -> Next): PipeBuilder<In, Next, E> =
        thenMap { current -> Ok(transform(current)) }
    ...
    ): PipeBuilder<In, Current, E> =
        PipeBuilder(id, command.filterOutput(predicate, onFailure), errorMapper)
     ...
     */
    fun tap(effect: suspend (Current) -> Unit): PipeBuilder<In, Current, E> =
        PipeBuilder(id, command.onSuccess(effect), errorMapper)
...
private class NamedCommand<I : Any, O : Any, E : IdkErrorType>(
    ...
    override suspend fun supports(args: Any) =
        delegate.supports(args)

    override suspend fun execute(args: I) =
 ...
 */
fun <I : Any, O : Any> pipe(id: String, first: BaseCommand<I, O, IdkError>) =
...
): Command<I, O, E> = object : Command<I, O, E> {
    override val id = id
...
): Command<I, O, IdkError> = object : Command<I, O, IdkError> {
    override val id = id
    override val isEnabled = true
    override val subsystem = EventSubsystems.CUSTOM
    override suspend fun supports(args: Any) = true
