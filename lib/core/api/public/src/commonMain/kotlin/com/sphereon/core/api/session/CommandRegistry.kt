...
class InMemoryCommandRegistry : CommandRegistry {
    ...
    override fun findBySubsystem(subsystem: String): Set<String> = entries.filter { it.value.subsystem == subsystem }.keys

    override fun findByTag(tag: String): Set<String> = entries.filter { tag in it.value.tags }.keys

    override fun findByPattern(pattern: String): Set<String> = entries.keys.filter { matchesAdvancedPattern(pattern, it) }.toSet()

    override fun isRegistered(commandId: String): Boolean = commandId in entries
}
