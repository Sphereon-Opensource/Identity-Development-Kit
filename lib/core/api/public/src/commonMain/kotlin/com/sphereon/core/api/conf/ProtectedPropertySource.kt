...
interface ProtectedPropertySource<T> : PropertySource<T> {
     ...
     */
    fun canSet(key: String, fromScope: ConfigLevel): Boolean
     ...
     */
    fun canInterpolate(key: String, fromScope: ConfigLevel): Boolean
}
...
open class ProtectedMutableMapPropertySource(
...
) : ProtectedPropertySource<MutableMap<String, Any>>, ScopedPropertySource<MutableMap<String, Any>> {
     ...
     */
    open fun addProperty(name: String, value: Any) = apply {
     ...
     */
    open fun addProperties(map: Map<String, Any>) = apply {
     ...
     */
    open fun addProtectedProperty(name: String, value: Any, protection: PropertyProtection) = apply {
     ...
     * Delete a property.
     */
    open fun deleteProperty(name: String) = apply {
        val normalizedKey = keyNormalizer.normalize(name)
    ...
    override fun canSet(key: String, fromScope: ConfigLevel): Boolean {
    ...
    override fun canInterpolate(key: String, fromScope: ConfigLevel): Boolean {
        val normalizedKey = keyNormalizer.normalize(key)
        val protection = protectionRegistry[normalizedKey] ?: return true
    ...
    override fun hasProperty(name: String): Boolean {
        return properties.containsKey(keyNormalizer.normalize(name))
    }
    ...
    override fun <T : Any> getProperty(name: String, targetType: KClass<T>): T? {
        ...
        // Coerce String values to the requested type
        if (value is String) {
...
open class ProtectedEnvPropertySource(
...
) : ProtectedPropertySource<Map<String, Any>>, ScopedPropertySource<Map<String, Any>> {

    private val keyNormalizerInstance: PropertyKeyNormalizer = PropertyKeyNormalizerImpl()
    private val delegateSource = MapPropertySource(
    ...
    override fun removeProperty(name: String) {
        throw UnsupportedOperationException("Cannot remove environment variables")
    }
