...
interface CacheConfigLoader {
     ...
     */
    fun loadRequirements(namespace: String, defaults: CacheDefaults<*, *>): CacheRequirements
}
...
class DefaultCacheConfigLoader(
    ...
    override fun loadRequirements(namespace: String, defaults: CacheDefaults<*, *>): CacheRequirements {
        val nsPrefix = "$CONFIG_PREFIX.$namespace"
        val globalPrefix = GLOBAL_PREFIX

        // Locality - config can override default LOCAL_ONLY
        val locality = getProperty("$nsPrefix.locality")
            ?.let { parseLocality(it) }
            ?: getProperty("$globalPrefix.locality")
                ?.let { parseLocality(it) }
            ?: CacheLocality.LOCAL_ONLY

        val distributedFallback = getBooleanProperty("$nsPrefix.distributed-fallback")
            ?: getBooleanProperty("$globalPrefix.distributed-fallback")
            ?: false

        val writeThrough = getBooleanProperty("$nsPrefix.write-through")
        ...
        val maxEntries = getIntProperty("$nsPrefix.max-entries")
            ?: getIntProperty("$globalPrefix.max-entries")
            ?: defaults.defaultMaxEntries

        val persistent = getBooleanProperty("$nsPrefix.persistent")
    ...
    private fun parseLocality(value: String): CacheLocality = when (value.uppercase().replace("-", "_")) {
    ...
    private fun loadTtlConfig(nsPrefix: String, globalPrefix: String, defaults: CacheTtlConfig): CacheTtlConfig {
        return CacheTtlConfig(
            app = parseDuration("$nsPrefix.ttl.app")
                ?: parseDuration("$globalPrefix.ttl.app")
                ?: defaults.app,
            tenant = parseDuration("$nsPrefix.ttl.tenant")
                ?: parseDuration("$globalPrefix.ttl.tenant")
                ?: defaults.tenant,
            principal = parseDuration("$nsPrefix.ttl.principal")
