...
sealed class StreamingBody {
    ...
    companion object {
         ...
         */
        fun ofText(value: String?, charset: String = "UTF-8"): StreamingBody = when {
         ...
         */
        fun ofBytes(value: ByteArray?): StreamingBody = when {
         ...
         */
        fun ofFlow(flow: Flow<ByteArray>, contentLength: Long? = null): StreamingBody =
            ByteStream(flow, contentLength)

        /**
         * Converts a GenericHttpBody to StreamingBody.
         */
        fun fromGenericHttpBody(body: GenericHttpBody): StreamingBody = when (body) {
            is GenericHttpBody.Empty -> Empty
            is GenericHttpBody.Text -> Text(body.value, body.charset)
            is GenericHttpBody.Bytes -> Bytes(body.value)
            is GenericHttpBody.LazyText -> {
                val text = body.value
                if (text.isNullOrEmpty()) Empty else Text(text, body.charset)
            }
            is GenericHttpBody.LazyBytes -> {
                val bytes = body.value
                if (bytes == null || bytes.isEmpty()) Empty else Bytes(bytes)
            }
        }
    }
     ...
     */
    fun toGenericHttpBody(): GenericHttpBody = when (this) {
        is Empty -> GenericHttpBody.Empty
        is Text -> GenericHttpBody.Text(value, charset)
        is Bytes -> GenericHttpBody.Bytes(value)
        is LazyBytes -> {
            val bytes = value
            if (bytes == null || bytes.isEmpty()) GenericHttpBody.Empty else GenericHttpBody.Bytes(bytes)
        }
        is ByteStream -> throw UnsupportedOperationException(
     ...
     */
    suspend fun toGenericHttpBodySuspend(): GenericHttpBody = when (this) {
        is Empty -> GenericHttpBody.Empty
        is Text -> GenericHttpBody.Text(value, charset)
        is Bytes -> GenericHttpBody.Bytes(value)
        is LazyBytes -> {
        ...
        is ByteStream -> {
            val bytes = collect()
            if (bytes.isEmpty()) GenericHttpBody.Empty else GenericHttpBody.Bytes(bytes)
        }
    }
}
