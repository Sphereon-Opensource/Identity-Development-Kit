...
class CachingPropertySourcesPropertyResolver(
    ...
    override fun <T : Any> getProperty(key: String, targetType: KClass<T>, defaultValue: T?) =
        delegate.getProperty(key, targetType, defaultValue)

    override fun getPropertyAsString(key: String, defaultValue: String?) =
    ...
    override fun getRequiredPropertyAsString(key: String, defaultValue: String?) =
    ...
    override fun getSubProperties(prefixes: Set<String>, stripPrefix: Boolean): Map<String, Any> {
        ...
        return if (stripPrefix) stripPrefixes(result, normalizedPrefixes) else result
    }

    override fun getSubPropertiesAsString(prefixes: Set<String>, stripPrefix: Boolean, redact: Boolean): Map<String, String> {
        return getSubProperties(prefixes, stripPrefix).mapValues { (key, value) ->
            redactIfNeeded(key, value, redact)
        }
    }
     ...
     */
    private fun redactIfNeeded(key: String, value: Any?, redact: Boolean): String {
        if (value == null) return "null"
        if (!redact) return value.toString()

        val metadata = ResolutionMetadata(
            source = "CachingPropertyResolver",
            scope = level,
            originalKey = key,
            normalizedKey = key,
            order = 0,
    ...
    private fun extractFromSnapshot(
        ...
        val values = snapshot.values.mapNotNull { (k, v) ->
            v.value?.let { k to it }
        }.toMap()
        return if (stripPrefix) stripPrefixes(values, normalizedPrefixes) else values
    }

    private fun stripPrefixes(props: Map<String, Any>, prefixes: Set<String>): Map<String, Any> {
        return props.mapKeys { (key, _) ->
            val normalizedKey = keyNormalizer.normalize(key)
            prefixes.find { normalizedKey.startsWith("$it.") }
