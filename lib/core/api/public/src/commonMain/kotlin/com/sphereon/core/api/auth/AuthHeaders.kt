...
data class AuthContext(
     ...
     */
    fun withScope(scope: String): AuthContext =
        copy(scopes = scopes + scope)

    /**
     * Creates a copy with additional policy context.
     */
    fun withPolicyContext(key: String, value: String): AuthContext =
        copy(policyContext = policyContext + (key to value))

    /**
     * Converts this context to headers for propagation.
     */
    /**
    ...
    fun toHeaders(): Map<String, String> = buildMap {
        ...
        if (policyContext.isNotEmpty()) {
            put(AuthHeaders.X_POLICY_CONTEXT, Json.encodeToString(
    ...
    companion object {
        ...
        fun fromHeaders(headers: Map<String, String>): AuthContext {
            ...
            val policyContext: Map<String, String> = if (policyContextJson != null) {
                try {
            ...
            val scopes = scopeHeader?.split(" ", ",")?.map { it.trim() }?.filter { it.isNotEmpty() }?.toSet() ?: emptySet()
 ...
 */
fun BinaryRequest.extractAuthContext(): AuthContext =
    AuthContext.fromHeaders(headers)

/**
 * Extension function to add auth context headers to a BinaryRequest.
 */
fun BinaryRequest.withAuthContext(authContext: AuthContext): BinaryRequest =
    withHeaders(authContext.toHeaders())
