...
class TenantContextDataImpl(override val tenantId: String) : TenantContextData {
...
class TenantResolutionHandlerImpl(tenantResolvers: Set<TenantResolver>) : TenantResolutionHandler {
    ...
    /**
     * Component interface to expose TenantResolutionHandler publicly from the AppComponent.
     */
...
class PrincipalResolutionHandlerImpl(principalResolvers: Set<PrincipalResolver>) : PrincipalResolutionHandler {
    val principalResolvers = principalResolvers.sorted()

    override fun resolvePrincipal(principalInput: PrincipalInput, tenantAware: TenantAware): PrincipalAware {
        val principal =
    ...
    /**
     * Component interface to expose PrincipalResolutionHandler publicly from the AppComponent.
     */
    @ContributesTo(AppScope::class)
    interface Component {
...
data class DefaultTenantInputString(override val tenant: String) : TenantInputString {
    override fun toString(): String {
        return "TenantInput(tenant='$tenant')"
    }
...
data class DefaultPrincipalInputString(override val principal: String) : PrincipalInputString {
    override fun toString(): String {
        return "PrincipalInput(principal='$principal')"
    }
...
data class HostTenantInput(val host: String) : TenantInput {
...
data class PathTenantInput(val path: String) : TenantInput {
...
class EmailDomainTenantResolver() : TenantResolver {
    ...
    override fun resolveTenant(tenantInput: TenantInput): String = tenantInput.asString().substringAfter("@").trim().lowercase()
}
...
class StaticPrincipalResolver() : PrincipalResolver {
    ...
    override fun resolvePrincipal(principalInput: PrincipalInput, tenant: TenantAware): String = principalInput.asString().trim()
}
...
class EmailPrincipalResolver() : PrincipalResolver {
    ...
    override fun resolvePrincipal(principalInput: PrincipalInput, tenant: TenantAware): String = principalInput.asString().trim().lowercase()
}
