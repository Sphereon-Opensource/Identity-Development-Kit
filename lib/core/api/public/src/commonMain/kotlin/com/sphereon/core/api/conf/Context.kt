...
enum class ConfigScope(scopeKey: String) {
    GLOBAL("global"),
    // PLUGIN,
    SERVICE("services")
}
enum class ConfigSource(key: String) {
    BUILD("build"),
    APP("app"),
    PROFILE("profile"),
    TENANT("tenant"),
 ...
 */
enum class ConfigLevel(val key: String, val level: Int) {
    APP("app", 10),
    TENANT("tenant", 20),
    ...
    companion object {
         ...
         */
        fun fromString(value: String): ConfigLevel? = entries.find {
...
data class ConfigContext(val scope: ConfigScope, val source: ConfigSource)

@OptIn(ExperimentalObjCName::class)
@ObjCName("ConfigContextHierarchy", exact = true)
@CoverageExcludedDataClass
data class ConfigContextHierarchy(val scope: ConfigScope, val sources: List<ConfigSource>)
