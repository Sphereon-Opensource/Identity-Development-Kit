...
class MultiLogService(
    ...
    LogService,
    MultiService<LogMessage, Unit, IdkErrorType> {
    override val policy: LogPolicy
        get() = logPolicy

    private val coroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.Default)

    val asyncDelegate: AsyncLogService =
        object : AsyncLogService {
            ...
            ): Boolean = this@MultiLogService.isEnabled(level = level, tag = tag, context = context)
            ...
            override suspend fun execute(args: LogMessage): IdkResult<Unit, IdkErrorType> = this@MultiLogService.execute(args)

            override fun toSync(): LogService = this@MultiLogService
        }

    private fun commandOrTag(
        message: LogMessage,
    ...
    private fun isEnabledForService(
        serviceId: String,
        level: LogLevel,
        commandOrTag: String?,
    ): Boolean =
    ...
    override suspend fun execute(args: LogMessage): IdkResult<Unit, IdkErrorType> {
        if (!isEnabled) {
            ...
            return LogError.NoLoggersConfigured
                .asError(
