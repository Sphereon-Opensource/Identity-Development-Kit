...
abstract class AbstractConfigEnvironment(
    ...
    private var _propertySourcesRevision: Long = 0L

    val propertyResolver: PropertyResolver
        get() {
            val currentRevision = structuralRevision()
            if (_propertyResolver == null || _propertySourcesRevision != currentRevision) {
                _propertyResolver = if (snapshotCache != null) {
                    CachingPropertySourcesPropertyResolver(
                        ...
                        ttlConfig = ttlConfig,
                        interpolator = interpolator ?: secretResolver?.let { DefaultPropertyInterpolator(secretResolver = it) }
                    )
                } else {
                    createResolver(getPropertySources(true), interpolator, secretResolver)
                }
                _propertySourcesRevision = currentRevision
            }
    ...
    override fun getConfigLocation(): Path {
        ...
        val location = Env.get(ENV_CONFIG_LOCATION)
    ...
    companion object {
        ...
        fun createResolver(
            ...
            val effectiveInterpolator = interpolator
    ...
    override fun <T : Any> getProperty(key: String, targetType: KClass<T>, defaultValue: T?): T? =
        propertyResolver.getProperty(key, targetType, defaultValue)

    override fun getPropertyAsString(key: String, defaultValue: String?): String? = propertyResolver.getPropertyAsString(key, defaultValue)
    override fun <T : Any> getRequiredProperty(key: String, targetType: KClass<T>, defaultValue: T?): T = propertyResolver.getRequiredProperty(key, targetType, defaultValue)
    override fun getRequiredPropertyAsString(key: String, defaultValue: String?): String = propertyResolver.getRequiredPropertyAsString(key, defaultValue)
    ...
    override fun getSubProperties(prefixes: Set<String>, stripPrefix: Boolean): Map<String, Any> = propertyResolver.getSubProperties(prefixes, stripPrefix)

    override fun getSubPropertiesAsString(prefixes: Set<String>, stripPrefix: Boolean, redact: Boolean): Map<String, String> = propertyResolver.getSubPropertiesAsString(prefixes, stripPrefix, redact)
