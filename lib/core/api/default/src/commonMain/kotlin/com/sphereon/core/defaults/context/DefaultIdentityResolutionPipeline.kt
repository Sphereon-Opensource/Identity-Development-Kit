...
class DefaultIdentityResolutionPipeline(
    ...
    internal fun detectPrincipalType(
        ...
        if (serviceId != null) return PrincipalType.SERVICE

        if (principalId == null && tokenClaims == null) return PrincipalType.ANONYMOUS
        ...
        if (tokenClaims != null) {
            ...
            if (grantType == "client_credentials") return PrincipalType.WORKLOAD
            ...
            if (sub != null) return PrincipalType.USER
        }

        // Has a principal but no token → could be header-based service identity
        if (principalId != null) return PrincipalType.SERVICE

        return PrincipalType.ANONYMOUS
    }

    private fun JsonElement.extractString(): String? = try {
