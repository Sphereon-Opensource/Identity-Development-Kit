 ...
 * Filters out NoOp stubs and builds an indexed catalog at startup.
 */
@Inject
...
class DefaultServiceCommandGroupCatalog(
    ...
    override val groups: List<ServiceCommandGroupDescription> = descriptorProviders
        .filter { it.groupId != NoOpServiceCommandGroupDescriptorProvider.ID }
        .map { it.describe() }
        .sortedBy { it.groupId }
    ...
    override fun findByModule(module: String): List<ServiceCommandGroupDescription> =
        groupsByModule[module] ?: emptyList()

    override fun findByGroupId(groupId: String): ServiceCommandGroupDescription? =
        groupsById[groupId]
