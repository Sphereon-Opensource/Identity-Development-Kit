...
class PromptCoordinatorImpl(
    ...
    override suspend fun <Req : PromptRequest, Res : PromptResponse> request(request: Req): PromptHandle<Req, Res> =
        mutex.withLock {
            // Create the handle
    ...
    @Suppress("UNCHECKED_CAST")
    override fun <Req : PromptRequest, Res : PromptResponse> getHandle(id: PromptId): PromptHandle<Req, Res>? {
        val handle = handles[id] ?: return null
        return if (handle.isActive) {
            handle as PromptHandle<Req, Res>
        } else {
            null
        }
    }

    override fun isActive(id: PromptId): Boolean = handles[id]?.isActive == true
}
