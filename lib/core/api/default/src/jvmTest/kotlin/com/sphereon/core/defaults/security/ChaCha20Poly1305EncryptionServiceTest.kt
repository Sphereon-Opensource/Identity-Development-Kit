...
/**
 * Pins the [ChaCha20Poly1305EncryptionService] contract: round-trip encrypt+decrypt recovers
 * the plaintext, AAD binding is enforced, an unknown key id surfaces as an error, and the
 * EncryptedBlob carries the active key id so a downstream rotation can route to the historical
 * key by id. Mirrors [AesGcmEncryptionServiceTest] so the two algorithms stay shape-compatible.
 *
 * JVM-only test (same placement as EDK's `Argon2idPasswordHasherTest`, the other
 * libsodium-backed component). The [ChaCha20Poly1305EncryptionService] impl is multiplatform
 * (commonMain, libsodium on JVM/JS/native), but the Kotlin/JS nodejs test runner executes the
 * raw `.mjs` through mocha without a webpack bundler, and the ionspin libsodium binding's
 * generated named imports from the emscripten `libsodium-sumo` module (`export default` only)
 * can only be satisfied by a bundler's CJS interop. ESM support is an open upstream item
 * (ionspin/kotlin-multiplatform-libsodium#70). wasmJs has no libsodium artifact at all and
 * falls back to AES-256-GCM ([AesGcmEncryptionService]).
 */
class ChaCha20Poly1305EncryptionServiceTest {
    /** Fixed 32-byte key carrying the C20P algorithm so tests are deterministic. */
    private val testKey =
    ...
    private fun service(provider: EncryptionKeyProvider): ChaCha20Poly1305EncryptionService = ChaCha20Poly1305EncryptionService(provider)
    ...
    fun encryptedBlobCarriesActiveKeyIdAndAlgorithm() =
        runTest {
            ...
            assertEquals(EncryptedBlob.ALG_CHACHA20_POLY1305, blob.algorithm)
            assertEquals(12, blob.iv.size, "ChaCha20-Poly1305 nonce must be 12 bytes")
            assertEquals(16, blob.authTag.size, "Poly1305 auth tag must be 16 bytes")
        }
    ...
    fun decryptingWithDifferentAadFails() =
        runTest {
            ...
            // tenant-B). The Poly1305 auth-tag check fails → decrypt returns Err.
    ...
    @Test
    fun decryptingWrongAlgorithmBlobFails() =
        runTest {
            // This service handles C20P only: an AES-GCM blob handed to it must be rejected
            // rather than silently mis-decrypted under the wrong primitive.
            val svc = service(fixedKeyProvider())
            val blob = svc.encrypt("payload".encodeToByteArray()).value
            val foreign = blob.copy(algorithm = EncryptedBlob.ALG_AES_256_GCM)
            assertTrue(svc.decrypt(foreign).isErr, "decrypt must reject a non-C20P blob")
        }

    @Test
    fun encryptingWithMismatchedKeyAlgorithmFails() =
        runTest {
            // Guard: the active key advertises AES-GCM but is wired to the ChaCha service.
            // Encrypt must fail rather than emit a C20P blob under an A256GCM-tagged key.
            val aesKey = testKey.copy(algorithm = EncryptedBlob.ALG_AES_256_GCM)
            val svc = service(fixedKeyProvider(aesKey))
            assertTrue(svc.encrypt("payload".encodeToByteArray()).isErr, "encrypt must fail on key/algorithm mismatch")
        }

    @Test
    fun ciphertextDiffersAcrossEncryptsForSamePlaintext() =
        runTest {
            // Fresh nonce per encrypt → distinct ciphertexts even for the same plaintext.
            // Catches a regression where nonce generation might be deterministic (which would
            // be catastrophic for ChaCha20-Poly1305 nonce reuse).
            val svc = service(fixedKeyProvider())
            val pt = "same".encodeToByteArray()
            val a = svc.encrypt(pt).value
            val b = svc.encrypt(pt).value
            assertTrue(!a.iv.contentEquals(b.iv), "nonce must differ across encrypts of the same plaintext")
    ...
    fun commandsAndFacadeProduceIdenticalResults() =
        runTest {
            ...
            // shape as calling the facade directly.
            val svc = service(fixedKeyProvider())
            ...
            val viaCommand =
                svc.commands.encrypt
                    .execute(
                        com.sphereon.core.api.security
            ...
            // nonces differ (each path generates its own); only the SHAPE matches.
    ...
    fun rotatesByLookingUpHistoricalKeyForOlderBlob() =
        runTest {
            // Rotation scenario: a row was encrypted under key k1, then the deployment rotated
            // the active key to k2. Decrypting an old blob must look up k1 by id (not assume the
            // active key) — proves the EncryptedBlob.keyId path works.
            val k1 = EncryptionKeyMaterial(keyId = "k1", keyBytes = ByteArray(32) { 1 }, algorithm = EncryptedBlob.ALG_CHACHA20_POLY1305)
            val k2 = EncryptionKeyMaterial(keyId = "k2", keyBytes = ByteArray(32) { 2 }, algorithm = EncryptedBlob.ALG_CHACHA20_POLY1305)
