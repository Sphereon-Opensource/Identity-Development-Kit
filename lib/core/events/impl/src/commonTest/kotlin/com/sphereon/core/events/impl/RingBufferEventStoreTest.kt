...
class RingBufferEventStoreTest {
    ...
    @Test
    fun testQueryWithFilter() = runTest {
        val store = RingBufferEventStore()

        // Store events with different categories
        store.store(DefaultEvent(
            id = Uuid.random(),
            type = EventTypes.COMMAND_COMPLETED,
            origin = "test",
            timestamp = Clock.System.now(),
    ...
    @Test
    fun testQueryWithLimit() = runTest {
        val store = RingBufferEventStore()
    ...
    @Test
    fun testQueryWithOffset() = runTest {
        val store = RingBufferEventStore()
    ...
    @Test
    fun testCapacityEviction() = runTest {
    ...
    @Test
    fun testNoneFilterMatchesNothing() = runTest {
        val store = RingBufferEventStore()
