...
class EventHubImpl : EventHub {
    ...
    override fun filteredEvents(filter: EventFilter): Flow<Event> = events.filter { filter.matches(it) }

    override fun eventsByTypePattern(pattern: String): Flow<Event> = events.filter { it.type.matches(pattern) }

    override fun eventsBySubsystem(subsystems: Set<EventSubsystem>): Flow<Event> = events.filter { it.subsystem in subsystems }

    override fun eventsByCategory(categories: Set<EventCategory>): Flow<Event> = events.filter { it.category in categories }
}
