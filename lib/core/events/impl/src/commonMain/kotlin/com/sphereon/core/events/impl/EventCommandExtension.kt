...
class EventCommandExtension<Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType>(
    ...
    override suspend fun beforeExecute(
        ...
        if (service is SilentCommand<*, *, *>) return BeforeExecuteResult.Continue(args)

        // Skip if not configured to emit for this command
        if (!config.shouldEmitStart(service.id)) return BeforeExecuteResult.Continue(args)
    ...
    override suspend fun afterExecute(
        ...
        // Skip silent commands (event system commands)
        if (service is SilentCommand<*, *, *>) return result

        // Calculate duration
        val startTime = startTimes.remove(service.id)
        val durationMs = if (startTime != null) {
        ...
        val (eventType, shouldEmit) = if (result.isOk) {
        ...
        scope.launch {
            ...
            val event = eventService.eventBuilder()
                ...
                .origin(service.id)
                .subsystem(service.subsystem)
                .category(if (result.isOk) EventCategories.LIFECYCLE else EventCategories.ERROR)
    ...
    private suspend fun emitEvent(eventService: SessionEventService, event: Event) {
        val encryptParts = if (config.encryptPayload) {
    ...
    companion object {
        ...
        ): EventCommandExtension<Arg, SuccessResult, ErrorResult> =
            EventCommandExtension(eventServiceProvider, scope = scope)
        ...
        fun <Arg : Any, SuccessResult : Any, ErrorResult : IdkErrorType> create(
            ...
            EventCommandExtension(eventServiceProvider, config, scope)
    }
}
