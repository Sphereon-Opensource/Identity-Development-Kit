...
class RingBufferEventStore(
    ...
    override suspend fun storeBatch(events: List<Event>): IdkResult<List<Event>, IdkError> =
        mutex.withLock {
    ...
    override suspend fun getById(id: Uuid): IdkResult<Event?, IdkError> =
        mutex.withLock {
            Ok(buffer.find { it.id == id })
        }

    override suspend fun count(filter: EventFilter): IdkResult<Long, IdkError> =
        mutex.withLock {
            Ok(buffer.count { filter.matches(it) }.toLong())
        }

    override suspend fun getRecent(count: Int): IdkResult<List<Event>, IdkError> =
        mutex.withLock {
            Ok(buffer.takeLast(count).reversed())
        }

    override suspend fun deleteOlderThan(olderThanMillis: Long): IdkResult<Long, IdkError> =
        mutex.withLock {
    ...
    override suspend fun clear(): IdkResult<Long, IdkError> =
        mutex.withLock {
    ...
    override suspend fun size(): IdkResult<Long, IdkError> =
        mutex.withLock {
