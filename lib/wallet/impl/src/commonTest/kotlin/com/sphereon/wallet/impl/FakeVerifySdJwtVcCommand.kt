...
/**
 * Test double for the SD-JWT VC verification command the wallet runs on receipt.
 *
 * The wallet calls this for every issued SD-JWT before storing it; the real command
 * resolves the issuer key from the JWS header `kid` (did:jwk) and checks the signature.
 * [accept] toggles between the success and rejection outcomes so tests can assert that the
 * wallet stores a credential when verification passes and refuses one when it fails.
 */
