...
interface ConfigurationStore<K : Any, V : Any> : Oid4vpStore<K, V> {
     ...
     */
    suspend fun putPersistent(key: K, value: V): IdkResult<StoreMetadata, IdkError>

    /**
     * Upsert a configuration persistently (no expiration).
     */
    suspend fun upsert(key: K, value: V): IdkResult<StoreMetadata, IdkError> = putPersistent(key, value)
