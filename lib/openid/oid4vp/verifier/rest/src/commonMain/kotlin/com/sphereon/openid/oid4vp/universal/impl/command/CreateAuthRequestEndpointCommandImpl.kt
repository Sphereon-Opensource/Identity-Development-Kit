...
@StringKey(CreateAuthRequestEndpointCommand.COMMAND_ID)
class CreateAuthRequestEndpointCommandImpl(
    execution: SessionExecution,
    private val createAuthRequestServiceCommand: CreateAuthRequestServiceCommand,
    ...
    override suspend fun doExecute(
        ...
        val input = request.requireJsonBody<CreateAuthorizationRequestInput>(json).getOrElse { return Err(it) }

        val output =
            createAuthRequestServiceCommand.execute(input).getOrElse { error ->
                return Err(error)
            }
