...
class ParseAuthorizationResponseCommandImplTest {
    ...
    @Test
    fun `test parse single query vp_token response`() = runTest {
        ...
        val args = ParseAuthorizationResponseArgs(
        ...
        assertEquals("eyJhbGciOiJFUzI1NiIsInR5cCI6InZjK3NkLWp3dCJ9.payload.signature",
            parsed.vpToken.getSinglePresentation("driver_license_query"))
        assertEquals("state123", parsed.state)
    }

    @Test
    fun `test parse multiple query vp_token response`() = runTest {
        ...
        assertEquals("eyJhbGciOiJFUzI1NiJ9.payload1.sig1", 
            parsed.vpToken.getSinglePresentation("driver_license_query"))
        assertEquals("eyJhbGciOiJFUzI1NiJ9.payload2.sig2", 
            parsed.vpToken.getSinglePresentation("employment_query"))
        assertEquals("state456", parsed.state)
    }

    @Test
    fun `test parse vp_token with multiple presentations per query`() = runTest {
    ...
    @Test
    fun `test parse response without state`() = runTest {
    ...
    @Test
    fun `test parse error response`() = runTest {
        ...
        assertIs<Err<*>>(result)
        assertTrue(result.error.message.defaultMessage.contains("invalid_request"))
    }

    @Test
    fun `test parse response with missing vp_token`() = runTest {
        val args = ParseAuthorizationResponseArgs(
            responseParams = mapOf(
                "state" to "state123"
                // Missing vp_token
    ...
    @Test
    fun `test state validation with original request`() = runTest {
        ...
        assertIs<Err<*>>(result)
        assertTrue(result.error.message.defaultMessage.contains("State mismatch"))
    }

    @Test
    fun `test state validation with matching state`() = runTest {
        val originalRequest = AuthorizationRequest(
            clientId = "https://verifier.example.com",
            redirectUri = "https://verifier.example.com/callback",
    ...
    @Test
    fun `test parse SD-JWT with disclosures`() = runTest {
    ...
    @Test
    fun `test reject non-object vp_token`() = runTest {
