...
internal class TestAuthorizationSessionStore : AuthorizationSessionStore {
    ...
    override suspend fun createSession(
        ...
        return Ok(
            AuthorizationSession(
                ...
                authorizationRequest = com.sphereon.oauth2.common.model.AuthorizationRequest(
    ...
    override suspend fun updateStatus(
        ...
        val existing = entries[correlationId] ?: return Ok(
            AuthorizationSession(
                sessionId = correlationId,
                correlationId = correlationId,
                dcqlQuery = com.sphereon.openid.oid4vp.dcql.DcqlQuery(credentials = emptyList()),
                authorizationRequest = com.sphereon.oauth2.common.model.AuthorizationRequest(
                    clientId = "test",
                    redirectUri = "test"
                ),
    ...
    override suspend fun storeResponse(correlationId: String, parsedResponse: ParsedAuthorizationResponse): IdkResult<AuthorizationSession, IdkError> {
        val existing = entries[correlationId] ?: return Ok(
            AuthorizationSession(
                sessionId = correlationId,
                correlationId = correlationId,
        ...
        val updated = existing.value.copy(
    ...
    override suspend fun storeValidationResult(correlationId: String, validationResult: ValidationResult): IdkResult<AuthorizationSession, IdkError> {
        val existing = entries[correlationId] ?: return Ok(
            AuthorizationSession(
                sessionId = correlationId,
                correlationId = correlationId,
    ...
    override suspend fun touch(key: String, ttlSeconds: Long): IdkResult<Boolean, IdkError> = Ok(entries.containsKey(key))
