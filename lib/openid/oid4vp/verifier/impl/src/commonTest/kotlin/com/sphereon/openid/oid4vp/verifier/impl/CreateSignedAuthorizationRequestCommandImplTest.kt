...
class CreateSignedAuthorizationRequestCommandImplTest {
    ...
    private fun createTestDcqlQuery(): DcqlQuery = DcqlQuery(
        credentials = listOf(
            DcqlCredentialQuery(
    ...
    private fun createCommand(
        mockJarCommand: MockCreateSignedJarCommand = MockCreateSignedJarCommand()
    ): CreateSignedAuthorizationRequestCommandImpl {
        val createAuthRequestCommand = CreateAuthorizationRequestCommandImpl(
    ...
    @Test
    fun `test create signed authorization request success`() = runTest {
        ...
        val args = CreateSignedAuthorizationRequestArgs(
    ...
    @Test
    fun `test signed request includes DCQL query`() = runTest {
        val mockJarCommand = MockCreateSignedJarCommand()
        val command = createCommand(mockJarCommand)
    ...
    @Test
    fun `test JAR signing failure propagates error`() = runTest {
        val mockJarCommand = MockCreateSignedJarCommand(
            shouldFail = true,
        ...
        assertIs<Err<*>>(result)
        assertTrue(result.error.message.defaultMessage.contains("JAR"))
    }

    @Test
    fun `test invalid request args propagates validation error`() = runTest {
        val mockJarCommand = MockCreateSignedJarCommand()
        val command = createCommand(mockJarCommand)
    ...
    @Test
    fun `test custom expiration time is passed to JAR`() = runTest {
        val mockJarCommand = MockCreateSignedJarCommand()
        val command = createCommand(mockJarCommand)
    ...
    @Test
    fun `test default expiration time is 300 seconds`() = runTest {
        val mockJarCommand = MockCreateSignedJarCommand()
        val command = createCommand(mockJarCommand)
    ...
    @Test
    fun `test client_id is used as JWT issuer`() = runTest {
        val mockJarCommand = MockCreateSignedJarCommand()
        val command = createCommand(mockJarCommand)
    ...
    @Test
    fun `test fragment response mode with signed request`() = runTest {
        val mockJarCommand = MockCreateSignedJarCommand()
        val command = createCommand(mockJarCommand)
