...
class ParseAuthorizationResponseCommandImpl(
...
), ParseAuthorizationResponseCommand, ParseAuthorizationResponseCommandService {
    ...
    override suspend fun doExecute(
        ...
        if (error != null) {
            ...
            val errorMessage = buildString {
                ...
                if (errorUri != null) append(" (see: $errorUri)")
            }
    ...
    private suspend fun parseJarmResponse(
        ...
        val verifyArgs = VerifyJarmResponseArgs(
            jarmJwt = jarmJwt,
        ...
        val jarmResult = verifyJarmCommand.execute(verifyArgs).getOrElse { error ->
        ...
        val vpTokenRaw = responseParams["vp_token"]?.let {
            when (it) {
                is kotlinx.serialization.json.JsonPrimitive -> it.content
                is kotlinx.serialization.json.JsonArray -> Json.encodeToString(kotlinx.serialization.json.JsonArray.serializer(), it)
                else -> it.toString()
            }
        ...
        val vpToken = parseVpToken(vpTokenRaw)
            ?: return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
        ...
        return Ok(ParsedAuthorizationResponse(
            vpToken = vpToken,
            state = payload.state,
     ...
     */
    private fun parseStandardResponse(
        args: ParseAuthorizationResponseArgs
    ): IdkResult<ParsedAuthorizationResponse, IdkError> {
        val responseParams = args.responseParams

        // Extract vp_token (REQUIRED)
        val vpTokenRaw = responseParams["vp_token"]
            ?: return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
    ...
    private fun parseVpToken(vpTokenRaw: String): VpToken? {
        return try {
            ...
            log.error("Failed to parse vp_token: ${e.message}")
            null
        }
    }
}
