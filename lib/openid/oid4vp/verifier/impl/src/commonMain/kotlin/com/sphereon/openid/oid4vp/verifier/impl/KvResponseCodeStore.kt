...
class KvResponseCodeStore(
    ...
    override suspend fun createResponseCode(
        ...
        return kv.put(namespace, responseCode, entry, ttlSeconds.seconds).map {
    ...
    override suspend fun put(key: String, value: StoredAuthorizationResponse, ttlSeconds: Long): IdkResult<StoreMetadata, IdkError> {
        val now = clock.now().toEpochMilliseconds()
        val expiresAt = now + (ttlSeconds * 1000)
        ...
        return kv.put(namespace, key, entry, ttlSeconds.seconds).map {
    ...
    override suspend fun get(key: String): IdkResult<StoredAuthorizationResponse?, IdkError> {
        return kv.get(namespace, key).map { entry ->
    ...
    override suspend fun getAndConsume(key: String, consume: Boolean): IdkResult<StoredAuthorizationResponse, IdkError> {
        return mutex.withLock {
            val now = clock.now().toEpochMilliseconds()

            val entry = kv.get(namespace, key).getOrElse { e ->
                return@withLock Err(
                    IdkError.fromString(
            ...
            val updatedEntry = if (consume) entry.copy(used = true) else entry
            if (consume) {
    ...
    override suspend fun isConsumed(key: String): IdkResult<Boolean, IdkError> {
        return kv.get(namespace, key).map { it?.used ?: false }.mapError { e ->
            IdkError.fromString(
    ...
    override suspend fun touch(key: String, ttlSeconds: Long): IdkResult<Boolean, IdkError> {
        return mutex.withLock {
            ...
            val updated = entry.copy(expiresAt = now + (ttlSeconds * 1000))
    ...
    private fun ResponseCodeEntry.toStoredAuthorizationResponse(responseCode: String): StoredAuthorizationResponse {
        val vpToken = parseVpToken(rawVpToken)
            ?: throw IllegalArgumentException("Invalid vp_token format")

        val jarmModeEnum: JarmMode? = jarmMode?.let {
            try {
        ...
        val parsed = ParsedAuthorizationResponse(
            vpToken = vpToken,
            state = state,
    ...
    private fun ByteArray.encodeBase64Url(): String {
        ...
        while (i < size) {
            ...
            if (i >= size) {
                result.append(base64Chars[(b1 and 0x03) shl 4])
                break
            }
            ...
            if (i >= size) {
                result.append(base64Chars[(b2 and 0x0F) shl 2])
                break
            }
