...
class KvClientMetadataConfigurationStore(
    ...
    private val indexNamespace = KvNamespace(
    ...
    private val clientIdIndexNamespace = KvNamespace(
    ...
    internal data class ClientMetadataConfigEntry(
        ...
        fun toPublic(): ClientMetadataConfiguration = ClientMetadataConfiguration(
    ...
    override suspend fun putPersistent(key: String, value: ClientMetadataConfiguration): IdkResult<StoreMetadata, IdkError> {
        val now = clock.now().toEpochMilliseconds()
        val entry = ClientMetadataConfigEntry(
            ...
            createdAt = if (value.createdAt > 0) value.createdAt else now,
            updatedAt = now
        )

        return mutex.withLock {
            val putResult = kv.put(namespace, key, entry, Duration.INFINITE).getOrElse { e ->
                return@withLock Err(
                    IdkError.fromString(
    ...
    override suspend fun put(key: String, value: ClientMetadataConfiguration, ttlSeconds: Long): IdkResult<StoreMetadata, IdkError> {
        val now = clock.now().toEpochMilliseconds()
        val entry = ClientMetadataConfigEntry(
            clientMetadataId = value.clientMetadataId,
            clientId = value.clientId,
        ...
        return mutex.withLock {
            val putResult = kv.put(namespace, key, entry, ttlSeconds.seconds).getOrElse { e ->
                return@withLock Err(
                    IdkError.fromString(
                        message = "Failed to store client metadata configuration: ${e.message}",
                        exception = IllegalStateException(e.toString()),
    ...
    override suspend fun get(key: String): IdkResult<ClientMetadataConfiguration?, IdkError> {
        return kv.get(namespace, key).map { it?.toPublic() }.mapError { e ->
    ...
    override suspend fun getEntry(key: String): IdkResult<com.sphereon.openid.oid4vp.verifier.store.StoredEntry<ClientMetadataConfiguration>?, IdkError> {
        return kv.getEntry(namespace, key).map { entry ->
            ...
            IdkError.fromString(message = "Failed to read client metadata configuration entry: ${e.message}", exception = IllegalStateException(e.toString()), code = "OID4VP_CLIENT_METADATA_STORE_ERROR")
        }
    }

    override suspend fun delete(key: String): IdkResult<Boolean, IdkError> {
        return mutex.withLock {
            val existing = kv.get(namespace, key).getOrElse { e ->
                return@withLock Err(
                    IdkError.fromString(
            ...
            val deleted = kv.delete(namespace, key).getOrElse { e ->
                return@withLock Err(
                    IdkError.fromString(
    ...
    override suspend fun exists(key: String): IdkResult<Boolean, IdkError> {
        return kv.exists(namespace, key).mapError { e ->
            IdkError.fromString(message = "Failed to check client metadata configuration existence: ${e.message}", exception = IllegalStateException(e.toString()), code = "OID4VP_CLIENT_METADATA_STORE_ERROR")
        }
    }
    ...
    override suspend fun cleanupExpired(): IdkResult<Int, IdkError> {
        return kv.cleanupExpired(namespace).mapError { e ->
            IdkError.fromString(message = "Failed to cleanup expired client metadata configurations: ${e.message}", exception = IllegalStateException(e.toString()), code = "OID4VP_CLIENT_METADATA_STORE_ERROR")
        }
    }
    ...
    override suspend fun getByClientId(clientId: String): IdkResult<ClientMetadataConfiguration?, IdkError> {
        return kv.get(clientIdIndexNamespace, clientId).flatMap { clientMetadataId ->
            if (clientMetadataId == null) return@flatMap Ok(null)
            get(clientMetadataId)
        }.mapError { e ->
            IdkError.fromString(message = "Failed to resolve client metadata by clientId: ${e.message}", exception = IllegalStateException(e.toString()), code = "OID4VP_CLIENT_METADATA_INDEX_ERROR")
        }
    }

    private suspend fun getIndex(): IdkResult<Set<String>, IdkError> {
        return kv.get(indexNamespace, indexKey).map { it ?: emptySet() }.mapError { e ->
            IdkError.fromString(message = "Failed to read client metadata configuration index: ${e.message}", exception = IllegalStateException(e.toString()), code = "OID4VP_CLIENT_METADATA_INDEX_ERROR")
        }
    }

    private suspend fun putIndex(ids: Set<String>): IdkResult<Unit, IdkError> {
        return kv.put(indexNamespace, indexKey, ids, Duration.INFINITE).map { }.mapError { e ->
            IdkError.fromString(message = "Failed to write client metadata configuration index: ${e.message}", exception = IllegalStateException(e.toString()), code = "OID4VP_CLIENT_METADATA_INDEX_ERROR")
        }
    }
