...
class ValidateAuthorizationResponseCommandImpl(
...
), ValidateAuthorizationResponseCommand, ValidateAuthorizationResponseCommandService {
    ...
    override suspend fun doExecute(
        ...
        for ((queryId, presentationList) in vpToken.presentations) {
            ...
            for ((presentationIndex, presentation) in presentationList.withIndex()) {
                ...
                val formatMatches = queryFormat == null ||
        ...
        val requiredCredentials = dcqlQuery.credentials?.filter { query ->
        ...
        dcqlQuery.credential_sets?.filter { it.required }?.forEach { credentialSet ->
            val anySatisfied = credentialSet.options.any { option ->
     ...
     */
    private fun extractDisclosedClaims(presentation: String, format: CredentialFormat): Map<String, Any?> {
        return when (format) {
