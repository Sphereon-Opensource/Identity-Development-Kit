...
class CreateAuthorizationRequestCommandImpl(
...
), CreateAuthorizationRequestCommand, CreateAuthorizationRequestCommandService {
    ...
    override suspend fun doExecute(
        ...
        val dcqlQueryJson = serializeDcqlQuery(processedArgs.dcqlQuery)
        ...
        val isDirectPost = processedArgs.responseMode == ResponseMode.DIRECT_POST ||
            processedArgs.responseMode == ResponseMode.DIRECT_POST_JWT
        val redirectUri = if (isDirectPost && processedArgs.responseUri != null) {
        ...
        val request = buildOid4vpAuthorizationRequest(
            ...
            // Client ID scheme (only set if not PRE_REGISTERED)
        ...
        log.info("Created authorization request with client_id: ${processedArgs.clientId}, " +
        ...
        return Ok(CreatedAuthorizationRequest(
            request = request,
    ...
    private fun validateArgs(args: CreateAuthorizationRequestArgs): IdkError? {
        ...
        // Validate nonce length (minimum 8 characters per best practices)
        if (args.nonce.length < 8) {
            return IdkError.ILLEGAL_ARGUMENT_ERROR(
        ...
        if ((args.responseMode == ResponseMode.DIRECT_POST ||
        ...
        if ((args.responseMode == ResponseMode.FRAGMENT ||
    ...
    /**
     * Generate a unique session ID for response correlation
     */
    private fun generateSessionId(): String {
        // Use a combination of timestamp and random bytes for uniqueness
        val timestamp = kotlin.time.TimeSource.Monotonic.markNow().hashCode()
        val random = kotlin.random.Random.nextInt()
        return "${timestamp.toUInt().toString(16)}-${random.toUInt().toString(16)}"
    }
}
