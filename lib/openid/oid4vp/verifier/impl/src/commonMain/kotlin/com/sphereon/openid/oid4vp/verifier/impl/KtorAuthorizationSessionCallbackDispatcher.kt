...
class KtorAuthorizationSessionCallbackDispatcher(
    ...
    override suspend fun dispatch(url: String, update: AuthorizationSessionStatusUpdate): IdkResult<Unit, IdkError> {
        return try {
            client.post(url) {
            ...
            Err(IdkError.fromString(message = "Failed to dispatch authorization session callback: ${e.message}", exception = e))
        }
    }
}
