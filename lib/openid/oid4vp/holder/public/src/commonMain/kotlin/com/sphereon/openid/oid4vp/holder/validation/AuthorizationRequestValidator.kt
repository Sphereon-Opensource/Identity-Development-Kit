 ...
 */
val validateOid4vpAuthorizationRequest = Validation<AuthorizationRequest> {
    ...
    run {
        constrain("valid client_id_scheme") { req ->
            val clientIdScheme = req.additionalParameters?.get("client_id_scheme")?.toString()?.removeSurrounding("\"")
            clientIdScheme == null || clientIdScheme in listOf(
                "pre-registered",
                "redirect_uri",
                "entity_id",
                "did",
