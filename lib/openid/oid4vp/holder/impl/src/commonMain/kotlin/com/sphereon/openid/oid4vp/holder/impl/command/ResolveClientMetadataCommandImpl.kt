...
class ResolveClientMetadataCommandImpl(
...
), ResolveClientMetadataCommand, ResolveClientMetadataCommandService {
    ...
    override suspend fun resolveClientMetadata(request: AuthorizationRequest): IdkResult<ResolvedClientMetadata, IdkError> {
    ...
    override suspend fun doExecute(
        ...
        request.additionalParameters["client_metadata_uri"]?.let { uriElement ->
            val metadataUri = try {
                ...
                return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
                    message = "Failed to parse client_metadata_uri: ${e.message}"
                ))
            }
        ...
        request.additionalParameters["client_metadata"]?.let { metadataJson ->
            return try {
                ...
                val normalized: JsonElement = when (metadataJson) {
                    is JsonPrimitive -> {
                ...
                Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
                    message = "Failed to parse embedded client_metadata: ${e.message}"
                ))
            }
        }
...
//                    engine = HttpClientEngineType.CIO,
            ...
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to fetch client metadata from $uri: ${e.message}"
            ))
        }
    }
}
