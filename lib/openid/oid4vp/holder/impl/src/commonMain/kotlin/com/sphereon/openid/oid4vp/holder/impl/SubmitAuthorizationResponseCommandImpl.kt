...
class SubmitAuthorizationResponseCommandImpl(
...
), SubmitAuthorizationResponseCommand, SubmitAuthorizationResponseCommandService {
    ...
    override suspend fun submitAuthorizationResponse(
        ...
        return execute(SubmitAuthorizationResponseArgs(resolvedRequest, response, responseMode))
    }
    ...
    override suspend fun doExecute(
        ...
        val effectiveResponseMode = processedArgs.responseMode
        ...
        return when (effectiveResponseMode) {
            ResponseMode.DIRECT_POST -> submitDirectPost(resolvedRequest, response)
            ResponseMode.DIRECT_POST_JWT -> submitDirectPostJwt(processedArgs)
            ResponseMode.FRAGMENT -> buildRedirectResponse(resolvedRequest, response, "#")
            ResponseMode.QUERY -> buildRedirectResponse(resolvedRequest, response, "?")
        }
    }
    ...
    private suspend fun submitDirectPost(
        ...
        val vpTokenElement = resolveVpTokenElement(response)
            ?: return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
        ...
        return try {
            ...
            if (httpResponse.status.isSuccess()) {
                ...
                val responseBody = try {
                    ...
                    log.debug("Could not parse response body as JSON: ${e.message}")
                    null
                }

                val redirectUri = responseBody?.get("redirect_uri")?.jsonPrimitive?.content

                log.info("Authorization response submitted successfully" +
                    if (redirectUri != null) ", redirect_uri received" else ""
                )
                ...
                val error = errorBody?.get("error")?.jsonPrimitive?.content
                    ?: "http_error_${httpResponse.status.value}"
                val errorDescription = errorBody?.get("error_description")?.jsonPrimitive?.content
            ...
            Err(IdkError.fromString(
                message = "Failed to submit authorization response: ${e.message}",
    ...
    private suspend fun submitDirectPostJwt(
        ...
        val jarmConfig = jarmOptions.jarmConfig ?: deriveJarmConfigFromClientMetadata(resolvedRequest)
            ?: return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
        ...
        val encryptionRecipient = if (jarmConfig.mode == JarmMode.ENCRYPTED || 
        ...
        val jarmArgs = CreateJarmResponseArgs(
        ...
        val jarmResult = createJarmCommand.execute(jarmArgs).getOrElse { error ->
        ...
        return try {
            ...
            if (httpResponse.status.isSuccess()) {
                ...
                log.info("JARM authorization response submitted successfully" +
                    if (redirectUri != null) ", redirect_uri received" else ""
                )
            ...
            Err(IdkError.fromString(
                message = "Failed to submit JARM authorization response: ${e.message}",
    ...
    private fun deriveJarmConfigFromClientMetadata(resolvedRequest: ResolvedOid4vpRequest): JarmConfig? {
        ...
        return when {
            // Sign + Encrypt mode
            signedAlg != null && encryptedAlg != null -> JarmConfig.signedEncrypted(
            ...
            // Sign only mode
            signedAlg != null -> JarmConfig.signed(signedAlg)
            // Encrypt only mode
            encryptedAlg != null -> JarmConfig.encrypted(
                keyEncryptionAlg = encryptedAlg,
                contentEncryptionAlg = encryptedEnc ?: JarmConfig.DEFAULT_CONTENT_ENCRYPTION_ALG
            )
            // No JARM parameters
    ...
    private suspend fun resolveEncryptionRecipient(
        ...
        val clientMetadata = resolvedRequest.clientMetadata
        ...
        val selectedKeyInfo = when {
            embeddedEncKey != null -> ResolvedKeyInfo.fromKey(embeddedEncKey)
            else -> {
                ...
                val resolved = externalIdentifierService.resolve(
                ...
                val selectedEnc = resolved.jwks.firstOrNull { keyInfo -> isEncKey(keyInfo.key) }
    ...
    private fun buildRedirectResponse(
        ...
        val fullRedirectUri = buildString {
            append(redirectUri)
            ...
            append("vp_token=")
            append(vpTokenJson.encodeUrlComponent())

            presentationSubmissionElement?.let {
                append("&presentation_submission=")
                append(encodeJsonElement(it).encodeUrlComponent())
            }
            response.state?.let {
                append("&state=")
        ...
        val modeName = if (separator == "#") "fragment" else "query"
    ...
    private fun encodeJsonElement(element: JsonElement): String {
