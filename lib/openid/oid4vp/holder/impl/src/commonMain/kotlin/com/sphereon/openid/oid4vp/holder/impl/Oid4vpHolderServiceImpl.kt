...
class Oid4vpHolderServiceImpl(
    ...
    inner class CommandsImpl : Oid4vpHolder.Commands {
    ...
    override val commands: Oid4vpHolder.Commands = CommandsImpl()
    ...
    override suspend fun parseAuthorizationRequest(requestUri: String, walletConfig: WalletConfig?): IdkResult<AuthorizationRequest, IdkError> {
        return parseAuthorizationRequestCommand.execute(ParseAuthorizationRequestArgs(requestUri, walletConfig))
    }

    override suspend fun resolveAuthorizationRequest(request: AuthorizationRequest): IdkResult<ResolvedOid4vpRequest, IdkError> {
        return resolveAuthorizationRequestCommand.execute(request)
    }
    ...
    override suspend fun createAuthorizationResponse(
        ...
        return createAuthorizationResponseCommand.execute(CreateAuthorizationResponseArgs(request, selectedCredentials))
    }
    ...
    override suspend fun submitAuthorizationResponse(
        ...
        return submitAuthorizationResponseCommand.execute(SubmitAuthorizationResponseArgs(resolvedRequest, response, responseMode))
    }
}
