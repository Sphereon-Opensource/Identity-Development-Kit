...
class CreateAuthorizationResponseCommandImplTest {
    ...
    @Test
    fun `test create response with single credential`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
        ...
        assertEquals("eyJhbGciOiJFUzI1NiJ9.eyJzdWIiOiIxMjM0In0.signature", 
    ...
    @Test
    fun `test create response with multiple credentials for different queries`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
            execution = TestExecutionContext.createExecution()
        )
        ...
        assertEquals("eyJhbGciOiJFUzI1NiJ9.eyJzdWIiOiIxMjM0In0.sig1", 
            vpToken.getSinglePresentation("driver_license_query"))
        assertEquals("eyJhbGciOiJFUzI1NiJ9.eyJzdWIiOiI1Njc4In0.sig2", 
            vpToken.getSinglePresentation("employment_query"))
    }

    @Test
    fun `test create response with multiple presentations for same query`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
            execution = TestExecutionContext.createExecution()
        )
    ...
    @Test
    fun `test create response with empty credentials list fails`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
            execution = TestExecutionContext.createExecution()
        )
        ...
        assertIs<Err<*>>(result)
        assertTrue(result.error.message.defaultMessage.contains("at least one credential", ignoreCase = true))
    }

    @Test
    fun `test create response with mdoc credential`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
            execution = TestExecutionContext.createExecution()
        )
        ...
        val vpToken = response.vpToken
        assertNotNull(vpToken)
        assertEquals("omlpc3N1ZXJBdXRoompkb2NUeXBlrW9yZy5pc28uMTgwMTM...", 
            vpToken.getSinglePresentation("mdl_query"))
    }

    @Test
    fun `test create response without state`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
            execution = TestExecutionContext.createExecution()
        )
    ...
    @Test
    fun `test create response with SD-JWT presentation`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
            execution = TestExecutionContext.createExecution()
        )
        ...
        val sdJwtPresentation = "eyJhbGciOiJFUzI1NiJ9.eyJzdWIiOiIxMjM0In0.sig~" +
    ...
    @Test
    fun `test create response with mixed credential formats`() = runTest {
        val command = CreateAuthorizationResponseCommandImpl(
            execution = TestExecutionContext.createExecution()
        )
