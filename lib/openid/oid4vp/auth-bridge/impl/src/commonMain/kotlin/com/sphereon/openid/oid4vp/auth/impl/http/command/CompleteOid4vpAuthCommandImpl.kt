...
class CompleteOid4vpAuthCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        val authResult = authBridge.completeAuthentication(sessionId).getOrElse { error ->
            val statusCode = when (error.code) {
                Oid4vpAuthErrorCode.SESSION_NOT_FOUND.name -> 404
                Oid4vpAuthErrorCode.SESSION_NOT_VERIFIED.name,
                Oid4vpAuthErrorCode.SESSION_ALREADY_COMPLETED.name -> 409
                Oid4vpAuthErrorCode.IDV_REQUIRED.name -> 202
                else -> 500
            }
