...
class CreateOid4vpAuthSessionCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        val input =
            parseInput(request.body).getOrElse { error ->
                return Err(error)
            }
        ...
        val result =
            authBridgeService.createSession(createArgs).getOrElse { error ->
                return Err(error)
            }
    ...
    private fun parseInput(body: String?): IdkResult<CreateOid4vpAuthSessionRequest?, IdkError> {
        ...
        return try {
            ...
            Err(Oid4vpAuthErrors.invalidRequestBody(expected.message ?: "Malformed JSON", expected))
        }
    }
}
