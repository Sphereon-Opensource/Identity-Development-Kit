...
class Oid4vpAuthFlowIntegrationTest {
    ...
    @Test
    fun `full auth flow should create session, update to verified, and complete`() = runTest {
        val bridge = TestOid4vpAuthBridge()

        // Step 1: Create session
        val createResult = bridge.createSession(CreateSessionArgs(
            queryId = "test-query",
    ...
    @Test
    fun `getSessionStatus should return PENDING while waiting`() = runTest {
        val bridge = TestOid4vpAuthBridge()
    ...
    @Test
    fun `completeAuthentication should fail if session not verified`() = runTest {
        val bridge = TestOid4vpAuthBridge()
    ...
    @Test
    fun `completeAuthentication should fail if already completed`() = runTest {
        val bridge = TestOid4vpAuthBridge()
    ...
    @Test
    fun `getSessionStatus should fail for non-existent session`() = runTest {
        val bridge = TestOid4vpAuthBridge()
    ...
    @Test
    fun `completeAuthentication should fail for non-existent session`() = runTest {
        val bridge = TestOid4vpAuthBridge()
    ...
    @Test
    fun `session store should persist sessions correctly`() = runTest {
        val store = InMemoryOid4vpAuthSessionStore()
    ...
    @Test
    fun `session store should update sessions correctly`() = runTest {
        val store = InMemoryOid4vpAuthSessionStore()
    ...
    @Test
    fun `session store should delete sessions correctly`() = runTest {
        val store = InMemoryOid4vpAuthSessionStore()
    ...
    fun `should handle multiple concurrent sessions`() = runTest {
        ...
        val sessions = (1..5).map { i ->
            val result = bridge.createSession(CreateSessionArgs(
                queryId = "query-$i",
...
class TestOid4vpAuthBridge : Oid4vpAuthBridge {
     ...
     */
    fun markSessionVerified(sessionId: String, verifiedData: VerifiedData) {
    ...
    override suspend fun completeAuthentication(sessionId: String): IdkResult<Oid4vpAuthResult, IdkError> {
        ...
        val claims = session.verifiedData?.credentials?.firstOrNull()?.claims ?: emptyMap()
