...
class CompleteOid4vpAuthCommandImplTest {
    ...
    @Test
    fun `should return user identity on successful completion`() = runTest {
        val now = Clock.System.now()
        val mockBridge = MockOid4vpAuthBridge(
            completeResult = Ok(Oid4vpAuthResult(
    ...
    @Test
    fun `should return error for session not verified`() = runTest {
        val mockBridge = MockOid4vpAuthBridge(
        ...
        assertTrue(result.isErr)
        assertTrue(result.error.message.defaultMessage.contains("not verified", ignoreCase = true))
    }

    @Test
    fun `should return error for already completed session`() = runTest {
        val mockBridge = MockOid4vpAuthBridge(
        ...
        val result = mockBridge.completeAuthentication("session-123")
        assertTrue(result.isErr)
        assertTrue(result.error.message.defaultMessage.contains("already completed", ignoreCase = true))
    }

    @Test
    fun `response should serialize correctly`() {
        val now = Clock.System.now()
        val response = CompleteOid4vpAuthResponse(
            userId = "user@example.com",
            jwtClaims = null,
            claims = mapOf(
                "sub" to JsonPrimitive("user@example.com"),
    ...
    @Test
    fun `response with new user flag should be set correctly`() = runTest {
        val now = Clock.System.now()
        val mockBridge = MockOid4vpAuthBridge(
            completeResult = Ok(Oid4vpAuthResult(
