...
class GetOid4vpAuthStatusCommandImplTest {
    ...
    @Test
    fun `should return PENDING status for new session`() =
        runTest {
            val mockBridge =
    ...
    @Test
    fun `should return VERIFIED status after wallet presentation`() =
        runTest {
            val mockBridge =
                MockOid4vpAuthBridge(
                    getStatusResult =
...
class MockOid4vpAuthBridge(
    ...
    override suspend fun createSession(args: CreateSessionArgs): IdkResult<CreateSessionResult, IdkError> = createResult ?: Err(Oid4vpAuthErrors.sessionNotFound("not-configured"))

    override suspend fun getSessionStatus(sessionId: String): IdkResult<Oid4vpAuthStatusResponse, IdkError> = getStatusResult ?: Err(Oid4vpAuthErrors.sessionNotFound("not-configured"))

    override suspend fun completeAuthentication(sessionId: String): IdkResult<Oid4vpAuthResult, IdkError> = completeResult ?: Err(Oid4vpAuthErrors.sessionNotFound("not-configured"))
}
