...
class Oid4vpAuthIdvCommandsTest {
    @Test
    fun `getSessionStatus returns IDV_REQUIRED with idvMessage`() =
        runTest {
            val mockBridge =
    ...
    @Test
    fun `completeAuthentication returns IDV_REQUIRED error when reconciliation needed`() =
        runTest {
            val mockBridge =
                MockOid4vpAuthBridge(
            ...
            assertTrue(
                result.error.message.defaultMessage
                    .contains("IDV", ignoreCase = true) ||
                    result.error.message.defaultMessage
                        .contains("identity verification", ignoreCase = true),
                "Error should mention IDV: ${result.error.message.defaultMessage}",
            )
        }

    @Test
    fun `session transitions through IDV flow states`() =
        runTest {
    ...
    @Test
    fun `sessionNotIdvRequired error for non-IDV session`() =
        runTest {
    ...
    @Test
    fun `noReconciliationMapping error includes credential type`() =
        runTest {
    ...
    @Test
    fun `reconciliationFailed error includes reason`() =
        runTest {
