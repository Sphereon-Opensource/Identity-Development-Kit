...
/**
 * Test app component for OID4VP Auth Bridge HTTP endpoint testing.
 *
 ...
 *
 * The commands and adapter are available via the merged session component.
 */
@DependencyGraph(AppScope::class)
abstract class Oid4vpAuthTestAppComponent : AbstractAppComponent() {
    ...
    fun interface Factory {
        ...
        ): Oid4vpAuthTestAppComponent
    }
}

fun createOid4vpAuthTestAppComponent(
    application: Any,
    appId: String = "oid4vp-auth-test",
    profile: String = "test",
    version: String = "1.0.0-test",
): Oid4vpAuthTestAppComponent {
    val component = dev.zacsweers.metro.createGraphFactory<Oid4vpAuthTestAppComponent.Factory>().create(
 ...
 *
 * Required because [Oid4vpUserAuthenticationProviderImpl.Component] exposes
...
private class NoOpTestUserAuthenticationProvider : UserAuthenticationProvider {
    ...
    override suspend fun initiateAuthentication(sessionId: String, returnUrl: String, hint: AuthenticationHint?): IdkResult<String, AuthenticationError> =
        com.sphereon.core.api.Err(AuthenticationError.Generic(message = "Not implemented in test"))
    ...
    fun provideWalletAttributeProjector(): com.sphereon.openid.oid4vp.auth.claims.WalletAttributeProjector =
        object : com.sphereon.openid.oid4vp.auth.claims.WalletAttributeProjector {
            override suspend fun projectWalletAttributes(
                credentials: List<com.sphereon.openid.oid4vp.universal.VerifiedCredential>,
            ): Map<String, kotlinx.serialization.json.JsonElement> {
                return credentials.flatMap { it.claims.entries }.associate { it.key to it.value }
            }
        }
}
 ...
 * which contributes HTTP adapters that require RequestUriSigningConfig.
 */
@SingleIn(SessionScope::class)
...
class TestRequestUriSigningConfig @Inject constructor() : RequestUriSigningConfig {
    override val signingKey = KeyInfo<Nothing>(kid = "test-request-uri-signing-key")
...
private class InMemoryTestIdentityLinkBindingStore : IdentityLinkBindingStore {
    ...
    override suspend fun findByMatchId(tenantId: String, matchId: String): IdentityLinkBinding? =
        bindings.values.find { it.tenantId == tenantId && it.matchId == matchId }

    override suspend fun findByHolderHash(tenantId: String, holderHash: String): IdentityLinkBinding? =
        bindings.values.find { it.tenantId == tenantId && it.holderIdentifierHash == holderHash }
    ...
    override suspend fun delete(tenantId: String, bindingId: String): Boolean =
        bindings.remove(bindingId) != null
...
interface Oid4vpAuthTestReconciliationOrchestratorModule {
    @Provides
    @SingleIn(SessionScope::class)
    fun provideReconciliationOrchestratorApi(): ReconciliationOrchestratorApi = object : ReconciliationOrchestratorApi {
        ...
        override suspend fun initiateReconciliation(
            ...
            Err(IdkError.fromString("Not implemented in test"))
