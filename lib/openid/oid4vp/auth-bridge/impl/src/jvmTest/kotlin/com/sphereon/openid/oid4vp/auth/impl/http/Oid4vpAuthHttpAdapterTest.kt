...
class Oid4vpAuthHttpAdapterTest {
    ...
    @Test
    fun `create session command should handle POST sessions`() = runTest {
    ...
    @Test
    fun `create session command should work with empty body`() = runTest {
        val createCommand = CreateOid4vpAuthSessionCommandImpl(execution, mockService)
    ...
    @Test
    fun `create session command should only match POST method`() = runTest {
        val createCommand = CreateOid4vpAuthSessionCommandImpl(execution, mockService)
    ...
    @Test
    fun `create session response should have correct headers`() = runTest {
        val createCommand = CreateOid4vpAuthSessionCommandImpl(execution, mockService)
...
class MockOid4vpAuthBridge : Oid4vpAuthBridge {
    ...
    override suspend fun getSessionStatus(sessionId: String): IdkResult<Oid4vpAuthStatusResponse, IdkError> {
        val session = sessions[sessionId]
            ?: return com.sphereon.core.api.Err(Oid4vpAuthErrors.sessionNotFound(sessionId))
    ...
    override suspend fun completeAuthentication(sessionId: String): IdkResult<Oid4vpAuthResult, IdkError> {
        ...
        if (session.status == Oid4vpAuthSessionStatus.COMPLETED) {
            return com.sphereon.core.api.Err(Oid4vpAuthErrors.sessionAlreadyCompleted(sessionId))
        }

        if (session.status != Oid4vpAuthSessionStatus.VERIFIED) {
            return com.sphereon.core.api.Err(Oid4vpAuthErrors.sessionNotVerified(sessionId))
        }
