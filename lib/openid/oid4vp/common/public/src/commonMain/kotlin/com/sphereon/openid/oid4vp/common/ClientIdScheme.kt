...
enum class JarSignerMethod {
    ...
    companion object {
         ...
         */
        fun detect(x5c: List<String>?, jwk: Any?, kid: String?): JarSignerMethod {
            return when {
...
@Serializable
@JsExportCompat
enum class ClientIdScheme(val prefix: String?) {
    /**
    ...
    val jarRequired: Boolean
        get() = when (this) {
            ...
            DECENTRALIZED_IDENTIFIER, VERIFIER_ATTESTATION -> true
            else -> false
        }
    ...
    val allowedSignerMethods: Set<JarSignerMethod>
        get() = when (this) {
            PRE_REGISTERED -> setOf(
                JarSignerMethod.CUSTOM,
                JarSignerMethod.DID,
                JarSignerMethod.JWK,
                JarSignerMethod.X5C
            )
            REDIRECT_URI -> emptySet() // JAR MUST NOT be used
            X509_SAN_DNS -> setOf(JarSignerMethod.X5C)
            X509_SAN_URI -> setOf(JarSignerMethod.X5C)
            X509_HASH -> setOf(JarSignerMethod.X5C)
            DECENTRALIZED_IDENTIFIER -> setOf(JarSignerMethod.DID)
            VERIFIER_ATTESTATION -> setOf(
                JarSignerMethod.DID,
            ...
            OPENID_FEDERATION -> emptySet() // Handled via federation trust chain
            ORIGIN -> emptySet() // Reserved for DC API
        }
     ...
     */
    fun isSignerMethodAllowed(method: JarSignerMethod): Boolean {
        return method in allowedSignerMethods
    }
