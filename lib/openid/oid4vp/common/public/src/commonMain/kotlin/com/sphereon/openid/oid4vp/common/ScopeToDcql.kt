...
class ScopeRegistry(
    ...
    private val registry: MutableMap<String, ScopeDefinition> = definitions
     ...
     */
    fun register(definition: ScopeDefinition): ScopeRegistry = apply {
     ...
     * @return This registry for chaining
     */
    fun registerAll(definitions: List<ScopeDefinition>): ScopeRegistry = apply {
     ...
     * @return This registry for chaining
     */
    fun unregister(scopeValue: String): ScopeRegistry = apply {
...
class ScopeResolver(
    ...
    fun resolveScopes(scopeValues: List<String>): ScopeResolutionResult {
        ...
        val mergedQuery = if (dcqlQueries.isNotEmpty()) {
    ...
    companion object {
        ...
        fun mergeDcqlQueries(queries: List<DcqlQuery>): DcqlQuery {
            if (queries.isEmpty()) {
                throw IllegalArgumentException("Cannot merge empty list of DCQL queries")
            }
            if (queries.size == 1) {
 ...
 */
val validateScopeDefinition = Validation<ScopeDefinition> {
 ...
 * ```
 */
inline fun buildScopeDefinition(block: ScopeDefinitionBuilder.() -> Unit): ScopeDefinition {
    return ScopeDefinitionBuilder().apply(block).build()
}
...
class ScopeRegistryBuilder {
     ...
     */
    fun scope(definition: ScopeDefinition) = apply {
     ...
     */
    fun scope(block: ScopeDefinitionBuilder.() -> Unit) = apply {
 ...
 * ```
 */
inline fun buildScopeRegistry(block: ScopeRegistryBuilder.() -> Unit): ScopeRegistry {
    return ScopeRegistryBuilder().apply(block).build()
}
