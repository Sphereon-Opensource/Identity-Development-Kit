...
class ParseTransactionDataCommandImpl(
...
), ParseTransactionDataCommand, ParseTransactionDataCommandService {
    ...
    override suspend fun parseTransactionData(
        transactionData: List<String>
    ): IdkResult<ParseTransactionDataResult, IdkError> {
        return execute(ParseTransactionDataArgs(transactionData))
    }
    ...
    override suspend fun doExecute(
        ...
        for ((index, encoded) in transactionData.withIndex()) {
            ...
            val jsonString = try {
                ...
                log.error("Failed to decode base64url transaction data at index $index: ${e.message}")
            ...
            val jsonObject = try {
                ...
                log.error("Failed to parse JSON transaction data at index $index: ${e.message}")
                return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
            ...
            val transactionDataHashesAlg = try {
            ...
            val additionalPropsObject = if (additionalProperties.isNotEmpty()) {
            ...
            val entry = TransactionDataEntry(
                type = type,
