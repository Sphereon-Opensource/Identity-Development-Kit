 ...
 * @Inject
 * class MyComponent(private val resolveScopeCommand: ResolveScopeCommand)
 * ```
 */
@Inject
...
class ResolveScopeCommandImpl(
...
), ResolveScopeCommand, ResolveScopeCommandService {
    ...
    // Create resolver with provided or empty registry
    private val resolver = ScopeResolver(scopeRegistry ?: ScopeRegistry.empty())

    override suspend fun resolveScope(scopeString: String?): IdkResult<ScopeResolutionResult, IdkError> {
        return execute(ResolveScopeArgs(scopeString))
    }
