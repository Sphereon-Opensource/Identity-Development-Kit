...
class VerifyTransactionDataCommandImpl(
...
), VerifyTransactionDataCommand, VerifyTransactionDataCommandService {
    ...
    override suspend fun verifyTransactionData(
        ...
        return execute(VerifyTransactionDataArgs(transactionData, credentials))
    }
    ...
    override suspend fun doExecute(
        ...
        if (transactionData.isEmpty()) {
            return Ok(VerifyTransactionDataResult(
                verified = true,
        ...
        // 1. Parse all transaction data entries
        val parseResult = parseTransactionDataCommand.execute(ParseTransactionDataArgs(transactionData))
        ...
        for (parsedEntry in parsedEntries) {
            val verificationResult = verifyEntry(
                ...
                if (algFromPresentation !in allowedAlgs) {
                    return VerifyEntryResult.Error(
                        TransactionDataError.UnsupportedHashAlgorithmError(
                            message = "Transaction data entry at index $index for credential '$credentialId' presentation $presentationIndex uses algorithm '$algFromPresentation' which is not in allowed algorithms: $allowedAlgs",
     ...
     */
    private fun computeHash(encoded: String, algorithm: TransactionDataHashAlgorithm): ByteArray {
    ...
    private sealed interface VerifyEntryResult {
        ...
        data class Error(val error: TransactionDataError) : VerifyEntryResult
    }
}
