...
class VerifyVerifierAttestationCommandImpl(
    ...
    VerifyVerifierAttestationCommand,
    VerifyVerifierAttestationCommandService {
    ...
    override suspend fun verifyVerifierAttestation(args: VerifyVerifierAttestationArgs): IdkResult<VerifyVerifierAttestationResult, IdkError> = execute(args)
    ...
    override suspend fun doExecute(
        ...
        if (cnf == null) {
            missingClaims.add("cnf")
        }
        if (cnfJwk == null) {
            missingClaims.add("cnf.jwk")
        }
        ...
        val jwsVerifyResult =
            verifyJwsCommand.execute(VerifyJwsArgs(jws = JwsCompact(processedArgs.attestationJwt)))
        ...
        log.debug(
            "Verifier attestation validation ${if (valid) {
                "passed"
            } else {
                "failed"
     ...
     */
    private fun jwkThumbprintsMatch(
        jwk1: Jwk,
        jwk2: Jwk,
    ): Boolean {
        // Compare key type
