...
import com.sphereon.openid.oid4vp.universal.impl.createUniversalOid4vpTestAppComponent
...
class UniversalOid4vpE2ETest {
    ...
    @Test
    fun `create session with inline DCQL query and get status`() = runTest {
        // 1. Create full DI hierarchy with static app ID
        val testScope = TestScope()
        val app = createUniversalOid4vpTestAppComponent(testScope)

        // 2. Configure real software KMS
        DefaultPrincipalMapPropertySource.addProperties(
            mapOf(
                "universal-oid4vp-test.test.kms.providers.test-software.type" to "software",
                "universal-oid4vp-test.test.kms.providers.test-software.id" to "test-software",
                "universal-oid4vp-test.test.kms.providers.test-software.keystore.type" to "memory",
                "universal-oid4vp-test.test.kms.providers.test-software.keystore.id" to "test-memory-keystore",
                "universal-oid4vp-test.test.kms.providers.test-software.keystore.keyVisibility" to "private",
                "universal-oid4vp-test.test.kms.providers.test-software.keystore.overwriteAlias" to "true"
            )
        )
        ...
        val dispatcher = (sessionComponent as DefaultHttpAdapterDispatcher.Component).httpAdapterDispatcher
        ...
        val createRequest = GenericHttpRequest.withTextBody(
            method = "POST",
        ...
        // 7. Get status
        val statusRequest = GenericHttpRequest(
            method = "GET",
        ...
        // 8. Delete session
        val deleteRequest = GenericHttpRequest(
            method = "DELETE",
            path = "/oid4vp/backend/auth/requests/${output.correlationId}"
        )
    ...
    @Test
    fun `create session and verify QR code data URI format`() = runTest {
        val testScope = TestScope()
        val app = createUniversalOid4vpTestAppComponent(testScope)
        ...
        val createResponse = dispatcher.dispatch(
    ...
    @Test
    fun `error handling - missing query parameters`() = runTest {
        val testScope = TestScope()
        val app = createUniversalOid4vpTestAppComponent(testScope)
    ...
    @Test
    fun `error handling - get non-existent session`() = runTest {
        val testScope = TestScope()
        val app = createUniversalOid4vpTestAppComponent(testScope)
        ...
        val statusResponse = dispatcher.dispatch(
            GenericHttpRequest(
                method = "GET",
    ...
    @Test
    fun `error handling - delete non-existent session`() = runTest {
        val testScope = TestScope()
        val app = createUniversalOid4vpTestAppComponent(testScope)
        ...
        val deleteResponse = dispatcher.dispatch(
            GenericHttpRequest(
                method = "DELETE",
                path = "/oid4vp/backend/auth/requests/non-existent-correlation-id"
            )
     ...
     */
    @Test
    fun `full e2e flow - create session, holder issues SD-JWT VC, creates VP, submits via direct_post`() = runTest {
        val testScope = TestScope()
        val app = createUniversalOid4vpTestAppComponent(testScope)
        ...
        val rpService = (verifierComponent as Oid4VpVerifierServiceImpl.Component).oid4vpVerifierService
        ...
        val dispatcher = (verifierComponent as DefaultHttpAdapterDispatcher.Component).httpAdapterDispatcher
        ...
        // Status is available via GET /backend/auth/requests/{correlationId}, not on CreateAuthorizationRequestOutput
        ...
        val sdJwtService = (holderComponent as SdJwtServiceImpl.Component).sdJwtService

        // Generate issuer key for SD-JWT VC
        val issuerKeyPair = holderKms.generateKeyAsync(
        ...
        val holderKeyPair = holderKms.generateKeyAsync(
            providerId = "test-software",
        ...
        val sessionNonce = sessionResult.authorizationRequest.nonce
            ?: error("Session should have a nonce")

        val vcPayload = sdJwtPayload {
        ...
        val vpTokenJson = json.encodeToString(kotlinx.serialization.json.JsonElement.serializer(), vpToken.toJson())
        ...
        val handleResult = rpService.handleDirectPostResponse(
        ...
        val retrievedResponse = rpService.retrieveAuthorizationResponse(
     ...
     */
    @Test
    fun `full e2e flow with JARM encrypted response`() = runTest {
        val testScope = TestScope()
        val app = createUniversalOid4vpTestAppComponent(testScope)
        ...
        val verifierEncryptionKeyPair = verifierKms.generateKeyAsync(
            providerId = "test-software",
        ...
        val responseParameters = kotlinx.serialization.json.buildJsonObject {
        ...
        val registry = (holderComponent as SimpleSessionScopedCommandRegistryComponent).sessionScopedCommandRegistry
        @Suppress("UNCHECKED_CAST")
        val createJarmCommand = registry.get(CreateJarmResponseCommand.COMMAND_ID) as CreateJarmResponseCommand
        val encryptionRecipient = ManagedOptsKeyInfo(
        ...
        val jarmResult = createJarmCommand.execute(
        ...
        val verifierDecryptionKey = ManagedOptsKeyInfo(
        ...
        val parseResult = rpService.parseAuthorizationResponse(
