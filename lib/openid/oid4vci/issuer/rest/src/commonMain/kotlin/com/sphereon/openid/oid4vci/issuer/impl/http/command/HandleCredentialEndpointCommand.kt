...
@StringKey(HandleCredentialEndpointCommand.COMMAND_ID)
class HandleCredentialEndpointCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        val selector = if (credentialRequest.credentialIdentifier != null) "credential_identifier" else "credential_configuration_id"
        log.debug {
            "Credential request parsed: selector=$selector, " +
                "proofType=${credentialRequest.proofs?.proofType ?: "none"}, " +
                "proofCount=${credentialRequest.proofs?.proofValues?.size ?: 0}, " +
                "responseEncryptionRequested=${credentialRequest.credentialResponseEncryption != null}"
        }
            ...
            handleCredentialRequestCommand
                .execute(
                    HandleCredentialRequestArgs(
                        ...
                        // Proof `aud` is the exact Credential Issuer Identifier advertised in
                        // metadata. In a tenant/instance routed deployment it can include an
                        // instance path even when the config provider's fallback is host-only.
