...
internal class NoOpContextConfig(
    private val principalConfigService: PrincipalConfigService? = null,
) : ContextConfig {
    ...
    override val principal: PrincipalConfigService
        get() = principalConfigService ?: throw NotImplementedError("Not needed for test")

    override fun conf(level: ConfigLevel): ConfigService =
        if (level == ConfigLevel.PRINCIPAL) principal else throw NotImplementedError("Not needed for test")
}

internal class TestSessionExecution(
    override val sessionContext: SessionContext = NoOpSessionContext,
    principalConfigService: PrincipalConfigService? = null,
) : SessionExecution {
    ...
    override val conf: ContextConfig = NoOpContextConfig(principalConfigService)
}
