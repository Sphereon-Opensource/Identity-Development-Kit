...
object ThemeValidator {
    ...
    private val CSS_SHADOW_LAYER = "(?:inset\\s+)?(?:-?\\d+(?:\\.\\d+)?(?:px|rem|em)?\\s+){2,4}(?:#[0-9A-Fa-f]{3,8}|rgba?\\([^)]+\\))"
     ...
     */
    fun validateCustomCss(css: String, policy: CssPolicyConfig = CssPolicyConfig()): List<String> {
        val errors = mutableListOf<String>()
