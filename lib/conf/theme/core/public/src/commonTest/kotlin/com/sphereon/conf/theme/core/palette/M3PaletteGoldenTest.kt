...
class M3PaletteGoldenTest {
    ...
    fun differentSeedsShouldProduceDifferentPrimaries() {
        ...
        assertTrue(purple.primary.tone40 != blue.primary.tone40,
            "Different seeds should produce different primary tone40 values")
    }

    @Test
    fun multiSeedShouldProduceDeterministicPalette() {
        val palette1 = M3PaletteGenerator.generateFromSeeds(
            primarySeed = "#6750A4",
            secondarySeed = "#FF5722",
            neutralSeed = "#9E9E9E",
        )
        val palette2 = M3PaletteGenerator.generateFromSeeds(
            primarySeed = "#6750A4",
            secondarySeed = "#FF5722",
            neutralSeed = "#9E9E9E",
        )
    ...
    fun multiSeedWithDifferentSecondaryShouldDiffer() {
        val p1 = M3PaletteGenerator.generateFromSeeds(
            primarySeed = "#6750A4",
            secondarySeed = "#FF5722",
        )
        val p2 = M3PaletteGenerator.generateFromSeeds(
            primarySeed = "#6750A4",
        )
        assertTrue(p1.secondary.tone40 != p2.secondary.tone40,
    ...
    private fun assertValidHex(hex: String) {
        ...
        assertTrue(clean.length == 6 || clean.length == 8,
            "Expected 6 or 8 hex digits, got ${clean.length} in: $hex")
        assertTrue(clean.all { it in "0123456789ABCDEFabcdef" },
