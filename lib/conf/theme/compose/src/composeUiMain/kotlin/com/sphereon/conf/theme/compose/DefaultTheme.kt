...
fun DefaultTheme(
    ...
    val themedTokens = if (a11y.isHighContrast) {
...
fun DefaultTheme(
    ...
    val variant = if (isDark) ThemeVariant.DARK else ThemeVariant.LIGHT
    ...
    val resolvedTheme = when {
        ...
        // Zero-config
        else -> if (isDark) DefaultThemeDefaults.dark else DefaultThemeDefaults.light
    }
...
internal fun DefaultThemeContent(
    ...
    val motion = if (accessibilityState.prefersReducedMotion) {
        MotionTokens.Zero
    } else {
        MotionTokenMapper.toMotionTokens(resolvedTheme)
    }
    val colorScheme = if (animateTransition) animateThemeColorScheme(rawColorScheme, motion) else rawColorScheme
