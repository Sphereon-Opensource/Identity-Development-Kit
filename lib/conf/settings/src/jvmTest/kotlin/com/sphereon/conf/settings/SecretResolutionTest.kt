...
class SecretResolutionTest {
    ...
    private fun cleanupTestKeys() {
        val keysToClean = listOf(
            "secret.ref", "db.connection", "combined.value", "missing.secret"
        )
    ...
    fun testSecretEnvResolutionWithExistingVar() {
        ...
        val envVar = if (System.getProperty("os.name").lowercase().contains("win")) {
            "USERPROFILE"
        } else {
            "HOME"
        }
    ...
    fun testRawSecretReferenceStored() {
        ...
        assertTrue(resolvedValue != secretRef || System.getenv("PATH") == null,
