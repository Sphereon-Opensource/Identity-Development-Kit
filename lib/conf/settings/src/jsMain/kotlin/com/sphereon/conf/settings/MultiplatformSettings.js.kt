...
actual class MultiplatformSettings actual constructor(app: App, configLevel: ConfigLevel, userContext: UserContext?) {
    ...
    actual inline fun <reified T : Any> get(key: String, defaultValue: T?): T? {
        ...
        return if (settings.hasKey(keyWithNS)) {
            when (T::class) {
                ...
                else -> throw IllegalArgumentException("Unsupported type: ${T::class} for key $keyWithNS")
            }
        } else defaultValue
    }
    ...
    actual fun getAsString(key: String): String? {
        ...
        val value = settings.getStringOrNull(keyWithNS) ?: settings.getIntOrNull(keyWithNS)
    ...
    actual fun getKeys(): Set<String> {
        ...
        return settings.keys.map { it.replace("$namespace/", "") }.toSet()
    }
