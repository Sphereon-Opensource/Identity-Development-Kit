...
interface IdvMethodDefinitionStore {
    suspend fun findById(id: IdvMethodId, tenantId: String? = null): IdvMethodDefinition?
    suspend fun list(tenantId: String? = null, enabledOnly: Boolean = false): List<IdvMethodDefinition>
    ...
    suspend fun delete(id: IdvMethodId, tenantId: String? = null): Boolean
}

interface IdvUseCaseDefinitionStore {
    suspend fun findById(id: IdvUseCaseId, tenantId: String): IdvUseCaseDefinition?
    suspend fun list(tenantId: String, enabledOnly: Boolean = false): List<IdvUseCaseDefinition>
    ...
    suspend fun delete(id: IdvUseCaseId, tenantId: String): Boolean
    suspend fun resolve(args: ResolveIdvUseCaseArgs): IdvUseCaseDefinition?
}

interface IdvExecutionStore {
    suspend fun findById(executionId: IdvExecutionId, tenantId: String): IdvExecution?
    suspend fun create(execution: IdvExecution): IdvExecution
    suspend fun update(execution: IdvExecution, expectedVersion: Long? = null): IdkResult<IdvExecution, IdvError>
    suspend fun delete(executionId: IdvExecutionId, tenantId: String): Boolean
    suspend fun findExpired(tenantId: String, cutoff: Instant): List<IdvExecution>
}
