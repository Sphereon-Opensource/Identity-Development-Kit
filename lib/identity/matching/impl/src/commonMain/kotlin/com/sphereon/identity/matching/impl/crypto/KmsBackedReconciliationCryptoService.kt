...
class KmsBackedReconciliationCryptoService(
    ...
    override suspend fun hashHolderKey(holderKey: String): HashedIdentifier = generateHmac(holderKey, holderKeyAlias, holderKeyVersion)

    override suspend fun hashExternalIdentifier(identifier: String): HashedIdentifier = generateHmac(identifier, institutionKeyAlias, institutionKeyVersion)
    ...
    override suspend fun encrypt(plaintext: String): EncryptedPayload {
        ...
        val encryptResult =
            checkNotNull(result.getOrNull()) { "Encryption failed: ${result.errorOrNull()}" }
    ...
    private suspend fun generateHmac(
        ...
        val macResult =
            checkNotNull(result.getOrNull()) { "HMAC generation failed: ${result.errorOrNull()}" }
