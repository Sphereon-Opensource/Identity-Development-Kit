...
class InMemoryIdentityMatchStore : IdentityMatchStore {
    ...
    private data class HashKey(val tenantId: String, val identifierHash: String, val identifierType: IdentifierType)
    ...
    private fun compositeId(tenantId: String, matchId: String) = "$tenantId:$matchId"
    ...
    override suspend fun findByInternalIdentityId(tenantId: String, internalIdentityId: String): List<IdentityMatch> {
        return byId.values.filter { it.tenantId == tenantId && it.internalIdentityId == internalIdentityId }
    }
