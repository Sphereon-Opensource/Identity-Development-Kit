...
class KottageIdentityLinkBindingStore(
    ...
    private val bindingNs = KvNamespace(
        name = "identity-link-binding",
        codec = KotlinxSerializationJsonKvCodec(json, IdentityLinkBinding.serializer())
    )
    private val matchIndexNs = KvNamespace(
        name = "identity-link-binding-match-idx",
        codec = KotlinxSerializationJsonKvCodec(json, String.serializer())
    )
    private val holderIndexNs = KvNamespace(
        name = "identity-link-binding-holder-idx",
        codec = KotlinxSerializationJsonKvCodec(json, String.serializer())
    )
    private val tenantIndexNs = KvNamespace(
        name = "identity-link-binding-tenant-idx",
        codec = KotlinxSerializationJsonKvCodec(json, ListSerializer(String.serializer()))
    )

    private fun bindingKey(tenantId: String, bindingId: String) = "$tenantId:$bindingId"
    private fun matchIndexKey(tenantId: String, matchId: String) = "$tenantId:$matchId"
    private fun holderIndexKey(tenantId: String, holderHash: String) = "$tenantId:$holderHash"
    ...
    override suspend fun findByMatchId(tenantId: String, matchId: String): IdentityLinkBinding? {
        val bindingId = store.get(matchIndexNs, matchIndexKey(tenantId, matchId)).getOrNull()
            ?: return null
    ...
    override suspend fun findByHolderHash(tenantId: String, holderHash: String): IdentityLinkBinding? {
        val bindingId = store.get(holderIndexNs, holderIndexKey(tenantId, holderHash)).getOrNull()
            ?: return null
        return findByIdInternal(tenantId, bindingId)
    }
    ...
    override suspend fun findExpired(tenantId: String, inactiveSince: Instant): List<IdentityLinkBinding> {
        ...
        return bindingIds.mapNotNull { bindingId ->
    ...
    private suspend fun findByIdInternal(tenantId: String, bindingId: String): IdentityLinkBinding? {
        return store.get(bindingNs, bindingKey(tenantId, bindingId)).getOrNull()
    }

    private suspend fun addToTenantIndex(tenantId: String, bindingId: String) {
    ...
    private suspend fun removeFromTenantIndex(tenantId: String, bindingId: String) {
        val ids = getTenantIndex(tenantId).toMutableList()
    ...
    private suspend fun getTenantIndex(tenantId: String): List<String> {
        return store.get(tenantIndexNs, tenantIndexKey(tenantId)).getOrNull() ?: emptyList()
    }
}
