...
class KottageIdentityLinkBindingStoreJvmTest {
    ...
    fun updateBinding() = runTest {
        val store = createStore()
        store.create(createBinding())
        val updated = createBinding().copy(
            updatedAt = Clock.System.now(),
            persistedAttributesEnvelope = PersistedAttributesEnvelope(
                encrypted = EncryptedPayload(ciphertext = "ciphertext", keyVersion = "C-v1"),
                canonicalSchemaVersion = "1",
    ...
    fun persistenceAcrossRestart() = runTest {
        ...
        val kvStoreFactory = KottageKvStoreFactory(kottage)
        val store1 = KottageIdentityLinkBindingStore(store = kvStoreFactory.create(KottageKvStoreConfig(id = "identity-link-bindings", scopeBinding = com.sphereon.data.store.kv.KvStoreScopeBinding.APP)))
        store1.create(createBinding())

        val store2 = KottageIdentityLinkBindingStore(store = kvStoreFactory.create(KottageKvStoreConfig(id = "identity-link-bindings", scopeBinding = com.sphereon.data.store.kv.KvStoreScopeBinding.APP)))
