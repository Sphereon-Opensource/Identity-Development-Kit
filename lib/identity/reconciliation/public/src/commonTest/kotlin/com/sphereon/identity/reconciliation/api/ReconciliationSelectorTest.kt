...
class ReconciliationSelectorTest {
    ...
    ): ReconciliationSelectorRule = ReconciliationSelectorRule(
        id = id,
        priority = priority,
        enabled = enabled,
        tenants = tenants,
    ...
    ): ReconciliationSelectorInput = ReconciliationSelectorInput(
        tenantId = tenantId,
        queryId = queryId,
    ...
    fun queryIdMatching() {
        ...
        val result = ReconciliationSelector.evaluate(
            rules,
    ...
    fun claimPredicateMatching() {
        val claims = AttributeBag.empty()
            .with(AttributePath("affiliation"), JsonPrimitive("student"))

        val rules = listOf(
            rule(
                id = "student",
                attributePredicates = listOf(
    ...
    fun multiRuleMatchHighestPrioritySelected() {
        ...
        val rules = listOf(
            rule(id = "fast-path", priority = 100, decision = ReconciliationDecision.USE_EXISTING_BINDING,
                knownHolderStates = setOf(KnownHolderState.MATCHED_HOLDER_KEY),
                providerId = null, materialProfileId = null),
            rule(id = "student-surf", priority = 50,
