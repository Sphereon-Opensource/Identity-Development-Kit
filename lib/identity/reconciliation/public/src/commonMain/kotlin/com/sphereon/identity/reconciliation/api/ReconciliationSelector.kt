...
object ReconciliationSelector {
    ...
    private fun matches(rule: ReconciliationSelectorRule, input: ReconciliationSelectorInput): Boolean =
    ...
    private fun matchesTenants(expected: Set<String>?, tenantId: String): Boolean =
        expected == null || expected.contains(tenantId)

    private fun matchesSet(expected: Set<String>?, actual: String?): Boolean =
        expected == null || (actual != null && expected.contains(actual))

    private fun matchesAnyOf(expected: Set<String>?, actual: Set<String>): Boolean =
        expected.isNullOrEmpty() || expected.any { actual.contains(it) }

    private fun matchesIssuers(patterns: Set<String>?, issuers: Set<String>): Boolean =
        patterns == null || patterns.any { pattern ->
            issuers.any { issuer ->
                try {
    ...
    private fun matchesKnownHolder(expected: Set<KnownHolderState>?, actual: KnownHolderState?): Boolean =
        expected == null || (actual != null && expected.contains(actual))

    private fun matchesPredicates(predicates: List<AttributePredicate>?, attributes: AttributeBag?): Boolean =
        predicates.isNullOrEmpty() || (attributes != null && AttributePredicateEvaluator.evaluate(predicates, attributes))

    private fun resolve(template: ReconciliationPlanTemplate, ruleVersion: String): ReconciliationPlan =
        when (template.decision) {
            ReconciliationDecision.SKIP_RECONCILIATION -> SkipReconciliation(
                selectorRuleVersion = ruleVersion,
            )
            ReconciliationDecision.USE_EXISTING_BINDING -> UseExistingBinding(
                requiredAttributeNames = template.requiredAttributeNames,
                materialProfileId = template.materialProfileId,
                selectorRuleVersion = ruleVersion,
            )
            ReconciliationDecision.RUN_IDV -> RunIdv(
            ...
            ReconciliationDecision.STEP_UP -> StepUp(
            ...
            ReconciliationDecision.FAIL_CLOSED -> FailClosed(
