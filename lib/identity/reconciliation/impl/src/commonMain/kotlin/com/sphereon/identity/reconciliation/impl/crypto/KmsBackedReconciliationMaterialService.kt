...
class KmsBackedReconciliationMaterialService(
    ...
    private val keyVersionsByDomain: Map<String, String> = mapOf(
        "holder" to "v1",
        "institution" to "v1",
        "material" to "v1",
    ),
    ...
    override suspend fun deriveMaterials(
        ...
        return profile.materials.mapNotNull { material ->
    ...
    private suspend fun deriveSingle(
        ...
        return when (material) {
            ...
            is AttributeTupleMaterial -> {
                val input = buildTupleInput(
                    material.attributePaths, material.normalizationProfile, material.saltRef,
                    canonicalAttributes.attributes, material.minRequiredAttributes,
                ) ?: return null
            ...
            is CredentialAttributeTupleMaterial -> {
                val scopedAttributes = resolveCredentialAttributes(material, credentialScopedAttributes)
                    ?: return null
                val input = buildTupleInput(
                    material.attributePaths, material.normalizationProfile, material.saltRef,
                    scopedAttributes, material.minRequiredAttributes,
                ) ?: return null
                val hash = hmac(input, material.hmacDomain, current)
    ...
    private fun resolveCredentialAttributes(
        ...
        if (credentialScopedAttributes == null) return null
    ...
    private fun buildTupleInput(
        ...
        val values = attributePaths.mapNotNull { path ->
        ...
        if (values.size < minRequiredAttributes) return null
    ...
    private suspend fun hmac(input: String, domain: String, current: Boolean): HashedIdentifier {
        val alias = "$keyAliasPrefix:$domain"
        val versions = if (current) keyVersionsByDomain else previousKeyVersionsByDomain
        val version = versions[domain]
            ?: throw IllegalStateException("No ${if (current) "current" else "previous"} key version for domain '$domain'")
        ...
        val macResult = result.getOrNull()
            ?: throw IllegalStateException("HMAC failed for domain '$domain': ${result.errorOrNull()}")
