...
class InMemoryReconciliationSessionStore(
    logManager: AppLogManager,
) : ReconciliationSessionStore {
    private val log = logManager.withTag("ReconciliationStore")
    ...
    override suspend fun findById(
        tenantId: String,
        sessionId: String,
    ): ReconciliationSession? = byId[compositeId(tenantId, sessionId)]

    override suspend fun findByState(
        tenantId: String,
        state: String,
    ): ReconciliationSession? {
        log.debug("findByState: tenantId=$tenantId, state=$state")
        log.debug("byState keys: ${byState.keys}")
        log.debug("byState contains state? ${byState.containsKey(state)}")
        val sessionKey =
            byState[state] ?: run {
                log.debug("No sessionKey found for state=$state")
                return null
            }
        val session =
            byId[sessionKey] ?: run {
                log.debug("No session found for key=$sessionKey")
                return null
            }
        log.debug("Found session: id=${session.id}, tenantId=${session.tenantId}")
        return if (session.tenantId == tenantId) {
            session
        } else {
            null
        }
    ...
    override suspend fun create(session: ReconciliationSession): ReconciliationSession {
        ...
        log.debug("Created session: id=${session.id}, tenantId=${session.tenantId}, state=${session.state}, cid=$cid")
        log.debug("Store now has ${byId.size} sessions, ${byState.size} state mappings")
        log.debug("Store instance: ${this.hashCode()}")
        return session
    }
    ...
    override suspend fun findExpired(
        tenantId: String,
        cutoff: Instant,
    ): List<ReconciliationSession> = byId.values.filter { it.tenantId == tenantId && it.expiresAt < cutoff }
}
