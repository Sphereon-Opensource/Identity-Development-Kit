...
class ReconciliationProviderConfigBinder(
    ...
    fun bindOidcClients(): List<OidcClientConfig> {
        val clientNames =
            configService
                .getPropertyAsString("$OIDC_CLIENTS_PREFIX.names", null)
                ?.split(",")
                ?.map { it.trim() }
        ...
        return clientNames.mapNotNull { name ->
            ...
            val discoveryUrl =
                configService.getPropertyAsString("$prefix.discovery-url", null)
                    ?: return@mapNotNull null

            val clientIdKey =
                configService.getPropertyAsString("$prefix.client-id-ref.key", null)
    ...
    fun readAttributeMappings(
        ...
        while (true) {
            val source =
                configService.getPropertyAsString("$providerPrefix.$mappingSuffix[$index].source", null)
                    ?: break
            val target =
        ...
        val entryNames =
            configService
                .getPropertyAsString("$providerPrefix.$mappingSuffix.entries", null)
                ?.split(",")
                ?.map { it.trim() }
                ?.filter { it.isNotEmpty() }
                ?: return emptyList()
    ...
    fun bindProviders(): List<ReconciliationProvider> {
        val providerNames =
            configService
                .getPropertyAsString("$PROVIDERS_PREFIX.names", null)
                ?.split(",")
                ?.map { it.trim() }
                ?: return emptyList()
        ...
        return providerNames.mapNotNull { name ->
            ...
            val oidcClientId =
                configService.getPropertyAsString("$providerPrefix.oidc-client-id", null)
                    ?: return@mapNotNull null

            val mappingMode =
                configService
                    .getPropertyAsString("$providerPrefix.mapping-mode", null)
                    ?.replace('-', '_')
                    ?.uppercase()
                    ?.let { runCatching { MappingMode.valueOf(it) }.getOrNull() }
                    ?: MappingMode.EXPLICIT

            val attributeMappings =
                if (mappingMode == MappingMode.EXPLICIT) {
                    readAttributeMappings(providerPrefix, "attribute-mappings")
                        .ifEmpty { readAttributeMappings(providerPrefix, "claim-mappings") }
                } else {
                    emptyList()
                }
            val userInfoAttributeMappings =
                if (mappingMode == MappingMode.EXPLICIT) {
                    readAttributeMappings(providerPrefix, "userinfo-attribute-mappings")
                        .ifEmpty { readAttributeMappings(providerPrefix, "userinfo-claim-mappings") }
                } else {
                    emptyList()
                }
            ...
            ReconciliationProvider(
                ...
                identifierAttributeName =
                    configService.getPropertyAsString("$providerPrefix.identifier-attribute-name", null)
                        ?: configService.getPropertyAsString("$providerPrefix.identifier-claim-name", "sub")
                        ?: "sub",
                ...
                assuranceAmr =
                    configService
                        .getPropertyAsString("$providerPrefix.assurance-amr", null)
                        ?.split(",")
                        ?.map { it.trim() }
                        ?.filter { it.isNotEmpty() },
            )
