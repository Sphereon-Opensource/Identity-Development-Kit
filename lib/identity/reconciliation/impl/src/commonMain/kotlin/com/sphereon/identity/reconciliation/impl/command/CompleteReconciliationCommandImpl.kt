...
class CompleteReconciliationCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        if (session.status != ReconciliationSessionStatus.CREATED && session.status != ReconciliationSessionStatus.REDIRECTED) {
            return Err(IdkError.fromDTO(ReconciliationError.SessionInvalidState(
        ...
        if (session.state != applied.state) {
            return Err(IdkError.fromDTO(ReconciliationError.StateMismatch(
        ...
        // Look up provider
        val provider = providerStore.findById(session.providerId)
        ...
        val oidcConnection = oidcConnectionResolver.resolve(provider.oidcClientId)
        ...
        val tokenEndpoint = oidcConnection.tokenEndpointOverride
        ...
        val tokenResponse = exchangeTokenCommand.execute(
            ...
            return Err(IdkError.fromDTO(ReconciliationError.TokenExchangeFailed(
        ...
        val idTokenClaims: Map<String, JsonElement> = tokenResponse.idToken?.let { idToken ->
            tokenClaimExtractor.extractAllClaims(idToken).getOrNull()
        } ?: emptyMap()

        // Fetch userinfo claims if enabled for this OIDC client
        val userInfoClaims: Map<String, JsonElement> = if (oidcConnection.userInfoEnabled) {
            val userinfoEndpoint = oidcDiscoveryService.getMetadata(discoveryIssuer).getOrNull()?.userinfoEndpoint
                ?: (discoveryIssuer.trimEnd('/') + "/userinfo")
            val userInfoResult = fetchUserInfoCommand.execute(
                FetchUserInfoArgs(
        ...
        val mappedClaims = mappedClaimsResult.getOrElse { error ->
        ...
        val finalClaimsResult = if (provider.userInfoAttributeMappings.isNotEmpty()) {
            applyAttributeMappings(mappedClaims, provider.userInfoAttributeMappings)
        } else {
            Ok(mappedClaims)
        }

        val finalClaims = finalClaimsResult.getOrElse { error ->
            sessionStore.update(session.copy(status = ReconciliationSessionStatus.ERROR, errorMessage = error.message.defaultMessage))
            return Err(error)
        }

        val externalSubject = finalClaims[provider.identifierAttributeName]?.jsonPrimitive?.content
            ?: return Err(IdkError.fromDTO(ReconciliationError.IdentifierAttributeMissing(
        ...
        val matchResult = createMatchCommand.execute(
        ...
        val match = matchResult.getOrElse { error ->
            val existing = identityMatchStore.findByIdentifierHash(
                applied.tenantId, session.identifierHash, session.identifierType,
            )
            if (existing != null) {
                existing
            } else {
                return Err(IdkError.fromDTO(ReconciliationError.MatchCreationFailed(
                    reason = error.message.defaultMessage ?: "Unknown error",
                )))
            }
        }
        ...
        val encryptedIdentity = try {
            ...
            return Err(IdkError.fromDTO(ReconciliationError.CryptoFailed(
                operation = "encrypt",
        ...
        val updatedSession = sessionStore.update(session.copy(
    ...
    private fun applyAttributeMappings(
        ...
        if (mappings.isEmpty()) return Ok(claims)
        ...
        if (missingRequired.isNotEmpty()) {
            return Err(IdkError.fromDTO(ReconciliationError.RequiredAttributesMissing(
