...
class CreateReconciliationSessionCommandTest {
    ...
    @Test
    fun createSessionSuccessfully() = runTest {
        ctx.providerStore.save(testProvider)

        val result = ctx.createSessionCommand.execute(
        ...
        assertTrue(authUrl.contains("redirect_uri=https%3A%2F%2Fapp.example.com%2Fcallback"),
            "Auth URL should contain URL-encoded redirect_uri, got: $authUrl")
        assertTrue(authUrl.contains("scope=openid+profile+email"))
    }

    @Test
    fun createSessionFailsWithUnknownProvider() = runTest {
        val result = ctx.createSessionCommand.execute(
            CreateReconciliationSessionArgs(
                identifierHash = "sha256:abc123",
                identifierType = IdentifierType.EMAIL,
        ...
        assertTrue(
            result.error.message.defaultMessage?.contains("not found") == true,
            "Error should mention provider not found"
        )
    }

    @Test
    fun createSessionWithDifferentProvider() = runTest {
        val otherProvider = testProvider.copy(
            id = "other-provider",
    ...
    @Test
    fun createSessionPkceVerifierIsRfc7636Compliant() = runTest {
        ctx.providerStore.save(testProvider)
