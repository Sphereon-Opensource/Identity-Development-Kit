 ...
 */
internal object CborDecoderRuntimeImpl {
    /**
    ...
    fun decode(
        ...
        val state = CborDecodeRuntimeState(config)
    ...
    internal fun decodeWithState(
        ...
        return try {
            ...
            when (majorType) {
                ...
                MajorType.ARRAY -> {
                    decodeArray(encodedCbor, offset, additionalInformation, state)
                }

                MajorType.MAP -> {
                    decodeMap(encodedCbor, offset, additionalInformation, state)
                }
                ...
                MajorType.SPECIAL -> {
                    decodeSpecial(encodedCbor, offset, additionalInformation)
                }
            }
        } catch (_: IndexOutOfBoundsException) {
            Err(CborError.OUT_OF_BOUNDS(offset, 1, encodedCbor.size - offset))
        } catch (expected: Throwable) {
            Err(CborError.DECODE_ERROR(expected.message ?: "Unknown error", expected))
        }
    }
    ...
    private fun decodeLength(
        encodedCbor: ByteArray,
        offset: Int,
    ): IdkResult<Pair<Int, ULong>, IdkError> {
        if (offset >= encodedCbor.size) {
            return Err(CborError.OUT_OF_BOUNDS(offset, 1, encodedCbor.size - offset))
        }
        ...
        if (additionalInformation < 24) {
            return Ok(Pair(offset + 1, additionalInformation.toULong()))
        }
        // Each case reads N extra bytes after offset; check bounds before accessing.
        // wasmJs traps on out-of-bounds array access instead of throwing IndexOutOfBoundsException.
        val (newOffset, requiredBytes) =
            when (additionalInformation) {
                ...
                31 -> return Ok(Pair(offset + 1, 0UL))

                // indefinite length marker
                else -> return Err(CborError.INVALID_ADDITIONAL_INFO(additionalInformation, offset))
            }
        if (newOffset > encodedCbor.size) {
            return Err(CborError.OUT_OF_BOUNDS(offset, requiredBytes, encodedCbor.size - offset))
        }
        return when (additionalInformation) {
            24 -> Ok(Pair(newOffset, encodedCbor.getUInt8(offset + 1).toULong()))
            25 -> Ok(Pair(newOffset, encodedCbor.getUInt16(offset + 1).toULong()))
            26 -> Ok(Pair(newOffset, encodedCbor.getUInt32(offset + 1).toULong()))
            27 -> Ok(Pair(newOffset, encodedCbor.getUInt64(offset + 1)))
    ...
    private fun decodeUnsignedInteger(
        ...
        decodeLength(encodedCbor, offset).map { (newOffset, value) ->
    ...
    private fun decodeArray(
        ...
        try {
            ...
            return if (isIndefinite) {
                ...
                while (currentOffset < encodedCbor.size) {
                    if (encodedCbor[currentOffset].toUByte() == CborRuntimeImpl.BREAK) {
                        state.exitContainer()
    ...
    private fun decodeTagged(
        ...
        return try {
            decodeLength(encodedCbor, offset).flatMap { (tagOffset, tagNumber) ->
                ...
                val item =
                    if (tagNum == CborTagged.ENCODED_CBOR && taggedItem is CborByteString) {
    ...
    private fun decodeSpecial(
        ...
        return when (additionalInformation) {
            ...
            in 0..19 -> {
                Err(CborError.DECODE_ERROR("Reserved simple value $additionalInformation at offset $offset"))
            }

            20 -> {
                Ok(Pair(offset + 1, CborSimple.FALSE))
            }

            21 -> {
                Ok(Pair(offset + 1, CborSimple.TRUE))
            }

            22 -> {
                Ok(Pair(offset + 1, CborSimple.NULL))
            }

            23 -> {
                Ok(Pair(offset + 1, CborSimple.UNDEFINED))
            }
            ...
            31 -> {
                Err(CborError.UNEXPECTED_BREAK(offset))
            }

            else -> {
                Err(CborError.INVALID_ADDITIONAL_INFO(additionalInformation, offset))
            }
        }
    }
        ...
        val value: Float =
            when {
                ...
                exp == 31 -> {
                    ...
                    (mant + 1024) * HALF_FLOAT_EXP_TABLE[exp]
                }
            }
