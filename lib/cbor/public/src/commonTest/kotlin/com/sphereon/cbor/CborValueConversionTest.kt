...
class CborValueConversionTest {
    ...
    fun testNestedCborMap() {
        val innerMap = CborMap(mutableMapOf(
    ...
    @Test
    fun testBuildCborArrayWithVariousTypes() {
    ...
    fun testCborByteStringArrayEncodeToHex() {
        val array: CborArray<CborByteString> = CborArray(mutableListOf(
    ...
    fun testCoseLabelFromCborStructureWithNInt() {
        // CborNInt stores the absolute value, and fromCborStructure negates it
    ...
    private class TestLabeledMap : NumberLabeledMap() {
        fun addItem(label: Int, item: CborItem<*>?) {
            putLabel(label, item)
        }
    ...
    @Test
    fun testCborMapEncodeItemBuilder() {
        val builder = CborMap.encodeItemBuilder("subject")
        builder.put("key", CborString("value"))
    ...
    // ========== CborSupport branches ==========

    @Test
    fun testCborSupportSerializer() {
        val serializer = CborSupport.serializer
    ...
    fun testCborSupportItemToValue() {
        ...
        val result = CborSupport.itemToValue(item)
    ...
    fun testCborSupportItemToByteArray() {
        val item = CborString("test")
        val bytes = CborSupport.itemToByteArray(item)
        assertNotNull(bytes)
    ...
    fun testCborSupportItemFromByteArray() {
        val item = CborString("test")
        val bytes = CborSupport.itemToByteArray(item)
        val decoded = CborSupport.itemFromByteArray<String>(bytes)
        assertIs<CborString>(decoded)
    ...
    fun testCborSupportDataItemFromValue() {
        ...
        val encodedItem = CborSupport.dataItemFromValue<ByteArray>(bytes)
    ...
    fun testCborSupportDataItemToByteArray() {
        ...
        val bytes = CborSupport.dataItemToByteArray(encodedItem)
        assertNotNull(bytes)
    }
    ...
    fun testCborSupportDataItemFromByteArray() {
        ...
        val encodedItem = CborEncodedItem<CborString>(encoded)
        val bytes = CborSupport.dataItemToByteArray(encodedItem)
        val decoded = CborSupport.dataItemFromByteArray<CborString>(bytes)
        assertNotNull(decoded)
    }

    @Test
    fun testCborSerializer() {
        assertNotNull(cborSerializer)
    }
    ...
    fun testByteArrayToNumberLabeledMap() {
        ...
        val encoded = Cbor.encode(innerMap)
        val labeledMap = encoded.toNumberLabeledMap()
    ...
    @Test
    fun testCborEncodedItemFromData() {
        val data = CborMap(mutableMapOf(CborString("key") to CborUInt(1)))
        val encoded = CborEncodedItem.fromData(data)
        assertNotNull(encoded)
    }
    ...
    // ========== ArrayBuilder branches ==========

    @Test
    fun testArrayBuilderWithAllTypes() {
        val array = buildCborArray {
            add(CborString("str"))
            add(CborUInt(42))
            add(CborNInt(5))
    ...
    fun testCborMapWithNestedMapValue() {
        ...
        val innerMap = CborMap(mutableMapOf(CborString("inner") to CborUInt(1)))
        val outerMap = CborMap(mutableMapOf(
    ...
    fun testNumberLabeledMapRequiredLabel() {
        @Suppress("UNCHECKED_CAST")
        val map = NumberLabeledMap(CborMap(mutableMapOf(
    ...
    @Test
    fun testCborEncodedItemFromDataMap() {
        val original = CborMap(mutableMapOf(CborString("key") to CborUInt(42)))
        val encodedItem = CborEncodedItem.fromData(original)
        assertNotNull(encodedItem)
        assertNotNull(encodedItem.value)
    }
    ...
    @Test
    fun testCborEncodedItemFromDataWithArray() {
        val array = CborArray(mutableListOf(CborUInt(1), CborUInt(2)))
        val encoded = CborEncodedItem.fromData(array)
        assertNotNull(encoded.value)
    }

    // Test CborEncodedItem fromData with nested structures
    @Test
    fun testCborEncodedItemFromDataWithNestedMap() {
        val innerMap = CborMap(mutableMapOf(CborString("inner") to CborUInt(1)))
        val outerMap = CborMap(mutableMapOf(CborString("outer") to innerMap))
        val encoded = CborEncodedItem.fromData(outerMap)
        assertNotNull(encoded.value)
    }
    ...
    fun testCborArrayToJsonWithCDDLWithEncodedItem() {
        val encoded = CborEncodedItem.fromData(CborString("test"))
    ...
    @Test
    fun testCborEncodedItemFromDataWithUInt() {
        val encoded = CborEncodedItem.fromData(CborUInt(42))
        assertNotNull(encoded.value)
    }

    @Test
    fun testCborEncodedItemFromDataWithNInt() {
        val encoded = CborEncodedItem.fromData(CborNInt(42))
        assertNotNull(encoded.value)
    }

    @Test
    fun testCborEncodedItemFromDataWithFloat() {
        val encoded = CborEncodedItem.fromData(CborFloat32(3.14f))
        assertNotNull(encoded.value)
    }

    @Test
    fun testCborEncodedItemFromDataWithDouble() {
        val encoded = CborEncodedItem.fromData(CborDouble(3.14))
        assertNotNull(encoded.value)
    }

    @Test
    fun testCborEncodedItemFromDataWithBoolean() {
        val encoded = CborEncodedItem.fromData(CborSimple.TRUE)
        assertNotNull(encoded.value)
    }

    @Test
    fun testCborEncodedItemFromDataWithByteString() {
        val encoded = CborEncodedItem.fromData(CborByteString(byteArrayOf(1, 2, 3)))
        assertNotNull(encoded.value)
    }
    ...
    fun testCDDLListFromJsonWithNestedObjectValue() {
        val jsonWithObject = buildJsonArray {
            add(buildJsonObject {
                put("key", JsonPrimitive("value"))
            })
        }
    ...
    fun testCDDLMapFromJsonWithArrayValue() {
        val jsonWithArray = buildJsonObject {
    ...
    fun testCborMapWithMixedLabelTypes() {
        val map = CborMap(mutableMapOf<CborItem<*>, CborItem<*>>(
