...
class CborSimpleValueTest {
    ...
    fun testFalse() {
        ...
        val decoded = Cbor.decode<CborBool>(encoded)
    ...
    fun testNull() {
        ...
        val decoded = Cbor.decode<CborSimple<*>>(encoded)
    ...
    fun testDecodeKnownVectors() {
        // false
        val falseDecoded = Cbor.decode<CborBool>("f4".decodeFromHex())
        assertEquals(false, falseDecoded.value)

        // true
        val trueDecoded = Cbor.decode<CborBool>("f5".decodeFromHex())
        assertEquals(true, trueDecoded.value)

        // null
        val nullDecoded = Cbor.decode<CborSimple<*>>("f6".decodeFromHex())
        assertSame(CborSimple.NULL, nullDecoded)

        // undefined
        val undefinedDecoded = Cbor.decode<CborSimple<*>>("f7".decodeFromHex())
    ...
    fun testInvalidSimpleValueTwoBytes() {
        ...
        assertFailsWith<IllegalArgumentException> {
            Cbor.decode<CborSimple<*>>("f810".decodeFromHex()) // simple(16) in two bytes
        }
    }
