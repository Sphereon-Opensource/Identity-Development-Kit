...
class CborCoreTypesTest {
    ...
    // ========== MapBuilder more branches ==========

    @Test
    fun testMapBuilderPutNullByteArrayNotOptional() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)

        assertFailsWith<IllegalArgumentException> {
            builder.put("key", null as ByteArray?, optional = false)
        }
    }
    ...
    fun testMapBuilderPutCborArray() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
        val array: CborArray<CborItem<*>> = CborArray(mutableListOf(CborUInt(1), CborUInt(2)))
    ...
    @Test
    fun testMapBuilderPutCborArrayNull() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
        builder.put("items", null as CborArray<CborItem<*>>?, optional = true)
    ...
    fun testMapBuilderPutCborMap() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
        val innerMap: CborMap<CborItem<*>, CborItem<*>> = CborMap(mutableMapOf(CborString("inner") to CborUInt(1)))
        builder.put("nested", innerMap)

        assertEquals(1, cborMap.value.size)
    }

    @Test
    fun testMapBuilderPutCborMapNull() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
        builder.put("nested", null as CborMap<CborItem<*>, CborItem<*>>?, optional = true)

        assertEquals(0, cborMap.value.size)
    }

    @Test
    fun testMapBuilderPutHasToCborNull() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
        builder.put("key", null as HasToCbor<*>?, optional = true)

        assertEquals(0, cborMap.value.size)
    }

    @Test
    fun testMapBuilderPutNullListOptional() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
        builder.put("key", null as List<Long>?, optional = true)

        assertEquals(0, cborMap.value.size)
    }

    @Test
    fun testMapBuilderPutNullMapOptional() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
    ...
    @Test
    fun testMapBuilderPutNullArrayOptional() {
        val cborMap = CborMap(mutableMapOf<CborItem<*>, CborItem<*>?>())
        val builder = MapBuilder(Unit, cborMap)
        builder.put("key", null as Array<Long>?, optional = true)
    ...
    fun testCoseLabelFromCborItemNegative() {
        // CborNInt stores the value, fromCborStructure creates NumberLabel with negated value
