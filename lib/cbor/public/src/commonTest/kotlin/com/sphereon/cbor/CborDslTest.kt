...
class CborDslTest {
    ...
    @Test
    fun testDslMatchesLegacyBuilder() {
        // Build with DSL
        val dslMap = cborMap {
            "name" to "test"
            1 to "numeric"
        ...
        val legacyMap = CborMap.builder(Unit)
            .put("name", "test")
            .put(1L, "numeric")
            .end()
            .build() as CborMap<*, *>
        ...
        // Compare
        assertEquals(dslMap.value.size, legacyMap.value.size)

        val dslEncoded = Cbor.encode(dslMap)
        val legacyEncoded = Cbor.encode(legacyMap)

        // Note: The exact byte order may differ due to map iteration order,
        // but both should decode to equivalent structures
        assertTrue(dslEncoded.isNotEmpty())
        assertTrue(legacyEncoded.isNotEmpty())
    }
}
