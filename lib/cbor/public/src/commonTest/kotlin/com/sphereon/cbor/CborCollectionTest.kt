...
class CborCollectionTest {
    ...
    @Test
    fun testEmptyArray() {
        val encoded = Cbor.encode(CborArray(mutableListOf()))
    ...
    fun testDecodeIndefiniteLengthArray() {
        // [_ 1, 2, 3]
        val hex = "9f010203ff"
        val decoded = Cbor.decode<CborArray<CborItem<*>>>(hex.decodeFromHex())
        assertEquals(3, decoded.value.size)
        assertTrue(decoded.indefiniteLength)
    }
    ...
    @Test
    fun testEmptyMap() {
        val encoded = Cbor.encode(CborMap(mutableMapOf()))
    ...
    fun testDecodeIndefiniteLengthMap() {
        ...
        val decoded = Cbor.decode<CborMap<CborItem<*>, CborItem<*>>>(hex.replace(" ", "").decodeFromHex())
        assertTrue(decoded.indefiniteLength)
        assertEquals(2, decoded.value.size)
    }
    ...
    fun testDecodeMapKnownVector() {
        // {1: 2, 3: 4}
        val decoded = Cbor.decode<CborMap<CborItem<*>, CborItem<*>>>("a201020304".decodeFromHex())
        assertEquals(2, decoded.value.size)
    }
