...
open class CborByteString(
    ...
    override fun toJsonSimple(): JsonElement = JsonPrimitive(encodeValueTo(Encoding.BASE64URL))
...
class CborByteStringIndefLength(
    value: List<cddl_bstr>,
) : CborItem<List<cddl_bstr>>(value, CDDL.bstr_indef_length) {
    ...
    override fun encode(builder: ByteStringBuilder) {
        ...
        value.forEach {
            cborEncodeLength(builder, majorType, it.size)
            builder.append(it)
        }
...
fun CborArray<CborByteString>.encodeToBase64Array(urlSafe: Boolean = false) =
    this.value
        .map {
            if (urlSafe) {
                it.encodeValueTo(Encoding.BASE64URL)
            } else {
                it.encodeValueTo(Encoding.BASE64)
            }
        }.toTypedArray()

fun CborArray<CborByteString>.encodeToBase64UrlArray() = this.value.map { it.encodeValueTo(Encoding.BASE64URL) }.toTypedArray()

fun CborArray<CborByteString>.encodeToArray(encoding: Encoding) = this.value.map { it.encodeValueTo(encoding) }.toTypedArray()
