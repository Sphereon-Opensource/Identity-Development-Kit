 ...
 */
@OptIn(ExperimentalContracts::class)
inline fun <T> cborMapBuilder(
    subject: T,
    builderAction: CborMapScope.() -> Unit,
): CborBuilder<T> {
    contract {
        callsInPlace(builderAction, InvocationKind.EXACTLY_ONCE)
    }
 ...
 */
@OptIn(ExperimentalContracts::class)
inline fun <T> cborArrayBuilder(
    subject: T,
    builderAction: CborArrayScope.() -> Unit,
): CborBuilder<T> {
    contract {
        callsInPlace(builderAction, InvocationKind.EXACTLY_ONCE)
    }
 ...
 */
fun CborItem<*>.encode(): ByteArray = encodeCbor()
