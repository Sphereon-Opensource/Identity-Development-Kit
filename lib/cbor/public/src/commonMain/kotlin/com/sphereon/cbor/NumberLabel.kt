...
@JsExportCompat
class NumberLabel(value: Long) : CoseLabel<Long>(abs(value), if (value >= 0) CDDL.uint else CDDL.nint, LabelType.Number) {
    @JsName("fromInt")
    ...
    override fun encode(builder: ByteStringBuilder) {
        return toCborStructure().encode(builder)/*
    ...
    override fun toCborStructure(): CborItem<Long> {
        return if (majorType == MajorType.NEGATIVE_INTEGER) {
            CborNInt(value)
        } else {
            CborUInt(value)
        }
    ...
    companion object {
        @JsStatic
        fun fromCborStructure(structure: CborItem<*>): NumberLabel {
