...
/**
 * Kotlin/JS raw DEFLATE — WHATWG `CompressionStream` / `DecompressionStream` with format
 * `"deflate-raw"` (RFC 1951, no zlib wrapper). Available in Node.js 18+ and every evergreen
 * browser. The API is async (TransformStream), wrapped via `kotlinx.coroutines.await`.
 */
internal actual suspend fun rawDeflate(data: ByteArray): ByteArray = transform(data, compress = true)

internal actual suspend fun rawInflate(data: ByteArray): ByteArray = transform(data, compress = false)

private suspend fun transform(
    input: ByteArray,
    compress: Boolean,
): ByteArray {
    val output = runCompressionPipeline(input, if (compress) "CompressionStream" else "DecompressionStream").await()
    return uint8ArrayToByteArray(output)
}
