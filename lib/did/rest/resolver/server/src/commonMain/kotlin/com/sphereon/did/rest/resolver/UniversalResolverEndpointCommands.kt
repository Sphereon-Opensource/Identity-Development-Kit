...
class ResolveDidEndpointCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        val did = req.pathParams["identifier"]
            ?: return Ok(errorResponse(400, "Missing path parameter: identifier"))
        ...
        val method = extractMethod(did)
            ?: return Ok(errorResponse(400, "Invalid DID format: $did"))

        // Get resolver for this method
        val resolver = resolverRegistry.getResolver(method)
            ?: return Ok(jsonResponse(
                404,
                json.encodeToString(DidResolutionErrorResponse(
                    didResolutionMetadata = DidResolutionMetadataResponse(
...
class GetResolverPropertiesEndpointCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        val methods = resolverRegistry.getSupportedMethods()
        val methodCapabilities = methods.associateWith { method ->
            resolverRegistry.getResolver(method)?.capabilities
        }

        val properties = ResolverPropertiesResponse(
            methods = methods,
            methodCapabilities = methodCapabilities.filterValues { it != null }
