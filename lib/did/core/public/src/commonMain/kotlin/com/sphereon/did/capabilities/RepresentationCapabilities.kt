...
data class RepresentationCapabilities(
    ...
    companion object {
         ...
         */
        val JWK_ONLY: RepresentationCapabilities = RepresentationCapabilities(
            jsonWebKey2020 = true
        )
         ...
         */
        val JWK_AND_MULTIKEY: RepresentationCapabilities = RepresentationCapabilities(
            jsonWebKey2020 = true,
            multikey = true
        )
         ...
         */
        val ALL: RepresentationCapabilities = RepresentationCapabilities(
            jsonWebKey2020 = true,
            multikey = true,
     ...
     */
    fun getSupportedTypes(): List<VerificationMethodType> = buildList {
        if (jsonWebKey2020) add(VerificationMethodType.JSON_WEB_KEY_2020)
        if (multikey) add(VerificationMethodType.MULTIKEY)
        if (ed25519VerificationKey2020) add(VerificationMethodType.ED25519_VERIFICATION_KEY_2020)
        if (ed25519VerificationKey2018) add(VerificationMethodType.ED25519_VERIFICATION_KEY_2018)
        if (ecdsaSecp256k1VerificationKey2019) add(VerificationMethodType.ECDSA_SECP256K1_VERIFICATION_KEY_2019)
    }

    /**
     * Checks if a specific verification method type is supported.
     */
    fun supportsType(type: VerificationMethodType): Boolean = when (type) {
