...
internal object VerificationMethodOrReferenceSerializer : KSerializer<VerificationMethodOrReference> {
    ...
    override fun serialize(
        encoder: Encoder,
        value: VerificationMethodOrReference,
    ) {
        val jsonEncoder =
            checkNotNull(encoder as? JsonEncoder) { "This serializer only works with JSON" }
    ...
    override fun deserialize(decoder: Decoder): VerificationMethodOrReference {
        val jsonDecoder =
            checkNotNull(decoder as? JsonDecoder) { "This serializer only works with JSON" }
        ...
        return when (element) {
            ...
            is JsonObject -> {
                ...
                throw IllegalArgumentException("Expected string or object, got: $element")
            }
        }
    }
}
