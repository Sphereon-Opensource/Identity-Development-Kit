...
class DidExternalIdentifierResolutionServiceImpl(
...
), DidExternalIdentifierResolutionService, Scoped {
    ...
    override suspend fun doExecute(
        ...
        // Parse the DID
        val parsedDid = parseDid(did) ?: return IdkError.ILLEGAL_ARGUMENT_ERROR(
            message = "Invalid DID format: $did"
        ).asErrorResult()

        // Get the resolver for this method
        val resolver = resolverRegistry.getResolver(parsedDid.method)
            ?: return IdkError.NOT_FOUND_ERROR(
                message = "No resolver found for DID method: ${parsedDid.method}"
            ).asErrorResult()

        // Resolve the DID
        val resolutionResult = resolver.resolve(did, DidResolutionOptions())
    ...
    override suspend fun supports(args: Any): Boolean {
        ...
        return when (args) {
            is ExternalIdentifierDidOpts -> isSupportedDidIdentifier(args.identifier) == null
    ...
    override suspend fun isSupportedIdentifier(identifier: Any): Boolean {
        return isSupportedDidIdentifier(identifier) == null
    }
    ...
    private fun isSupportedDidIdentifier(identifier: Any): IdkErrorType? {
        ...
        val parsed = parseDid(identifier)
    ...
    override suspend fun asSupportedOpts(opts: ExternalIdentifierOptsOrResult): IdkResult<ExternalIdentifierDidOpts, IdkErrorType> {
        return if (isSupportedOpts(opts))
    ...
    private fun parseDid(did: String): ParsedDID? {
        val parts = did.split(":")
        if (parts.size < 3 || parts[0] != "did") return null
        ...
        val (idPart, fragment) = if (fragmentIndex >= 0) {
        ...
        val (methodSpecificId, path) = if (pathIndex >= 0) {
    ...
    private fun extractDidDocumentJwks(doc: com.sphereon.did.models.DidDocument?): DidDocumentJwks? {
        ...
        doc.authentication?.let { refs ->
            val jwks = refs.mapNotNull { ref ->
                getJwkFromRef(doc, ref)
            }
            if (jwks.isNotEmpty()) result["authentication"] = jwks
        }
        ...
        doc.assertionMethod?.let { refs ->
            ...
            if (jwks.isNotEmpty()) result["assertionMethod"] = jwks
        }
        ...
        doc.keyAgreement?.let { refs ->
            ...
            if (jwks.isNotEmpty()) result["keyAgreement"] = jwks
        }

        return result.ifEmpty { null }
    }

    private fun getJwkFromRef(doc: com.sphereon.did.models.DidDocument, ref: com.sphereon.did.models.VerificationMethodOrReference): JwkType? {
        ...
        val vm = doc.verificationMethod?.find {
