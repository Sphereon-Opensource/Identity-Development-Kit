...
class ResolveDidCommandImpl(
    ...
    ResolveDidCommand,
    ResolveDidCommandService {
    override val commandId: String get() = ResolveDidCommand.COMMAND_ID

    override suspend fun resolve(args: ResolveDidArgs): IdkResult<DidResolutionResult, IdkError> = execute(args)
    ...
    override suspend fun doExecute(
        ...
        val result =
            resolver.resolve(did, options).getOrElse {
                return Err(it)
            }
        ...
        if (resolver.capabilities.resolution.allowsCaching) {
            val ttl =
                resolver.capabilities.resolution.cacheTtlSeconds
                    ?.seconds
    ...
    private fun applyFilter(
        ...
        val afterKeyTypeFilter =
            if (keyTypesFilter != null) {
        ...
        val finalFilteredVmByPurpose =
            if (vmTypesFilter != null) {
