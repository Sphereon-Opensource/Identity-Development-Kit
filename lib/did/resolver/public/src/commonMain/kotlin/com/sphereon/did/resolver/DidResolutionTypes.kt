...
data class DidResolutionMetadata(
    ...
    companion object {
         ...
         */
        fun success(contentType: String = "application/did+json"): DidResolutionMetadata =
            DidResolutionMetadata(contentType = contentType)

        /**
         * Creates metadata for a not-found error.
         */
        fun notFound(message: String? = null): DidResolutionMetadata =
            DidResolutionMetadata(error = ERROR_NOT_FOUND, message = message)

        /**
         * Creates metadata for an invalid DID error.
         */
        fun invalidDid(message: String? = null): DidResolutionMetadata =
            DidResolutionMetadata(error = ERROR_INVALID_DID, message = message)
    }
...
data class DidResolutionResult(
    ...
    companion object {
         ...
         */
        fun notFound(did: String): DidResolutionResult = DidResolutionResult(
            didDocument = null,
         ...
         */
        fun error(errorCode: String, message: String? = null): DidResolutionResult = DidResolutionResult(
            didDocument = null,
     ...
     */
    fun getAuthenticationMethods(): List<VerificationMethod> =
        verificationMethodsByPurpose[VerificationPurpose.AUTHENTICATION] ?: emptyList()

    /**
     * Gets all assertion method verification methods (for signing credentials).
     */
    fun getAssertionMethods(): List<VerificationMethod> =
        verificationMethodsByPurpose[VerificationPurpose.ASSERTION_METHOD] ?: emptyList()

    /**
     * Gets all key agreement verification methods (for encryption).
     */
    fun getKeyAgreementMethods(): List<VerificationMethod> =
        verificationMethodsByPurpose[VerificationPurpose.KEY_AGREEMENT] ?: emptyList()

    /**
     * Finds a verification method by kid (key ID / fragment).
     */
    fun getVerificationMethodByKid(kid: String): VerificationMethod? =
        didDocument?.getVerificationMethodById(kid)

    /**
     * Finds a service by ID.
     */
    fun getServiceById(serviceId: String): DidService? =
        didDocument?.getServiceById(serviceId)
}
...
data class DidResolutionFilter(
    ...
    companion object {
         ...
         */
        fun forAuthentication(): DidResolutionFilter =
            DidResolutionFilter(purposes = listOf(VerificationPurpose.AUTHENTICATION))

        /**
         * Creates a filter for assertion methods (signing credentials).
         */
        fun forAssertionMethod(): DidResolutionFilter =
            DidResolutionFilter(purposes = listOf(VerificationPurpose.ASSERTION_METHOD))

        /**
         * Creates a filter for key agreement methods (encryption).
         */
        fun forKeyAgreement(): DidResolutionFilter =
            DidResolutionFilter(purposes = listOf(VerificationPurpose.KEY_AGREEMENT))

        /**
         * Creates a filter for a specific kid.
         */
        fun forKid(kid: String): DidResolutionFilter =
            DidResolutionFilter(kid = kid)

        /**
         * Creates a filter for a specific service.
         */
        fun forService(serviceId: String): DidResolutionFilter =
            DidResolutionFilter(serviceId = serviceId)
    }
}
