...
class SqliteDidRepositoryImpl(
    ...
    override suspend fun save(record: DidRecord): IdkResult<Unit, IdkError> = withContext(Dispatchers.Default) {
        try {
            // Check if DID already exists
            val existing = queries.findByDid(record.did).executeAsOneOrNull()
            if (existing != null) {
                return@withContext Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
            ...
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to save DID record: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun findByDid(did: String): IdkResult<DidRecord?, IdkError> = withContext(Dispatchers.Default) {
        try {
            val row = queries.findByDid(did).executeAsOneOrNull()
            Ok(row?.toDidRecord())
        } catch (e: Exception) {
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to find DID record: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun findByAlias(alias: String): IdkResult<DidRecord?, IdkError> = withContext(Dispatchers.Default) {
        try {
            val row = queries.findByAlias(alias).executeAsOneOrNull()
            Ok(row?.toDidRecord())
        } catch (e: Exception) {
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to find DID record by alias: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun findAll(filter: DidRecordFilter?): IdkResult<List<DidRecord>, IdkError> = withContext(Dispatchers.Default) {
        try {
            val rows = if (filter != null) {
                queries.findAllFiltered(
            ...
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to list DID records: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun update(record: DidRecord): IdkResult<Unit, IdkError> = withContext(Dispatchers.Default) {
        try {
            // Check if record exists
            val existing = queries.findById(record.id).executeAsOneOrNull()
            if (existing == null) {
                return@withContext Err(IdkError.NOT_FOUND_ERROR(
            ...
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to update DID record: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun delete(did: String): IdkResult<Unit, IdkError> = withContext(Dispatchers.Default) {
        try {
            // Check if DID exists
            ...
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to delete DID record: ${e.message}",
                exception = e
            ))
        }
    }
    ...
    ): IdkResult<Unit, IdkError> = withContext(Dispatchers.Default) {
        try {
            // Check for duplicate
            ...
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to save key mapping: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun getKeyMappings(didRecordId: String): IdkResult<List<DidKeyMappingRecord>, IdkError> = withContext(Dispatchers.Default) {
        try {
            val rows = queries.findKeyMappingsByDidRecordId(didRecordId).executeAsList()
            Ok(rows.map { it.toDidKeyMappingRecord() })
        } catch (e: Exception) {
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to get key mappings: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun deleteKeyMapping(mappingId: String): IdkResult<Unit, IdkError> = withContext(Dispatchers.Default) {
        try {
            ...
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to delete key mapping: ${e.message}",
                exception = e
            ))
        }
    }

    override suspend fun deleteKeyMappingsForDid(didRecordId: String): IdkResult<Unit, IdkError> = withContext(Dispatchers.Default) {
        try {
            queries.deleteKeyMappingsByDidRecordId(didRecordId)
            Ok(Unit)
        } catch (e: Exception) {
            Err(IdkError.UNKNOWN_ERROR(
                message = "Failed to delete key mappings: ${e.message}",
                exception = e
            ))
        }
    }
     ...
     */
    private fun Did_record.toDidRecord(): DidRecord = DidRecord(
        id = id,
        did = did,
        method = method,
        alias = alias,
     ...
     */
    private fun Did_key_mapping.toDidKeyMappingRecord(): DidKeyMappingRecord = DidKeyMappingRecord(
        id = id,
