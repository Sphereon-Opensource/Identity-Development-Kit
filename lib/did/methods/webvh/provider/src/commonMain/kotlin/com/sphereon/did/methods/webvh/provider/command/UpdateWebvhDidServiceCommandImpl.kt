...
@ContributesBinding(SessionScope::class, binding = binding<UpdateWebvhDidServiceCommand>())
class UpdateWebvhDidServiceCommandImpl(
...
) : TypedServiceCommandAdapter<UpdateWebvhDidInput, UpdateWebvhDidOutput, IdkError>(
        commandId = UpdateWebvhDidServiceCommand.COMMAND_ID,
        execution = execution,
        inputTypeToken = typeToken<UpdateWebvhDidInput>(),
        outputTypeToken = typeToken<UpdateWebvhDidOutput>(),
    ),
    UpdateWebvhDidServiceCommand {
    override val commandId: String get() = UpdateWebvhDidServiceCommand.COMMAND_ID

    override suspend fun supports(args: Any): Boolean = args is UpdateWebvhDidInput

    override suspend fun doExecute(
        args: UpdateWebvhDidInput,
        applyDuring: (UpdateWebvhDidInput) -> UpdateWebvhDidInput,
    ): IdkResult<UpdateWebvhDidOutput, IdkError> {
        val input = applyDuring(args)
        if (input.existingLog.isEmpty()) {
            return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "existingLog is required for update"))
        }
        ...
        val verificationMethodId =
            input.newDocument.verificationMethod
                ?.firstOrNull()
                ?.id
                ?: "${input.did}#key-1"
        ...
        val entryWithoutProof =
            WebvhLogEntry(
                // Predecessor versionId baked into the entry-hash input.
                versionId = prior.versionId,
                versionTime = versionTime,
                parameters = input.parameterChanges,
                state = input.newDocument,
                proof = emptyList(),
            )
        val signedEntry =
            signer
        ...
        return Ok(
            UpdateWebvhDidOutput(
                newEntry = signedEntry,
