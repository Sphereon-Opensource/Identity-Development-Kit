...
class KeyDidProviderImpl(
    ...
    override suspend fun create(
        ...
        val multicodecBytes = jwkToMulticodecBytes(jwk).getOrElse {
            return Err(it)
        }
        ...
        return when (jwk.kty) {
            JwaKeyType.OKP -> {
                ...
                val xBytes = try {
                    ...
                    return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
                        message = "Failed to decode x coordinate: ${e.message}"
                    ))
                }

                val prefix = when (curve) {
            ...
            JwaKeyType.EC -> {
                ...
                val yBytes = try {
                    ...
                    return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
                        message = "Failed to decode y coordinate: ${e.message}"
                    ))
                }
    ...
    companion object {
        ...
        fun didFromSecp256k1PublicKey(compressedPublicKey: ByteArray): String {
            require(compressedPublicKey.size == 33) { "Secp256k1 compressed public key must be 33 bytes" }
        ...
        fun didFromP256PublicKey(compressedPublicKey: ByteArray): String {
            require(compressedPublicKey.size == 33) { "P-256 compressed public key must be 33 bytes" }
        ...
        fun didFromP384PublicKey(compressedPublicKey: ByteArray): String {
            require(compressedPublicKey.size == 49) { "P-384 compressed public key must be 49 bytes" }
