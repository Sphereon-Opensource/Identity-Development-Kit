...
import com.sphereon.did.manager.impl.testutil.createDidManagerTestAppComponent
...
class DidWebCreationDslE2ETest {
    ...
    fun setUp() {
        ...
        // Create the full component hierarchy via DI
        val app = createDidManagerTestAppComponent(testInstance = this)
        ...
        dslProcessor = (sessionComponent as DidCreationDslProcessorImpl.Component).didCreationDslProcessor
        didManager = (sessionComponent as DidManagerServiceImpl.Component).didManager
    }

    // ==================== did:web with Multiple Keys ====================

    @Test
    fun testCreateDidWebWithSingleVerificationMethod() = runTest {
        val result = dslProcessor.create {
            method("web")
            domain("example.com")
    ...
    @Test
    fun testCreateDidWebWithMultipleVerificationMethods() = runTest {
        val result = dslProcessor.create {
            method("web")
    ...
    @Test
    fun testCreateDidWebWithThreeKeysAndServices() = runTest {
        val result = dslProcessor.create {
            method("web")
    ...
    @Test
    fun testCreateDidWebWithPath() = runTest {
        val result = dslProcessor.create {
            method("web")
            domain("example.com")
            path("users", "alice")
        ...
        assertEquals("did:web:example.com:users:alice", managedDid.did,
    ...
    @Test
    fun testCreateDidWebWithServices() = runTest {
        val result = dslProcessor.create {
            method("web")
            domain("example.com")
        ...
        assertEquals("https://example.com/.well-known/did-configuration.json",
            linkedDomains.serviceEndpoint, "Service endpoint should match")
        ...
        assertEquals("https://example.com/didcomm", messaging.serviceEndpoint,
    ...
    @Test
    fun testAddServiceAfterCreation() = runTest {
        // Create DID without services
        val createResult = dslProcessor.create {
            method("web")
            domain("example.com")
        ...
        assertEquals("https://example.com/hub", services[0].serviceEndpoint,
            "Service endpoint should match")
    ...
    @Test
    fun testUpdateServiceEndpoint() = runTest {
        ...
        val removeResult = didManager.update(
            managedDid.did,
            DidUpdateOptions(
                currentDocument = managedDid.document,
        ...
        val updatedService = DidService(
            id = "hub",
            type = "IdentityHub",
            serviceEndpoint = "https://new-endpoint.example.com/hub"
        )

        val addResult = didManager.update(
            managedDid.did,
            DidUpdateOptions(
        ...
        assertEquals(1, services.size, "Should have 1 service")
        assertEquals("hub", services[0].id, "Service ID should match")
        assertEquals("https://new-endpoint.example.com/hub", services[0].serviceEndpoint,
    ...
    @Test
    fun testDidWebFullLifecycle() = runTest {
        ...
        val addServiceResult = didManager.update(
            managedDid.did,
            DidUpdateOptions(
                currentDocument = managedDid.document,
                addServices = listOf(newService)
    ...
    @Test
    fun testCreateDidWebWithoutDomainFails() = runTest {
        val result = dslProcessor.create {
            method("web")
            // Missing domain
            autoGenerateKey {
        ...
        assertTrue(error.message.defaultMessage.contains("domain"),
            "Error should mention domain: ${error.message.defaultMessage}")
    }

    @Test
    fun testCreateDidWebWithInvalidDomainFails() = runTest {
        val result = dslProcessor.create {
            method("web")
        ...
        val error = result.getErrorOrNull()
        assertNotNull(error, "Should have error")
        assertTrue(error.message.defaultMessage.contains("protocol"),
