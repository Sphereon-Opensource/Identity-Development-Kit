...
class DidManagerServiceImpl(
    ...
    override suspend fun create(options: DidCreateOptions): IdkResult<ManagedDid, IdkError> {
        ...
        val createResult = provider.create(options).getOrElse {
            return Err(it)
        }
        ...
        val keyMappings = options.verificationMethods.map { config ->
            DidKeyMapping(
                id = generateId("km"),
    ...
    override suspend fun get(did: String): IdkResult<ManagedDid, IdkError> {
        val record = repository.findByDid(did).getOrElse {
            return Err(it)
        } ?: return Err(IdkError.NOT_FOUND_ERROR(message = "DID not found: $did"))

        val keyMappings = repository.getKeyMappings(record.id).getOrElse {
            return Err(it)
        }
    ...
    override suspend fun getByAlias(alias: String): IdkResult<ManagedDid, IdkError> {
        val record = repository.findByAlias(alias).getOrElse {
            return Err(it)
    ...
    override suspend fun list(filter: DidFilter?): IdkResult<List<ManagedDid>, IdkError> {
        ...
        val records = repository.findAll(persistenceFilter).getOrElse {
            return Err(it)
        }
    ...
    override suspend fun update(
        ...
        val updateResult = provider.update(did, options).getOrElse {
            return Err(it)
        }

        // Update the managed DID record
        val updated = managedDid.copy(
    ...
    override suspend fun addVerificationMethod(
        ...
        val addKeyOptions = AddKeyOptions(
            currentDocument = managedDid.document,
            config = config
        )

        val updateResult = provider.addKey(did, addKeyOptions).getOrElse {
            return Err(it)
        }

        // Add the key mapping
        val newMapping = DidKeyMapping(
            id = generateId("km"),
            verificationMethodId = config.verificationMethodId,
            kmsKeyAlias = config.kmsKeyAlias,
            kmsProviderId = config.kmsProviderId,
    ...
    override suspend fun removeVerificationMethod(
        ...
        val updateResult = provider.removeKey(did, verificationMethodId).getOrElse {
            return Err(it)
        }
    ...
    override suspend fun addService(
        ...
        val updateResult = provider.addService(did, service).getOrElse {
            return Err(it)
        }
    ...
    override suspend fun removeService(
        ...
        val updateResult = provider.removeService(did, serviceId).getOrElse {
            return Err(it)
        }
    ...
    override suspend fun import(
        ...
        val existing = repository.findByDid(did).getOrElse {
            return Err(it)
        }
        if (existing != null) {
