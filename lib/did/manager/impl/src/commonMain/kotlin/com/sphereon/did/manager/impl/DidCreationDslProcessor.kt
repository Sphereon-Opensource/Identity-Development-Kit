...
class DidCreationDslProcessorImpl(
    ...
    override suspend fun create(dslResult: DidCreationDslResult): IdkResult<ManagedDid, IdkError> {
        ...
        for ((index, keyConfig) in keyConfigs.withIndex()) {
            val processedKey =
                processKeyConfig(keyConfig, index).getOrElse {
                    return Err(it)
                }
    ...
    private suspend fun processAutoGenerate(
        ...
        val algorithm =
            config.algorithm ?: deriveSignatureAlgorithm(config)
                ?: return Err(
            ...
            try {
                ...
                return Err(
                    IdkError.NOT_FOUND_ERROR(
                        message = "Key with alias '${config.alias}' not found in provider '$providerId': ${expected.message}",
                    ),
                )
            }

        val publicJwk =
            when (val key = keyInfo.key) {
                is Jwk -> key
 ...
 * Example usage:
 * ```kotlin
 * val processor = sessionGraph.didCreationDslProcessor
