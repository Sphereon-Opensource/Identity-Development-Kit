...
class X509VerifyServiceAppleAdapterImpl() : X509CoroutinesCallbackApple, Scoped {
        ...
        return memScoped {
            try {
                ...
                for (cert in certificateChain) {
                    val sec = derToSecCertificate(cert.der)
                ...
                val certsArray = platform.CoreFoundation.CFArrayCreateMutable(
                    allocator = null,
                    capacity = 0,
                    callBacks = null
                )
                ...
                val trustCreateStatus = SecTrustCreateWithCertificates(
                    certsArray,
                    policy,
                    trustVar.ptr
                )
                ...
                val pemAnchors = pemAndDerToCertificateChain(
                    pemChain = verifyContext.request.chainPEM,
                    derChain = verifyContext.request.chainDER
                )

                if (pemAnchors.isNotEmpty()) {
                    val anchorArray = platform.CoreFoundation.CFArrayCreateMutable(
                        allocator = null,
                        capacity = 0,
                        callBacks = null
                    )
    ...
    private fun derToSecCertificate(der: ByteArray): CFTypeRef? = memScoped {
