...
class KmsSupportClassesTest {
     ...
     * because the cache may have cached empty results from component initialization.
     */
    ...
    fun noOpKmsProviderFactoryShouldThrowOnCreate() {
        val factory = NoOpKmsProviderFactoryImpl()
        val config = KmsProviderConfig(
            id = "test-provider",
            kmsProviderType = "test"
        )

        assertFailsWith<IllegalArgumentException> {
            factory.create(config, session.asCoreApiServiceComponent().serviceExecution)
        }
    }
    ...
    fun keyStoreManagerShouldBeAvailable() {
        ...
        val manager = (app as KeyStoreManagerImpl.Component).keyStoreManager
    ...
    fun keyStoreManagerErrorMessageShouldContainAvailableFactories() {
        ...
        assertTrue(exception.message?.contains("available factories") == true ||
    ...
    fun kmsProviderManagerShouldBeAvailable() {
        app as JvmCryptoTestAppComponent
        val manager = (app as KmsProviderManagerImpl.Component).kmsProviderManager
    ...
    fun kmsProviderManagerShouldThrowForUnknownType() {
        ...
        assertFailsWith<IllegalArgumentException> {
            manager.createFromProviderConfig(config, session.asCoreApiServiceComponent().serviceExecution)
        }
    }
    ...
    fun kmsProviderManagerWithEmptyFactoriesShouldThrow() {
        ...
        val managerWithNoFactories = KmsProviderManagerImpl(
        ...
        val exception = assertFailsWith<IllegalArgumentException> {
            managerWithNoFactories.createFromProviderConfig(config, session.asCoreApiServiceComponent().serviceExecution)
        }
    ...
    fun kmsProviderManagerShouldSuccessfullyCreateProviderWhenFactoryExists() {
        ...
        val provider = manager.createFromProviderConfig(config, session.asCoreApiServiceComponent().serviceExecution)
    ...
    fun kmsProviderManagerFactoriesFilterComparison() {
        ...
        val manager = KmsProviderManagerImpl(
    ...
    fun kmsProviderConfigBinderShouldBeAvailable() {
        app as JvmCryptoTestAppComponent
        val binder = (app as KmsProviderConfigBinderImpl.Component).kmsProviderConfigBinder
    ...
    fun keyStoreConfigBinderShouldFindKeystoreIdsWithTypeSuffix() {
        app as JvmCryptoTestAppComponent
        val configService = (app as AppConfigService.Component).appConfigService
        val binder = (app as KeyStoreConfigBinderImpl.Component).keyStoreConfigBinder
    ...
    // =========== KeyStoreConfigBinderImpl Component Access Test ===========
