...
class PlatformKeyTest {
    ...
    @Test
    fun privatePlatformKeyWithInvalidEncodingShouldThrow() {
        // Test that invalid DER encoding throws
        val mockPrivateKey =
            object : PrivateKey {
                override fun getAlgorithm(): String = "EC"
                ...
                override fun getEncoded(): ByteArray = byteArrayOf(0x30, 0x03, 0x01, 0x01, 0x00) // invalid PKCS#8
            }
    ...
    @Test
    fun privatePlatformKeyWithNullEncodingShouldThrow() {
        // Mock returns "EC" algorithm but doesn't implement ECPrivateKey → falls to else branch
        // Null encoding causes failure during PKCS#8 parsing
        val mockPrivateKey =
            object : PrivateKey {
                override fun getAlgorithm(): String = "EC"
    ...
    fun privatePlatformKeyElseBranchWithNonRsaDecodingShouldThrow() {
        // DSA is not a supported key type — fails at construction (KeyTypeMapping.fromValue)
        // or at toPublicKey() depending on the key type mapping
        val mockPrivateKey =
            object : PrivateKey {
                override fun getAlgorithm(): String = "DSA"

                override fun getFormat(): String = "PKCS#8"

                override fun getEncoded(): ByteArray = byteArrayOf(0x30, 0x03, 0x01, 0x01, 0x00)
            }
