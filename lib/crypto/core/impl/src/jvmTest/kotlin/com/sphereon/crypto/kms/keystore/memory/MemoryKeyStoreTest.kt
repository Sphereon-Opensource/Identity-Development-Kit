...
class MemoryKeyStoreTest {
    ...
    fun setUp() {
        ...
        val memoryConfig = MemoryKeyStoreConfig(
        ...
        memoryKeyStore = factory.create(memoryConfig, session.asCoreApiServiceComponent().serviceExecution)
    }

    // =========== Key Storage Tests ===========

    @Test
    fun storeKeyShouldSucceed() = runTest {
    ...
    @Test
    fun getKeyByAliasShouldSucceed() = runTest {
        val keyPair = keyManagerService.generateKey(
            alg = SignatureAlgorithm.ECDSA_SHA256
        )
        val keyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
    ...
    @Test
    fun listKeysShouldReturnStoredKeys() = runTest {
    ...
    @Test
    fun deleteKeyShouldSucceed() = runTest {
    ...
    @Test
    fun deleteKeyNotFoundShouldReturnFalse() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        // Create a ManagedKeyInfo with a non-existent alias
        val nonExistentKeyInfo = com.sphereon.crypto.core.ManagedKeyInfo(
    ...
    @Test
    fun getKeyAsPublicShouldReturnPublicKey() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
    ...
    @Test
    fun listCertificateChainAliasesShouldReturnEmpty() = runTest {
    ...
    @Test
    fun listCertificateAliasesShouldReturnEmpty() = runTest {
    ...
    fun factoryShouldCreateMemoryKeystore() {
        ...
        app as JvmCryptoTestAppComponent
        val factory = app.memoryKeyStore
        val keystore = factory.create(memoryConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    fun factoryShouldRejectNonMemoryConfig() {
        ...
        val invalidConfig = com.sphereon.crypto.core.kms.KeyStoreConfigImpl(
            id = "invalid",
            keyStoreType = "other"
        )

        assertFailsWith<IllegalArgumentException> {
            factory.create(invalidConfig, session.asCoreApiServiceComponent().serviceExecution)
        }
    }

    @Test
    fun factoryShouldCreateAppScopedKeystore() {
        val appScopedConfig = MemoryKeyStoreConfig(
        ...
        app as JvmCryptoTestAppComponent
        val factory = app.memoryKeyStore
        val keystore = factory.create(appScopedConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    fun factoryShouldCreateTenantScopedKeystore() {
        val tenantScopedConfig = MemoryKeyStoreConfig(
        ...
        val keystore = factory.create(tenantScopedConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    fun factoryShouldCreatePrincipalTenantScopedKeystore() {
        val principalTenantConfig = MemoryKeyStoreConfig(
        ...
        app as JvmCryptoTestAppComponent
        val factory = app.memoryKeyStore
        val keystore = factory.create(principalTenantConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    fun factoryShouldCreateSessionScopedKeystore() {
        val sessionScopedConfig = MemoryKeyStoreConfig(
        ...
        app as JvmCryptoTestAppComponent
        val factory = app.memoryKeyStore
        val keystore = factory.create(sessionScopedConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    @Test
    fun overwriteEnabledShouldAllowReplace() = runTest {
        val keyPair1 = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
    ...
    @Test
    fun noOverwriteShouldFailOnDuplicate() = runTest {
        // Create keystore with overwriteAlias=false
        val noOverwriteConfig = MemoryKeyStoreConfig(
        ...
        app as JvmCryptoTestAppComponent
        val factory = app.memoryKeyStore
        val noOverwriteKeystore = factory.create(noOverwriteConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    fun backingStorageShouldSupportMultiTenantIsolation() {
        ...
        val resolvedKeyInfo = com.sphereon.crypto.core.ResolvedKeyInfo(
            key = com.sphereon.crypto.core.jose.Jwk(kty = com.sphereon.crypto.core.jose.JwaKeyType.EC),
            keyVisibility = KeyVisibility.PUBLIC
        )
        val managedKeyInfo = com.sphereon.crypto.core.ManagedKeyInfo(
    ...
    fun storagePartitionKeyShouldSupportDifferentScopes() {
        ...
        val tenantAndPrincipal = StoragePartitionKey.forPrincipalTenant(
            keystoreId = "scope-keystore",
            tenantId = "tenant-A",
            principalId = "user-1"
        )
        val fullScope = StoragePartitionKey.forSession(
            keystoreId = "scope-keystore",
            tenantId = "tenant-A",
            principalId = "user-1",
    ...
    fun publicFromPrivateKeyStoreShouldRequirePrivateKeyStore() {
        ...
        val publicConfig = MemoryKeyStoreConfig(
            id = "public-only-keystore",
            keyVisibility = KeyVisibility.PUBLIC.keyVisibility
        )
        app as JvmCryptoTestAppComponent
        val publicKeyStore = app.memoryKeyStore.create(publicConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    @Test
    fun publicFromPrivateKeyStoreListKeysShouldReturnPublicKeys() = runTest {
        // Create a fresh keystore for this test to avoid state pollution
        val freshConfig = MemoryKeyStoreConfig(
        ...
        app as JvmCryptoTestAppComponent
        val freshKeyStore = app.memoryKeyStore.create(freshConfig, session.asCoreApiServiceComponent().serviceExecution)
        ...
        // Keys should be converted to public visibility
        val keyForAlias = keys.find { it.alias == "pub-list-key" }
        assertNotNull(keyForAlias)
        // The toManagedPublicKeyInfo() should set keyVisibility to PUBLIC
        assertEquals(KeyVisibility.PUBLIC, keyForAlias.keyVisibility)
    }

    @Test
    fun publicFromPrivateKeyStoreGetKeyShouldReturnPublicKey() = runTest {
    ...
    @Test
    fun publicFromPrivateKeyStoreStoreKeyShouldRequirePrivateKey() = runTest {
        // Create a fresh keystore for this test
        val freshConfig = MemoryKeyStoreConfig(
    ...
    @Test
    fun publicFromPrivateKeyStoreStoreKeyShouldSucceedWithPrivateKey() = runTest {
        // Create a fresh keystore for this test
        val freshConfig = MemoryKeyStoreConfig(
    ...
    @Test
    fun publicFromPrivateKeyStoreDeleteKeyShouldDelegate() = runTest {
        // Create a fresh keystore for this test
        val freshConfig = MemoryKeyStoreConfig(
    ...
    @Test
    fun getKeyByKidShouldFindMatchingKey() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val lookupKeyInfo = com.sphereon.crypto.core.KeyInfo(
            kid = storedKey.kid,
            key = null,
            alias = null
        )
    ...
    @Test
    fun publicKeystoreShouldNotReturnPrivateKey() = runTest {
        // Create a PUBLIC keystore
        val publicConfig = MemoryKeyStoreConfig(
        ...
        val privateRequestKeyInfo = com.sphereon.crypto.core.ManagedKeyInfo(
            providerId = storedKey.providerId,
            alias = storedKey.alias,
            resolvedKeyInfo = com.sphereon.crypto.core.ResolvedKeyInfo(
                key = storedKey.key,
    ...
    @Test
    fun storeKeyOnPublicKeystoreShouldRejectPrivateKey() = runTest {
        // Create a PUBLIC keystore
        val publicConfig = MemoryKeyStoreConfig(
    ...
    @Test
    fun storeCertificateChainShouldSucceed() = runTest {
        val alias = "cert-chain-test-${System.currentTimeMillis()}"
        val testCert = com.sphereon.crypto.core.x509.Certificate(
            der = byteArrayOf(0x30, 0x82.toByte(), 0x01, 0x00),
            fingerPrint = "ABC123",
            serialNumber = "123456",
            issuerDN = "CN=Test Issuer",
            subjectDN = "CN=Test Subject",
            notBefore = kotlinx.datetime.Instant.parse("2024-01-01T00:00:00Z"),
            notAfter = kotlinx.datetime.Instant.parse("2025-12-31T23:59:59Z")
        )
    ...
    @Test
    fun getCertificateChainShouldReturnStoredChain() = runTest {
    ...
    @Test
    fun getCertificateChainShouldThrowForNonExistent() = runTest {
    ...
    @Test
    fun deleteCertificateChainShouldSucceed() = runTest {
        val alias = "delete-cert-chain-test-${System.currentTimeMillis()}"
        val testCert1 = com.sphereon.crypto.core.x509.Certificate(
            der = byteArrayOf(0x30, 0x82.toByte(), 0x01, 0x00),
            fingerPrint = "GHI789",
            serialNumber = "789012",
            issuerDN = "CN=Test Issuer",
        ...
        val testCert2 = com.sphereon.crypto.core.x509.Certificate(
    ...
    @Test
    fun deleteCertificateChainShouldReturnFalseForNonExistent() = runTest {
    ...
    @Test
    fun storeTrustedCertificateShouldSucceed() = runTest {
    ...
    @Test
    fun getCertificateShouldReturnStoredCert() = runTest {
    ...
    @Test
    fun getCertificateShouldFallbackToCertificateChain() = runTest {
    ...
    @Test
    fun getCertificateShouldThrowForNonExistent() = runTest {
        assertFailsWith<NotFoundException> {
    ...
    @Test
    fun deleteCertificateShouldSucceed() = runTest {
    ...
    @Test
    fun deleteCertificateShouldReturnFalseForNonExistent() = runTest {
    ...
    @Test
    fun storeCertificateChainWithSingleCertShouldAlsoStoreAsCert() = runTest {
    ...
    @Test
    fun noOverwriteCertificateChainShouldFailOnDuplicate() = runTest {
        val noOverwriteConfig = MemoryKeyStoreConfig(
        ...
        app as JvmCryptoTestAppComponent
        val noOverwriteKeystore = app.memoryKeyStore.create(noOverwriteConfig, session.asCoreApiServiceComponent().serviceExecution)
    ...
    @Test
    fun noOverwriteTrustedCertificateShouldFailOnDuplicate() = runTest {
        val noOverwriteConfig = MemoryKeyStoreConfig(
    ...
    @Test
    fun keystoreWithBackingStorageShouldUsePartition() = runTest {
        val backingStorage = MemoryKeyStoreBackingStorageImpl()
        val partitionKey = StoragePartitionKey(keystoreId = "partition-test", tenantId = "test-tenant")

        val config = MemoryKeyStoreConfig(
            id = "partition-test",
            keyVisibility = KeyVisibility.PRIVATE.keyVisibility,
            overwriteAlias = true
        )
    ...
    @Test
    fun keystoreWithBackingStorageShouldStoreCertsInPartition() = runTest {
        val backingStorage = MemoryKeyStoreBackingStorageImpl()
    ...
    @Test
    fun getKeyRequestingPrivateFromPublicStoredKeyShouldFail() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val publicKeyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PUBLIC)
        ...
        val privateRequest = com.sphereon.crypto.core.ManagedKeyInfo(
            providerId = storedKey.providerId,
            alias = alias,
            resolvedKeyInfo = com.sphereon.crypto.core.ResolvedKeyInfo(
                key = storedKey.key,
    ...
    @Test
    fun getCertificateChainShouldFallbackToCertificatesMap() = runTest {
    ...
    @Test
    fun getCertificateShouldFallbackToCertificateChainsMap() = runTest {
    ...
    @Test
    fun getCertificateChainNotFoundShouldThrow() = runTest {
        assertFailsWith<NotFoundException> {
    ...
    @Test
    fun getCertificateNotFoundShouldThrow() = runTest {
        assertFailsWith<NotFoundException> {
    ...
    @Test
    fun getKeyWithNullVisibilityShouldUseConfigDefault() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val privateKeyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val nullVisRequest = com.sphereon.crypto.core.ManagedKeyInfo(
            providerId = storedKey.providerId,
            alias = alias,
            resolvedKeyInfo = ResolvedKeyInfo(
                key = storedKey.key,
    ...
    @Test
    fun deleteNonExistentKeyShouldReturnFalse() = runTest {
        // First generate a real key to get a valid key object, then use a non-existent alias
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)

        val nonExistentRequest = com.sphereon.crypto.core.ManagedKeyInfo(
            providerId = "test-provider",
    ...
    @Test
    fun publicKeystoreRejectingPrivateKeyStoreShouldThrow() = runTest {
        val publicConfig = MemoryKeyStoreConfig(
    ...
    @Test
    fun keystoreWithoutBackingStorageShouldUseLocalMaps() = runTest {
    ...
    @Test
    fun keystoreWithBackingButNoPartitionKeyShouldUseLocalMaps() = runTest {
        val backingStorage = MemoryKeyStoreBackingStorageImpl()
        val config = MemoryKeyStoreConfig(
    ...
    @Test
    fun publicKeystoreGetPrivateKeyShouldThrow() = runTest {
        val publicConfig = MemoryKeyStoreConfig(
    ...
    @Test
    fun storeKeyWithNullVisibilityShouldUseConfigDefault() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        // Create key info with null visibility using the public JWK
        val keyInfoWithNullVis = ResolvedKeyInfo(
    ...
    fun factoryPrincipalTenantScopedWithNullPrincipalShouldThrow() {
        val principalConfig = MemoryKeyStoreConfig(
    ...
    fun factorySessionScopedWithNullPrincipalShouldThrow() {
        val sessionConfig = MemoryKeyStoreConfig(
