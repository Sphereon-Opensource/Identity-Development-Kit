...
class ManagedKeyStoreWithProviderLookupsTest {
    ...
    private val testManagedKeyInfo =
    ...
    private class TestResolverRegistry(
        resolvers: Set<KeyResolverService>,
    ) : KeyResolverRegistry {
        ...
        override fun getResolverById(id: String) =
            resolverMap[id]
    ...
    fun settingsShouldBeNull() {
        val keyStore =
            ManagedKeyStoreWithProviderLookups(
    ...
    @Test
    fun listKeysShouldAggregateFromAllProviders() =
        runTest {
            ...
            coEvery { provider1.listKeys() } returns arrayOf(key1.toKeyReference())
            coEvery { provider2.listKeys() } returns arrayOf(key2.toKeyReference())
    ...
    @Test
    fun listKeysShouldIgnoreProviderExceptions() =
        runTest {
            val provider1 = mockk<KmsProvider>()
            val provider2 = mockk<KmsProvider>()
    ...
    @Test
    fun listKeysShouldReturnEmptyWhenNoProviders() =
        runTest {
    ...
    @Test
    fun getKeyShouldUseProviderIdDirectly() =
        runTest {
    ...
    @Test
    fun getKeyShouldTryResolversFirst() =
        runTest {
            val provider = mockk<KmsProvider>()
            val resolver = mockk<KeyResolverService>()

            val resolvedWithProvider =
    ...
    @Test
    fun getKeyShouldFallbackToProvidersWhenResolverFails() =
        runTest {
            val provider = mockk<KmsProvider>()
            val resolver = mockk<KeyResolverService>()
    ...
    @Test
    fun getKeyShouldTryNextProviderWhenFirstFails() =
        runTest {
            val provider1 = mockk<KmsProvider>()
            val provider2 = mockk<KmsProvider>()
    ...
    @Test
    fun getKeyShouldThrowWhenAllProvidersFail() =
        runTest {
            val provider1 = mockk<KmsProvider>()
            val provider2 = mockk<KmsProvider>()
    ...
    @Test
    fun getKeyShouldThrowWhenNoAliasOrKid() =
        runTest {
            val keyStore = ManagedKeyStoreWithProviderLookups(testProviderRegistry(emptySet()), testResolverRegistry(emptySet()))
    ...
    @Test
    fun getKeyShouldThrowWithEmptyTriedSourcesWhenNoProvidersOrResolvers() =
        runTest {
    ...
    @Test
    fun getKeyShouldWorkWithKidOnly() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
            coEvery { provider.getKey(any<KeyInfoType<*>>()) } returns testManagedKeyInfo
    ...
    @Test
    fun getKeyShouldSkipResolverWhenResolvedKeyHasNoProviderId() =
        runTest {
            val provider = mockk<KmsProvider>()
            val resolver = mockk<KeyResolverService>()

            // Resolver returns a key without providerId
            val resolvedWithoutProvider =
                ResolvedKeyInfo<KeyType>(
                    key = testJwk,
                    keyVisibility = KeyVisibility.PRIVATE,
                    providerId = null,
    ...
    @Test
    fun storeKeyShouldDelegateToProvider() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
    ...
    @Test
    fun storeKeyShouldThrowWhenProviderIdMismatch() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
            ...
            val keyInfoWithDifferentProvider =
                ResolvedKeyInfo<KeyType>(
                    key = testJwk,
                    keyVisibility = KeyVisibility.PRIVATE,
    ...
    @Test
    fun storeKeyShouldThrowWhenAliasMismatch() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
    ...
    @Test
    fun storeKeyShouldSucceedWhenProviderIdMatches() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
            coEvery { provider.storeKey(any(), any(), any(), any()) } returns testManagedKeyInfo
            ...
            val keyInfoWithMatchingProvider =
                ResolvedKeyInfo<KeyType>(
                    key = testJwk,
                    keyVisibility = KeyVisibility.PRIVATE,
                    providerId = "test-provider",
    ...
    @Test
    fun storeKeyShouldSucceedWhenAliasMatches() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
            coEvery { provider.storeKey(any(), any(), any(), any()) } returns testManagedKeyInfo
    ...
    @Test
    fun deleteKeyShouldThrowWhenNoAliasOrKid() =
        runTest {
            val keyStore = ManagedKeyStoreWithProviderLookups(testProviderRegistry(emptySet()), testResolverRegistry(emptySet()))
    ...
    @Test
    fun deleteKeyShouldReturnFalseWhenProviderNotFound() =
        runTest {
            val provider = mockk<KmsProvider>()
    ...
    @Test
    fun deleteKeyShouldDelegateToProvider() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
    ...
    @Test
    fun deleteKeyShouldReturnFalseOnNotFoundException() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
    ...
    @Test
    fun deleteKeyShouldWorkWithKidOnly() =
        runTest {
            val provider = mockk<KmsProvider>()
            every { provider.id } returns "test-provider"
            coEvery { provider.deleteKey(any()) } returns true
