...
class X5cExternalIdentifierResolutionServiceImpl(execution: SessionExecution, val x509VerifyService: X509VerifyService) :
    ...
    ), X5cExternalIdentifierResolutionService, Scoped {
    override suspend fun doExecute(
        ...
        val verificationResult = x509VerifyService.verifyCertificateChain(
        ...
        return ExternalIdentifierResult.X5c(
            identifierOpts = opts,
            x5c = opts.identifier,
        ...
        ).asOkResult().also { log.debug("Resolved external X5C identifier: ${args.identifier.toString().take(100)}") }

    }

    override suspend fun supports(args: Any): Boolean {
        val externalArgs = args as? ExternalIdentifierOptsOrResult ?: return false
        val methodSupported = externalArgs is ExternalIdentifierX5cOpts ||
    ...
    override suspend fun isSupportedIdentifier(identifier: Any): Boolean {
        ...
        val strings: List<String>? = when (identifier) {
    ...
    override suspend fun asSupportedOpts(opts: ExternalIdentifierOptsOrResult): IdkResult<ExternalIdentifierX5cOpts, IdkErrorType> {
        return if (isSupportedOpts(opts)) (opts as ExternalIdentifierX5cOpts).asOkResult() else IdkError.Companion.COMMAND_ARG_NOT_SUPPORTED_ERROR().asErrorResult()
    }
    ...
    /**
     * Component interface for DI access to the X5C external identifier resolution service.
     */
