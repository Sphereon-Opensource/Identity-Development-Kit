...
class ResolvePublicKeyCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        return try {
            val resolver = resolverRegistry.getResolverByKeyTypeOrIdentifier(
                identifierMethod = appliedArgs.identifierMethod,
                keyType = keyInfo.keyType,
                resolverId = keyInfo.providerId
            )
            val resolvedKey = resolver.resolvePublicKey(
                keyInfo,
            ...
            log.warn("Public key resolution failed: ${e.message}")
            IdkError.fromString(message = "Failed to resolve public key: ${e.message}", code = "CRYPTO_ERROR", exception = e).asErrorResult()
        }
    }

    override suspend fun supports(args: Any): Boolean {
        return args is ResolvePublicKeyArgs && args.keyInfo != null
    }
}
