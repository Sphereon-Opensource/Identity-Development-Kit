...
class DefaultSignatureService(
    ...
    override suspend fun createRawSignature(keyInfo: KeyInfoType<*>, input: ByteArray, requireX5Chain: Boolean): ByteArray =
        keyManagerService.createRawSignature(keyInfo, input, requireX5Chain)

    override suspend fun isValidRawSignature(keyInfo: KeyInfoType<*>, input: ByteArray, signature: ByteArray): Boolean =
        keyManagerService.isValidRawSignature(keyInfo, input, signature)
    ...
    private suspend fun signJws(
        ...
        val prepareArgs = CreateJwsJsonArgs(
        ...
        val signatureBytes = try {
            ...
            return Err(IdkError.UNKNOWN_ERROR(message = "JWS signing failed: ${e.message}", exception = e))
        }
    ...
    override suspend fun validate(
        ...
        return try {
            ...
            log.error("Signature validation failed: ${e.message}", e)
            Err(IdkError.UNKNOWN_ERROR(message = "Signature validation failed: ${e.message}", exception = e))
        }
    }
