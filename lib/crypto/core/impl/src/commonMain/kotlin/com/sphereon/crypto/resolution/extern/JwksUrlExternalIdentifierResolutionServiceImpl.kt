    ...
    override suspend fun doExecute(
        ...
        val jwksJson =
            try {
                ...
                return IdkError.UNKNOWN_ERROR(message = "Failed to fetch JWKS from $url: ${expected.message}", exception = expected).asErrorResult()
            } finally {
                try {
                    httpClient.close()
                } catch (expected: Exception) {
        ...
        val resolvedKeys =
            jwkSet.keys
                .mapNotNull {
                    try {
                        ResolvedKeyInfo.fromKey(it) as com.sphereon.crypto.core.ResolvedKeyInfoType<JwkType>
                    } catch (expected: Exception) {
