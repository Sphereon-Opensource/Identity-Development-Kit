...
class CreateRawSignatureCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        return try {
            ...
            log.warn("Failed to create signature: ${e.message}")
            IdkError.fromString(message = "Failed to create signature: ${e.message}", code = "CRYPTO_ERROR", exception = e).asErrorResult()
        }
    }

    override suspend fun supports(args: Any): Boolean {
        return args is CreateRawSignatureArgs && args.keyInfo != null
    }
}
...
class VerifyRawSignatureCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        return try {
            ...
            log.warn("Failed to verify signature: ${e.message}")
            IdkError.fromString(message = "Failed to verify signature: ${e.message}", code = "CRYPTO_ERROR", exception = e).asErrorResult()
        }
    }

    override suspend fun supports(args: Any): Boolean {
        return args is VerifyRawSignatureArgs && args.keyInfo != null
    }
}
