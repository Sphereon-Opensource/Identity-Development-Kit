...
class QueryProviderCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        return try {
            ...
            val matchingProvider = providerIds.asSequence()
            ...
            log.warn("Error querying providers: ${e.message}")
            IdkError.NOT_FOUND_ERROR(
                resource = "KMS provider matching query criteria",
                message = e.message ?: "No provider found"
            ).asErrorResult()
        }
    }

    override suspend fun supports(args: Any): Boolean {
        return args is QueryProviderArgs && args.query != null
    }
}
...
class QueryProvidersCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        return try {
            ...
            val matchingProviders = allProviderIds
                .map { providerRegistry.getProviderById(it) }
            ...
            val matches = matchingProviders.map { provider ->
                ProviderMatch(
            ...
            log.error("Error querying providers: ${e.message}", e)
            IdkError.UNKNOWN_ERROR(
                message = "Error querying KMS providers: ${e.message}"
            ).asErrorResult()
        }
    }

    override suspend fun supports(args: Any): Boolean {
        return args is QueryProvidersArgs && args.query != null
    }
}
...
class GetAllCapabilitiesCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        return try {
            val providerIds = providerRegistry.getProviderIds()
            val capabilitiesMap = providerIds.mapNotNull { id ->
            ...
            log.error("Error getting provider capabilities: ${e.message}", e)
            IdkError.UNKNOWN_ERROR(
                message = "Error retrieving provider capabilities: ${e.message}"
            ).asErrorResult()
        }
    }

    override suspend fun supports(args: Any): Boolean {
        return args is GetAllCapabilitiesArgs
    }
}
...
interface KmsQueryCommandsComponent {
