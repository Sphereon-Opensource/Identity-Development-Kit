...
class NoOpKeyStoreFactory : KeyStoreFactory {
    ...
    override fun create(config: KeyStoreConfig): KeyStore = throw IllegalArgumentException("Please register your Keystore Factory or Factories!. No KeyStore Factory found")
...
class KeyStoreManagerImpl(
    factories: Set<KeyStoreFactory>,
    private val binder: KeyStoreConfigBinder,
) : KeyStoreManager {
    ...
    override fun createFromKeyStoreConfig(config: KeyStoreConfig): KeyStore = createFromKeyStoreConfig(config, null)

    override fun createFromKeyStoreConfig(
        config: KeyStoreConfig,
        execution: SessionExecution?,
    ): KeyStore {
        require(factories.isNotEmpty()) { "No keystore factories found! Make sure to register a key store module if you need keystore support" }
        val factory =
            factories.find { it.keyStoreType == config.keyStoreType }
    ...
    override fun createFromProperties(configService: ConfigService): Set<KeyStore> = binder.getKeyStoreConfigs(configService).map { createFromKeyStoreConfig(it) }.toSet()
}
...
class KeyStoreConfigBinderImpl(
    logManager: AppLogManager,
) : KeyStoreConfigBinder {
    ...
    private fun buildPrefixes(_configService: ConfigService) = setOf(KMS_KEYSTORES_PREFIX)
    ...
    override fun getKeyStoreConfig(
...
private val KeyStorePropertyNameAliases =
    mapOf(
