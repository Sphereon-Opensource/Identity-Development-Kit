...
class MemoryKeyStoreFactoryImpl(
    ...
    override fun create(config: KeyStoreConfig): MemoryKeyStoreService {
        return create(config, null)
    }

    override fun create(config: KeyStoreConfig, execution: SessionExecution?): MemoryKeyStoreService {
        val memoryConfig = config as? MemoryKeyStoreConfig
        ...
        val partitionKey = when (scopeBinding) {
            ...
            MemoryKeyStoreScopeBinding.TENANT -> {
                val tenantId = execution?.sessionContext?.context?.tenant?.tenantId
                    ?: throw IllegalStateException("Cannot create TENANT-scoped keystore without execution context with tenant information")
            ...
            MemoryKeyStoreScopeBinding.PRINCIPAL_TENANT -> {
                val context = execution?.sessionContext?.context
                    ?: throw IllegalStateException("Cannot create PRINCIPAL_TENANT-scoped keystore without execution context")
                val tenantId = context.tenant.tenantId
                val principalId = context.principal?.toString()
                    ?: throw IllegalStateException("Cannot create PRINCIPAL_TENANT-scoped keystore without principal information")
            ...
            MemoryKeyStoreScopeBinding.SESSION -> {
                val sessionContext = execution?.sessionContext
                    ?: throw IllegalStateException("Cannot create SESSION-scoped keystore without execution context with session information")
                val tenantId = sessionContext.context.tenant.tenantId
                val principalId = sessionContext.context.principal?.toString()
                    ?: throw IllegalStateException("Cannot create SESSION-scoped keystore without principal information")
