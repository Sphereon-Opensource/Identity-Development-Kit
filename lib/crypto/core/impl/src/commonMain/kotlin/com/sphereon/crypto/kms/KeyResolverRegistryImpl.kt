...
class KeyResolverRegistryImpl(
    ...
    private val keyResolversById: MutableMap<String, KeyResolverService> = keyResolvers
        .sortedBy { it.getId() }
        .associateBy { it.getId() }
        .toMutableMap()

    private var _defaultResolverId: String? = null

    override fun defaultResolverId(): String {
        require(keyResolversById.isNotEmpty()) { "At least one key resolver is required" }
        return _defaultResolverId ?: keyResolversById.values.first().getId()
    }

    override fun getResolverIds(): Array<String> {
        return keyResolversById.keys.toTypedArray()
    }

    override fun getResolverById(id: String): KeyResolverService {
        return keyResolversById[id]
    ...
    override fun getResolverByKeyTypeOrIdentifier(
        ...
        val resolver = keyResolversById.values.firstOrNull {
    ...
    override fun registerResolver(resolver: KeyResolverService, makeDefaultResolver: Boolean?) {
        ...
        if (makeDefaultResolver == true) {
            _defaultResolverId = resolver.getId()
        }
    }
}
