...
class KeyInfoIdentifierResolutionServiceImpl(execution: SessionExecution, val kms: KeyManagerService) :
    ...
    ), KeyInfoIdentifierResolutionService, Scoped {
    override suspend fun doExecute(
        ...
        if (!supports(args)) {
            return IdkError.COMMAND_ARG_NOT_SUPPORTED_ERROR(message = "Managed identifier opts for Key expected. Type ${args.method ?: args.identifier} not supported")
                .asErrorResult()
        }
        val optsResult = asSupportedOpts(args)
        if (optsResult.isErr) { return optsResult.error.asErrorResult() }
        ...
        if (algorithmHint != null && managedKeyInfo.signatureAlgorithm != algorithmHint) {
            ...
            val updatedResolvedKeyInfo = ResolvedKeyInfo(
    ...
    override suspend fun supports(args: Any): Boolean {
        return supportsManagedIdentifierArgs(args)
    }
    ...
    override suspend fun resolve(opts: ManagedIdentifierOptsOrResult): IdkResult<ManagedIdentifierKeyResult, IdkErrorType> {
        return execute(opts)
    }
    ...
    override suspend fun asSupportedOpts(opts: ManagedIdentifierOptsOrResult): IdkResult<ManagedOptsKeyInfo, IdkErrorType> {
        if (!isSupportedOpts(opts)) {
            return IdkError.COMMAND_ARG_NOT_SUPPORTED_ERROR(message = "Managed identifier opts or result not supported : ${opts::class.simpleName}, id: ${opts.identifier}")
                .asErrorResult()
        }
        return when (opts) {
            is ManagedIdentifierResult<*> -> ManagedOptsKeyInfo(identifier = opts.keyInfo, context = opts.context, lookup = opts.keyInfo)
            is ManagedOptsKeyInfo -> opts
            is ManagedOptsAlias -> ManagedOptsKeyInfo(
            ...
            is ManagedOptsKid -> {
                ...
                val identifier = if (lookupKeyInfo.kid == null) {
            ...
            is ManagedOptsJwk -> ManagedOptsKeyInfo(
                identifier = ResolvedKeyInfo.fromKey(opts.identifier),
                context = opts.context,
                lookup = opts.lookup
            )

            is ManagedOptsCoseKey -> ManagedOptsKeyInfo(
                identifier = ResolvedKeyInfo.fromKey(opts.identifier),
                context = opts.context,
                lookup = opts.lookup
            )

            is ManagedOptsKey -> ManagedOptsKeyInfo(identifier = ResolvedKeyInfo.fromKey(opts.identifier), context = opts.context, lookup = opts.lookup)
            else -> return IdkError.COMMAND_ARG_NOT_SUPPORTED_ERROR(message = "Supplied managed identifier opts are not supported ${opts::class.simpleName}, id: ${opts.identifier}")
                .asErrorResult()
        }.asOkResult()
    }
    ...
    interface Component {
        val keyInfoIdentifierResolutionService: KeyInfoIdentifierResolutionService
    }
