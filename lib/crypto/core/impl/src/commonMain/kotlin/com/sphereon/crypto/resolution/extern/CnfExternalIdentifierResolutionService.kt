...
class NoOpVerificationMethodKeyResolverImpl : VerificationMethodKeyResolver {
    ...
    override suspend fun resolveVerificationMethodKey(did: String, verificationMethodId: String?): IdkResult<JwkType, IdkErrorType> {
        return IdkError.COMMAND_ARG_NOT_SUPPORTED_ERROR(
            message = "DID resolution is not available. Cannot resolve DID-based kid: $did. " +
...
class CnfExternalIdentifierResolutionServiceImpl(
    ...
    override suspend fun doExecute(
        ...
        return IdkError.ILLEGAL_ARGUMENT_ERROR(
            message = "CNF contains only a non-DID kid '$kid' without jwk or jku. " +
    ...
    private suspend fun resolveFromDid(
        ...
        val verificationMethodId = if (kid.contains('#')) kid.substringAfter('#') else null
    ...
    private suspend fun resolveFromJwk(
        ...
        return jwkResolver.resolve(jwkOpts).fold(
            success = { jwkResult ->
                ...
                val keyInfo = jwkResult.keyInfo.let { info ->
    ...
    private suspend fun resolveFromJku(
        ...
        val jwksOpts = ExternalIdentifierJwksUrlOpts(
            identifier = jku,
            lookup = if (kid != null) AdditionalIdentifierLookup(kid = kid) else AdditionalIdentifierLookup()
        )
    ...
    override suspend fun asSupportedOpts(opts: ExternalIdentifierOptsOrResult): IdkResult<ExternalIdentifierCnfOpts, IdkErrorType> {
        return if (isSupportedOpts(opts)) (opts as ExternalIdentifierCnfOpts).asOkResult()
        else IdkError.COMMAND_ARG_NOT_SUPPORTED_ERROR().asErrorResult()
    }

    @ContributesTo(SessionScope::class)
    interface Component {
        val cnfExternalIdentifierResolutionService: CnfExternalIdentifierResolutionService
    }
