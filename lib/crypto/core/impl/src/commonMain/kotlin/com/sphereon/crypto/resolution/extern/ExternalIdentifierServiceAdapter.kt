...
abstract class ExternalIdentifierServiceAdapter<ResultType : ExternalIdentifierResult>(
    ...
    Command<ExternalIdentifierOptsOrResult, ResultType, IdkErrorType>, ExternalIdentifierService {
    override val order: Int get() = Order.MEDIUM.orderValue

    override suspend fun supports(args: Any): Boolean {
        return args is ExternalIdentifierOptsOrResult
    }
    ...
    override suspend fun isSupportedOpts(opts: ExternalIdentifierOptsOrResult): Boolean {
        return supports(opts)
    }

    override suspend fun asSupportedOpts(opts: ExternalIdentifierOptsOrResult): IdkResult<ExternalIdentifierOpts, IdkErrorType> {
        return if (isSupportedOpts(opts)) (opts as ExternalIdentifierOpts).asOkResult() else IdkError.Companion.COMMAND_ARG_NOT_SUPPORTED_ERROR().asErrorResult()
    }
