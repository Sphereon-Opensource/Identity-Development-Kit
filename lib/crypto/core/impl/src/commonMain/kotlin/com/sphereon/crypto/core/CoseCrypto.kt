...
//expect fun coseCryptoService(platformCallback: ICoseCryptoCallbackMarkerType = CallbacksImpl.coseCrypto()): ICoseCryptoService
...
abstract class AbstractCoseCryptoService(
...
) : HasPlatformCallback<CoseCryptoCallbackCoroutines>, CoseCryptoService {
    ...
    override fun platform(): CoseCryptoCallbackCoroutines {
        return this.platformCallback ?: DefaultCallbacks.coseCrypto().also { this.platformCallback = it }
    }

    override fun setPlatform(platform: CoseCryptoCallbackCoroutines): HasPlatformCallback<CoseCryptoCallbackCoroutines> = apply {
        this.platformCallback = platform
    }

    protected fun assertEnabled() {
        if (!isEnabled()) {
            throw IllegalStateException("COSE session is disabled; cannot sign")
        } else if (!hasPlatform()) {
    ...
    protected suspend fun preSign1(
        ...
        assertEnabled()
        val (unprotectedHeader, protectedHeader, cborKeyInfo) = verifyAndAmendKeyInfo(
    ...
    protected fun <CborType : Any> postSign1(
        ...
        val coseSign1 = CoseSign1<CborType>(
    ...
    private fun defaultAlgorithmForKeyType(keyType: KeyTypeMapping?): CoseAlgorithm? {
        ...
        return if (effectiveKeyType == CoseKeyTypeEnum.EC2) SignatureAlgorithm.ECDSA_SHA256.cose else null
    }
    ...
    protected fun buildKeyInfoFromJwk(
        ...
        val coseKey = CoseKey(
            x5chain = x5chain,
            kty = algKeyType.toCbor(),
            kid = kid?.toCborByteString(Encoding.UTF8),
            alg = jwk.alg?.toCoseAlgorithm()?.value?.let { CborUInt(value = it) },
            generateKid = false,
            // EC parameters - EC algorithms always have a curve by definition
            crv = if (isEcKey) sigAlg.curve!!.toCbor() else null,
            x = if (isEcKey) jwk.x?.toCborByteString(Encoding.BASE64URL) else null,
            y = if (isEcKey) jwk.y?.toCborByteString(Encoding.BASE64URL) else null,
            d = if (isEcKey) jwk.d?.toCborByteString(Encoding.BASE64URL) else null,
            // RSA parameters
            n = if (isRsaKey) jwk.n?.toCborByteString(Encoding.BASE64URL) else null,
            rsaE = if (isRsaKey) jwk.e?.toCborByteString(Encoding.BASE64URL) else null,
        )
    ...
    private fun buildFinalHeaders(
        ...
        val finalProtectedHeader = if (baseProtectedHeader.alg == null && sigAlg != null) {
    ...
    protected suspend fun verifyAndAmendKeyInfo(
        ...
        val resolvedKeyInfo: KeyInfoType<*> = when {
        ...
        val key: CoseKeyType = CoseJoseKeyMappingService.toCoseKey(
        ...
        val (finalUnprotectedHeader, finalProtectedHeader) = buildFinalHeaders(
            protectedHeader, unprotectedHeader, sigAlg, x5chain
        )
    ...
    override suspend fun verify1(
        ...
        val (_, _, info) = verifyAndAmendKeyInfo(
            protectedHeader = input.protectedHeader,
            unprotectedHeader = input.unprotectedHeader,
            keyInfo = keyInfo,
...
suspend fun defaultCreateMac0UsingKeys(
    ...
    macCallback: suspend (
        provider: CryptographyProvider?, input: CoseMac0InputCbor,
        sharedSecret: ByteArray,
        alg: SignatureAlgorithm,
    ) -> CoseMac0Result,
): CoseMac0Result {
