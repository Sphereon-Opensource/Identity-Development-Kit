...
class MultiExternalIdentifierResolutionServiceImpl(execution: SessionExecution, val external: Set<ExternalIdentifierService>) :
    ...
    override suspend fun doExecute(
        ...
        return IdkError.COMMAND_ARG_NOT_SUPPORTED_ERROR(
            message = "External identifier opts or result not supported: $argsType (method: $identifierMethod, identifier type: $identifierType). " +
    ...
    override suspend fun isSupportedIdentifier(identifier: Any): Boolean {
        return external.any { it.isSupportedIdentifier(identifier) }
    }
    ...
    /**
     * Component interface for DI access to the multi external identifier resolution service.
     */
    @ContributesTo(SessionScope::class)
    interface Component {
        val multiExternalIdentifierService: MultiExternalIdentifierService
    }
}