...
class CoseCryptoProviderToCallbackAdapter(
...
) : CoseCryptoCallbackCoroutinesMarker, Scoped {
    ...
    private fun assertedSignatureProvider(alg: SignatureAlgorithm? = null, kms: String? = null): SimpleSignatureService {
        return (keyManagerService?.getProvider(providerId = kms, alg = alg) ?: rawSignatureService)
    ...
    private fun assertedPublicKeyProvider(keyInfo: KeyInfoType<*>): KeyResolverService {
        ...
        val providerAsResolver = keyInfo.providerId?.let { providerId ->
            try {
     ...
     */
    override suspend fun verify1(input: CoseSign1<*>, keyInfo: KeyInfoType<*>?, requireX5Chain: Boolean?): VerifySignatureResultType<CoseKeyType> {
        ...
        if (chain?.isNotEmpty() == true && keyInfo == null) {
            val cert = signumX509CertificateFromDer(chain.first().value)
        ...
        val recalculatedToBeSignedCbor = input.toBeSignedCbor(keyInfo = resolvedKeyInfo)
        val validSig = assertedSignatureProvider(alg = alg, kms = resolvedKeyInfo.providerId).isValidRawSignature(
            resolvedKeyInfo,
