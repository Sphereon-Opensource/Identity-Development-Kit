...
open class KeyManagerServiceImpl @Inject constructor(
    ...
    override fun getProvider(providerId: String?, alg: SignatureAlgorithm?) = providerRegistry.getProvider(providerId, alg)
    ...
    override fun registerProvider(provider: KmsProvider, makeDefaultKms: Boolean?) = providerRegistry.registerProvider(provider, makeDefaultKms)
    ...
    override fun registerResolver(resolver: KeyResolverService, makeDefaultResolver: Boolean?) = resolverRegistry.registerResolver(resolver, makeDefaultResolver)


    override suspend fun queryProvider(query: KmsProviderQuery): IdkResult<QueryProviderResult, IdkError> {
        val command = queryProviderCommand
    ...
    override suspend fun queryProviders(query: KmsProviderQuery): IdkResult<QueryProvidersResult, IdkError> {
        val command = queryProvidersCommand
    ...
    override suspend fun getAllCapabilities(includeDisabled: Boolean): IdkResult<GetAllCapabilitiesResult, IdkError> {
        val command = getAllCapabilitiesCommand
    ...
    override suspend fun <KT : KeyType> resolvePublicKey(
        ...
        if (command != null && exec != null) {
            ...
            val resolvedKeyInfo = result.getOrElse { throw PKIException(it.message.defaultMessage ?: "Resolve public key failed") }.resolvedKeyInfo
    ...
    override suspend fun isValidRawSignature(keyInfo: KeyInfoType<*>, input: ByteArray, signature: ByteArray): Boolean {
        ...
        val resolvedKeyInfo = if (keyInfo.key == null && keyInfo.kid == null && keyInfo.alias == null && !keyInfo.x5c.isNullOrEmpty()) {
    ...
    override suspend fun performKeyAgreement(
        ...
        val exec = execution
        if (command != null && exec != null) {
            val args = PerformKeyAgreementArgs(
    ...
    override suspend fun storeKey(keyInfo: ResolvedKeyInfoType<*>, providerId: String, alias: String, certChain: Array<com.sphereon.crypto.core.x509.Certificate>?): com.sphereon.crypto.core.ManagedKeyInfoType<*> {
        val command = storeKeyCommand
        val exec = execution
        if (command != null && exec != null) {
            val args = StoreKeyArgs(
                keyInfo = keyInfo,
                providerId = providerId,
                alias = alias,
                certChain = certChain
    ...
    override suspend fun listKeys(): Array<com.sphereon.crypto.core.ManagedKeyInfoType<*>> {
