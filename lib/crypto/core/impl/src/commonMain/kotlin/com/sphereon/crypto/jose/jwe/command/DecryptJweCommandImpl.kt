...
import com.sphereon.crypto.jose.jwe.*
...
class DecryptJweCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        val (header, encryptedKey, iv, ciphertext, authTag) = when (jwe) {
            is JweCompact -> {
                Tuple5(jwe.header, jwe.encryptedKey, jwe.iv, jwe.ciphertext, jwe.authTag)
            }
            is JweJsonFlattened -> {
                val hdr = jwe.getProtectedHeader() ?: jwe.getUnprotectedHeader()
            ...
            else -> return IdkResult.err(IdkError.fromString("Unsupported JWE type"))
        }
        ...
        val encAlg = ContentEncryptionAlgorithm.fromIdentifier(encAlgStr)
        ...
        if (keyAgreementAlg != null) {
            ...
            cek = if (keyWrapAlg == KeyWrapAlgorithm.DIR) {
                ...
                try {
                    ...
                    return IdkResult.err(IdkError.fromString("Key unwrapping failed: ${e.message}"))
                }
            }
        } else {
        ...
        val plaintext = try {
            ...
            return IdkResult.err(IdkError.fromString("Decryption failed: ${e.message}"))
        }
        ...
        val result = JweDecryptionResult(
            plaintext = plaintext,
            header = header,
            aad = aad
        )
    ...
    private suspend fun performEcdhEsDecryption(
        ...
        val ourPrivateJwk = decryptorKeyInfo.key as? Jwk
        ...
        val derivedKey = ConcatKdf.deriveKey(
    ...
    private fun extractSymmetricKeyForDir(
        ...
        val expectedKeySize = when (encAlgStr) {
            "A128GCM" -> 16        // 128 bits
            "A192GCM" -> 24        // 192 bits
            "A256GCM" -> 32        // 256 bits
            "A128CBC-HS256" -> 32  // 256 bits (128 for encryption + 128 for MAC)
            "A192CBC-HS384" -> 48  // 384 bits (192 for encryption + 192 for MAC)
            "A256CBC-HS512" -> 64  // 512 bits (256 for encryption + 256 for MAC)
            else -> return IdkResult.err(IdkError.fromString(
    ...
    private suspend fun decryptJweJsonGeneral(
        ...
        val orderedRecipients = if (decryptorKid != null) {
        ...
        for (recipient in orderedRecipients) {
            ...
            val effectiveHeader = if (keyAgreementAlg != null && recipientHeader?.epk != null) {
            ...
            val cekResult = try {
                if (keyAgreementAlg != null) {
                    ...
                    if (keyWrapAlg == KeyWrapAlgorithm.DIR) {
                        ...
                        try {
                            val cek = keyManagerService.unwrapKey(
                                unwrappingKeyInfo = resolvedDecryptor.asResult().keyInfo,
                                wrappedKey = encryptedKey,
                                algorithm = keyWrapAlg
                            )
                            ...
                            IdkResult.err(IdkError.fromString("Key unwrapping failed: ${e.message}"))
                        }
                    }
                } else {
                ...
                IdkResult.err(IdkError.fromString("CEK derivation failed: ${e.message}"))
            }
            ...
            try {
                ...
                val plaintext = keyManagerService.decrypt(
                    keyInfo = cekInfo,
                    ciphertext = ciphertext,
                    algorithm = encAlg,
                    iv = iv,
                ...
                lastError = IdkError.fromString("Decryption failed: ${e.message}")
                // Try next recipient
            }
        }
