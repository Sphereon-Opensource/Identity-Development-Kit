...
class JwsBuildersTest {
    ...
    fun testCreateJwsArgsBuilderWithBuilders() {
        ...
        assertEquals("JWT", args.opts.protectedHeader!!["typ"]?.jsonPrimitive?.content)
    }
    ...
    fun testCreateJwsArgsBuilderDSL() {
        ...
        assertNotNull(args.opts.protectedHeader)
        assertEquals("PS256", args.opts.protectedHeader!!["alg"]?.jsonPrimitive?.content)
    }
    ...
    fun testCreateJwsJsonArgsBuilder() {
        ...
        assertNotNull(args.opts.protectedHeader)
        assertEquals("ES384", args.opts.protectedHeader!!["alg"]?.jsonPrimitive?.content)
    }
    ...
    fun testCreateJwsJsonArgsBuilderDSL() {
        ...
        assertEquals("test", args.opts.unprotectedHeader!!["x-metadata"]?.jsonPrimitive?.content)
    }
    ...
    fun testPayloadMerge() {
        val basePayload = jwsPayload {
            iss("https://issuer.example.com")
            sub("user-123")
        }

        val extendedPayload = JwsPayloadBuilder.from(basePayload)
            .claim("email", "user@example.com")
            .claim("role", "admin")
            .build()
