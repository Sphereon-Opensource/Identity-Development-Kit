...
class CoseJoseKeyMappingServiceTest {
    ...
    private val testJwkWithKid = Jwk(
        generateKid = false,
        kty = JwaKeyType.EC,
        crv = JwaCurve.P_256,
        x = "uxHN3W6ehp0VWXKaMNie1J82MVJCFZYScau74o17cx8",
    ...
    @Test
    fun tryToJoseJwkShouldReturnSameJwkWhenInputIsJwk(): TestResult = runTest {
    ...
    @Test
    fun tryToJoseJwkShouldConvertCoseKeyToJwk(): TestResult = runTest {
        val coseKey = CoseKey.decodeCbor(coseKeyHex.decodeFrom(Encoding.HEX))
    ...
    @Test
    fun tryToJoseJwkShouldConvertCoseKeyJsonToJwk(): TestResult = runTest {
        val coseKeyJson = CoseKeyJson(
    ...
    @Test
    fun toJoseJwkShouldReturnJwkWhenInputIsValid(): TestResult = runTest {
    ...
    @Test
    fun tryToCoseKeyShouldReturnSameCoseKeyWhenInputIsCoseKey(): TestResult = runTest {
        val coseKey = CoseKey.decodeCbor(coseKeyHex.decodeFrom(Encoding.HEX))
        val result = CoseJoseKeyMappingService.tryToCoseKey(coseKey)

        assertTrue(result.isOk)
        assertEquals(coseKey.x?.value?.toList(), result.value.x?.value?.toList())
    }

    @Test
    fun tryToCoseKeyShouldConvertJwkToCoseKey(): TestResult = runTest {
        val result = CoseJoseKeyMappingService.tryToCoseKey(testJwk)

        assertTrue(result.isOk)
        assertEquals(CoseKeyTypeEnum.EC2.toCbor(), result.value.kty)
        assertEquals(CoseCurve.P_256.toCbor(), result.value.crv)
    }

    @Test
    fun tryToCoseKeyShouldConvertCoseKeyJsonToCoseKey(): TestResult = runTest {
        val coseKeyJson = CoseKeyJson(
            kty = CoseKeyTypeEnum.EC2,
            crv = CoseCurve.P_256,
            x = "uxHN3W6ehp0VWXKaMNie1J82MVJCFZYScau74o17cx8",
    ...
    @Test
    fun toCoseKeyShouldReturnCoseKeyWhenInputIsValid(): TestResult = runTest {
        val coseKey = CoseJoseKeyMappingService.toCoseKey(testJwk)
        assertEquals(CoseKeyTypeEnum.EC2.toCbor(), coseKey.kty)
    }

    // =========== X5c Conversion Tests ===========

    @Test
    fun tryToJoseX5cShouldReturnNullForNullInput(): TestResult = runTest {
    ...
    @Test
    fun tryToJoseX5cShouldConvertStringArray(): TestResult = runTest {
    ...
    @Test
    fun tryToJoseX5cShouldConvertCborByteStringArray(): TestResult = runTest {
    ...
    @Test
    fun tryToJoseX5cShouldReturnErrorForUnsupportedType(): TestResult = runTest {
    ...
    @Test
    fun toJoseX5cShouldThrowForUnsupportedType(): TestResult = runTest {
    ...
    @Test
    fun tryToCoseX5chainShouldReturnNullForNullInput(): TestResult = runTest {
    ...
    @Test
    fun tryToCoseX5chainShouldConvertCborByteStringArray(): TestResult = runTest {
        val certBytes = "test-cert".encodeToByteArray()
        val cborByteString = CborByteString(certBytes)
        val x5c = arrayOf<Any>(cborByteString)
    ...
    @Test
    fun tryToCoseX5chainShouldConvertBase64StringArray(): TestResult = runTest {
        // Base64 encoded "test"
    ...
    @Test
    fun tryToCoseX5chainShouldReturnErrorForUnsupportedType(): TestResult = runTest {
        val x5c = arrayOf<Any>(123)
        val result = CoseJoseKeyMappingService.tryToCoseX5chain(x5c)
    ...
    @Test
    fun toCoseX5chainShouldThrowForUnsupportedType(): TestResult = runTest {
        val x5c = arrayOf<Any>(123)
        assertFailsWith<IllegalArgumentException> {
    ...
    @Test
    fun toJwkKeyInfoShouldConvertKeyInfoWithKey(): TestResult = runTest {
        val keyInfo = KeyInfo(
            key = testJwk,
            kid = "test-kid",
    ...
    @Test
    fun toJwkKeyInfoShouldHandleNullKey(): TestResult = runTest {
    ...
    @Test
    fun toCoseKeyInfoShouldConvertKeyInfoWithKey(): TestResult = runTest {
        val keyInfo = KeyInfo(
            key = testJwk,
            kid = "test-kid",
            signatureAlgorithm = SignatureAlgorithm.ECDSA_SHA256,
    ...
    @Test
    fun toResolvedJwkKeyInfoShouldConvertResolvedKeyInfo(): TestResult = runTest {
    ...
    @Test
    fun toResolvedCoseKeyInfoShouldConvertResolvedKeyInfo(): TestResult = runTest {
        val resolvedKeyInfo = ResolvedKeyInfo(
            key = testJwk,
            kid = "test-kid",
            signatureAlgorithm = SignatureAlgorithm.ECDSA_SHA256,
        ...
        assertEquals("test-kid", result.kid)
        assertEquals(SignatureAlgorithm.ECDSA_SHA256, result.signatureAlgorithm)
        assertEquals(CoseKeyTypeEnum.EC2.toCbor(), result.key.kty)
    }

    // =========== isResolvedKeyInfo Tests ===========

    @Test
    fun isResolvedKeyInfoShouldReturnTrueWhenKeyIsPresent(): TestResult = runTest {
        val keyInfo = KeyInfo(
            key = testJwk,
            kid = "test-kid"
        )
    ...
    @Test
    fun isResolvedKeyInfoShouldReturnFalseWhenKeyIsNull(): TestResult = runTest {
        val keyInfo = KeyInfo<Jwk>(
            key = null,
            kid = "test-kid"
        )
    ...
    @Test
    fun toResolvedKeyInfoShouldCreateResolvedKeyInfoFromKeyInfo(): TestResult = runTest {
        val keyInfo = KeyInfo(
            key = testJwk,
            kid = "test-kid",
            signatureAlgorithm = SignatureAlgorithm.ECDSA_SHA256
    ...
    @Test
    fun tryToResolvedKeyInfoWithResolverShouldReturnKeyInfoIfAlreadyResolved(): TestResult = runTest {
        val resolvedKeyInfo = ResolvedKeyInfo(
            key = testJwk,
            kid = "test-kid",
            signatureAlgorithm = SignatureAlgorithm.ECDSA_SHA256
        ...
        val result = CoseJoseKeyMappingService.tryToResolvedKeyInfoWithResolver(
    ...
    @Test
    fun tryToResolvedKeyInfoWithResolverShouldReturnErrorWhenUnresolvedAndNoCallback(): TestResult = runTest {
        val keyInfo = KeyInfo<Jwk>(
            key = null,
            kid = "test-kid"
        )

        val result = CoseJoseKeyMappingService.tryToResolvedKeyInfoWithResolver<Jwk>(
            keyInfo = keyInfo,
            resolveCallback = null
        )
    ...
    @Test
    fun tryToResolvedKeyInfoWithResolverShouldUseCallbackWhenUnresolved(): TestResult = runTest {
        val keyInfo = KeyInfo<Jwk>(
            key = null,
            kid = "test-kid"
        )
    ...
    @Test
    fun toResolvedKeyInfoWithResolverShouldThrowWhenUnresolvedAndNoCallback(): TestResult = runTest {
        val keyInfo = KeyInfo<Jwk>(
            key = null,
            kid = "test-kid"
        )
    ...
    @Test
    fun getJoseX5cShouldReturnNullWhenKeyHasNoX5c(): TestResult = runTest {
        val x5c = CoseJoseKeyMappingService.getJoseX5c(testJwk)
        assertNull(x5c)
    }

    @Test
    fun getJoseX5cShouldReturnX5cWhenKeyHasIt(): TestResult = runTest {
...
""".trimIndent()
    ...
    @Test
    fun getCoseX5chainShouldReturnNullWhenKeyHasNoX5chain(): TestResult = runTest {
    ...
    @Test
    fun jwkToCoseKeyAndBackShouldPreserveKeyData(): TestResult = runTest {
        val coseKey = CoseJoseKeyMappingService.toCoseKey(testJwk)
    ...
    @Test
    fun coseKeyToJwkAndBackShouldPreserveKeyData(): TestResult = runTest {
        val coseKey = CoseKey.decodeCbor(coseKeyHex.decodeFrom(Encoding.HEX))
