...
fun Any.asSignatureServiceComponent(): DefaultSignatureService.Component =
    this as DefaultSignatureService.Component
...
class JwsSigningFlowsE2ETest {
    ...
    fun setUp() {
        ...
        signatureService = session.component.asSignatureServiceComponent().signatureService
    }
    ...
    private val testPayload = JsonObject(
        mapOf(
    ...
    @Test
    fun twoStepJwsCompactPrepareSignAssemble() = runTest {
        initKeys()
    ...
    @Test
    fun twoStepJwsFlattenedPrepareSignAssemble() = runTest {
        initKeys()
        // Given: a prepared JWS object
        val prepareArgs = CreateJwsJsonArgs(issuer = issuer, payload = testPayload)
        val prepared = jwtService.prepareJws(prepareArgs).getOrFail("prepareJws")
    ...
    @Test
    fun twoStepJwsGeneralPrepareSignAssemble() = runTest {
        initKeys()
        // Given: a prepared JWS object
        val prepareArgs = CreateJwsJsonArgs(issuer = issuer, payload = testPayload)
        val prepared = jwtService.prepareJws(prepareArgs).getOrFail("prepareJws")
    ...
    @Test
    fun twoStepJwsViaJwtServiceAssembleMethods() = runTest {
        initKeys()
        // Given: a prepared JWS object
        val prepareArgs = CreateJwsJsonArgs(issuer = issuer, payload = testPayload)
        val prepared = jwtService.prepareJws(prepareArgs).getOrFail("prepareJws")
    ...
    @Test
    fun signatureServiceOneStepRawSign() = runTest {
        initKeys()
    ...
    @Test
    fun signatureServiceTwoStepCreateDigestComplete() = runTest {
        initKeys()
    ...
    @Test
    fun signatureServiceTwoStepWithJwsSigningInput() = runTest {
        initKeys()
        // Given: a prepared JWS object
        val prepareArgs = CreateJwsJsonArgs(issuer = issuer, payload = testPayload)
        val prepared = jwtService.prepareJws(prepareArgs).getOrFail("prepareJws")
    ...
    @Test
    fun signatureServiceJwsOneStepSign() = runTest {
        initKeys()
    ...
    @Test
    fun signatureServiceJwsOneStepWithJsonPayload() = runTest {
        initKeys()
        ...
        val params = JwsSignatureParameters(
            issuer = issuer,
            payload = testPayload
        )
    ...
    @Test
    fun twoStepJwsWithMultipleAlgorithms() = runTest {
        ...
        for (alg in algorithms) {
            ...
            val algIssuer = ManagedOptsKeyInfo(
                identifier = keyInfo,
                context = IdentifierContext(
