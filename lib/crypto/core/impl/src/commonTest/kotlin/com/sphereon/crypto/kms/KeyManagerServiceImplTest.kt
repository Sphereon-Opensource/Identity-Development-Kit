...
class KeyManagerServiceImplTest {
    ...
    fun registerProviderShouldAddProvider() {
        val config2 = SoftwareKmsProviderConfig(
            id = "second-software-provider",
            cryptographyProvider = CryptographyProvider.Default.name
        )
        app as SoftwareKmsProviderFactoryImpl.Component
        val provider2 = app.softwareKmsProvider.create(config2, session.asCoreApiServiceComponent().serviceExecution)
    ...
    @Test
    fun generateKeyWithAliasShouldSucceed() = runTest {
    ...
    @Test
    fun generateKeyWithProviderIdShouldSucceed() = runTest {
        val keyPair = keyManagerService.generateKey(
    ...
    @Test
    fun createRawSignatureShouldSucceed() = runTest {
    ...
    @Test
    fun isValidRawSignatureShouldReturnTrueForValidSignature() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
    ...
    @Test
    fun isValidRawSignatureShouldReturnFalseForInvalidSignature() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val publicKeyInfo = keyPair.joseToManagedKeyInfo(KeyVisibility.PUBLIC)
    ...
    @Test
    fun isValidRawSignatureWithProviderIdShouldUseProvider() = runTest {
        val keyPair = keyManagerService.generateKey(
            providerId = "test-software-provider",
            alg = SignatureAlgorithm.ECDSA_SHA256
        )
        ...
        val keyInfoWithProvider = ManagedKeyInfo(
            providerId = "test-software-provider",
    ...
    @Test
    fun performKeyAgreementShouldSucceedWithValidKeys() = runTest {
        ...
        val sharedSecret = keyManagerService.performKeyAgreement(
    ...
    @Test
    fun performKeyAgreementShouldFailWithoutPrivateKey() = runTest {
    ...
    @Test
    fun performKeyAgreementShouldProduceSameSecretBothWays() = runTest {
        ...
        val aliceShared = keyManagerService.performKeyAgreement(
            privateKeyInfo = alicePrivate,
            publicKeyInfo = bobPublic,
            algorithm = KeyAgreementAlgorithm.ECDH_ES,
            keyDataLen = null
        ...
        val bobShared = keyManagerService.performKeyAgreement(
    ...
    @Test
    fun performKeyAgreementWithP384ShouldSucceed() = runTest {
    ...
    @Test
    fun listKeysShouldReturnArray() = runTest {
        val keys = keyManagerService.listKeys()
        assertNotNull(keys)
    }

    @Test
    fun storeAndGetKeyShouldWork() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
    ...
    @Test
    fun deleteKeyShouldWork() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        // Create ResolvedKeyInfo without alias/providerId to avoid validation conflicts
        val resolvedKeyInfo = ResolvedKeyInfo(
            key = keyPair.jose.privateJwk!!,
    ...
    @Test
    fun resolvePublicKeyShouldSucceed() = runTest {
        val keyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
