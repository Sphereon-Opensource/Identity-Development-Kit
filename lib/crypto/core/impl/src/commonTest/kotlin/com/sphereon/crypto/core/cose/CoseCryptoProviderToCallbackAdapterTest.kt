...
class CoseCryptoProviderToCallbackAdapterTest {
    ...
    @Test
    fun adapterShouldHandleKeyManagerServiceProviderException() = runTest {
        // Create adapter with provider that throws exception
        val adapter = CoseCryptoProviderToCallbackAdapter(
    ...
    @Test
    fun adapterShouldHandleSimpleSignatureServiceProviderException() = runTest {
    ...
    @Test
    fun adapterShouldHandlePublicKeyResolverProviderException() = runTest {
    ...
    @Test
    fun assertedSignatureProviderShouldThrowWhenNoProvidersAvailable() = runTest {
        val adapter = CoseCryptoProviderToCallbackAdapter(
        ...
        val exception = assertFailsWith<PKIException> {
            adapter.sign(
                ToBeSignedCbor(
                    value = byteArrayOf(1, 2, 3),
                    keyInfo = KeyInfo<Jwk>(kid = "test-key")
    ...
    @Test
    fun assertedPublicKeyProviderShouldThrowWhenNoResolversAvailable() = runTest {
        val adapter = CoseCryptoProviderToCallbackAdapter(
            keyManagerServiceProvider = { throw RuntimeException("Unavailable") },
            rawSignatureServiceProvider = null,
            publicKeyResolverServiceProvider = null
    ...
    @Test
    fun signShouldUseKeyInfoAlgorithmWhenPresent() = runTest {
        ...
        val coseCryptoService = (session.component as CryptoServicesImpl.Component).cryptoServices.cose
    ...
    @Test
    fun signShouldUseInputAlgWhenKeyInfoAlgIsNull() = runTest {
        val managedKeyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        val coseKeyInfo = CoseJoseKeyMappingService.toCoseKeyInfo(keyInfo)
        ...
        val keyInfoWithoutAlg = KeyInfo<CoseKeyType>(
            kid = coseKeyInfo.kid,
    ...
    @Test
    fun signWithRequireX5ChainTrue() = runTest {
        val managedKeyPair = keyManagerService.generateKey(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        val coseKeyInfo = CoseJoseKeyMappingService.toCoseKeyInfo(keyInfo)
    ...
    @Test
    fun adapterWithNullSimpleSignatureServiceProviderShouldWork() = runTest {
    ...
    @Test
    fun adapterWithNullPublicKeyResolverProviderShouldWork() = runTest {
