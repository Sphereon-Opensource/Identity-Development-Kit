...
actual class SoftwareKeyStoreService actual constructor(
     ...
     */
    private fun getNativeKeyAsJwk(alias: String, keyInfo: KeyInfoType<*>): ManagedKeyInfoType<*> {
        val secKeyRef = Keychain.getNativeKey(alias)
            ?: throw PKIException("Native key not found: $alias")

        val publicKeyRef = platform.Security.SecKeyCopyPublicKey(secKeyRef)
            ?: throw PKIException("Failed to extract public key from native key: $alias")

        val publicKeyData = platform.Security.SecKeyCopyExternalRepresentation(publicKeyRef, null)
        ...
        val pubKeyBytes = pubKeyBytesPtr?.readBytes(pubKeyLength)
        ...
        val jwk = if (pubKeyBytes.size == 65 && pubKeyBytes[0] == 0x04.toByte()) {
        ...
        val key = when (requestedEncoding) {
     ...
     */
    private fun getImportedKeyAsJwk(alias: String, keyInfo: KeyInfoType<*>): ManagedKeyInfoType<*> {
        val keyData = Keychain.findKeyByAlias(alias)
            ?: throw PKIException("Key not found: $alias")

        // Convert DER to JWK
        val jwk = if (keyData.isPrivate) {
    ...
    actual override suspend fun storeKey(
        ...
        // Check if this is actually a private key (has 'd' component in JWK)
        ...
        // Import the key from JWK/DER
        val (secKeyRef, keyDer) = if (isPrivate) {
            when (keyType) {
                KeyTypeMapping.EC -> {
                    val signumKey = jwk.toSignumPrivateKey() as CryptoPrivateKey.EC.WithPublicKey
                    val secKey = signumKey.toSecKey().getOrThrow()
                    val der = signumKey.encodeToDer()
                    secKey to der
                }
                KeyTypeMapping.RSA -> {
                    val signumKey = jwk.toSignumPrivateKey() as CryptoPrivateKey.RSA
                    val withPubKey = signumKey as CryptoPrivateKey.WithPublicKey<*>
                    val secKey = withPubKey.toSecKey().getOrThrow()
                    val der = signumKey.encodeToDer()
                    secKey to der
                }
            ...
            val signumKey = jwk.toSignumPublicKey()
            val secKey = signumKey.toSecKey().getOrThrow()
            val der = signumKey.encodeToDer()
            secKey to der
        }
        ...
        Keychain.addKey(
            ...
            keyType = keyType,
            isPrivate = isPrivate,
            keySizeBits = keySizeGuess(secKeyRef, keyType)
        )
    ...
    actual override suspend fun listCertificateAliases(): Array<String> {
        return Keychain.listCertificateAliases().toTypedArray()
    }

    actual override suspend fun getCertificate(alias: String): Certificate {
        val der = Keychain.findCertificateDERByAlias(alias)
    ...
    actual override suspend fun deleteCertificate(alias: String): Boolean {
        return Keychain.deleteCertificateByAlias(alias)
    }
    ...
    private fun decodeBase64(input: String): ByteArray =
        try {
            ...
            throw PKIException("Invalid Base64 ${e.message}")
        }

    private fun keySizeGuess(secKeyRef: OwnedCFValue<SecKeyRef>, keyType: KeyTypeMapping): Int? =
        when (keyType) {
            KeyTypeMapping.EC -> 256 // default P-256 if curve not provided
            KeyTypeMapping.RSA -> null // let Security infer
            else -> null
        }
...
internal object Keychain {
    ...
    private fun saveKeyIndex(index: Map<String, KeyMetadata>) {
        val indexData = index.entries.joinToString("\n") { (alias, meta) ->
            val keyTypeStr = when (meta.keyType) {
    ...
    fun addKey(
        ...
        // iOS will auto-compute kSecAttrApplicationLabel from the public key component
        // This allows iOS to later auto-link with certificates for identity creation
        val keyTypeAttr = when (keyType) {
    ...
    fun findKeyByAlias(alias: String): KeychainKeyData? {
        ...
        val query: CFDictionaryRef? = cfDict(
            kSecClass to kSecClassKey,
    ...
    fun addCertificate(alias: String, der: ByteArray) {
        val certRef = SecCertificateCreateWithData(null, der.toCFData())
            ?: throw PKIException("Invalid certificate DER")

        // For certificates, use kSecAttrLabel instead of kSecAttrApplicationTag
        val addQuery: CFDictionaryRef? = cfDict(
        ...
        if (status != 0) {
            ...
            if (status.toInt() == -25299) {
                val deleteQuery: CFDictionaryRef? = cfDict(
                    kSecClass to kSecClassCertificate,
                    kSecAttrLabel to alias.toCFData()
                )
                SecItemDelete(deleteQuery)
    ...
    fun listCertificateAliases(): List<String> {
        return loadCertIndex().toList()
    }
    ...
    fun generateAndStoreNativeKey(
        ...
        val attributes = cfDict(
            platform.Security.kSecAttrKeyType to when (keyType) {
     ...
     */
    fun markAsNativeKey(alias: String, keyType: KeyTypeMapping) {
        val index = loadKeyIndex()
     ...
     */
    fun getNativeKey(alias: String): SecKeyRef? = memScoped {
        val tag = tagFor(alias)
 ...
 */
fun markAsNativeKeychainKey(alias: String, keyType: KeyTypeMapping) = Keychain.markAsNativeKey(alias, keyType)
...
private fun ByteArray.toCFData(): CFDataRef? =
    CFDataCreate(null, this.refToU(0), this.size.toLong())
...
@OptIn(ExperimentalForeignApi::class)
private fun Boolean.toCFBoolean(): CFBooleanRef =
    if (this) kCFBooleanTrue!! else kCFBooleanFalse!!

@OptIn(ExperimentalForeignApi::class)
private fun Int.toCFNumber(): CFNumberRef? = memScoped {
    val v = alloc<IntVar>()
...
@OptIn(ExperimentalForeignApi::class)
private fun cfDict(vararg pairs: Pair<CFTypeRef?, CFTypeRef?>): CFDictionaryRef? = memScoped {
