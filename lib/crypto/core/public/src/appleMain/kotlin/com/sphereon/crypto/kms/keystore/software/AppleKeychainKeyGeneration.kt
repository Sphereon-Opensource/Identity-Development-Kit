...
object AppleKeychainKeyGeneration {
    ...
    fun generateNativeKeyPair(
        ...
        val keySizeBits =
            when (keyType) {
                ...
                KeyTypeMapping.RSA -> {
                    ...
                    throw PKIException("Unsupported key type: $keyType")
                }
            }
        ...
        val (privateKey, publicKey) =
            memScoped {
                val privateTagData =
                    privateTag.encodeToByteArray().usePinned { pinned ->
                ...
                val publicTagData =
                    publicTag.encodeToByteArray().usePinned { pinned ->
        ...
        // Convert to JWK
        val (privateJwk, publicJwk) =
            convertNativeKeyToJwk(
    ...
    private fun convertNativeKeyToJwk(
        ...
        // Get public key data
        val publicKeyData =
            SecKeyCopyExternalRepresentation(publicKey, null) as? CFDataRef
    ...
    private fun convertECKeyToJwk(
        ...
        // Construct SubjectPublicKeyInfo and convert to JWK
        val spki = publicKeyECFrom(curveName, x, y)
        val publicJwk =
            spki
                .toJwk(
                    x5c = null,
     ...
     */
    private fun deleteNativeKeysForAlias(
        alias: String,
        privateTag: String,
        publicTag: String,
    ) = memScoped {
