...
class ExternalIdentifierResolverRegistry(
     ...
     */
    fun findResolvers(identifier: ExternalIdentifierBase): List<ExternalIdentifierResolver> {
        return sortedResolvers.filter { it.supports(identifier) }
    }
     ...
     */
    fun findResolver(identifier: ExternalIdentifierBase): ExternalIdentifierResolver? {
        return sortedResolvers.firstOrNull { it.supports(identifier) }
    }
     ...
     */
    suspend fun resolve(identifier: ExternalIdentifierBase): IdkResult<ExternalIdentifierResult, IdkError> {
        val resolver = findResolver(identifier)
            ?: return Err(IdkError.NOT_FOUND_ERROR(
                resource = "ExternalIdentifierResolver",
                message = "No resolver found for identifier type: ${identifier::class.simpleName}. " +
    ...
    fun getDiagnostics(): ExternalIdentifierRegistryDiagnostics {
        return ExternalIdentifierRegistryDiagnostics(
            resolverCount = resolvers.size,
            resolvers = sortedResolvers.map {
