...
object ConcatKdf {
    ...
    fun deriveKey(
        ...
        require(keyDataLen % 8 == 0) { "keyDataLen must be a multiple of 8 bits" }

        val keyDataLenBytes = keyDataLen / 8
        val hashLen = 32 // SHA-256 output is 32 bytes
    ...
    private fun buildOtherInfo(
        ...
        val size = 4 + algorithmBytes.size +  // AlgorithmID
                   4 + apu.size +              // PartyUInfo
                   4 + apv.size +              // PartyVInfo
                   4                           // SuppPubInfo (keyDataLen)
     ...
     */
    private fun buildHashInput(counter: Int, z: ByteArray, otherInfo: ByteArray): ByteArray {
        val result = ByteArray(4 + z.size + otherInfo.size)
        ...
        // Copy OtherInfo
        otherInfo.copyInto(result, 4 + z.size)
     ...
     */
    private fun writeInt32BE(array: ByteArray, offset: Int, value: Int) {
     ...
     */
    fun getKeyLengthBits(algorithm: KeyAgreementAlgorithm, encAlgorithm: String): Int {
        return when (algorithm) {
            // Direct key agreement - derive CEK directly
            KeyAgreementAlgorithm.ECDH_ES -> when (encAlgorithm) {
                "A128GCM" -> 128
                "A192GCM" -> 192
                "A256GCM" -> 256
                "A128CBC-HS256" -> 256  // 128 enc + 128 mac
                "A192CBC-HS384" -> 384  // 192 enc + 192 mac
                "A256CBC-HS512" -> 512  // 256 enc + 256 mac
            ...
            KeyAgreementAlgorithm.ECDH_ES_A128KW -> 128
            KeyAgreementAlgorithm.ECDH_ES_A192KW -> 192
            KeyAgreementAlgorithm.ECDH_ES_A256KW -> 256
        }
    }
     ...
     */
    fun getAlgorithmId(algorithm: KeyAgreementAlgorithm, encAlgorithm: String): String {
        return when (algorithm) {
            KeyAgreementAlgorithm.ECDH_ES -> encAlgorithm  // Use enc for direct
