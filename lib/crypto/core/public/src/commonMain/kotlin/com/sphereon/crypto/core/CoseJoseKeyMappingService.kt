...
object CoseJoseKeyMappingService {
    ...
    fun tryToJoseJwk(key: KeyType): IdkResult<Jwk, IdkError> {
        return when (key) {
            ...
            is CoseKeyJsonType -> {
                ...
                if (key.kty is CborUInt) {
                    return Ok(CoseKey.fromDTO(key as CoseKeyDTOType).cborToJwk())
                }
                if (key.kty is CoseKeyTypeEnum) {
                    return Ok(CoseKeyJson.fromJsonDTO(key as CoseKeyJsonDTOType).jsonToJwk())
                }
                if (key.kty is JwaKeyType) {
                    return Ok(Jwk.from(key as JwkType))
                }
                if (key.kty is String) {
                    return Ok(Jwk.fromDTO(key as JwkDTOType))
                }
    ...
    fun tryToCoseKey(key: KeyType): IdkResult<CoseKey, IdkError> {
        return when (key) {
            ...
            is CoseKeyJson -> {
                ...
                if (key.kty is CborUInt) {
                    return Ok(CoseKey.fromDTO(key as CoseKeyDTOType))
                }
                if (key.kty is CoseKeyTypeEnum) {
                    return Ok(CoseKeyJson.fromJsonDTO(key as CoseKeyJsonDTOType).toCbor())
                }
                if (key.kty is JwaKeyType) {
                    return Ok(Jwk.from(key as JwkType).jwkToCoseKey())
                }
                if (key.kty is String) {
                    return Ok(Jwk.fromDTO(key as JwkDTOType).jwkToCoseKey())
                }
    ...
    fun tryToJoseX5c(x5c: Array<Any>?): IdkResult<Array<String>?, IdkError> {
        ...
        for (item in x5c) {
            when (item) {
                ...
                is CborByteString -> result.add(item.encodeValueTo(Encoding.BASE64))

                // x5c is always base64
     ...
     */
    fun <KeyType : com.sphereon.crypto.core.KeyType> toResolvedKeyInfo(
        keyInfo: KeyInfoType<*>,
        key: KeyType? = null,
    ): ResolvedKeyInfoType<KeyType> = ResolvedKeyInfo.fromKeyInfo(keyInfo, key)
    ...
    fun <KeyType : com.sphereon.crypto.core.KeyType> toResolvedKeyInfoWithResolver(
        ...
        tryToResolvedKeyInfoWithResolver(keyInfo, resolveCallback).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun tryToCoseX5chain(x5c: Array<Any>?): IdkResult<CborArray<CborByteString>?, IdkError> {
        ...
        for (item in x5c) {
            when (item) {
                ...
                is String -> result.add(item.toCborByteString(Encoding.BASE64))
    ...
    fun toCoseX5chain(x5c: Array<Any>?): CborArray<CborByteString>? =
        tryToCoseX5chain(x5c).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
}
