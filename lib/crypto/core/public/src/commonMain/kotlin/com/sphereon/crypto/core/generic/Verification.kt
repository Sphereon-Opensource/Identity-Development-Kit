...
@ObjCName("VerifyResultType", exact = true)
@JsExportCompat
 ...
 *
 * @param KeyType The specific type of key implementing the Key interface.
 */
@JsExportCompat
interface VerifyResultsType<out KeyType : com.sphereon.crypto.core.KeyType> : HasToJsonString {
    /**
     * Indicates whether an error has occurred.
     *
 ...
 *
 * @param KeyType The type of key being verified, which must implement the `Key` interface.
...
@Serializable
@JsExportCompat
data class VerifyResults<out KeyType : com.sphereon.crypto.core.KeyType>(
    override val error: Boolean,
    override val verifications: Array<VerifyResult>,
    override val keyInfo: KeyInfoType<KeyType>?,
) : VerifyResultsType<KeyType> {
    /**
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (!verifications.contentEquals(other.verifications)) {
            return false
        }
     ...
     */
    override fun toString(): String = "VerifyResults(error=$error, verifications=${verifications.contentToString()}, keyInfo=$keyInfo)"
...
open class VerifyResult(
     ...
     */
    override fun toString(): String = "VerifyResult(name='$name', error=$error, message=$message, critical=$critical)"
 ...
 */
@JsExportCompat
sealed interface VerifySignatureResultType<out KeyType : com.sphereon.crypto.core.KeyType> : VerifyResultType {
    /**
     * Represents information related to a specific key.
     *
     * This variable holds an instance of `KeyInfo<KeyType>`, which provides metadata and functionality
 ...
 *
 * @param KT The type of the key implementing the Key interface.
...
@JsExportCompat
@Serializable
class VerifySignatureResult<out KT : KeyType>(
    override val error: Boolean,
    override val name: String,
    override val critical: Boolean,
    override val message: String? = null,
    override val detailMessage: String? = null,
    override val keyInfo: KeyInfoType<KT>? = null,
) : VerifySignatureResultType<KT> {
    /**
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is VerifySignatureResult<Any>) {
            return false
        }
     ...
     */
    override fun toString(): String = "VerifySignatureResult(keyInfo=$keyInfo)"
}
