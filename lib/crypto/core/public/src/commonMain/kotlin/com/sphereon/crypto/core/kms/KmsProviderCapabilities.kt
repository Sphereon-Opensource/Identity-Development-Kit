...
data class OperationCapability(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (!signatureAlgorithms.contentEquals(other.signatureAlgorithms)) return false
        if (!contentEncryptionAlgorithms.contentEquals(other.contentEncryptionAlgorithms)) return false
        if (!keyWrapAlgorithms.contentEquals(other.keyWrapAlgorithms)) return false
        if (!keyAgreementAlgorithms.contentEquals(other.keyAgreementAlgorithms)) return false
...
data class KmsProviderCapabilities(
     ...
     */
    fun supportsOperation(operation: KmsProviderOperation): Boolean =
        operations.any { it.operation == operation && it.supported }
     ...
     */
    fun getOperationCapability(operation: KmsProviderOperation): OperationCapability? =
        operations.firstOrNull { it.operation == operation }

    /**
     * Checks if a specific crypto algorithm is supported.
     */
    fun supportsCryptoAlgorithm(algorithm: CryptoAlg): Boolean =
        supportedCryptoAlgorithms.contains(algorithm)

    /**
     * Checks if a specific digest algorithm is supported.
     */
    fun supportsDigestAlgorithm(algorithm: DigestAlg): Boolean =
        supportedDigestAlgorithms.contains(algorithm)

    /**
     * Checks if a specific content encryption algorithm is supported.
     */
    fun supportsContentEncryption(algorithm: ContentEncryptionAlgorithm): Boolean =
        contentEncryptionAlgorithms.contains(algorithm)

    /**
     * Checks if a specific identifier resolution method is supported.
     */
    fun supportsIdentifierMethod(method: IIdentifierMethod): Boolean =
        resolutionMethods.any { it.methodName == method.methodName }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (!storageTypes.contentEquals(other.storageTypes)) return false
        if (supportsKeyImport != other.supportsKeyImport) return false
        if (supportsKeyExport != other.supportsKeyExport) return false
        if (exposePrivateKeys != other.exposePrivateKeys) return false
        if (!operations.contentEquals(other.operations)) return false
        if (!supportedKeyTypes.contentEquals(other.supportedKeyTypes)) return false
        if (!supportedCurves.contentEquals(other.supportedCurves)) return false
        if (!supportedCryptoAlgorithms.contentEquals(other.supportedCryptoAlgorithms)) return false
        if (!supportedDigestAlgorithms.contentEquals(other.supportedDigestAlgorithms)) return false
        if (!signatureAlgorithms.contentEquals(other.signatureAlgorithms)) return false
        if (!contentEncryptionAlgorithms.contentEquals(other.contentEncryptionAlgorithms)) return false
        if (supportsX509 != other.supportsX509) return false
        if (supportsAttestation != other.supportsAttestation) return false
        if (supportsHardwareBacking != other.supportsHardwareBacking) return false
        if (supportsPublicKeyResolution != other.supportsPublicKeyResolution) return false
        if (!resolutionMethods.contentEquals(other.resolutionMethods)) return false
...
enum class ContentEncryptionAlgorithm(
    ...
    companion object {
        @JsStatic
        fun fromIdentifier(identifier: String): ContentEncryptionAlgorithm? =
...
class KmsProviderQueryBuilder {
    ...
    fun build(): KmsProviderQuery = KmsProviderQuery(
        operation = operation,
 ...
 * ```
 */
fun kmsQuery(block: KmsProviderQueryBuilder.() -> Unit): KmsProviderQuery {
    return KmsProviderQueryBuilder().apply(block).build()
}
