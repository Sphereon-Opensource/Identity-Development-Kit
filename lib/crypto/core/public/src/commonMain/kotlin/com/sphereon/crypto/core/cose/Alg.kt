...
enum class CoseAlgorithm(
     ...
     */
    EdDSA("EdDSA", -8, CoseKeyTypeEnum.OKP, null, null, CoseCurve.Ed25519, "EdDSA", AlgorithmType.SIGNATURE),
     ...
     */
    HS256("HS256", 5, CoseKeyTypeEnum.Symmetric, DigestAlg.SHA256, 256, null, "HMAC w/ SHA-256", AlgorithmType.SIGNATURE),
     ...
     */
    HS384("HS384", 6, CoseKeyTypeEnum.Symmetric, DigestAlg.SHA384, 384, null, "HMAC w/ SHA-384", AlgorithmType.SIGNATURE),
     ...
     */
    HS512("HS512", 7, CoseKeyTypeEnum.Symmetric, DigestAlg.SHA512, 512, null, "HMAC w/ SHA-512", AlgorithmType.SIGNATURE),
...
// Encryption
    ...
    companion object {
        ...
        fun fromValue(value: Int?, type: AlgorithmType? = null): CoseAlgorithm? {
            return entries.find { entry -> entry.value == value && (type === null || type === entry.type) }
        }
        ...
        fun fromName(name: String): CoseAlgorithm =
            entries.find { it.id == name || it.name == name } ?: throw IllegalArgumentException("Unknown cose algorithm: $name")
        ...
        fun asList(type: AlgorithmType? = null): Array<CoseAlgorithm> {
            return entries.filter { entry -> type === null || entry.keyType == type }.toTypedArray()
        }

    }

    override fun toString(): String {
        return "CoseAlgorithm(id='$id', value=$value, keyType=$keyType, hash=$digestAlg, tagLength=$tagLength, curve=$curve, description='$explanation', type=$type)"
    }
