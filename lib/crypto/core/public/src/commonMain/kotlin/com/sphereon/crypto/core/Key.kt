...
object ResolvedKeyInfoSerializer : KSerializer<ResolvedKeyInfoType<*>> {
    ...
    override fun deserialize(decoder: Decoder): ResolvedKeyInfoType<*> = decoder.decodeSerializableValue(delegate)
}
...
/**
 * Sealed interface for metadata-only key references within the [KeyInfoType] hierarchy.
 *
 * A key reference carries identifying metadata (alias, kid, provider) and optional
 * classification fields, but never carries actual key material. Used as the return
 * type of [com.sphereon.crypto.core.kms.KeyStoreService.listKeys] to avoid exposing
 * key material during listing operations.
 *
...
/**
 * Data class implementation of [ManagedKeyReferenceType].
 *
 * Metadata-only key reference — contains no key material, certificates, or JWKs.
 * Part of the [KeyInfoType] sealed hierarchy so it can be used anywhere
...
data class ManagedKeyInfo<out KT : KeyType>(
    ...
    ResolvedKeyInfoType<KT> by resolvedKeyInfo {
    override val key = resolvedKeyInfo.key

    override fun toResolvedPublicKeyInfo() = resolvedKeyInfo.toResolvedPublicKeyInfo()

    override fun toManagedPublicKeyInfo(): ManagedKeyInfoType<KT> = this.copy(resolvedKeyInfo = resolvedKeyInfo.toResolvedPublicKeyInfo(), alias = alias, providerId = providerId)
    ...
    companion object {
        ...
        fun <T : KeyType> build(
            ...
            providerId: String? = keyInfo.providerId,
        ): ManagedKeyInfo<T> {
