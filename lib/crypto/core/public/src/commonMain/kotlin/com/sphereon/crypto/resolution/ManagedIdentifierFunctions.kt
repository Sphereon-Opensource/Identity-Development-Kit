 ...
 */
fun isManagedIdentifierResult(identifier: ManagedIdentifierOptsOrResult): Boolean {
    return identifier is ManagedIdentifierResult<*>
}
...
suspend fun tryEnsureManagedIdentifierResult(
    identifier: ManagedIdentifierOptsOrResult,
): IdkResult<ManagedIdentifierResult<*>, IdkError> = withContext(Dispatchers.Default) {
    if (isManagedIdentifierResult(identifier)) {
        ...
        when (opts) {
            is ManagedOptsJwk -> {
                ...
                val keyInfo = opts.lookup as? com.sphereon.crypto.core.ResolvedKeyInfoType<com.sphereon.crypto.core.jose.JwkType>
                    ?: return@withContext Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "ManagedOptsJwk lookup must be a ResolvedKeyInfoType"))
                val alias = keyInfo.alias ?: jwk.kid
            ...
            is ManagedOptsKey -> {
                val keyInfo = opts.lookup as? com.sphereon.crypto.core.ResolvedKeyInfoType<com.sphereon.crypto.core.KeyType>
                    ?: return@withContext Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "ManagedOptsKey lookup must be a ResolvedKeyInfoType"))
                val alias = keyInfo.alias ?: keyInfo.kid
            ...
            is ManagedOptsCoseKey -> {
                ...
                val keyInfo = opts.lookup as? com.sphereon.crypto.core.ResolvedKeyInfoType<com.sphereon.crypto.core.cose.CoseKeyType>
                ...
                Ok(ManagedIdentifierCoseKeyResult(
                    identifier = coseKey,
                    keyInfo = managedKeyInfo,
                    context = opts.context
                ))
            }
            else -> Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
                message = "Cannot resolve ${opts::class.simpleName} without a ManagedIdentifierService. " +
 ...
 */
suspend fun ensureManagedIdentifierResult(
    identifier: ManagedIdentifierOptsOrResult,
): ManagedIdentifierResult<*> {
    return tryEnsureManagedIdentifierResult(identifier).getOrElse {
 ...
 */
@Beta(message = "Safe conversion API - may have minor changes in future versions")
suspend fun tryManagedIdentifierToKeyResult(
    identifier: ManagedIdentifierOptsOrResult,
): IdkResult<ManagedIdentifierKeyResult, IdkError> {
 ...
 * @throws IllegalArgumentException if conversion fails.
 */
suspend fun managedIdentifierToKeyResult(
    identifier: ManagedIdentifierOptsOrResult,
): ManagedIdentifierKeyResult {
    return tryManagedIdentifierToKeyResult(identifier).getOrElse {
        throw IllegalArgumentException(it.message.defaultMessage)
    }
}
 ...
 */
@Beta(message = "Safe conversion API - may have minor changes in future versions")
suspend fun tryManagedIdentifierToJwk(
    identifier: ManagedIdentifierOptsOrResult,
): IdkResult<ManagedIdentifierJwkResult, IdkError> {
    val managedResult = tryEnsureManagedIdentifierResult(identifier).getOrElse { return Err(it) }
 ...
 * @throws IllegalArgumentException if conversion fails.
 */
suspend fun managedIdentifierToJwk(
    identifier: ManagedIdentifierOptsOrResult,
): ManagedIdentifierJwkResult {
    return tryManagedIdentifierToJwk(identifier).getOrElse {
        throw IllegalArgumentException(it.message.defaultMessage)
    }
}