...
) : AbstractKeyStoreConfig(), KeyStoreConfig
...
interface KeyStoreFactory : IWithKeyStoreType {
     ...
     */
    fun create(config: KeyStoreConfig, execution: SessionExecution?): KeyStore {
...
interface KeyStore : IWithKeyStoreType, KeyStoreService, CertificateStoreService {
    val order: Int
...
interface KeyStoreManager {
    ...
    fun createFromKeyStoreConfig(config: KeyStoreConfig, execution: SessionExecution?): KeyStore
...
interface KeyStoreManagerAsync : KeyStoreManager {
     ...
     */
    suspend fun createFromPropertiesAsync(configService: ConfigService): Set<KeyStore> =
        createFromProperties(configService)
}
...
interface KeyStoreConfigBinder {
     ...
     * Requires sync cache to be warmed up if using external cache.
     */
    fun getKeyStoreConfig(configService: ConfigService, providerId: String): KeyStoreConfig
     ...
     */
    suspend fun getKeyStoreIdsAsync(configService: ConfigService): Array<String> =
        getKeyStoreIds(configService)
     ...
     * Default implementation delegates to sync version.
     */
    suspend fun getKeyStoreConfigAsync(configService: ConfigService, providerId: String): KeyStoreConfig =
        getKeyStoreConfig(configService, providerId)
     ...
     * Default implementation delegates to sync version.
     */
    suspend fun getKeyStoreConfigsAsync(configService: ConfigService): Array<KeyStoreConfig> =
        getKeyStoreConfigs(configService)
