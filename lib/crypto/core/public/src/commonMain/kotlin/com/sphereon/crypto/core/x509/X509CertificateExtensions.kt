...
suspend fun Certificate.downloadCertificateChain(httpClient: HttpClient): List<Certificate> {
    return downloadExtraCertificatesInternal(this, httpClient, mutableSetOf())
}

suspend fun X509Certificate.downloadCertificateChain(
    httpClient: HttpClient
): List<X509Certificate> {
    return downloadExtraCertificatesInternal(this.toCertificateDto(), httpClient, mutableSetOf()).map { it.toSignumX509Certificate() }
}

private suspend fun downloadExtraCertificatesInternal(
    certificate: Certificate, httpClient: HttpClient, seen: MutableSet<Certificate>
): List<Certificate> {
    if (certificate in seen) {
        throw IllegalStateException("Circular Authority Information Access (AIA) reference detected for certificate: ${certificate.subjectDN}")
    }
    seen.add(certificate)
    val signumCertificate = certificate.toSignumX509Certificate()
    ...
    for (accessDesc in seqElement.children) {
        ...
        if (!isAccessMethodOid(
                firstChild, listOf(
                    X509ExtensionOids.AUTHORITY_INFORMATION_ACCESS, X509ExtensionOids.CA_ISSUERS
                )
            )
        ) {
            continue
