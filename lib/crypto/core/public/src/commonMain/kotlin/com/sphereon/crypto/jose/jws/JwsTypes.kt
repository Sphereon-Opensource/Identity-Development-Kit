...
@JsExportCompat
@Serializable
data class JwsCompact(val value: String) : Jws {
...
data class PreparedJws(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (existingSignatures != other.existingSignatures) return false
...
data class PreparedJwsObject(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (b64 != other.b64) return false
        if (identifier != other.identifier) return false
        if (!signingInput.contentEquals(other.signingInput)) return false
...
object JwsTypeUtils {
    ...
    fun isJwsCompact(jws: Jws): Boolean {
        return jws is JwsCompact
    }

    fun isJwsJsonFlattened(jws: Jws): Boolean {
        return jws is JwsJsonFlattened
    }

    fun isJwsJsonGeneral(jws: Jws): Boolean {
        return jws is JwsJsonGeneral
    }
    
    fun isValidCompactFormat(value: String): Boolean {
        return value.split("~")[0].matches(COMPACT_JWS_REGEX)
    }
}
...
fun PreparedJwsObject.assembleGeneral(signatureBytes: ByteArray): JwsJsonGeneral {
    ...
    val signature = JwsJsonSignature(
