...
) : ManagedIdentifierOptsOrResult(identifier = identifier, method = method, context = context),
    ManagedKeyInfoType<KeyType> by keyInfo {
...
object ManagedIdentifierOptsTypeGuards {
    fun isManagedIdentifierDidOpts(opts: ManagedIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.DID) || IdentifierTypeUtils.isDidIdentifier(opts.identifier)

    fun isManagedIdentifierKidOpts(opts: ManagedIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.KID) || IdentifierTypeUtils.isKidIdentifier(opts.identifier)

    fun isManagedIdentifierKeyOpts(opts: ManagedIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.KEY) || IdentifierTypeUtils.isKeyIdentifier(opts.identifier)

    fun isManagedIdentifierCoseKeyOpts(opts: ManagedIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.COSE_KEY) || IdentifierTypeUtils.isCoseKeyIdentifier(opts.identifier)

    fun isManagedIdentifierOID4VCIssuerOpts(opts: ManagedIdentifierOpts): Boolean =
        (opts.method == IdentifierMethodDefaults.OID4VCI_ISSUER) || IdentifierTypeUtils.isOID4VCIssuerIdentifier(opts.identifier)

    fun isManagedIdentifierJwkOpts(opts: ManagedIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.JWK) || IdentifierTypeUtils.isJwkIdentifier(opts.identifier)

    fun isManagedIdentifierX5cOpts(opts: ManagedIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.X5C) || IdentifierTypeUtils.isX5cIdentifier(opts.identifier)

    fun asManagedIdentifierDidOpts(opts: ManagedIdentifierOpts): ManagedOptsDid = require(isManagedIdentifierDidOpts(opts)) { "opts is not a ManagedOptsDid" }.let { opts as ManagedOptsDid }

    fun asManagedIdentifierKidOpts(opts: ManagedIdentifierOpts): ManagedOptsKid = require(isManagedIdentifierKidOpts(opts)) { "opts is not a ManagedIdentifierOpts.Kid" }.let { opts as ManagedOptsKid }

    fun asManagedIdentifierKeyOpts(opts: ManagedIdentifierOpts): ManagedOptsKey = require(isManagedIdentifierKeyOpts(opts)) { "opts is not a ManagedIdentifierOpts.Key" }.let { opts as ManagedOptsKey }

    fun asManagedIdentifierCoseKeyOpts(opts: ManagedIdentifierOpts): ManagedOptsCoseKey =
        require(isManagedIdentifierCoseKeyOpts(opts)) {
            "opts is not a ManagedIdentifierOpts.CoseKey"
        }.let { opts as ManagedOptsCoseKey }

    fun asManagedIdentifierJwkOpts(opts: ManagedIdentifierOpts): ManagedOptsJwk = require(isManagedIdentifierJwkOpts(opts)) { "opts is not a ManagedIdentifierOpts.Jwk" }.let { opts as ManagedOptsJwk }

    fun asManagedIdentifierOID4VCIssuerOpts(opts: ManagedIdentifierOpts): ManagedOptsOID4VCIssuer =
        require(isManagedIdentifierOID4VCIssuerOpts(opts)) {
            "opts is not a ManagedIdentifierOID4VCIssuerOpts"
        }.let { opts as ManagedOptsOID4VCIssuer }

    fun asManagedIdentifierX5cOpts(opts: ManagedIdentifierOpts): ManagedOptsX5c = require(isManagedIdentifierX5cOpts(opts)) { "opts is not a ManagedIdentifierOpts.X5c" }.let { opts as ManagedOptsX5c }
}
...
object ManagedIdentifierResultTypeGuards {
    fun isManagedIdentifierCoseKeyResult(result: ManagedIdentifierResult<*>): Boolean = result.method == IdentifierMethodDefaults.COSE_KEY

    fun isManagedIdentifierDidResult(result: ManagedIdentifierResult<*>): Boolean = result.method == IdentifierMethodDefaults.DID

    fun isManagedIdentifierX5cResult(result: ManagedIdentifierResult<*>): Boolean = result.method == IdentifierMethodDefaults.X5C

    fun isManagedIdentifierJwkResult(result: ManagedIdentifierResult<*>): Boolean = result.method == IdentifierMethodDefaults.JWK

    fun isManagedIdentifierKidResult(result: ManagedIdentifierResult<*>): Boolean = result.method == IdentifierMethodDefaults.KID

    fun isManagedIdentifierKeyResult(result: ManagedIdentifierResult<*>): Boolean = result.method == IdentifierMethodDefaults.KEY
}
