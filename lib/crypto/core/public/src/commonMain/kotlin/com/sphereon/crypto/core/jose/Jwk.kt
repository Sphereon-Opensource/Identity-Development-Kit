...
data class Jwk(
...
) : JwkType, HasToJsonString, HasToJsonDTO {
    ...
    override fun getSignatureAlgorithm(): SignatureAlgorithm? {
        ...
        alg?.let { return SignatureAlgorithm.fromJose(it) }
        ...
        return when (kty.value) {
            ...
            "oct" -> {
                ...
                when {
                    keyBytes == null -> null
                    keyBytes >= 64 -> SignatureAlgorithm.HMAC_SHA512  // 512 bits
                    keyBytes >= 48 -> SignatureAlgorithm.HMAC_SHA384  // 384 bits
                    keyBytes >= 32 -> SignatureAlgorithm.HMAC_SHA256  // 256 bits
                    else -> null
                }
            }
            "OKP" -> {
    ...
    override fun getKeyType(): KeyTypeMapping {
        return KeyTypeMapping.fromJose(this.kty)
    }
    ...
    override fun getKeyOperations(): Array<KeyOperations>? {
        return key_ops?.map { KeyOperations.fromJose(it) }?.toTypedArray()
    }
    ...
    override fun getX509CertificateChain(): Array<String>? {
        return x5c?.map { x509DerOrPemToDer(it) }?.toTypedArray()
    }
    ...
    override fun getX509Certificate(): Certificate? =
        getX509CertificateChain()?.firstOrNull()?.let { cert -> certificateFromDer(cert.decodeFrom(Encoding.BASE64)) }
    ...
    override fun getKeyId(generate: Boolean) = kid ?: if (generate) determineKid() else kid
    ...
    fun tryToMinimalJwk(): IdkResult<Jwk, IdkError> {
        return when (kty) {
            JwaKeyType.EC -> Ok(Jwk(
                kty = kty,
                crv = crv,
                x = x,
                y = y
            ))
            JwaKeyType.RSA -> Ok(Jwk(
                kty = kty,
                n = n,
                e = e
            ))
            JwaKeyType.oct -> Ok(Jwk(
                kty = kty,
                k = k
            ))
            else -> Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unsupported key type for minimal JWK: $kty"))
        }
    }
     ...
     */
    fun toMinimalJwk(): Jwk = tryToMinimalJwk().getOrElse {
     ...
     */
    override fun publicKeyPem(): String = tryPublicKeyPem().getOrElse {
        throw IllegalArgumentException(it.message.defaultMessage)
    }
     ...
     */
    private fun toRSAPublicKeyPem(): String = tryToRSAPublicKeyPem().getOrElse {
        throw IllegalArgumentException(it.message.defaultMessage)
    }
     ...
     */
    private fun toECPublicKeyPem(): String = tryToECPublicKeyPem().getOrElse {
        throw IllegalArgumentException(it.message.defaultMessage)
    }
    ...
    class Builder {
         ...
         */
        fun withKid(kid: String? = null, generate: Boolean? = null) = apply { this.kid = kid; this.generateKid = generate ?: generateKid }
         ...
         */
        fun build(): Jwk = tryBuild().getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
...
// Name is like other extensions functions to not class with JS
    fun jwkToCoseKeyJson(): CoseKeyJson = tryJwkToCoseKeyJson().getOrElse {
        throw IllegalArgumentException(it.message.defaultMessage)
    }
     ...
     */
    fun tryJwkToCoseKey(): IdkResult<CoseKey, IdkError> = tryJwkToCoseKeyJson().let { result ->
        when {
...
// Name is like other extensions functions to not class with JS
    ...
    override fun <T> toJsonDTO() = com.sphereon.cbor.json.toJsonDTO<T>(this)
    ...
    companion object {
         ...
         */
        @JsStatic
        fun fromDTO(jwk: JwkDTOType): Jwk = with(jwk) {
            return Jwk(
         ...
         */
        @JsStatic
        fun tryFromJsonObject(jwk: JsonObject): IdkResult<Jwk, IdkError> = with(jwk) {
            val keyType = get("kty")?.jsonPrimitive?.content?.let { JwaKeyType.fromValue(it) }
                ?: return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "kty is required in JWK"))
            return@tryFromJsonObject Ok(Jwk(
                ...
                qInv = get("qInv")?.jsonPrimitive?.content,
         ...
         */
        @JsStatic
        fun fromJsonObject(jwk: JsonObject): Jwk = tryFromJsonObject(jwk).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun from(jwk: JwkType): Jwk = with(jwk) {
            return@from Jwk(
                alg = alg,
                crv = crv,
                d = d,
...
fun tryGenerateJwkThumbprint(jwk: JwkType): IdkResult<String, IdkError> {
    val jwkSubset = when (jwk.kty) {
        ...
        else -> return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unsupported key type for thumbprint: ${jwk.kty}"))
    }
 ...
 */
fun generateJwkThumbprint(jwk: JwkType): String = tryGenerateJwkThumbprint(jwk).getOrElse {
    throw IllegalArgumentException(it.message.defaultMessage)
}
