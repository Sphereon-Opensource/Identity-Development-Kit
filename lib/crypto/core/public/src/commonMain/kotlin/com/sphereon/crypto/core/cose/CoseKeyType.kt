...
import com.sphereon.crypto.core.interop.signumX509CertificateChainFromPem
...
@JsExportCompat
@Serializable
sealed interface CoseKeyJsonType : CoseKeyJsonDTOType, KeyType {
...
data class CoseKeyJson(
...
) : JsonView(), CoseKeyJsonType, HasToJsonString {
    init {
    ...
    override fun getSignatureAlgorithm(): SignatureAlgorithm? {
        return alg?.let { SignatureAlgorithm.fromCose(it) }
    }
    ...
    override fun getKeyType(): KeyTypeMapping {
        return KeyTypeMapping.fromCose(this.kty)
    }
    ...
    override fun getKeyOperations(): Array<KeyOperations>? {
        return key_ops?.map { KeyOperations.fromCose(it) }?.toTypedArray()
    }
     ...
     */
    override fun getX509Certificate(): Certificate? =
        getX509CertificateChain()?.firstOrNull()?.let { certBytes -> certificateFromPem(certBytes) }
     ...
     */
    override fun getX509CertificatePem(): String? =
        getX509CertificateChain()?.firstOrNull()?.let { cert -> wrapX509CertificatePem(cert) }

    override fun getKeyId(generate: Boolean): String? = if (!generate) kid else determineKid()
    ...
    override fun toCbor(): CoseKey =
        CoseKey.Builder().withKty(kty).withKid(kid, false).withAlg(alg).withKeyOps(key_ops).withBaseIV(baseIV)
            .withCrv(crv).withX(x).withY(y).withD(d).withX5Chain(x5chain)
            .withN(n).withRsaE(rsaE).withP(p).withQ(q).withDP(dP).withDQ(dQ).withQInv(qInv)
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other !is CoseKeyJsonType) return false
        ...
        if (rsaE != other.rsaE) return false
    ...
    override fun toString(): String {
        return "CoseKeyJsonImpl(kty=$kty, kid=$kid, alg=$alg, key_ops=${key_ops?.contentToString()}, baseIV=$baseIV, crv=$crv, x=$x, y=$y, d=$d, x5chain=${x5chain?.contentToString()}, n=$n, rsaE=$rsaE, p=$p, q=$q, dP=$dP, dQ=$dQ, qInv=$qInv, additional=$additional)"
    }
    ...
    companion object {
         ...
         */
        fun fromJsonDTO(dto: CoseKeyJsonDTOType) = with(dto) {
            CoseKeyJson(
                generateKid = false,
                kty = kty,
                kid = kid,
        ...
        fun fromX509CertificatePem(pem: String): CoseKeyJson {
            val certificates = signumX509CertificateChainFromPem(pem).map { it.toCertificateDto() }
...
@JsExportCompat
@Serializable
sealed interface CoseKeyType : CoseKeyDTOType, KeyType {
...
data class CoseKey(
...
) : CoseKeyType, CborJsonViewStructure<CoseKeyType, CoseKeyJsonType, CborMap<NumberLabel, CborItem<*>>>(cddl = CDDL.map, original = original) {
    ...
    /**
     * Constructs and returns a CBOR (Concise Binary Object Representation) builder for the CoseKey object.
     *
     * This method configures the CBOR map with known properties and any additional key-value pairs.
     * Known properties include kty, kid, alg, key_ops, baseIV, and key-type-specific parameters.
     * For EC/OKP keys: crv, x, y, d. For RSA keys: n, e, d, p, q, dP, dQ, qInv.
     * Properties in the additional map will not overwrite the known properties.
     *
     * @return a CborBuilder configured with the properties of the CoseKey object.
     */
    override fun cborBuilder(): CborBuilder<CoseKeyType> {
        ...
        // Note: COSE labels are overloaded - same label means different things for different key types
        val keyType = kty.value?.let { CoseKeyTypeEnum.fromValue(it) }
        if (keyType == CoseKeyTypeEnum.RSA) {
            // RSA key parameters: -1=n, -2=e, -3=d, -4=p, -5=q, -6=dP, -7=dQ, -8=qInv
            mapBuilder
                .put(N, n, true)
                .put(E, rsaE, true)
                .put(D_RSA, d, true)  // RSA uses d at label -3
                .put(P, rsaP, true)
                .put(Q, rsaQ, true)
                .put(DP, rsaDP, true)
                .put(DQ, rsaDQ, true)
                .put(QINV, rsaQInv, true)
        } else {
            // EC/OKP key parameters: -1=crv, -2=x, -3=y, -4=d
            mapBuilder
                .put(CRV, crv, true)
                .put(X, x, true)
                .put(Y, y, true)
...
//todo: Probably  nice to be able to provide an encoding parm, but then we have to adjust the whole interface
    override fun toJson(): CoseKeyJson {
        ...
        val builder = CoseKeyJson.Builder()
            .withKty(keyType)
     ...
     */
    override fun getSignatureAlgorithm(): SignatureAlgorithm? {
        return alg?.let { CoseAlgorithm.fromValue(it.value.toInt())?.let { coseAlg -> SignatureAlgorithm.fromCose(coseAlg) } }
    }
     ...
     */
    override fun getKeyOperations(): Array<KeyOperations>? {
        return key_ops?.value?.map { KeyOperations.fromCose(CoseKeyOperations.fromValue(it.value.toInt())) }?.toTypedArray()
    }
    ...
    override fun getX509Certificate(): Certificate? =
        getX509CertificateChain()?.firstOrNull()?.let { derBase64 -> certificateFromDer(derBase64.decodeFrom(Encoding.BASE64)) }

    override fun getX509CertificatePem(): String? =
        getX509CertificateChain()?.firstOrNull()?.let { cert -> wrapX509CertificatePem(cert) }

    override fun getKeyId(generate: Boolean) = (if (!generate) this.kid else determineKid())?.value?.encodeTo(Encoding.UTF8)
    ...
    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is CoseKeyType) return false
        ...
        if (n != other.n) return false
        if (rsaE != other.rsaE) return false
        if (rsaP != other.rsaP) return false
        if (rsaQ != other.rsaQ) return false
        if (rsaDP != other.rsaDP) return false
        if (rsaDQ != other.rsaDQ) return false
        if (rsaQInv != other.rsaQInv) return false
        if (additional != other.additional) return false
     ...
     */
    override fun toString(): String {
        return "CoseKeyImpl(kty=$kty, kid=$kid, alg=$alg, key_ops=$key_ops, baseIV=$baseIV, crv=$crv, x=$x, y=$y, d=$d, x5chain=$x5chain, n=$n, rsaE=$rsaE, rsaP=$rsaP, rsaQ=$rsaQ, rsaDP=$rsaDP, rsaDQ=$rsaDQ, rsaQInv=$rsaQInv, additional=$additional)"
    }
...
// EC (kty 2) + OKP (kty 1)
        ...
        @JsName("fromDTO")
        @JsStatic
        fun fromDTO(dto: CoseKeyDTOType) = with(dto) {
            CoseKey(
                generateKid = false,
                kty = kty,
                kid = kid,
        ...
        /**
         * Decodes the provided CBOR-encoded byte array into a `CoseKey` object.
         *
         * @param encodedKey The CBOR-encoded byte array representing the device engagement data.
         * @return The decoded `CoseKey` object.
         */
        @JsName("decodeCbor")
        @JsStatic
        fun decodeCbor(encodedKey: ByteArray): CoseKey =

            fromCborStructure(cborSerializer.decode(encodedKey)).copy(original = encodedKey)

        @JsStatic
        fun fromEncodedCborItem(encoded: CborEncodedItem<CoseKey>): CoseKey {
            return decodeCbor(encoded.value.taggedItem.value)
        }
         ...
         * @return The constructed `CoseKey` object based on the provided CBOR map.
         */
        @JsName("fromCborStructure")
        @JsStatic
        fun fromCborStructure(m: CborMap<NumberLabel, CborItem<*>>): CoseKey {
            ...
            // Key-type specific parameter parsing
            // Note: Labels -1 to -4 are overloaded between EC/OKP and RSA key types
            return if (keyType == CoseKeyTypeEnum.RSA) {
                // RSA key: -1=n, -2=e, -3=d, -4=p, -5=q, -6=dP, -7=dQ, -8=qInv
                CoseKey(
                    generateKid = false,
                    kty = kty,
                    kid = KID.optional(m),
                    ...
                    // RSA parameters (using RSA-specific labels)
                    n = N.optional(m),
                    rsaE = E.optional(m),
                    d = D_RSA.optional(m),  // RSA uses d at label -3
                    rsaP = P.optional(m),
                    rsaQ = Q.optional(m),
                    rsaDP = DP.optional(m),
                    rsaDQ = DQ.optional(m),
                    rsaQInv = QINV.optional(m),
                    additional = if (additional.isEmpty()) null else CborMap(additional)
                )
            } else {
                // EC/OKP key: -1=crv, -2=x, -3=y, -4=d
                CoseKey(
                    generateKid = false,
                    kty = kty,
                    kid = KID.optional(m),
