...
object JwsUtils {
     ...
     */
    fun encodeBytesToBase64Url(bytes: ByteArray): String {
        return bytes.encodeTo(Encoding.BASE64URL)
    }
     ...
     */
    fun decodeBase64UrlToBytes(base64Url: String): ByteArray {
        return base64Url.decodeFrom(Encoding.BASE64URL)
    }

    /**
     * Creates the signing input for a JWS (base64url(header).base64url(payload))
     */
    fun createSigningInput(base64UrlHeader: String, base64UrlPayload: String): ByteArray {
        return "$base64UrlHeader.$base64UrlPayload".encodeToByteArray()
    }
