...
internal object KeyTypeSerializer : KSerializer<KeyTypeMapping> {
    ...
    override fun serialize(encoder: Encoder, value: KeyTypeMapping) {
...
sealed class KeyTypeMapping(private val coseKeyType: CoseKeyTypeEnum, private val joseKeyType: JwaKeyType) {
    ...
    companion object {
        ...
        fun tryToJose(cose: CoseKeyTypeEnum): IdkResult<JwaKeyType, IdkError> =
            asList.find { it.coseKeyType == cose || it.coseKeyType.toString() == cose.toString() }?.joseKeyType
                ?.let { Ok(it) }
        ...
        fun tryToCose(jose: JwaKeyType): IdkResult<CoseKeyTypeEnum, IdkError> =
            asList.find { it.joseKeyType == jose || it.joseKeyType.toString() == jose.toString() }?.coseKeyType
                ?.let { Ok(it) }
        ...
        fun tryFromJose(jose: JwaKeyType): IdkResult<KeyTypeMapping, IdkError> =
            asList.find { it.joseKeyType == jose || it.joseKeyType.toString() == jose.toString() }
                ?.let { Ok(it) }
                ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "JOSE key type $jose not found"))
        ...
        fun tryFromCose(cose: CoseKeyTypeEnum): IdkResult<KeyTypeMapping, IdkError> =
            asList.find { it.coseKeyType == cose || it.coseKeyType.toString() == cose.toString() }
                ?.let { Ok(it) }
                ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "COSE key type $cose not found"))
         ...
         */
        @JsStatic
        fun toJose(cose: CoseKeyTypeEnum) = tryToJose(cose).getOrElse {
         ...
         */
        @JsStatic
        fun toCose(jose: JwaKeyType) = tryToCose(jose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun fromJose(jose: JwaKeyType) = tryFromJose(jose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun fromCose(cose: CoseKeyTypeEnum) = tryFromCose(cose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
        ...
        fun tryToJoseKty(kty: Any): IdkResult<JwaKeyType, IdkError> {
            return when (kty) {
                is String -> JwaKeyType.fromValue(kty)?.let { Ok(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown JOSE key type: $kty"))
                is Int -> CoseKeyTypeEnum.fromValue(kty.toLong())?.let { tryToJose(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown COSE key type value: $kty"))
                is CborNumber<*> -> CoseKeyTypeEnum.fromValue(kty.value.toLong())?.let { tryToJose(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown COSE key type value: ${kty.value}"))
                is CoseKeyTypeEnum -> tryToJose(kty)
                is JwaKeyType -> Ok(kty)
                else -> Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Cannot convert ${kty::class.simpleName} to JOSE key type"))
            }
        }
        ...
        fun tryToCoseKty(kty: Any): IdkResult<CoseKeyTypeEnum, IdkError> {
            return when (kty) {
                is Long -> CoseKeyTypeEnum.fromValue(kty)?.let { Ok(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown COSE key type value: $kty"))
                is String -> JwaKeyType.fromValue(kty)?.let { tryToCose(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown JOSE key type: $kty"))
                is Int -> CoseKeyTypeEnum.fromValue(kty.toLong())?.let { Ok(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown COSE key type value: $kty"))
                is CborNumber<*> -> CoseKeyTypeEnum.fromValue(kty.value.toLong())?.let { Ok(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown COSE key type value: ${kty.value}"))
                is CoseKeyTypeEnum -> Ok(kty)
                is JwaKeyType -> tryToCose(kty)
                else -> Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Cannot convert ${kty::class.simpleName} to COSE key type"))
            }
        }
         ...
         */
        @JsStatic
        fun toJoseKty(kty: Any): JwaKeyType = tryToJoseKty(kty).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toCoseKty(kty: Any): CoseKeyTypeEnum = tryToCoseKty(kty).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
...
internal object SignatureAlgorithmSerializer : KSerializer<SignatureAlgorithm> {
    ...
    override fun serialize(encoder: Encoder, value: SignatureAlgorithm) {
        encoder.encodeString(value::class.simpleName ?: value.toString())
    }
    ...
    */
    ...
    companion object {
        ...
        fun tryToJose(cose: CoseAlgorithm?): IdkResult<JwaAlgorithm, IdkError> {
            if (cose == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "COSE algorithm is null"))
            return tryFromCose(cose).let { result ->
                when {
                    result.isOk -> result.value.joseAlgorithm?.let { Ok(it) }
        ...
        fun tryToCose(jose: JwaAlgorithm?): IdkResult<CoseAlgorithm, IdkError> {
            if (jose == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "JOSE algorithm is null"))
            return tryFromJose(jose).let { result ->
                when {
                    result.isOk -> result.value.coseAlgorithm?.let { Ok(it) }
        ...
        fun tryFromJose(jose: JwaAlgorithm?): IdkResult<SignatureAlgorithm, IdkError> {
            if (jose == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "JOSE algorithm is null"))
            return asList.find { it.joseAlgorithm == jose || it.joseAlgorithm.toString() == jose.toString() }
                ?.let { Ok(it) }
        ...
        fun tryFromCose(cose: CoseAlgorithm?): IdkResult<SignatureAlgorithm, IdkError> {
            if (cose == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "COSE algorithm is null"))
            return asList.find { it.coseAlgorithm == cose || it.coseAlgorithm.toString() == cose.toString() }
                ?.let { Ok(it) }
        ...
        fun tryToJoseAlg(alg: Any?): IdkResult<JwaAlgorithm, IdkError> {
            if (alg == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Algorithm is null"))
            return when (alg) {
                is String -> JwaAlgorithm.fromValue(alg)?.let { Ok(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown JOSE algorithm: $alg"))
                is Int -> tryToJoseAlgFromCose(alg)
                is CborNumber<*> -> tryToJoseAlgFromCose(alg.value.toInt())
                is CoseAlgorithm -> tryToJose(alg)
                is JwaAlgorithm -> Ok(alg)
                else -> Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Cannot convert ${alg::class.simpleName} to JOSE algorithm"))
            }
        }
        ...
        fun tryToCoseAlg(alg: Any?): IdkResult<CoseAlgorithm, IdkError> {
            if (alg == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Algorithm is null"))
            return when (alg) {
                is String -> tryToCoseAlgFromJose(alg)
                is Int -> CoseAlgorithm.fromValue(alg)?.let { Ok(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown COSE algorithm value: $alg"))
                is CborNumber<*> -> CoseAlgorithm.fromValue(alg.value.toInt())?.let { Ok(it) }
                    ?: Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Unknown COSE algorithm value: ${alg.value}"))
                is CoseAlgorithm -> Ok(alg)
                is JwaAlgorithm -> tryToCose(alg)
                else -> Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Cannot convert ${alg::class.simpleName} to COSE algorithm"))
            }
        }
        ...
        fun tryToJoseAlgFromCose(algorithm: Int?): IdkResult<JwaAlgorithm, IdkError> {
            if (algorithm == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Algorithm is null"))
            val coseAlg = CoseAlgorithm.fromValue(algorithm)
        ...
        fun tryToCoseAlgFromJose(algorithm: String?): IdkResult<CoseAlgorithm, IdkError> {
            if (algorithm == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "Algorithm is null"))
            val joseAlg = JwaAlgorithm.fromValue(algorithm)
         ...
         */
        @JsStatic
        fun toJose(cose: CoseAlgorithm?) = tryToJose(cose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toCose(jose: JwaAlgorithm?) = tryToCose(jose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun fromJose(jose: JwaAlgorithm?) = tryFromJose(jose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun fromCose(cose: CoseAlgorithm?) = tryFromCose(cose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toJoseAlg(alg: Any?): JwaAlgorithm = tryToJoseAlg(alg).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toCoseAlg(alg: Any?): CoseAlgorithm = tryToCoseAlg(alg).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toJoseAlgFromCose(algorithm: Int?) = tryToJoseAlgFromCose(algorithm).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toCoseAlgFromJose(algorithm: String?): CoseAlgorithm = tryToCoseAlgFromJose(algorithm).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }

        @JsStatic
        fun fromValue(value: String): SignatureAlgorithm? =
            asList.find { it::class.simpleName == value }
    }
...
sealed class Curve(
    ...
    companion object {
        ...
        fun tryFromJose(jose: JwaCurve?): IdkResult<Curve, IdkError> {
            if (jose == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "JOSE curve is null"))
            return asList.find { it.joseCurve == jose || it.joseCurve.toString() == jose.toString() }
                ?.let { Ok(it) }
        ...
        fun tryFromCose(cose: CoseCurve?): IdkResult<Curve, IdkError> {
            if (cose == null) return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "COSE curve is null"))
            return asList.find { it.coseCurve == cose || it.coseCurve.toString() == cose.toString() }
                ?.let { Ok(it) }
        ...
        @JsStatic
        fun fromJose(jose: JwaCurve?) = tryFromJose(jose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }

        @JsStatic
        fun fromCose(cose: CoseCurve?) = tryFromCose(cose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
...
internal object KeyOperationsSerializer : KSerializer<KeyOperations> {
    ...
    override fun serialize(encoder: Encoder, value: KeyOperations) {
...
sealed class KeyOperations(
    ...
    companion object {
        ...
        fun tryFromJose(jose: JoseKeyOperations): IdkResult<KeyOperations, IdkError> =
            asList.find { it.joseKeyOperations == jose || it.joseKeyOperations.toString() == jose.toString() }
                ?.let { Ok(it) }
        ...
        fun tryFromCose(cose: CoseKeyOperations): IdkResult<KeyOperations, IdkError> =
            asList.find { it.coseKeyOperations == cose || it.coseKeyOperations.toString() == cose.toString() }
                ?.let { Ok(it) }
         ...
         */
        @JsStatic
        fun tryFromValue(value: String): IdkResult<KeyOperations, IdkError> = when (value) {
            "sign" -> Ok(SIGN)
            "verify" -> Ok(VERIFY)
            "wrapKey" -> Ok(WRAP_KEY)
            "decrypt" -> Ok(DECRYPT)
         ...
         */
        @JsStatic
        fun fromJose(jose: JoseKeyOperations) = tryFromJose(jose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun fromCose(cose: CoseKeyOperations) = tryFromCose(cose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toJose(cose: CoseKeyOperations) = tryToJose(cose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
         ...
         */
        @JsStatic
        fun toCose(jose: JoseKeyOperations) = tryToCose(jose).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }

        @JsStatic
        fun fromValue(value: String): KeyOperations = tryFromValue(value).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }

        @JsStatic
        fun toValue(value: KeyOperations): String = when (value) {
            SIGN -> "sign"
            VERIFY -> "verify"
            WRAP_KEY -> "wrapKey"
            DECRYPT -> "decrypt"
