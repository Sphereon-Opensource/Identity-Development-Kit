...
interface X509VerificationRequestType {
    ...
    fun toJsonString(): String =
        """{"chainDER":${chainDER?.joinToString(prefix = "[", postfix = "]") { it.toString() }}, "chainPEM":${
        ...
        }, "verificationProfile":"$verificationProfile", "verificationTime":"$verificationTime"}"""
        ...
        if (chainDER == null && chainPEM == null) {
            ...
            return Err(
                X509VerificationResult(
                    ...
                    message =
                        if (enabled) {
                            "Please provide either a chain in DER format or PEM format"
                        } else {
                            "X509 verification has been disabled"
                        },
                    critical = enabled,
 ...
 */
fun X509ValidationContext.successResult(message: String = "Certificate chain validated"): X509VerificationResult =
    X509VerificationResult(
        name = CryptoConst.X509_LITERAL,
        message = message,
        error = false,
 ...
 */
fun X509ValidationContext.disabledResult(): X509VerificationResult =
    X509VerificationResult(
        name = CryptoConst.X509_LITERAL,
        message = "X509 verification has been disabled",
        error = false,
