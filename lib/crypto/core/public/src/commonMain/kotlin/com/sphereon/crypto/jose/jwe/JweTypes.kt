...
data class JweCompact(
    ...
    companion object {
        ...
        fun parse(jwe: String): JweCompact {
            ...
            val encryptedKey = if (parts[1].isEmpty()) {
                ByteArray(0)
            } else {
...
data class JweJsonFlattened(
     ...
     */
    fun getUnprotectedHeader(): JweHeader? {
        return unprotected?.let { JweHeader.fromJson(it) }
    }
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): JweJsonFlattened {
            return cryptoJsonSerializer.decodeFromString(serializer(), json)
        }
    }
}
...
data class JweJsonGeneral(
    ...
    companion object {
         ...
         */
        fun fromJson(json: String): JweJsonGeneral {
            return cryptoJsonSerializer.decodeFromString(serializer(), json)
        }
    }
}
...
data class JweRecipient(
     ...
     */
    fun getHeader(): JweHeader? {
        return header?.let { JweHeader.fromJson(it) }
    }
}
 ...
 */
object JweTypeUtils {
    fun isJweCompact(jwe: Jwe): Boolean {
        return jwe is JweCompact
    }

    fun isJweJsonFlattened(jwe: Jwe): Boolean {
        return jwe is JweJsonFlattened
    }

    fun isJweJsonGeneral(jwe: Jwe): Boolean {
        return jwe is JweJsonGeneral
    }
}
