...
enum class PredefinedKmsProviderTypes : WithKmsProviderType {
    SOFTWARE, MOBILE, REST, AZURE_KEYVAULT, AWS_KMS, DIGIDENTITY, EPHEMERAL;
...
) : AbstractKmsProviderConfig(), KmsProviderConfigBase
...
interface KmsProviderFactory : WithKmsProviderType {
    fun create(config: KmsProviderConfigBase, execution: SessionExecution): KmsProvider
}
...
interface KmsProviderManager {
     ...
     */
    fun createFromProviderConfig(config: KmsProviderConfigBase, execution: SessionExecution): KmsProvider

    /**
     * Creates KMS providers from config properties (sync).
     * Requires sync cache to be warmed up if using external cache.
     */
    fun createFromProperties(configService: ConfigService, execution: SessionExecution): Set<KmsProvider>
     ...
     */
    suspend fun createFromPropertiesAsync(configService: ConfigService, execution: SessionExecution): Set<KmsProvider> =
        createFromProperties(configService, execution)
}
 ...
 */
@JsExportCompat
interface KmsProvider : WithKmsProviderType, SimpleSignatureService, EncryptionService, KeyStoreService {
    val order: Int
     ...
     */
    suspend fun generateMac(keyId: String, message: ByteArray, digestAlgorithm: DigestAlg): ByteArray =
        throw UnsupportedOperationException("Provider $id does not support MAC operations")
     ...
     * @throws UnsupportedOperationException if this provider does not support MAC operations
     */
    suspend fun verifyMac(keyId: String, message: ByteArray, mac: ByteArray, digestAlgorithm: DigestAlg): Boolean =
        throw UnsupportedOperationException("Provider $id does not support MAC operations")
}
...
interface KmsProviderConfigBinder {
     ...
     * Requires sync cache to be warmed up if using external cache.
     */
    fun getKmsProviderConfig(configService: ConfigService, providerId: String): KmsProviderConfigBase
     ...
     * Default implementation delegates to sync version.
     */
    suspend fun getKmsProviderIdsAsync(configService: ConfigService): Array<String> =
        getKmsProviderIds(configService)
     ...
     * Default implementation delegates to sync version.
     */
    suspend fun getKmsProviderConfigAsync(configService: ConfigService, providerId: String): KmsProviderConfigBase =
        getKmsProviderConfig(configService, providerId)
     ...
     * Default implementation delegates to sync version.
     */
    suspend fun getKmsProviderConfigsAsync(configService: ConfigService): Array<KmsProviderConfigBase> =
