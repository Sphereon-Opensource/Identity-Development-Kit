...
object ExternalIdentifierOptsGuards {
    fun isExternalIdentifierDidOpts(opts: ExternalIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.DID) || IdentifierTypeUtils.isDidIdentifier(opts.identifier)

    fun isExternalIdentifierKidOpts(opts: ExternalIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.KID) || IdentifierTypeUtils.isKidIdentifier(opts.identifier)

    fun isExternalIdentifierJwkOpts(opts: ExternalIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.JWK) || IdentifierTypeUtils.isJwkIdentifier(opts.identifier)

    fun isExternalIdentifierCoseKeyOpts(opts: ExternalIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.COSE_KEY) || IdentifierTypeUtils.isCoseKeyIdentifier(opts.identifier)

    fun isExternalIdentifierOidcDiscoveryOpts(opts: ExternalIdentifierOpts): Boolean =
        (opts.method == IdentifierMethodDefaults.OIDC_DISCOVERY) || IdentifierTypeUtils.isOidcDiscoveryIdentifier(opts.identifier)

    fun isExternalIdentifierJwksUrlOpts(opts: ExternalIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.JWKS_URL) || IdentifierTypeUtils.isJwksUrlIdentifier(opts.identifier)

    fun isExternalIdentifierOIDFEntityIdOpts(opts: ExternalIdentifierOpts): Boolean =
        (opts.method == IdentifierMethodDefaults.ENTITY_ID) || IdentifierTypeUtils.isOIDFEntityIdIdentifier(opts.identifier)

    fun isExternalIdentifierX5cOpts(opts: ExternalIdentifierOpts): Boolean = (opts.method == IdentifierMethodDefaults.X5C) || IdentifierTypeUtils.isX5cIdentifier(opts.identifier)

    fun isExternalIdentifierCnfOpts(opts: ExternalIdentifierOpts): Boolean = opts.method == IdentifierMethodDefaults.CNF && opts is ExternalIdentifierCnfOpts
    ...
    fun tryAsExternalIdentifierKidOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierKidOpts, IdkError> =
        if (isExternalIdentifierKidOpts(opts)) {
            Ok(opts as ExternalIdentifierKidOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierKidOpts"))
        }
    ...
    fun tryAsExternalIdentifierDidOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierDidOpts, IdkError> =
        if (isExternalIdentifierDidOpts(opts)) {
            Ok(opts as ExternalIdentifierDidOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierDidOpts"))
        }
    ...
    fun tryAsExternalIdentifierJwkOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierJwkOpts, IdkError> =
        if (isExternalIdentifierJwkOpts(opts)) {
            Ok(opts as ExternalIdentifierJwkOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierJwkOpts"))
        }
    ...
    fun tryAsExternalIdentifierCoseKeyOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierCoseKeyOpts, IdkError> =
        if (isExternalIdentifierCoseKeyOpts(opts)) {
            Ok(opts as ExternalIdentifierCoseKeyOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierCoseKeyOpts"))
        }
    ...
    fun tryAsExternalIdentifierOidcDiscoveryOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierOidcDiscoveryOpts, IdkError> =
        if (isExternalIdentifierOidcDiscoveryOpts(opts)) {
            Ok(opts as ExternalIdentifierOidcDiscoveryOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierOidcDiscoveryOpts"))
        }
    ...
    fun tryAsExternalIdentifierJwksUrlOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierJwksUrlOpts, IdkError> =
        if (isExternalIdentifierJwksUrlOpts(opts)) {
            Ok(opts as ExternalIdentifierJwksUrlOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierJwksUrlOpts"))
        }
    ...
    fun tryAsExternalIdentifierOIDFEntityIdOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierOIDFEntityIdOpts, IdkError> =
        if (isExternalIdentifierOIDFEntityIdOpts(opts)) {
            Ok(opts as ExternalIdentifierOIDFEntityIdOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierOIDFEntityIdOpts"))
        }
    ...
    fun tryAsExternalIdentifierX5cOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierX5cOpts, IdkError> =
        if (isExternalIdentifierX5cOpts(opts)) {
            Ok(opts as ExternalIdentifierX5cOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierX5cOpts"))
        }
    ...
    fun tryAsExternalIdentifierCnfOpts(opts: ExternalIdentifierOpts): IdkResult<ExternalIdentifierCnfOpts, IdkError> =
        if (isExternalIdentifierCnfOpts(opts)) {
            Ok(opts as ExternalIdentifierCnfOpts)
        } else {
            Err(IdkError.ILLEGAL_ARGUMENT_ERROR(message = "opts is not an instance of ExternalIdentifierCnfOpts"))
        }
    ...
    fun asExternalIdentifierKidOpts(opts: ExternalIdentifierOpts): ExternalIdentifierKidOpts =
        tryAsExternalIdentifierKidOpts(opts).getOrElse {
    ...
    fun asExternalIdentifierDidOpts(opts: ExternalIdentifierOpts): ExternalIdentifierDidOpts =
        tryAsExternalIdentifierDidOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun asExternalIdentifierJwkOpts(opts: ExternalIdentifierOpts): ExternalIdentifierJwkOpts =
        tryAsExternalIdentifierJwkOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun asExternalIdentifierCoseKeyOpts(opts: ExternalIdentifierOpts): ExternalIdentifierCoseKeyOpts =
        tryAsExternalIdentifierCoseKeyOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun asExternalIdentifierOidcDiscoveryOpts(opts: ExternalIdentifierOpts): ExternalIdentifierOidcDiscoveryOpts =
        tryAsExternalIdentifierOidcDiscoveryOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun asExternalIdentifierJwksUrlOpts(opts: ExternalIdentifierOpts): ExternalIdentifierJwksUrlOpts =
        tryAsExternalIdentifierJwksUrlOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun asExternalIdentifierOIDFEntityIdOpts(opts: ExternalIdentifierOpts): ExternalIdentifierOIDFEntityIdOpts =
        tryAsExternalIdentifierOIDFEntityIdOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun asExternalIdentifierX5cOpts(opts: ExternalIdentifierOpts): ExternalIdentifierX5cOpts =
        tryAsExternalIdentifierX5cOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
    ...
    fun asExternalIdentifierCnfOpts(opts: ExternalIdentifierOpts): ExternalIdentifierCnfOpts =
        tryAsExternalIdentifierCnfOpts(opts).getOrElse {
            throw IllegalArgumentException(it.message.defaultMessage)
        }
}
...
object ExternalIdentifierResultGuards {
    fun isExternalIdentifierDidResult(result: ExternalIdentifierResult): Boolean = result.method == IdentifierMethodDefaults.DID && result is ExternalIdentifierResult.Did

    fun isExternalIdentifierJwkResult(result: ExternalIdentifierResult): Boolean = result.method == IdentifierMethodDefaults.JWK && result is ExternalIdentifierResult.Jwk

    fun isExternalIdentifierCoseKeyResult(result: ExternalIdentifierResult): Boolean = result.method == IdentifierMethodDefaults.COSE_KEY && result is ExternalIdentifierResult.CoseKey

    fun isExternalIdentifierX5cResult(result: ExternalIdentifierResult): Boolean = result.method == IdentifierMethodDefaults.X5C && result is ExternalIdentifierResult.X5c

    fun isExternalIdentifierOidfEntityIdResult(result: ExternalIdentifierResult): Boolean = result.method == IdentifierMethodDefaults.ENTITY_ID && result is ExternalIdentifierResult.OIDFEntityId
    ...
    fun isExternalIdentifierJwksResult(result: ExternalIdentifierResult): Boolean = result.method == IdentifierMethodDefaults.JWKS_URL && result is ExternalIdentifierResult.JwksUrl

    fun isExternalIdentifierCnfResult(result: ExternalIdentifierResult): Boolean = result.method == IdentifierMethodDefaults.CNF && result is ExternalIdentifierResult.Cnf

    fun asExternalIdentifierDidResult(result: ExternalIdentifierResult): ExternalIdentifierResult.Did =
        require(isExternalIdentifierDidResult(result)) {
            "result is not an instance of ExternalIdentifierResult.Did"
        }.let { result as ExternalIdentifierResult.Did }

    fun asExternalIdentifierJwkResult(result: ExternalIdentifierResult): ExternalIdentifierResult.Jwk =
        require(isExternalIdentifierJwkResult(result)) {
            "result is not an instance of ExternalIdentifierResult.Jwk"
        }.let { result as ExternalIdentifierResult.Jwk }

    fun asExternalIdentifierCoseKeyResult(result: ExternalIdentifierResult): ExternalIdentifierResult.CoseKey =
        require(isExternalIdentifierCoseKeyResult(result)) {
            "result is not an instance of ExternalIdentifierResult.CoseKey"
        }.let { result as ExternalIdentifierResult.CoseKey }

    fun asExternalIdentifierX5cResult(result: ExternalIdentifierResult): ExternalIdentifierResult.X5c =
        require(isExternalIdentifierX5cResult(result)) {
            "result is not an instance of ExternalIdentifierResult.X5c"
        }.let { result as ExternalIdentifierResult.X5c }

    fun asExternalIdentifierOidfEntityIdResult(result: ExternalIdentifierResult): ExternalIdentifierResult.OIDFEntityId =
        require(isExternalIdentifierOidfEntityIdResult(result)) {
            "result is not an instance of ExternalIdentifierResult.OIDFEntityId"
        }.let { result as ExternalIdentifierResult.OIDFEntityId }
    ...
    fun asExternalIdentifierJwksUrlResult(result: ExternalIdentifierResult): ExternalIdentifierResult.JwksUrl =
        require(isExternalIdentifierJwksResult(result)) {
            "result is not an instance of ExternalIdentifierResult.JwksUrl"
        }.let { result as ExternalIdentifierResult.JwksUrl }

    fun asExternalIdentifierCnfResult(result: ExternalIdentifierResult): ExternalIdentifierResult.Cnf =
        require(isExternalIdentifierCnfResult(result)) {
            "result is not an instance of ExternalIdentifierResult.Cnf"
        }.let { result as ExternalIdentifierResult.Cnf }
}
