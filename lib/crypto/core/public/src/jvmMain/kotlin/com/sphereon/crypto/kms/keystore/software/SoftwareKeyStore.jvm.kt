...
actual class SoftwareKeyStoreService actual constructor(
    ...
    private val keyStoreData: Deferred<LoadedKeyStoreData> by lazy {
        backgroundScope.async {
            val loadedKeyStore = withContext(Dispatchers.IO) {
            ...
            val resolvedSource = when (val src = source) {
    ...
    actual override suspend fun listKeys(): Array<ManagedKeyInfoType<*>> {
        ...
        return withContext(Dispatchers.Default) {
            ...
            keyAliases.chunked(chunkSize).flatMap { aliasChunk ->
                aliasChunk.map { alias ->
                    async {
                        try {
...
//        require(managedKeyInfo.key != null || managedKeyInfo.kid != null || managedKeyInfo.alias != null) { "Either a kid needs to be provided or a key needs to be passed in" }
        ...
        val cachedResolvedKeyInfo = if (isCacheValid) resolvedKeyCache[alias] else null
        val resolvedKeyInfo = if (cachedResolvedKeyInfo != null) {
        ...
        return if (visibility === KeyVisibility.PUBLIC) ManagedKeyInfo(
            alias = alias, providerId = managedKeyInfoResult.providerId, resolvedKeyInfo = managedKeyInfoResult.toResolvedPublicKeyInfo()
        ) else managedKeyInfoResult
    }
    ...
    actual override suspend fun storeKey(
        ...
        val (privateKey, certificates) = withContext(Dispatchers.Default) {
            val privateKey = convertToJavaPrivateKey(keyInfo)

            val certChain: MutableList<X509Certificate> = when {
     ...
     */
    actual override suspend fun storeCertificateChain(alias: String, certificates: Array<Certificate>, keyInfo: ResolvedKeyInfoType<*>?) {
        ...
        val (certChain, resolvedKeyInfo) = withContext(Dispatchers.Default) {
    ...
    actual override suspend fun storeTrustedCertificate(alias: String, certificate: Certificate) {
        ...
        val javaCert = withContext(Dispatchers.Default) {
    ...
    private suspend fun matchKey(keyInfo: KeyInfoType<*>): KeyInfoType<*> {
        ...
        if (managedKeyInfo.alias == null) {
            // First try exact alias match (most reliable)
            var matchingKey = if (keyInfo.kid != null) {
                listKeys().find { it.alias == keyInfo.kid }
            } else {
                null
            }

            // If no alias match, try kid match
            if (matchingKey == null) {
                matchingKey = listKeys().find {
                    if (keyInfo.kid != null) {
                        it.kid == keyInfo.kid
                    } else {
    ...
    private suspend fun persistenceLoop() {
        persistenceMutex.withLock {
            ...
            if (isPersistenceRunning) return@withLock
            ...
            if (resolvedSource is KeyStoreLoaderOpts.Source.File) {
                ...
                java.io.FileOutputStream(tmpFile).use { fos ->
                    ...
                    try {
                        ...
                        // Some devices/filesystems may not support force; ignore
                    }
                }
                ...
                if (!moved && tmpFile.exists()) {
                    ...
                    try {
                        ...
                        // Last resort: copy/replace (only if tmp still exists)
                        if (tmpFile.exists()) {
                            java.nio.file.Files.copy(tmpFile.toPath(), targetFile.toPath(), java.nio.file.StandardCopyOption.REPLACE_EXISTING)
                            tmpFile.delete()
                        }
                    }
                }
    ...
    private fun resolvedKeyInfoFromSecretEntry(alias: String, entry: KeyStore.SecretKeyEntry): ResolvedKeyInfoType<*> {
    ...
    private fun resolvedKeyInfoFrom(alias: String, entry: KeyStore.PrivateKeyEntry): ResolvedKeyInfoType<*> {
        val jwk = derPrivateKeyToJwk(entry.privateKey.encoded)
        ...
        return ResolvedKeyInfo(
            key = jwk,
            // TODO: Persist kid separately (e.g., in certificate extension) for proper kid support
//            kid = alias,
            alias = alias,
            providerId = config.id,
            keyVisibility = KeyVisibility.fromValue(config.keyVisibility),
     ...
     */
    private fun getCachedOrResolveKeyInfo(alias: String, entry: KeyStore.PrivateKeyEntry): ResolvedKeyInfoType<*> {
