...
abstract class BaseKeyStoreLoader(private val type: String) : KeyStoreLoader {
    override suspend fun load(opts: KeyStoreLoaderOpts): KeyStore {
        val ks = KeyStore.getInstance(type)

        val effectiveSource: KeyStoreLoaderOpts.Source = when (val src = opts.source) {
    ...
    private fun resolveFileSource(src: KeyStoreLoaderOpts.Source.File): Pair<KeyStoreLoaderOpts.Source.File, File> {
        ...
        val resolvedSrc =
            if (resolvedFile.absolutePath != src.path) src.copy(path = resolvedFile.absolutePath) else src
