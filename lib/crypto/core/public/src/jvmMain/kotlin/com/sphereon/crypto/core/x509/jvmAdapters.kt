...
fun convertToJavaPrivateKey(keyInfo: Any): PrivateKey {
    ...
    val algorithm = when (key.getKeyType()) {
        ...
        KeyTypeMapping.Symmetric -> throw IllegalArgumentException(
...
private fun extractPrivateKeyBytes(key: KeyType): ByteArray {
    return when (key.getKeyType()) {
        ...
        KeyTypeMapping.OKP -> {
            ...
            constructOKPPrivateKeyPKCS8(Base64.getDecoder().decode(privateBytes), key.crv.toString())
        }

        else -> throw IllegalArgumentException("Unsupported key type: ${key.getKeyType()}")
    }
}

private fun constructECPrivateKeyPKCS8(privateBytes: ByteArray, curve: String): ByteArray {
    // 1. Turn raw 'd' into a positive BigInteger
    val s = BigInteger(1, privateBytes)

    val jdkCurve = when (curve) {
        "P-256" -> "secp256r1"
        "P-384" -> "secp384r1"
        "P-521" -> "secp521r1"
        else -> curve
...
private fun constructRSAPrivateKeyPKCS8(key: KeyType): ByteArray {
    ...
    val n = BigInteger(
        1, decoder.decode(
            (jwk.n)
                ?: error("Missing modulus 'n'")
        )
    )
    val e = BigInteger(
        1, decoder.decode(
            (jwk.e)
                ?: error("Missing public exponent 'e'")
        )
    )
    val d = BigInteger(
        1, decoder.decode(
            (jwk.d)
                ?: error("Missing private exponent 'd'")
        )
    )
    val p = BigInteger(
        1, decoder.decode(
            (jwk.p)
                ?: error("Missing prime1 'p'")
        )
    )
    val q = BigInteger(
        1, decoder.decode(
            (jwk.q)
                ?: error("Missing prime2 'q'")
        )
    )
    val dp = BigInteger(
        1, decoder.decode(
            (jwk.dP)
                ?: error("Missing exponent1 'dp'")
        )
    )
    val dq = BigInteger(
        1, decoder.decode(
            (jwk.dQ)
                ?: error("Missing exponent2 'dq'")
        )
    )
    val qi = BigInteger(
        1, decoder.decode(
            (jwk.qInv)
                ?: error("Missing crtCoefficient 'qi'")
        )
    )
...
private fun constructOKPPrivateKeyPKCS8(privateBytes: ByteArray, curve: String): ByteArray {
    // Uses NamedParameterSpec + XECPrivateKeySpec (requires Java 11+)
    val keyFactory = when (curve) {
