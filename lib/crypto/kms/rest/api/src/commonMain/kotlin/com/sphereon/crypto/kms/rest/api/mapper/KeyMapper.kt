...
fun Array<KeyInfoRest>.toRestResponse(): ListKeysResponseRest =
    ListKeysResponseRest(
        keyInfos = this,
    )

/** Convert a REST list-keys response to an array of SDK [ManagedKeyReference] (metadata only). */
fun ListKeysResponseRest.toSdkReferences(): Array<ManagedKeyReference> =
    keyInfos
        .map { keyInfo ->
            ManagedKeyReference(
                alias = keyInfo.alias ?: "",
                ...
                keyType =
                    keyInfo.keyType
                        ?.let { KeyTypeMapping.fromValue(it.value) },
                keyVisibility =
                    keyInfo.keyVisibility
                        ?.let { runCatching { KeyVisibility.valueOf(it.value.uppercase()) }.getOrNull() },
                keyEncoding =
                    keyInfo.keyEncoding
