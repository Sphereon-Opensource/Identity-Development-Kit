...
class KmsKtorUniversalAdapterTest {
    ...
    fun setUp() {
        ...
        server = embeddedServer(CIO, port = port) {
            install(KotlinInjectPlugin) {
                this.appComponent = this@KmsKtorUniversalAdapterTest.appComponent
            }
        ...
        restClientKmsProvider = RestClientKmsProviderImpl(
            config,
            execution = sessionInstance.asCoreApiServiceComponent().serviceExecution
        )
    }
    ...
    @Test
    fun `REST provider can generate key through HTTP adapter`() = runTest {
        val keyAlias = "ktor-rest-key-${System.currentTimeMillis()}"

        val managedKeyPair = restClientKmsProvider.generateKeyAsync(
            alias = keyAlias,
    ...
    @Test
    fun `REST provider can list keys through HTTP adapter`() = runTest {
    ...
    @Test
    fun `REST provider can sign and verify through HTTP adapter`() = runTest {
        // Generate a key
        ...
        val signature = restClientKmsProvider.createRawSignature(
            keyInfo = keyInfo,
            input = data,
        ...
        // Verify through REST
        val isValid = restClientKmsProvider.isValidRawSignature(
            keyInfo = keyInfo,
            signature = signature,
            input = data
        )
        ...
        val isInvalid = restClientKmsProvider.isValidRawSignature(
            keyInfo = keyInfo,
            signature = signature,
    ...
    @Test
    fun `REST provider full key lifecycle through HTTP adapter`() = runTest {
    ...
    @Test
    fun `multiple algorithms work through HTTP adapter`() = runTest {
