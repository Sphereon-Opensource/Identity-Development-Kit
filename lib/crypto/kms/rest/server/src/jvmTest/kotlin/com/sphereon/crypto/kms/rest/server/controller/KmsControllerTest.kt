...
class KmsControllerTest {
    ...
    fun setUp() {
        ...
        server = embeddedServer(CIO, port = port) {
            install(KotlinInjectPlugin) {
                this.appComponent = this@KmsControllerTest.appComponent
            }
    ...
    fun getKey() = runTest {
        ...
        val storeResponse = client.post("http://localhost:$port/keys") {
            header("X-Tenant-ID", TEST_TENANT_ID)
            header("X-User-ID", TEST_USER_ID)
            contentType(ContentType.Application.Json)
        ...
        // Then get it
        val getResponse = client.get("http://localhost:$port/keys/$storedKid") {
            header("X-Tenant-ID", TEST_TENANT_ID)
            header("X-User-ID", TEST_USER_ID)
            accept(ContentType.Application.Json)
        }
    ...
    fun deleteKey() = runTest {
        ...
        // Then delete it
        val deleteResponse = client.delete("http://localhost:$port/keys/$storedKid") {
            header("X-Tenant-ID", TEST_TENANT_ID)
            header("X-User-ID", TEST_USER_ID)
        }
