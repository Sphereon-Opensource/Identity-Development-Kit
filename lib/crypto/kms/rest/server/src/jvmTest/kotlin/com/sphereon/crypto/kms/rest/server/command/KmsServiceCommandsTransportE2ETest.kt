...
class KmsServiceCommandsTransportE2ETest {
    ...
    fun setUp() {
        ...
        server = embeddedServer(CIO, port = port) {
            install(KotlinInjectPlugin) {
                this.appComponent = this@KmsServiceCommandsTransportE2ETest.appComponent
            }
        ...
        val config = RestClientKmsProviderConfig(
            ...
            authConfig = RestClientAuthConfig(
        ...
        restClient = RestClientKmsProviderImpl(
            config,
    ...
    @Test
    fun generateKeyViaRestShouldCreateKeyAndReturnResponse() = runTest {
        val alias = "transport-generate-test-${System.currentTimeMillis()}"

        val keyPair = restClient.generateKeyAsync(
            alias = alias,
        ...
        assertNotNull(keyPair.jose.publicJwk.x, "Public key should have x component")
    }

    // ==================== List Keys Transport Test ====================

    @Test
    fun listKeysViaRestShouldReturnAllKeys() = runTest {
    ...
    @Test
    fun getKeyViaRestShouldReturnSpecificKey() = runTest {
    ...
    @Test
    fun deleteKeyViaRestShouldRemoveKey() = runTest {
    ...
    @Test
    fun fullCrudWorkflowViaRestTransport() = runTest {
        val alias = "transport-crud-test-${System.currentTimeMillis()}"

        // CREATE
        val generatedKey = restClient.generateKeyAsync(
            alias = alias,
    ...
    fun serviceCommandMetadataShouldMatchRestEndpoints() {
        ...
        val getKey = object : GetKeyServiceCommand {
        ...
        val listKeys = object : ListKeysServiceCommand {
            override val isEnabled = true
            override val inputTypeToken get() = throw UnsupportedOperationException()
            override val outputTypeToken get() = throw UnsupportedOperationException()
        ...
        val generateKey = object : GenerateKeyServiceCommand {
            override val isEnabled = true
            override val inputTypeToken get() = throw UnsupportedOperationException()
            override val outputTypeToken get() = throw UnsupportedOperationException()
        ...
        val deleteKey = object : DeleteKeyServiceCommand {
            override val isEnabled = true
            override val inputTypeToken get() = throw UnsupportedOperationException()
            override val outputTypeToken get() = throw UnsupportedOperationException()
    ...
    @Test
    fun signatureCreationAndVerificationViaRestTransport() = runTest {
        ...
        val signature = restClient.createRawSignature(
            keyInfo = keyInfo,
            input = testData,
        ...
        val isValid = restClient.isValidRawSignature(
            keyInfo = keyInfo,
            signature = signature,
            input = testData
        )
        ...
        val isInvalidForWrongData = restClient.isValidRawSignature(
            keyInfo = keyInfo,
            signature = signature,
