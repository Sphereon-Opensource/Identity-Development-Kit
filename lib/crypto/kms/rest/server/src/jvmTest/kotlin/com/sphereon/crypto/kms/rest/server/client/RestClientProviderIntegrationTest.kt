...
class RestClientProviderIntegrationTest {
    ...
    fun setUp() {
        ...
        server = embeddedServer(CIO, port = port) {
            install(KotlinInjectPlugin) {
                this.appComponent = this@RestClientProviderIntegrationTest.appComponent
            }
        ...
        val sessionComponent = userContextInstance.sessionContextManager.createOrGetFromId("test-session")
        restClientKmsProvider = RestClientKmsProviderImpl(
            config,
            execution = sessionComponent.asCoreApiServiceComponent().serviceExecution
        )
    }
    ...
    @Test
    fun shouldIsolateKeysBetweenTenantKeystores() = runTest {
        ...
        val tenantBConfig = RestClientKmsProviderConfig(
            id = "testsoftware",
            restKmsUrl = "http://localhost:$port",
            authConfig = RestClientAuthConfig(
                usePrincipalFromContext = true,
        ...
        val tenantBContext = appComponent.userContextManager.createOrGetFromInputs(
        ...
        val tenantBClient = RestClientKmsProviderImpl(
            tenantBConfig,
            execution = tenantBSession.asCoreApiServiceComponent().serviceExecution
        )

        try {
            // When - generate keys for both tenants
            val tenantAKey = tenantAClient.generateKeyAsync(
                alias = "tenant-a-isolation-key",
                alg = SignatureAlgorithm.ECDSA_SHA256
            )
            assertNotNull(tenantAKey, "Tenant A key should be generated")

            val tenantBKey = tenantBClient.generateKeyAsync(
    ...
    @Test
    fun shouldMaintainKeyIsolationWithSameAliasAcrossTenants() = runTest {
