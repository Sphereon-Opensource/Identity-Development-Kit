 ...
 * the RegistrableServiceCommandDescriptor multibinding and resolvable via SessionScopedCommandRegistry.
 *
 ...
 *   -> SessionScopedCommandRegistry.get(commandId) -> descriptor.create() -> execute()
 *
 * The registry discovers commands through Set<RegistrableServiceCommandDescriptor> multibinding.
 * This test proves that real DI-injected KMS commands are in that set and executable
 * through the commandId-based lookup + execute path.
 */
class KmsCommandRegistryIntegrationTest {

    /**
     * Component interface to access the command registry from the session component.
     * KSP merges this into the test session component.
     *
     * Note: only exposes the registry, NOT Set<RegistrableServiceCommandDescriptor>
     * (exposing the set causes a KSP codegen ordering bug with @IntoSet multibindings).
     */
    ...
    companion object {
        ...
        private const val PROPERTY_PREFIX = "kms.registry.app.profile.kms.providers"
    }
    ...
    fun setUp() {
        ...
        // Access the command registry from the session component via the @ContributesTo interface
        registry = (session.component as TestRegistryComponent).sessionScopedCommandRegistry

        // Build command map from registry (which lazily creates commands from descriptors)
        val kmsCommandIds = listOf(
    ...
    @Test
    fun generateKeyViaExecute() = runTest {
        ...
        // Then
        assertTrue(result.isOk, "GenerateKey via execute should succeed: ${if (result.isErr) result.error else ""}")
    }

    @Test
    fun getKeyViaExecute() = runTest {
        // Given - first generate a key
        val alias = "registry-get-${System.currentTimeMillis()}"
        val generateCommand = commandMap[GenerateKeyServiceCommand.COMMAND_ID]!!
        ...
        val getCommand = commandMap[GetKeyServiceCommand.COMMAND_ID]!!
        val result = getCommand.execute(
            GetKeyInput(aliasOrKid = alias, providerId = TEST_PROVIDER_ID)
        )

        // Then
        assertTrue(result.isOk, "GetKey via execute should succeed: ${if (result.isErr) result.error else ""}")
    }

    @Test
    fun listKeysViaExecute() = runTest {
        ...
        val listCommand = commandMap[ListKeysServiceCommand.COMMAND_ID]!!
        val result = listCommand.execute(
            ListKeysInput(providerId = TEST_PROVIDER_ID)
        )

        // Then
        assertTrue(result.isOk, "ListKeys via execute should succeed: ${if (result.isErr) result.error else ""}")
    }

    @Test
    fun deleteKeyViaExecute() = runTest {
        // Given - generate a key first
        ...
        val deleteCommand = commandMap[DeleteKeyServiceCommand.COMMAND_ID]!!
        val result = deleteCommand.execute(
            DeleteKeyInput(aliasOrKid = alias, providerId = TEST_PROVIDER_ID)
        )

        // Then
        assertTrue(result.isOk, "DeleteKey via execute should succeed: ${if (result.isErr) result.error else ""}")
    }

    @Test
    fun getNonExistentKeyViaExecuteReturnsError() = runTest {
        // Given
        val getCommand = commandMap[GetKeyServiceCommand.COMMAND_ID]!!
