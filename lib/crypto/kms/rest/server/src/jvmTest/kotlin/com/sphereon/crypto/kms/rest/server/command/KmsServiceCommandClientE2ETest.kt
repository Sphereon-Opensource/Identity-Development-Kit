...
class KmsServiceCommandClientE2ETest {
    ...
    fun setUp() {
        ...
        server = embeddedServer(CIO, port = port) {
            install(KotlinInjectPlugin) {
                this.appComponent = this@KmsServiceCommandClientE2ETest.appComponent
            }
        ...
        transport = HttpServiceCommandTransport(
    ...
    @Test
    fun clientGenerateKeyServiceCommandSameAsServer() = runTest {
    ...
    @Test
    fun clientListKeysServiceCommandSameAsServer() = runTest {
        val generateInput = GenerateKeyInput(
            generateKey = GenerateKeyGlobal(
    ...
    @Test
    fun clientGetKeyServiceCommandSameAsServer() = runTest {
        val alias = "client-get-test-${System.currentTimeMillis()}"
        generateKeyCommand.execute(
            GenerateKeyInput(generateKey = GenerateKeyGlobal(alias = alias)))
    ...
    @Test
    fun clientDeleteKeyServiceCommandSameAsServer() = runTest {
    ...
    @Test
    fun fullCrudWorkflowUsingClientServiceCommands() = runTest {
        val alias = "client-crud-test-${System.currentTimeMillis()}"

        // CREATE
        val createResult = generateKeyCommand.execute(
            GenerateKeyInput(
                generateKey = GenerateKeyGlobal(
                    alias = alias,
                    use = JwkUse.sig,
    ...
    @Test
    fun clientCommandReturnsErrorForNonExistentKey() = runTest {
        val result = getKeyCommand.execute(
            GetKeyInput(aliasOrKid = "non-existent-${System.currentTimeMillis()}"))
        assertTrue(result.isErr, "Should return error for non-existent key")
    }

    @Test
    fun clientCommandReturnsErrorForDeleteNonExistentKey() = runTest {
        val result = deleteKeyCommand.execute(
            DeleteKeyInput(aliasOrKid = "non-existent-${System.currentTimeMillis()}"))
