...
class KmsKeysServiceFacadeImpl(
    ...
    override suspend fun getKey(
        ...
        return getKeyCommand.execute(GetKeyInput(aliasOrKid = aliasOrKid, providerId = providerId))
    }

    override suspend fun listKeys(
        providerId: String?
    ): IdkResult<ListKeysResponse, IdkError> {
        return listKeysCommand.execute(ListKeysInput(providerId = providerId))
    }

    override suspend fun storeKey(
        storeKey: StoreKey
    ): IdkResult<StoreKeyResponse, IdkError> {
        return storeKeyCommand.execute(StoreKeyInput(storeKey = storeKey))
    }

    override suspend fun generateKey(
        generateKey: GenerateKeyGlobal
    ): IdkResult<GenerateKeyResponse, IdkError> {
        return generateKeyCommand.execute(GenerateKeyInput(generateKey = generateKey))
    }
    ...
    override suspend fun deleteKey(
        ...
        return deleteKeyCommand.execute(DeleteKeyInput(aliasOrKid = aliasOrKid, providerId = providerId))
    }

    /**
     * This @ContributesTo interface makes the facade accessible as a property
     * in the kotlin-inject SessionComponent.
     */
