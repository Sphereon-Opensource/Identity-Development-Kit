...
class RestProviderConfigTest {
    ...
    val app = createRestProviderTestAppGraph(application = this)
    ...
    @Test
    fun `test rest kms config to json`() =
        runTest {
            val config =
    ...
    @Test
    fun `test json to rest kms config`() =
        runTest {
    ...
    fun `test provider using config object`() =
        runTest {
            app as RestProviderTestAppGraph

            val restKmsProvider =
                app.restClientKmsProvider.create(
                    RestClientKmsProviderConfig(
                        id = "test-rest",
                        restKmsUrl = "http://localhost:8080/kms/v1",
                    ),
    ...
    fun `test config binder using config object`() =
        runTest {
            ...
            val restKmsProvider = app.kmsProviderManager.createFromProviderConfig(config, session.asCoreApiServiceGraph().serviceExecution)
            assertNotNull(restKmsProvider)
            assertEquals("test-rest", restKmsProvider.id)
        }
    ...
    fun `test config binder getting all kms-es`() =
        runTest {
            ...
            val restKmsProvider1 = app.kmsProviderManager.createFromProviderConfig(configs.first(), session.asCoreApiServiceGraph().serviceExecution)
            assertNotNull(restKmsProvider1)
            val restKmsProvider2 = app.kmsProviderManager.createFromProviderConfig(configs.last(), session.asCoreApiServiceGraph().serviceExecution)
    ...
    @Test
    fun `test multiple kms providers with an actual KMS`() =
        runTest {
            app as RestProviderTestAppGraph
            ...
            val context = app.userContextManager.getAnonymous()
            val session = context.sessionContextManager.createOrGetFromId("test-multiple-kms").graph
            val kms = session.asKeyManagerServiceGraph().keyManagerService
