...
class MobileCryptoProviderTest {
    ...
    @BeforeTest
    fun setUp() {
        val app = staticMinimalTestAppComponent(application = this, appId = "test-app", profile = "test-profile", version = "test-version")
        val user = app.userContextManager.getAnonymous()
        val session = user.sessionContextManager.createOrGetFromId("test-session")


        mobileCryptoProvider = MobileKmsProviderImpl(MobileKmsProviderConfig(id = "test-mobile"), execution = session.asCoreApiServiceComponent().serviceExecution)
    }
    ...
    fun testSupportedAlg() {
        ...
        assertContentEquals(
            arrayOf( SignatureAlgorithm.ECDSA_SHA256,
