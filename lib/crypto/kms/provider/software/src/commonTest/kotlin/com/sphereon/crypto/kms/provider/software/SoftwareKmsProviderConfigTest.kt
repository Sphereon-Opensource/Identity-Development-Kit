...
class SoftwareKmsProviderConfigTest {
    ...
    private val decodeConfig = """{"id":"test-software","enabled":false,"exposePrivateKeysDuringGeneration":true,"order":10,"cryptographyProvider":"JDK","type":"software","autoCreateCertificate":true,"keyStore":{"type":"pkcs12","id":"test-software","enabled":false,"order":10,"password":"password","path":"test/path","accessMode":"read_write","keyVisibility":"private","persist":true,"overwriteAlias":true}}"""
    private val encodeConfig = """{"id":"test-software","enabled":false,"exposePrivateKeysDuringGeneration":true,"order":10,"cryptographyProvider":"$defaultProviderName","autoCreateCertificate":true,"keyStore":{"type":"pkcs12","id":"test-software","enabled":false,"order":10,"password":"password","path":"test/path","accessMode":"read_write","keyVisibility":"private","persist":true,"overwriteAlias":true}}"""
    ...
    @Test
    fun testSoftwareKmsConfigToJson() = runTest {
    ...
    @Test
    fun testJsonToSoftwareKmsConfig() = runTest {
    ...
    @Test
    fun testProviderUsingConfigObject() = runTest {
        val softwareKmsProvider = ctx.softwareKmsProviderFactory.create(
    ...
    @Test
    fun testKmsProviderManagerUsingConfigObject() = runTest {
        val softwareKmsProvider = ctx.kmsProviderManager.createFromProviderConfig(
            SoftwareKmsProviderConfig(
                id = "test-software",
                cryptographyProvider = CryptographyProvider.Default.name
            ),
    ...
    @Test
    fun testConfigBinderUsingConfigObject() = runTest {
        val binder = KmsProviderConfigBinderImpl(ctx.appLogManager)
        clearConfigCache()
        DefaultAppMapPropertySource.addProperties(
            mapOf(
                "software.kms.app.profile.kms.providers.test-software.type" to "software",
                "software.kms.app.profile.kms.providers.test-software.id" to "test-software"
            )
        )
    ...
    @Test
    fun testConfigBinderGettingAllKmses() = runTest {
        val binder = KmsProviderConfigBinderImpl(ctx.appLogManager)
        clearConfigCache()
        DefaultAppMapPropertySource.addProperties(
            mapOf(
                "software.kms.app.profile.kms.providers.test-software.type" to "software",
                "software.kms.app.profile.kms.providers.test-software.id" to "test-software",
                "software.kms.app.profile.kms.providers.test-software2.type" to "software",
                "software.kms.app.profile.kms.providers.test-software2.id" to "test2"
            )
        )
    ...
    @Test
    fun testMultipleKmsesFromProviderManagerAndProperties() = runTest {
        clearConfigCache()
        DefaultAppMapPropertySource.addProperties(
            mapOf(
                "software.kms.app.profile.kms.providers.test-software.type" to "software",
    ...
    @Test
    fun testMultipleKmsProvidersWithActualKms() = runTest {
        clearConfigCache()
        DefaultPrincipalMapPropertySource.addProperties(
            mapOf(
                "software.kms.app.profile.kms.providers.test-software.type" to "software",
                "software.kms.app.profile.kms.providers.test-software.id" to "test-software",
                "software.kms.app.profile.kms.providers.test-software.keystore.type" to "memory",
                "software.kms.app.profile.kms.providers.test-software.keystore.id" to "test-memory-keystore1",
                "software.kms.app.profile.kms.providers.test-software.keystore.keyVisibility" to "private",
                "software.kms.app.profile.kms.providers.test-software.keystore.overwriteAlias" to "true",

                "software.kms.app.profile.kms.providers.test-software2.type" to "software",
                "software.kms.app.profile.kms.providers.test-software2.id" to "test-software2",
                "software.kms.app.profile.kms.providers.test-software2.keystore.type" to "memory",
                "software.kms.app.profile.kms.providers.test-software2.keystore.id" to "test-memory-keystore2",
                "software.kms.app.profile.kms.providers.test-software2.keystore.keyVisibility" to "private",
                "software.kms.app.profile.kms.providers.test-software2.keystore.overwriteAlias" to "false",
            )
        )

        ctx.app.userContextManager.destroyAll()
        val contextInstance = ctx.app.userContextManager.getAnonymous()
        val sessionComponent = contextInstance.sessionContextManager.createOrGetFromId("test-2kms").component
    ...
    @Test
    fun testEcdhKeyAgreementBetweenTwoParties() = runTest {
        clearConfigCache()
        DefaultPrincipalMapPropertySource.addProperties(
            mapOf(
                "software.kms.app.profile.kms.providers.test-ecdh.type" to "software",
                "software.kms.app.profile.kms.providers.test-ecdh.id" to "test-ecdh",
                "software.kms.app.profile.kms.providers.test-ecdh.keystore.type" to "memory",
                "software.kms.app.profile.kms.providers.test-ecdh.keystore.id" to "test-ecdh-keystore",
                "software.kms.app.profile.kms.providers.test-ecdh.keystore.keyVisibility" to "private"
            )
        )

        ctx.app.userContextManager.destroyAll()
        val contextInstance = ctx.app.userContextManager.getAnonymous()
        val sessionComponent = contextInstance.sessionContextManager.createOrGetFromId("test-ecdh").component
        val kms = sessionComponent.asKeyManagerServiceComponent().keyManagerService

        val aliceKeyPair = kms.generateKeyAsync(
        ...
        val bobKeyPair = kms.generateKeyAsync(
            providerId = "test-ecdh",
            alias = "bob-key",
            use = JwkUse.enc,
            keyOperations = arrayOf(KeyOperations.DERIVE_KEY),
