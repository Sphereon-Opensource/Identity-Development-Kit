...
internal actual suspend fun aesKeyWrap(kek: ByteArray, plaintext: ByteArray): ByteArray {
    ...
    val wrappingKey = (subtle.importKey(
        "raw", kekBuffer,
        js("({name:'AES-KW'})"),
        false, js("['wrapKey']")
    ) as Promise<dynamic>).await()

    val keyToWrap = (subtle.importKey(
        "raw", plaintextBuffer,
        js("({name:'HMAC',hash:'SHA-256'})"),
        true, js("['sign']")
    ) as Promise<dynamic>).await()

    val wrappedBuffer = (subtle.wrapKey(
        "raw", keyToWrap, wrappingKey,
        js("({name:'AES-KW'})")
    ) as Promise<dynamic>).await()
...
internal actual suspend fun aesKeyUnwrap(kek: ByteArray, ciphertext: ByteArray): ByteArray {
    val subtle = js("globalThis.crypto.subtle")

    val kekBuffer = kek.toJsInt8Array().buffer
    val ciphertextBuffer = ciphertext.toJsInt8Array().buffer

    val unwrappingKey = (subtle.importKey(
        "raw", kekBuffer,
        js("({name:'AES-KW'})"),
        false, js("['unwrapKey']")
    ) as Promise<dynamic>).await()
    ...
    try {
        val unwrappedKey = (subtle.unwrapKey(
            "raw", ciphertextBuffer, unwrappingKey,
            js("({name:'AES-KW'})"),
            js("({name:'HMAC',hash:'SHA-256'})"),
            true, js("['sign']")
        ) as Promise<dynamic>).await()
        ...
        throw IllegalStateException(
            "AES Key Unwrap integrity check failed - wrong key or corrupted data", e
        )
    }
}
