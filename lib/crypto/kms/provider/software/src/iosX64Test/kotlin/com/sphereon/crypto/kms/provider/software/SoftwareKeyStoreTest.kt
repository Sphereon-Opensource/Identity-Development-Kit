...
class SoftwareKeyStoreTest {
    ...
    private val certBase64 =
        "MIIByTCCAW6gAwIBAgIQF+SpO+EPJj42boA4WHPL2zAKBggqhkjOPQQDAjBAMT4wPAYDVQQDEzVBY2MgS2l3YSBEaWdpdGFsIENlcnRpZmljYXRpb24" +
            "gV2FsbGV0IEludGVybWVkaWF0ZSBDQTAeFw0yNTA2MjUxMzA4MzJaFw0yNjA2MjUxMzA4MzJaMDYxNDAyBgNVBAMTK3dhbGxldC02NDA2MzA3OS02Z" +
            "mM3LTRhZTgtOWEwMi0yNTE4YTRhYmQyZmIwWTATBgcqhkjOPQIBBggqhkjOPQMBBwNCAASnaLC4eYSs/8XkDz8rDANemjmyI+N5g0yIJIiScgCnjZo" +
            "gmlDGGQ8HYCdvs9SJJJ3c17YSax44vVl6LMnidvKno1QwUjAJBgNVHRMEAjAAMB0GA1UdDgQWBBRI7w4/H9JCty7T4aVl+im74XEV0zAOBgNVHQ8BAf" +
            "8EBAMCB4AwFgYDVR0lAQH/BAwwCgYIKwYBBQUHAwIwCgYIKoZIzj0EAwIDSQAwRgIhAPnLEppg5TGMCqp/Nn+2os6vupEclyKv1yk/JQFQME8TAiEAi" +
            "YrVm/6J8zGmhtiG958kZB0afXhM/i3DuY8+0kpTUzA="

    @BeforeTest
    fun beforeEach() {
        app as NativeSoftwareProviderTestAppGraph
    ...
    fun afterEach() {
        ...
        runBlocking {
            ...
            keys.forEach { ref ->
                softwareStoreService.deleteKey(KeyInfo<Jwk>(alias = ref.alias, kid = ref.kid))
            }
    ...
    @Test
    fun `should store a certificate chain`() =
        runTest {
            val keyPair = softwareKmsProvider.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
            val alias = "test-alias"
            val resolvedKeyInfo =
    ...
    @Test
    fun `should delete a certificate chain`() =
        runTest {
            val keyPair = softwareKmsProvider.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
            val alias = "test-alias"
            val resolvedKeyInfo =
            ...
            val certChainDelete = softwareStoreService.getCertificateChain(alias)

            assertNull(certChainDelete)
        }

    @Test
    fun `should list all certificate chains`() =
        runTest {
            val keyPair = softwareKmsProvider.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
            val alias1 = "test-alias1"
            val alias2 = "test-alias2"
            val resolvedKeyInfo1 =
                ResolvedKeyInfo(
                    key = keyPair.jose.privateJwk ?: keyPair.jose.publicJwk,
                    keyVisibility = KeyVisibility.PRIVATE,
                    keyType = KeyTypeMapping.EC,
    ...
    @Test
    fun `should get a certificate chain by alias`() =
        runTest {
            val keyPair = softwareKmsProvider.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
            val alias = "test-alias"
            val resolvedKeyInfo =
    ...
    @Test
    fun `should store a certificate`() =
        runTest {
            val alias = "test-alias"
            val derBytes = certBase64.decodeFrom(Encoding.BASE64)
    ...
    fun `should delete certificate`() =
        runTest {
            ...
            val certDelete = softwareStoreService.getCertificate(alias)

            assertNull(certDelete)
        }

    @Test
    fun `should list all certificates`() =
        runTest {
            val derBytes = certBase64.decodeFrom(Encoding.BASE64)
            val certificate = certificateFromDer(derBytes)
    ...
    @Test
    fun `should get certificate by alias`() =
        runTest {
            val derBytes = certBase64.decodeFrom(Encoding.BASE64)
            val certificate = certificateFromDer(derBytes)
            val alias = "test-alias"
