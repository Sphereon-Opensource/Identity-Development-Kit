...
class AzureKeyVaultProviderTest {
    ...
    private val app = staticMinimalTestAppComponent(application = Any(), appId = "azure-keyvault-test", profile = "test", version = "1.0.0")
    private val session = app.userContextManager.getAnonymous().sessionContextManager.getAnonymous()

    @BeforeTest
    fun setupProvider() {
        fun assertConfigValue(getter: () -> String?): String =
            getter() ?: throw IllegalStateException("Missing required configuration value")
    ...
    private suspend fun getOrCreateTestKeyPair(): ManagedKeyPair {
        if (managedKeyPair == null) {
            managedKeyPair = azureKeyVaultCryptoProvider.generateKeyAsync(
    ...
    fun tearDown() = runTest {
        managedKeyPair?.let {
            try {
                ...
                println("Error deleting key in tearDown (might not have been created or already deleted): ${e.message}")
            }
        }
        managedKeyPair = null
    }
    ...
    fun testGenerateKeyAsyncECDSA_SHA384() = runTest {
        ...
        azureKeyVaultCryptoProvider.deleteKey(
            managedKeyPair.toManagedKeyInfo<Jwk>(
                visibility = KeyVisibility.PUBLIC, keyEncoding = KeyEncoding.JOSE
            )
        )
    }
    ...
    fun testValidRawSignatureAndVerification() = runTest {
        ...
        val signature = azureKeyVaultCryptoProvider.createRawSignature(
            keyInfo = keyInfo, input = "test".encodeToByteArray(), requireX5Chain = false
        )
        assertNotNull(signature)
        val verification = azureKeyVaultCryptoProvider.isValidRawSignature(
    ...
    @Test
    fun testCreateAndVerifySignature() = runTest {
        val keyInfo = getOrCreateTestKeyPair().joseToManagedKeyInfo()
        assertNotNull(keyInfo)
        ...
        val signOutput = azureKeyVaultCryptoProvider.createSignature(
            signInput = signInput,
            keyInfo = keyInfo,
    ...
    fun testListKeys() = runTest(timeout = 60_000.milliseconds) {
...
//    }
    ...
    fun testCertificateOperations() = runTest(timeout = 60_000.milliseconds) {
