...
class DidTrustValidationService(
    ...
    override suspend fun validate(request: TrustValidationRequest): TrustValidationResult {
        ...
        val did = request.context.parameters["did"]
        ...
        val allowedMethods = request.context.parameters["allowedMethods"]
            ?.split(",")?.map { it.trim() }
            ?: didConfig.allowedMethods
        val trustedDids = request.context.parameters["trustedDids"]
            ?.split(",")?.map { it.trim() }
        ...
        if (trustedDids.isNotEmpty()) {
            ...
            return try {
                val result = resolveDidCommand.execute(
                ...
                logger.error("DID resolution failed during trust validation", exception = e)
                TrustValidationResult(
                    trusted = false,
                    status = TrustStatus.VALIDATION_ERROR,
                    details = "DID resolution failed: ${e.message}",
                    validatedAt = Clock.System.now()
                )
            }
        }
