...
class X509TrustValidationService(
    ...
    private val logger = execution.log.logManager.withTagAsync("X509TrustValidationService")

    private val httpClient by lazy { httpClientFactory.createClient(HttpClientOptions()) }

    private val trustedCertsCache by lazy {
        cacheService.getStringCache<List<String>>(
            CacheRequirements(
    ...
    override suspend fun validate(request: TrustValidationRequest): TrustValidationResult {
        ...
        return try {
            ...
            val verificationRequest = if (trustedCerts.isNotEmpty()) {
            ...
            logger.error("X.509 trust validation failed", exception = e)
            TrustValidationResult(
                trusted = false,
                status = TrustStatus.VALIDATION_ERROR,
                details = "X.509 validation error: ${e.message}",
                validatedAt = Clock.System.now()
            )
        }
    }
    ...
    private suspend fun loadTrustedCerts(): List<String> {
        ...
        for (path in x509Config.caBundlePaths) {
            try {
                ...
                failedSources.add(path)
                logger.error("Failed to load CA bundle from path: $path", exception = e)
            }
        }
        ...
        for (url in x509Config.caBundleUrls) {
            try {
                ...
                failedSources.add(url)
                logger.error("Failed to fetch CA bundle from URL: $url", exception = e)
            }
        }

        if (failedSources.isNotEmpty()) {
            if (failedSources.size > x509Config.maxFailedSources) {
        ...
        if (certs.isNotEmpty()) {
            trustedCertsCache.putApp("trusted-certs", certs)
        }
     ...
     */
    private fun checkFingerprints(leafCertPem: String, trustedFingerprints: List<String>): Boolean {
        return try {
