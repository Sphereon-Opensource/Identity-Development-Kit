...
import com.sphereon.trust.etsi.lote.model.ServiceHistoryInstance
...
object LoTEXml {
    ...
    private fun writeListAndSchemeInformation(
        writer: XmlWriter,
        info: ListAndSchemeInformation,
    ) {
    ...
    private fun writePolicyOrLegalNotice(
        writer: XmlWriter,
        entries: List<PolicyOrLegalNoticeEntry>,
    ) {
    ...
    private fun writeOperatorAddress(
        writer: XmlWriter,
        elementName: String,
    ...
    private fun writePostalAddress(
        writer: XmlWriter,
        postal: LoTEPostalAddress,
    ) {
    ...
    private fun writeOtherLoTEPointer(
        writer: XmlWriter,
        pointer: OtherLoTEPointer,
    ) {
    ...
    private fun writeLoTEQualifier(
        writer: XmlWriter,
        qualifier: LoTEQualifier,
    ) {
    ...
    private fun writeTrustedEntity(
        writer: XmlWriter,
    ...
    private fun writeTrustedEntityInformation(
        writer: XmlWriter,
        info: TrustedEntityInformation,
    ) {
    ...
    private fun writeTrustedEntityService(
        writer: XmlWriter,
    ...
    private fun writeServiceInformation(
        writer: XmlWriter,
        info: LoTEServiceInformation,
        elementName: String,
    ) {
        writer.startTag(NAMESPACE_602, elementName, "")
    ...
    private fun writeServiceDigitalIdentity(
        writer: XmlWriter,
        identity: LoTEServiceDigitalIdentity,
    ) {
    ...
    private fun XmlWriter.writeSimpleElement(
        ns: String,
        localName: String,
        value: String,
    ) {
    ...
    private fun XmlWriter.writeMultiLangStrings(
        ns: String,
        containerName: String,
        elementName: String,
        items: List<MultiLangString>,
    ) {
        if (items.isEmpty()) {
            return
        }
    ...
    private fun XmlWriter.writeMultiLangURIs(
        ns: String,
        containerName: String,
        elementName: String,
        items: List<MultiLangURI>,
    ) {
        if (items.isEmpty()) {
            return
        }
    ...
    private fun parseWithReader(reader: XmlReader): LoTE {
        ...
        while (reader.hasNext()) {
            reader.next()
            if (reader.hasExitedElement(depth, elementName)) {
                break
            }
