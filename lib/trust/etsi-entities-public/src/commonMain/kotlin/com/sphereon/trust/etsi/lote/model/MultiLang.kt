 ...
 */
fun List<MultiLangString>.forLang(lang: String): String? =
    firstOrNull { it.lang.equals(lang, ignoreCase = true) }?.value

/**
 * Converts to a map of language code to value.
 */
fun List<MultiLangString>.toLangMap(): Map<String, String> =
    associate { it.lang to it.value }

/**
 * Finds the URI value for the given language code, or null if not found.
 */
@JvmName("forLangURI")
fun List<MultiLangURI>.forLang(lang: String): String? =
    firstOrNull { it.lang.equals(lang, ignoreCase = true) }?.uriValue

/**
 * Converts to a map of language code to URI value.
 */
fun List<MultiLangURI>.toURIMap(): Map<String, String> =
    associate { it.lang to it.uriValue }

/**
 * Creates a MultiLangString list from a map of language code to value.
 */
fun Map<String, String>.toMultiLangStrings(): List<MultiLangString> =
    map { (lang, value) -> MultiLangString(lang, value) }

/**
 * Creates a MultiLangURI list from a map of language code to URI value.
 */
fun Map<String, String>.toMultiLangURIs(): List<MultiLangURI> =
    map { (lang, uriValue) -> MultiLangURI(lang, uriValue) }
