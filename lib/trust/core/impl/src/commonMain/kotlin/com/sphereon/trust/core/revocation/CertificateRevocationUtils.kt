...
object CertificateRevocationUtils {
    ...
    fun extractOcspUrl(certificateDer: ByteArray): String? {
        return try {
            ...
            val x509Cert = cert.toX509Certificate()
            val exts = x509Cert.tbsCertificate.extensions ?: return null
            val aiaExt =
                exts.firstOrNull {
            ...
            // Ignored: failed to extract OCSP URL from certificate
            null
        }
    }
    ...
    fun extractCdpUrls(certificateDer: ByteArray): List<String> {
        return try {
            val cert = certificateFromDer(certificateDer)
            val x509Cert = cert.toX509Certificate()
            val exts = x509Cert.tbsCertificate.extensions ?: return emptyList()
            val cdpExt =
                exts.firstOrNull {
            ...
            // Ignored: failed to extract CDP URLs from certificate
            emptyList()
        }
    }
    ...
    fun extractSerialNumber(certificateDer: ByteArray): String? =
        try {
            ...
            // Ignored: failed to extract serial number from certificate
            null
        }
    ...
    private fun collectUris(
        ...
        when (element) {
            is Asn1Sequence -> {
                element.children.forEach { collectUris(it, urls) }
            }
