...
class DefaultOCSPChecker(
    ...
    private val logger = execution.log.logManager.withTagAsync("DefaultOCSPChecker")
    private val httpClient by lazy { httpClientFactory.createClient(HttpClientOptions()) }

    private val cache by lazy {
        cacheService.getStringCache<RevocationCheckResult>(
            CacheRequirements(
    ...
    override suspend fun checkOCSP(
        ...
        return try {
            val serialNumber = CertificateRevocationUtils.extractSerialNumber(certificate)
                ?: return unavailable("Could not extract serial number from certificate")

            val ocspUrl = options.ocspResponderUrl
            ...
            if (options.useCache) {
                ...
                if (cached.isOk && cached.value != null) {
                    ...
                    return cached.value!!.copy(fromCache = true)
                }
            }
            ...
            unavailable("OCSP check failed: ${e.message}")
        }
    }
    ...
    private fun buildOcspRequest(
        ...
        return try {
            val issuerCert = com.sphereon.crypto.core.x509.certificateFromDer(issuerCertificate)
            val issuerSignum = issuerCert.toSignumX509Certificate()

            // SHA-1 hash of issuer's Distinguished Name (DER encoded)
            val issuerNameDer = Asn1.Sequence {
                issuerSignum.tbsCertificate.issuerName.forEach { +it }
            }.derEncoded
            val issuerNameHash = org.kotlincrypto.hash.sha1.SHA1().digest(issuerNameDer)

            // SHA-1 hash of issuer's public key (raw key bytes)
            val issuerKeyBytes = issuerSignum.publicKey.iosEncoded
            val issuerKeyHash = org.kotlincrypto.hash.sha1.SHA1().digest(issuerKeyBytes)

            // Serial number of the certificate being checked
            val cert = com.sphereon.crypto.core.x509.certificateFromDer(certificate)
            val certSignum = cert.toSignumX509Certificate()
            val serialBytes = certSignum.tbsCertificate.serialNumber

            // Build the OCSPRequest ASN.1 structure
            // SHA-1 AlgorithmIdentifier OID: 1.3.14.3.2.26
            val sha1Oid = byteArrayOf(
                0x06, 0x05, 0x2B, 0x0E, 0x03, 0x02, 0x1A.toByte() // OID 1.3.14.3.2.26
            )
    ...
    private fun parseOcspResponse(responseBytes: ByteArray): RevocationCheckResult {
        return try {
            ...
            val seq = element as? Asn1Sequence
                ?: return unavailable("OCSP response is not a valid ASN.1 SEQUENCE")

            val statusPrim = seq.children.firstOrNull() as? Asn1Primitive
            ...
            when (statusByte) {
                ...
                3 -> unavailable("OCSP responder returned: tryLater")
                5 -> unavailable("OCSP responder returned: sigRequired")
                6 -> unavailable("OCSP responder returned: unauthorized")
                else -> unavailable("OCSP responder returned unknown status: $statusByte")
            }
        } catch (e: Exception) {
            unavailable("Failed to parse OCSP response: ${e.message}")
        }
    }
    ...
    private fun parseSuccessfulOcspResponse(responseSeq: Asn1Sequence): RevocationCheckResult {
        try {
            ...
            val innerSeq = when (responseBytesWrapper) {
                is Asn1Sequence -> responseBytesWrapper
                else -> {
                    // Context-tagged: parse its content
                    try {
                        val content = (responseBytesWrapper as? Asn1Primitive)?.content
            ...
            val basicResponseBytes = (responseOctet as? Asn1Primitive)?.content
                ?: (responseOctet as? Asn1EncapsulatingOctetString)?.content
                ?: return goodWithNote("Could not extract BasicOCSPResponse")

            val basicResponse = Asn1Element.parse(basicResponseBytes) as? Asn1Sequence
                ?: return goodWithNote("BasicOCSPResponse is not a SEQUENCE")

            // tbsResponseData is the first child
            val tbsResponseData = basicResponse.children.firstOrNull() as? Asn1Sequence
            ...
            for (child in tbsResponseData.children) {
                ...
                if (responsesSeq.children.isEmpty()) continue
                ...
                if (firstResponse.children.size < 3) continue
                val certId = firstResponse.children[0] as? Asn1Sequence ?: continue
                if (certId.children.size < 4) continue // CertID has 4 fields
                ...
                return when (tag) {
                    0 -> RevocationCheckResult(
                        status = RevocationStatus.GOOD,
                        method = RevocationCheckMethod.OCSP,
                        checkedAt = Clock.System.now().toEpochMilliseconds()
                    )
                    1 -> RevocationCheckResult(
                        status = RevocationStatus.REVOKED,
                        method = RevocationCheckMethod.OCSP,
                        checkedAt = Clock.System.now().toEpochMilliseconds()
                    )
                    2 -> RevocationCheckResult(
                        status = RevocationStatus.UNKNOWN,
                        method = RevocationCheckMethod.OCSP,
                        checkedAt = Clock.System.now().toEpochMilliseconds()
                    )
                    else -> goodWithNote("Unknown certStatus tag: $tag")
                }
            }

            return goodWithNote("No SingleResponse found in BasicOCSPResponse")
        } catch (e: Exception) {
            return goodWithNote("Failed to parse BasicOCSPResponse: ${e.message}")
        }
    }

    private fun goodWithNote(note: String) = RevocationCheckResult(
        status = RevocationStatus.GOOD,
        method = RevocationCheckMethod.OCSP,
        checkedAt = Clock.System.now().toEpochMilliseconds(),
    ...
    private fun unavailable(message: String) = RevocationCheckResult(
