...
/**
 * Cross-platform CRL checker using Ktor HTTP client and signum ASN.1 parsing.
 *
...
class DefaultCRLChecker(
    ...
    override suspend fun checkCRL(
        ...
        return try {
            ...
            val cdpUrl = options.crlDistributionPoint
            ...
            val response = httpClient.get(cdpUrl) {
            ...
            logger.error("CRL check failed", exception = e)
            RevocationCheckResult(
                status = RevocationStatus.UNAVAILABLE,
                method = RevocationCheckMethod.CRL,
                checkedAt = Clock.System.now().toEpochMilliseconds(),
                errorMessage = "CRL check failed: ${e.message}"
            )
        }
    }
     ...
     * }
     */
    private fun checkSerialInCrl(crlBytes: ByteArray, serialNumber: String): RevocationCheckResult {
        return try {
            val element = Asn1Element.parse(crlBytes)
            val certList = element as? Asn1Sequence
                ?: return unavailable("CRL is not a valid ASN.1 SEQUENCE")

            val tbsCertList = certList.children.firstOrNull() as? Asn1Sequence
            ...
            for (child in tbsCertList.children) {
                ...
                if (seq.children.isEmpty()) continue
                ...
                // Tag 0x02 = INTEGER
                if (firstPrim.tag.tagValue != 2uL) continue
                ...
                for (entry in seq.children) {
                    ...
                    if (serialPrim.tag.tagValue != 2uL) continue

                    val revokedSerial = serialPrim.content.joinToString("") {
            ...
            unavailable("Failed to parse CRL: ${e.message}")
        }
    }
    ...
    private fun normalizeSerial(serial: String): String {
        return serial.lowercase().replace(":", "").replace(" ", "").removePrefix("0x")
    }
