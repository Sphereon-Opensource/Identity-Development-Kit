...
class SignumX509JsCompatTest {
    ...
    private suspend fun extractCertBase64(): String {
        ...
        val parser = com.sphereon.trust.etsi.parser.StreamingETSITrustListParser()
        val tl = parser.parseFromString(tlXml)
        return tl.trustedEntities.first()
    ...
    @Test
    fun base64DecodingMatchesStdlib() = runTest {
        val certBase64 = extractCertBase64()
        val ourResult = certBase64.decodeFromBase64()
        val stdResult = kotlin.io.encoding.Base64.decode(certBase64)

        assertEquals(stdResult.size, ourResult.size,
            "Decoded sizes differ: stdlib=${stdResult.size}, ours=${ourResult.size}")

        for (i in ourResult.indices) {
            assertEquals(stdResult[i], ourResult[i],
    ...
    @OptIn(kotlin.io.encoding.ExperimentalEncodingApi::class)
    @Test
    fun signumCanParseWithStdlibBase64() = runTest {
        // Test with stdlib Base64 decoder to isolate whether the issue is
        // in our base64 decoding or in signum's ASN.1 parser
        val certBase64 = extractCertBase64()
        val der = kotlin.io.encoding.Base64.decode(certBase64)
        val parsed = X509Certificate.decodeFromByteArray(der)
        assertNotNull(parsed, "Signum should parse cert decoded with stdlib Base64")
    }

    @Test
    fun signumCanParseExtractedCertificate() = runTest {
        val certBase64 = extractCertBase64()
        val parsed = signumX509CertificateFromBase64(certBase64)
    ...
    @Test
    fun x509DerOrPemToPemRoundTrip() = runTest {
        val certBase64 = extractCertBase64()

        // x509DerOrPemToPem internally calls signumX509CertificateFromBase64 + encodeToPEM
