...
class CrossFormatConversionTest {
    ...
    @Test
    fun testFidesTL612XmlTo602JsonRoundTrip() = runTest {
    ...
    @Test
    fun testFidesTLSchemeInfoSurvivesRoundTrip() = runTest {
        val xml = ctx.fetchUrl(FIDES_TL_URL)
        val etsiLoTE = parser.parseFromString(xml)
        val roundTripped = roundTripViaJson(etsiLoTE)
    ...
    @Test
    fun testFidesTLSchemeOperatorAddressSurvivesRoundTrip() = runTest {
        val xml = ctx.fetchUrl(FIDES_TL_URL)
        val etsiLoTE = parser.parseFromString(xml)
        val roundTripped = roundTripViaJson(etsiLoTE)
    ...
    @Test
    fun testFidesTLAllTSPsSurviveRoundTrip() = runTest {
        val xml = ctx.fetchUrl(FIDES_TL_URL)
        val etsiLoTE = parser.parseFromString(xml)
        val roundTripped = roundTripViaJson(etsiLoTE)
    ...
    @Test
    fun testFidesTLServiceInfoSurvivesRoundTrip() = runTest {
        val xml = ctx.fetchUrl(FIDES_TL_URL)
        val etsiLoTE = parser.parseFromString(xml)
        val roundTripped = roundTripViaJson(etsiLoTE)
        ...
        for (i in etsiLoTE.trustedEntities.indices) {
            ...
            for (j in originalServices.indices) {
                ...
                assertEquals(original.serviceTypeIdentifier, result.serviceTypeIdentifier,
                    "Service type mismatch at TSP $i, service $j")
                assertEquals(original.serviceName.forLang("en"), result.serviceName.forLang("en"),
                    "Service name mismatch at TSP $i, service $j")
                assertEquals(original.serviceStatus, result.serviceStatus,
                    "Service status mismatch at TSP $i, service $j")
                assertEquals(original.statusStartingTime, result.statusStartingTime,
    ...
    @Test
    fun testFidesTLDigitalIdentitySurvivesRoundTrip() = runTest {
        val xml = ctx.fetchUrl(FIDES_TL_URL)
        val etsiLoTE = parser.parseFromString(xml)
        val roundTripped = roundTripViaJson(etsiLoTE)

        for (i in etsiLoTE.trustedEntities.indices) {
            val originalId = etsiLoTE.trustedEntities[i].trustedEntityServices[0]
                .serviceInformation.serviceDigitalIdentity
            val resultId = roundTripped.trustedEntities[i].trustedEntityServices[0]
                .serviceInformation.serviceDigitalIdentity

            assertEquals(originalId.x509Certificates.size, resultId.x509Certificates.size,
                "Certificate count mismatch at TSP $i")
            if (originalId.x509Certificates.isNotEmpty()) {
                assertEquals(originalId.x509Certificates[0], resultId.x509Certificates[0],
                    "Certificate mismatch at TSP $i")
            }
            assertEquals(originalId.subjectName, resultId.subjectName,
                "Subject name mismatch at TSP $i")
            assertEquals(originalId.x509SKIs, resultId.x509SKIs,
                "X509 SKI mismatch at TSP $i")
        }
    }

    @Test
    fun testFidesTLParseFromJsonProducesEquivalentResult() = runTest {
        val xml = ctx.fetchUrl(FIDES_TL_URL)
    ...
    @Test
    fun testFidesLOTL612XmlTo602JsonRoundTrip() = runTest {
    ...
    @Test
    fun testFidesLOTLPointersSurviveRoundTrip() = runTest {
        val xml = ctx.fetchUrl(FIDES_LOTL_URL)
        val etsiLoTE = parser.parseFromString(xml)
        val roundTripped = roundTripViaJson(etsiLoTE)

        assertTrue(etsiLoTE.pointersToOtherLoTE.isNotEmpty(), "Expected at least 1 pointer in FIDES LOTL")
        assertEquals(etsiLoTE.pointersToOtherLoTE.size, roundTripped.pointersToOtherLoTE.size,
    ...
    @Test
    fun testFidesLOTLJsonOutputIsValidAndReparseable() = runTest {
        val xml = ctx.fetchUrl(FIDES_LOTL_URL)
        val etsiLoTE = parser.parseFromString(xml)
    ...
    @Test
    fun testFidesTL602XmlRoundTripViaLoTE() = runTest {
        ...
        for (i in etsiLoTE.trustedEntities.indices) {
            assertEquals(
                etsiLoTE.trustedEntities[i].trustedEntityInformation.name.forLang("en"),
                reparsedETSI.trustedEntities[i].trustedEntityInformation.name.forLang("en"),
