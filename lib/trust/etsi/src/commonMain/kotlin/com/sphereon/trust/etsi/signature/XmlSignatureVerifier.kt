...
class XmlUtilSignatureVerifier(
    ...
    override suspend fun verifyFromBytes(xmlData: ByteArray, options: XmlSignatureVerificationOptions): XmlSignatureVerificationResult {
        return verifyFromString(xmlData.decodeToString(), options)
    }
    ...
    private suspend fun verifyDocument(
        ...
        val root = document.getDocumentElement() ?: return XmlSignatureVerificationResult(
            valid = false,
            signaturePresent = false,
            errorMessage = "No root element in document"
        )
        val signatureNodes = root.getElementsByTagNameNS("http://www.w3.org/2000/09/xmldsig#", "Signature")
        ...
        return try {
            ...
            val isValid = keyManagerService.isValidRawSignature(
                keyInfo = keyInfo,
            ...
            val signingTime = qualifyingProperties?.signedProperties
    ...
    private fun extractCertificatesFromSignature(signatureElement: Element): Pair<List<String>, List<ByteArray>?> {
        try {
            val keyInfoNodes = signatureElement.getElementsByTagNameNS("http://www.w3.org/2000/09/xmldsig#", "KeyInfo")
            if (keyInfoNodes.length == 0) {
                return Pair(emptyList(), null)
            }

            val keyInfoElement = keyInfoNodes[0] as? Element ?: return Pair(emptyList(), null)
            val x509DataNodes = keyInfoElement.getElementsByTagNameNS("http://www.w3.org/2000/09/xmldsig#", "X509Data")
            if (x509DataNodes.length == 0) {
                return Pair(emptyList(), null)
            }

            val x509DataElement = x509DataNodes[0] as? Element ?: return Pair(emptyList(), null)
            val x509CertNodes = x509DataElement.getElementsByTagNameNS("http://www.w3.org/2000/09/xmldsig#", "X509Certificate")
    ...
    private fun extractSignatureValue(signatureElement: Element): ByteArray? {
        try {
            val signatureValueNodes = signatureElement.getElementsByTagNameNS("http://www.w3.org/2000/09/xmldsig#", "SignatureValue")
            if (signatureValueNodes.length == 0) {
                return null
            }

            val signatureValueText = signatureValueNodes[0]?.getTextContent()?.trim()?.replace("\\s".toRegex(), "")
                ?: return null
    ...
    private fun extractSignedInfo(signatureElement: Element): Element? {
        val signedInfoNodes = signatureElement.getElementsByTagNameNS("http://www.w3.org/2000/09/xmldsig#", "SignedInfo")
...
interface XmlSignatureVerifier {
     ...
     */
    suspend fun verifyFromBytes(xmlData: ByteArray, options: XmlSignatureVerificationOptions = XmlSignatureVerificationOptions()): XmlSignatureVerificationResult
     ...
     * @return Verification result
     */
    suspend fun verifyFromString(xmlString: String, options: XmlSignatureVerificationOptions = XmlSignatureVerificationOptions()): XmlSignatureVerificationResult
}
