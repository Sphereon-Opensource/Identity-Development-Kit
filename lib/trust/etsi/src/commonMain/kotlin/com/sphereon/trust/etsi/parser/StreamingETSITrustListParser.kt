...
class StreamingETSITrustListParser : ETSITrustListParser {
    ...
    override fun parseFromBytes(xmlData: ByteArray): ETSILoTE {
        return parseFromString(xmlData.decodeToString())
    }
    ...
    private fun parseMultiLangStrings(reader: XmlReader): List<MultiLangString> {
        ...
        while (reader.hasNext()) {
            ...
            if (reader.eventType == EventType.START_ELEMENT) {
                val lang = reader.getAttributeValue("http://www.w3.org/XML/1998/namespace", "lang")
    ...
    private fun parseOtherTSLPointer(reader: XmlReader): ETSIOtherLoTEPointer {
        ...
        while (reader.hasNext()) {
            ...
            if (reader.eventType == EventType.START_ELEMENT) {
                when (reader.localName) {
                    "TSLLocation" -> tslLocation = reader.readSimpleElement()
                    "SchemeTerritory" -> schemeTerritory = reader.readSimpleElement()
                    "SchemeOperatorName" -> schemeOperatorName = parseMultiLangStrings(reader)
    ...
    private fun parseAdditionalInformation(reader: XmlReader): AdditionalInformationParsed {
        ...
        val additionalInfo = if (textualInformation.isEmpty() && otherInformation.isEmpty() && mimeType == null) {
            null
        } else {
        ...
        while (reader.hasNext()) {
            when (reader.next()) {
                ...
                EventType.START_ELEMENT -> {
                    ...
                    when (elName) {
                        "MimeType" -> result["mimeType"] = reader.readSimpleElement().trim()
                        "SchemeTerritory" -> result["schemeTerritory"] = reader.readSimpleElement().trim()
                        "TSLType" -> result["tslType"] = reader.readSimpleElement().trim()
                        else -> {
    ...
    private fun parseTSPInformation(reader: XmlReader): ETSITrustedEntityInformation {
        ...
        return ETSITrustedEntityInformation(
            name = name,
            tradeName = tradeName,
            address = address ?: ETSIOperatorAddress(
        ...
        while (reader.hasNext()) {
            ...
            if (reader.eventType == EventType.START_ELEMENT) {
                when (reader.localName) {
                    ...
                    "ServiceDigitalIdentity" -> digitalIdentity = parseServiceDigitalIdentity(reader)
                    ...
                    "ServiceSupplyPoints" -> serviceSupplyPoints.addAll(parseServiceSupplyPoints(reader))
                    "SchemeServiceDefinitionURI" -> schemeServiceDefinitionURI = parseMultiLangURIs(reader)
                    "TSPServiceDefinitionURI" -> tspServiceDefinitionURI = parseMultiLangURIs(reader)
        ...
        while (reader.hasNext()) {
            ...
            if (reader.eventType == EventType.START_ELEMENT) {
                when (reader.localName) {
                    ...
                    "InformationValue" -> informationValue = reader.readSimpleElement()
                    "OtherInformation" -> {
                        val lang = reader.getAttributeValue("http://www.w3.org/XML/1998/namespace", "lang")
                            ?: reader.getAttributeValue(null, "xml:lang")
                            ?: "en"
