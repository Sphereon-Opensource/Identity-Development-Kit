...
class ETSITrustListIdentifierResolutionServiceImpl(
    ...
    override suspend fun doExecute(
        ...
        return when (val opts = optsResult.value) {
            is ExternalIdentifierETSITslOpts -> {
                resolveTrustList(opts)
            }

            is ExternalIdentifierETSITspOpts -> {
                resolveTSP(opts)
            }
     ...
     */
    private suspend fun resolveTrustList(opts: ExternalIdentifierETSITslOpts): IdkResult<ExternalIdentifierETSITslResult, IdkErrorType> {
        try {
            logger.info("Resolving ETSI trust list from ${opts.identifier}")

            // Find appropriate resolver
            val resolver =
                trustListResolvers.firstOrNull { it.supports(opts.identifier) }
                    ?: return IdkError
                        .UNKNOWN_ERROR(
     ...
     */
    private suspend fun resolveTSP(opts: ExternalIdentifierETSITspOpts): IdkResult<ExternalIdentifierETSITspResult, IdkErrorType> {
        try {
            // First resolve the trust list
            val tslUri =
                opts.tslUri ?: return IdkError
                    .COMMAND_ARG_NOT_SUPPORTED_ERROR(
    ...
    private fun applyFilters(
        ...
        opts.serviceTypeFilter?.let { serviceTypes ->
            filtered =
                filtered.copy(
                    trustedEntities =
                        filtered.trustedEntities
                            .map { entity ->
                                entity.copy(
                                    trustedEntityServices =
                                        entity.trustedEntityServices.filter { service ->
