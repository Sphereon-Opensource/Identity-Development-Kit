...
object XAdESParser {
    ...
    private val XADES_NAMESPACES = setOf(
        XADES_NS,
        XADES_141_NS,
    ...
    fun parse(signatureElement: Element): QualifyingProperties? {
        ...
        for (objNode in objectNodes) {
            if (objNode !is Element) continue
    ...
    private fun parseQualifyingProperties(element: Element, ns: String): QualifyingProperties {
        val target = element.getAttribute("Target")
        val signedProperties = findChildElement(element, ns, "SignedProperties")?.let {
            parseSignedProperties(it, ns)
        }
        val unsignedProperties = findChildElement(element, ns, "UnsignedProperties")?.let {
    ...
    private fun parseSignedProperties(element: Element, ns: String): SignedProperties {
        val id = element.getAttribute("Id")
        val signedSigProps = findChildElement(element, ns, "SignedSignatureProperties")?.let {
            parseSignedSignatureProperties(it, ns)
        }
        val signedDataObjProps = findChildElement(element, ns, "SignedDataObjectProperties")?.let {
    ...
    private fun parseSignedSignatureProperties(element: Element, ns: String): SignedSignatureProperties {
        // Signing time
        val signingTime = findChildElement(element, ns, "SigningTime")?.let { el ->
        ...
        val signingCertV2 = findChildElement(element, ns, "SigningCertificateV2")?.let {
            parseCertDigests(it, ns, v2 = true)
        }

        // SigningCertificate (legacy v1)
        val signingCert = findChildElement(element, ns, "SigningCertificate")?.let {
    ...
    private fun parseCertDigests(element: Element, ns: String, v2: Boolean): List<CertDigest> {
        ...
        for (certNode in certNodes) {
            ...
            val digestValueText = digestValueEl?.getTextContent()
            ...
            if (v2) {
                ...
                issuerSerialV2 = issuerSerialEl?.getTextContent()
                    ?.trim()?.replace("\\s".toRegex(), "")
                    ?.let { it.decodeFrom(Encoding.BASE64) }
            } else {
                val issuerSerialEl = findChildElement(certNode, ns, "IssuerSerial")
                if (issuerSerialEl != null) {
                    issuerName = findChildElement(issuerSerialEl, XMLDSIG_NS, "X509IssuerName")
                        ?.getTextContent()?.trim()
                    serialNumber = findChildElement(issuerSerialEl, XMLDSIG_NS, "X509SerialNumber")
                        ?.getTextContent()?.trim()
                }
            }
    ...
    private fun parseSignedDataObjectProperties(element: Element, ns: String): SignedDataObjectProperties {
        ...
        for (formatNode in formatNodes) {
            if (formatNode !is Element) continue
            val objectRef = formatNode.getAttribute("ObjectReference")
            val mimeType = findChildElement(formatNode, ns, "MimeType")
                ?.getTextContent()?.trim()
    ...
    private fun parseUnsignedProperties(element: Element, ns: String): UnsignedProperties {
        val unsignedSigProps = findChildElement(element, ns, "UnsignedSignatureProperties")?.let {
            parseUnsignedSignatureProperties(it, ns)
        }
        return UnsignedProperties(unsignedSignatureProperties = unsignedSigProps)
    }

    private fun parseUnsignedSignatureProperties(element: Element, ns: String): UnsignedSignatureProperties {
        ...
        for (tsNode in tsNodes) {
            if (tsNode !is Element) continue
            val encapsulated = findChildElement(tsNode, ns, "EncapsulatedTimeStamp")
            val tsValue = encapsulated?.getTextContent()
                ?.trim()?.replace("\\s".toRegex(), "")
            if (tsValue != null) {
    ...
    private fun findChildElement(parent: Element, nsUri: String, localName: String): Element? {
        ...
        return if (nodes.length > 0) nodes[0] as? Element else null
    }
}
