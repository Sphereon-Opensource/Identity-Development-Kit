...
object LoTE612Converter {
    ...
    fun LoTE.toETSILoTE(): ETSILoTE {
        ...
        return ETSILoTE(
            ...
            policyOrLegalNotice = info.policyOrLegalNotice.map {
    ...
    fun ETSILoTE.toLoTE(): LoTE {
        return LoTE(
            listAndSchemeInformation = ListAndSchemeInformation(
                ...
                policyOrLegalNotice = policyOrLegalNotice.map {
    ...
    private fun LoTEServiceInformation.toETSI(): ETSIServiceInformation {
        return ETSIServiceInformation(
            ...
            statusStartingTime = statusStartingTime ?: kotlinx.datetime.Instant.fromEpochMilliseconds(0),
    ...
    private fun buildAdditionalInfoFromQualifiers(qualifiers: List<LoTEQualifier>): ETSIAdditionalInformation? {
        ...
        for (q in qualifiers) {
            ...
            if (mimeType == null) mimeType = q.mimeType
            q.schemeTypeCommunityRules.forEach { otherInfo.add(it.uriValue) }
        }
        if (otherInfo.isEmpty() && mimeType == null) return null
    ...
    private fun ETSIServiceDigitalIdentity.toLoTE(): LoTEServiceDigitalIdentity {
        return LoTEServiceDigitalIdentity(
            ...
            x509SubjectNames = if (subjectName != null) listOf(subjectName) else emptyList(),
            x509SKIs = x509SKIs
        )
    }

    private fun ETSIOtherLoTEPointer.toLoTEOtherPointer(): LoTE602OtherPointer {
        val qualifier = LoTEQualifier(
            schemeOperatorName = schemeOperatorName,
            schemeTerritory = schemeTerritory.ifEmpty { null },
