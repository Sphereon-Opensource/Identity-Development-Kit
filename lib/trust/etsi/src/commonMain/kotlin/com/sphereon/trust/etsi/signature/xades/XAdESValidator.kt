...
data class XAdESValidationResult(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (signaturePresent != other.signaturePresent) {
            return false
        }
        if (signatureValid != other.signatureValid) {
            return false
        }
        if (referencesValid != other.referencesValid) {
            return false
        }
        if (xadesPresent != other.xadesPresent) {
            return false
        }
        if (signingCertificateValid != other.signingCertificateValid) {
            return false
        }
...
class XAdESValidatorImpl(
        ...
        try {
            val document =
                try {
                    ...
                    return errorResult("XML parsing failed: ${expected.message}")
                }
            val root =
            ...
            // Extract components
            val signedInfo =
                firstChild(signatureElement, XMLDSIG_NS, "SignedInfo")
                    ?: return errorResult("No SignedInfo in Signature")
            val signatureValueEl =
                firstChild(signatureElement, XMLDSIG_NS, "SignatureValue")
                    ?: return errorResult("No SignatureValue in Signature")

            val signatureValueBase64 =
                signatureValueEl
                    .getTextContent()
                    ?.trim()
                    ?.replace("\\s".toRegex(), "")
            ...
            val signatureValid =
                try {
                    ...
                    logger.error("Signature verification failed", exception = expected)
            ...
            return errorResult("XAdES validation failed: ${expected.message}")
        }
    }
    ...
    private fun diagnoseSignatureFailure(
        ...
        if (isAsciiBase64 && signatureValue.size > MIN_DOUBLE_ENCODED_SIZE) {
            try {
    ...
    private fun validateCertificateDigest(
        certDigest: CertDigest,
        certDer: ByteArray,
    ): Boolean {
        val digestAlg = digestAlgMap[certDigest.digestAlgorithm] ?: return false
