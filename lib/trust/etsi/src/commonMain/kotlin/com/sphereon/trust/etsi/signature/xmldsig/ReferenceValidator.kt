...
object ReferenceValidator {
    ...
    fun validateReferences(
        ...
        for (refNode in references) {
            if (refNode !is Element) continue
    ...
    private fun validateReference(
        ...
        try {
            ...
            val digestMethodEl = firstChildElement(reference, XMLDSIG_NS, "DigestMethod")
                ?: return ReferenceResult(uri, type, "", false, "Missing DigestMethod")
            val digestAlgUri = digestMethodEl.getAttribute("Algorithm")
                ?: return ReferenceResult(uri, type, "", false, "Missing DigestMethod Algorithm")

            val digestValueEl = firstChildElement(reference, XMLDSIG_NS, "DigestValue")
                ?: return ReferenceResult(uri, type, digestAlgUri, false, "Missing DigestValue")
            val expectedDigest = digestValueEl.getTextContent()
            ...
            val content = resolveReference(uri, type, document, signatureElement)
            ...
            // Compute digest
            val digestAlg = DIGEST_ALG_MAP[digestAlgUri]
    ...
    private fun resolveReference(
        ...
        return when {
            ...
            uri.isEmpty() -> document.getDocumentElement()
            ...
            else -> null // External references not supported
        }
    }

    private fun findElementById(document: Document, id: String, type: String?): Element? {
        val root = document.getDocumentElement() ?: return null
        return findElementByIdRecursive(root, id)
    }

    private fun findElementByIdRecursive(element: Element, id: String): Element? {
        // Check common id attributes
        val elemId = element.getAttribute("Id")
    ...
    private fun extractTransforms(reference: Element): List<Transform> {
        val transformsEl = firstChildElement(reference, XMLDSIG_NS, "Transforms")
            ?: return emptyList()
    ...
    private fun applyTransforms(
        ...
        for (transform in transforms) {
            when (transform.algorithm) {
                TRANSFORM_ENVELOPED -> hasEnvelopedTransform = true
        ...
        return if (content is Element) {
            val excludeNode = if (hasEnvelopedTransform) signatureElement else null
    ...
    private fun firstChildElement(parent: Element, nsUri: String, localName: String): Element? {
