...
class BleCentralServiceImpl(
    ...
    /**
     * Listener for BLE connection state changes.
     * This detects when the remote peripheral (reader) disconnects and notifies the transport.
     * Per ISO 18013-5, in forward engagement the reader should initiate the disconnect
     * after receiving the DeviceResponse.
     */
    private val connectionStateListener = object : com.sphereon.data.link.ble.client.BleEvent.Listener {
        ...
        // Handle notifications from the peripheral, particularly the END command on State characteristic
        ...
        // Other required listener methods - no-op
    ...
    init {
        // Register connection state listener to detect remote disconnect
        blePlatformClient.addBleEventListener(connectionStateListener)
        log.info("Connection state listener registered for remote disconnect detection")
    }
    ...
    override suspend fun awaitConnected(
        ...
        val service = if (targetServiceUuid != null) {
        ...
        log.info("Found service: ${service.id} (matched by ${if (service.id == targetServiceUuid) "UUID" else "characteristic"})")

        // Step 5: Discover characteristics
        val characteristicIds = listOfNotNull(
        ...
        val notificationCharacteristics = setOf(
    ...
    override suspend fun start(service: GattService): IdkResult<Unit, BleError> {
        ...
        // Register incoming channel as BLE event listener BEFORE starting data transfer
        // This ensures the channel receives notification events from the peripheral
        if (incomingChannel != null && incomingChannel is com.sphereon.data.link.ble.client.BleEvent.Listener) {
            blePlatformClient.addBleEventListener(incomingChannel)
            log.info("Incoming channel registered as BLE event listener")
        } else {
            log.warn("Incoming channel is null or not a BleEvent.Listener, notifications may not work")
        }
        ...
        val writeResult = blePlatformClient.writeCharacteristic(
            service = service,
     ...
     */
    override suspend fun writeDeviceEngagement(service: GattService, deviceEngagement: ByteArray): IdkResult<Unit, BleError> {
    ...
    override fun close() {
        ...
        // Remove the connection state listener first to avoid receiving events during close
        try {
            blePlatformClient.removeBleEventListener(connectionStateListener)
        } catch (e: Throwable) {
            log.warn("Error removing connection state listener: ${e.message}")
        }
        ...
        if (!closedCallbackInvoked) {
            ...
            try {
                ...
                log.error("Error in onClosed callback", exception = e)
            }
        } else {
