...
class BleTransportFactory(
    ...
    override fun supports(connectionMethod: ConnectionMethod): Boolean = connectionMethod is BleConnectionMethod

    override fun getConnectionMethodFactory(): ConnectionMethodBase.Factory = BleConnectionMethod.Companion
    ...
    override fun createTransfer(
        ...
        val shouldAttemptBothModes =
            engagementData?.let { data ->
                when (role) {
                    MdocRole.MDOC -> data.isDeviceEngagement()

                    // Holder with DeviceEngagement
        ...
        val useCentralMode =
            when {
                ...
                options.peripheralServerMode -> {
                    ...
                    error("BLE connection method must specify at least one mode (central or peripheral)")
                }
            }

        log.info(
            "Creating BLE transfer: role=$role, mode=${if (useCentralMode) {
                "central"
            } else {
                "peripheral"
            }}"
    ...
    private fun createCentralClientTransfer(
        ...
        val placeholderServiceUuid =
            connectionMethod.options.centralClientModeUuid
                ?: Uuid.random()

        val outgoingChannel =
            BleOutgoingDataChannelImpl(
                logManager = logManager,
                role = role,
                instanceId = instanceId,
    ...
    private fun createPeripheralServerTransfer(
        ...
        val serviceUuid =
            connectionMethod.options.peripheralServerModeUuid
                ?: connectionMethod.options.centralClientModeUuid
                ?: error("Either peripheralServerModeUuid (key 10) or centralClientModeUuid (key 11) must be set when acting as peripheral server")
        ...
        val peripheralService =
            BlePeripheralServiceImpl(
                blePlatformPeripheral = blePlatformPeripheral,
                logManager = logManager,
                role = role,
                characteristics = characteristics,
                serviceUuid = serviceUuid,
                incomingDataChannelImpl = incomingChannel,
                outgoingDataChannelImpl = outgoingChannel,
