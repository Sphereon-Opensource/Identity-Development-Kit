...
class BleCentralClientTransport(
    ...
    init {
        ...
        bleCentralService.setCallbacks(
            ...
            onClosed = {
                log.info("onClosed callback: BLE connection closed by remote, state before=${mutableEngagementState.value}")
                // Mark the transport as closed so engagement state becomes DISCONNECTED
                markClosed()
                log.info("markClosed() completed, state after=${mutableEngagementState.value}")
            },
        )
    }
    ...
    ): IdkResult<Uuid, BleError> {
        log.info("open() called for BLE central client mode, scanning for UUID: $id")

        mutex.withLock {
            check(mutableEngagementState.value == MdocEngagementState.INIT) {
                "open() called in wrong state: ${mutableEngagementState.value}. Expected ${MdocEngagementState.INIT}"
            }
            setEngagementData(engagementData)

            try {
                // Step 1: Scan for peripheral with the service UUID
                mutableEngagementState.value = MdocEngagementState.BLE_SCANNING
                ...
                if (deviceResult.isErr) {
                    ...
                    mutableEngagementState.value = MdocEngagementState.ERROR
                ...
                mutableEngagementState.value = MdocEngagementState.CONNECTING
    ...
    override suspend fun messageReceiveBlocking(): ByteArray {
        ...
        return try {
            ...
            if (mutableEngagementState.value == MdocEngagementState.DISCONNECTED) {
    ...
    private fun failTransport(error: Throwable) {
        ...
        if (mutableEngagementState.value.order >= MdocEngagementState.ERROR.order) {
    ...
    override fun getIncomingDataChannel(): Any = incomingChannel

    override fun getOutgoingDataChannel(): Any = outgoingChannel
    ...
    ): IdkResult<Unit, BleError> = bleCentralService.writeDeviceEngagement(service, deviceEngagement)
