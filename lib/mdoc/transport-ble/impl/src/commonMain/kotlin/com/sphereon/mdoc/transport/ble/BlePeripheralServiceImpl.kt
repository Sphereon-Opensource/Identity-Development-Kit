...
class BlePeripheralServiceImpl(
    ...
    private val incomingDataChannelImpl: BleIncomingDataChannel,
    private val outgoingDataChannelImpl: BleOutgoingDataChannel,
    ...
    // Job tracking the peripheral event collection
    private var eventCollectionJob: Job? = null

    /**
     * Start collecting BLE events from the peripheral's SharedFlow.
     * Handles connection state, characteristic writes (START/END commands, DeviceEngagement),
     * and descriptor writes (subscription tracking).
     */
    private fun startPeripheralEventCollection() {
        eventCollectionJob?.cancel()
        eventCollectionJob =
            eventScope.launch {
                blePlatformPeripheral.bleEvents.collect { event ->
                    when (event) {
    ...
    override suspend fun awaitAdvertising(serviceUuid: Uuid): IdkResult<Unit, BleError> {
        ...
        return try {
            ...
            // CRITICAL: Start event collection BEFORE adding service to catch early connections
            ...
            log.info("*** STARTING BLE EVENT COLLECTION ***")
            log.info("  - peripheralEventCollection for connection/subscriptions/START")
            log.info("  - incomingDataChannel (${incomingDataChannelImpl::class.simpleName}) for Client2Server writes")
            log.info("  - outgoingDataChannel (${outgoingDataChannelImpl::class.simpleName}) for Server2Client notifications")
            startPeripheralEventCollection()
            (incomingDataChannelImpl as? BleIncomingDataChannelImpl)?.startEventCollection(blePlatformPeripheral.bleEvents, eventScope)
            (outgoingDataChannelImpl as? BleOutgoingDataChannelImpl)?.startEventCollection(blePlatformPeripheral.bleEvents, eventScope)
            log.info("*** BLE EVENT COLLECTION STARTED ***")
            ...
            (outgoingDataChannelImpl as? BleOutgoingDataChannelImpl)?.setServiceAndCharacteristics(createdService!!)
    ...
    override suspend fun awaitConnection(senderKey: CoseKeyType): IdkResult<GattService, BleError> {
        ...
        return try {
            ...
            log.error("Failed to establish connection", exception = expected)
            BleErrors.connectionFailed("Failed to establish connection: ${expected.message}", expected).asErrorResult()
        }
    }
    ...
    override fun incomingChannel(): BleIncomingDataChannel = incomingDataChannelImpl

    override fun outgoingChannel(): BleOutgoingDataChannel = outgoingDataChannelImpl
    ...
    private suspend fun calculateAndSetIdentValue(senderKey: CoseKeyType) {
        ...
        val ikm = coseKeyCborCodec.encode(coseKey).getOrThrow()
    ...
    private suspend fun platformAdvertiseService(serviceUuid: Uuid) {
        ...
        check(result.isOk) { "Failed to start advertising: ${result.error}" }
    ...
    private suspend fun platformWaitForConnection(): GattService {
        ...
        return createdService ?: error("Service not created")
    }
