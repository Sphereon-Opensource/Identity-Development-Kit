...
class BleIncomingDataChannelImpl(
        ...
        processingScope.launch {
            ...
            for (chunk in incomingChunkQueue) {
                try {
                    ...
                    log.error("[$instanceId] Error processing chunk: ${expected.message}", exception = expected)
                }
            }
    ...
    override suspend fun receiveSessionEstablishment(): SessionEstablishment {
        ...
        return decodeSessionEstablishment(receiveRaw())
    }
    ...
    override suspend fun receiveSessionData(): SessionData {
        ...
        return decodeSessionData(receiveRaw())
    }

    override suspend fun awaitExternalTermination(): Boolean = receiveRaw().contentEquals(byteArrayOf(0x02))
        ...
        when (role) {
            MdocRole.MDOC -> {
                ...
                try {
                    ...
                    val sessionEstablishment = decodeSessionEstablishment(message)
                    ...
                    log.error("[$instanceId] Error handling session establishment: ${expected.message}", exception = expected)
            ...
            MdocRole.MDOC_READER -> {
                ...
                try {
                    ...
                    val sessionData = decodeSessionData(message)
                    ...
                    log.error("[$instanceId] Error validating session data: ${expected.message}", exception = expected)
     ...
     *
     * Events handled:
     * - CharacteristicChanged: incoming data chunks from the remote party
     * - MtuChanged: updates the characteristic size
     * - ConnectionStateChanged: logs connection state transitions
     * - CharacteristicWrite: logs write confirmations
                ...
                bleEvents.collect { event ->
                    when (event) {
                        is BleEvent.CharacteristicChanged -> {
                            ...
                            log.info("[$instanceId] *** CharacteristicChanged: char=${event.characteristic.id}, size=${event.value.size}, expectedChar=$incomingCharacteristicId ***")
                        ...
                        is BleEvent.CharacteristicWrite -> {
                            ...
                            log.debug("[$instanceId] CharacteristicWrite: char=${event.characteristic.id}, status=${event.status}")
                        }
    ...
    private fun decodeSessionEstablishment(message: ByteArray): SessionEstablishment =
        sessionEstablishmentCborCodec
            .decode(message)
            .getOrElse { throw it.toException() }
            .value

    private fun decodeSessionData(message: ByteArray): SessionData =
