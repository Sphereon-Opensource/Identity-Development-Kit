...
class BleOutgoingDataChannelImpl(
...
) : BleOutgoingDataChannel, BleEvent.Listener {
        ...
        processingScope.launch {
            for (req in sendQueue) {
                ...
                try {
                    ...
                    val status = characteristicWriter.writeCharacteristic(
                        currentService,
                        currentOutgoingChar,
                        req.packet
                    )
                    ...
                    log.error("[$instanceId] Exception during write: ${e.message}", exception = e)
                    req.ack.completeExceptionally(e)
                    sendQueue.close()
                    break
                }
            }
    ...
    override suspend fun sendRaw(data: ByteArray): Int = sendRawMutex.withLock {
        ...
        while (offset < data.size) {
            ...
            val flag: Byte = if (offset + chunkSize < data.size) 0x01 else 0x00

            val msg = if (flag == 0x01.toByte()) "CONTINUATION" else "END"
            log.debug("[$instanceId] Sending $msg chunk of $chunkSize bytes")

            val packet = ByteArray(chunkSize + 1).apply {
                this[0] = flag
    ...
    override suspend fun sendSessionEstablishment(sessionEstablishment: SessionEstablishment): Int {
        ...
        return sendRaw(sessionEstablishment.encodeCbor())
    }
    ...
    override suspend fun sendSessionData(sessionData: SessionData): Int {
        ...
        return sendRaw(sessionData.encodeCbor())
    }
    ...
    // Unused BleEvent.Listener methods
