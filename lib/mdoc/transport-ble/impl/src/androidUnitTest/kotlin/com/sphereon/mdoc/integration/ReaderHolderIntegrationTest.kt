...
class ReaderHolderIntegrationTest {
    ...
    companion object {
        init {
            // Configure KMS properties BEFORE creating the graph
    ...
    // The context is only needed to satisfy the type check in AndroidUnitTestAppGraph
    // None of our test services (KMS, FakeBLE, MdocEngagementManager) actually use it
    private val mockContext = mockk<Context>(relaxed = true)

    // Create app graph once at class level (singleton for all tests)
    private val app = createAndroidUnitTestAppGraph(
    ...
    private val readerSessionGraph = readerSession.graph.asKeyManagerServiceGraph()

    // Access services from session components (automatically injected via DI)
    private val holderKms = holderSessionGraph.keyManagerService
    private val readerKms = readerSessionGraph.keyManagerService

    // ===== STEP 2: INTEGRATE MDOCENGAGEMENTMANAGER =====
    private val holderEngagementManagerGraph = holderSession.graph as MdocEngagementManagerImpl.Graph
    private val holderEngagementManager = holderEngagementManagerGraph.mdocEngagementManager
    ...
    fun setup() {
        ...
        readerManager = (readerSession.graph as MdocReaderEngagementManagerImpl.Graph).mdocReaderEngagementManager
    }
    ...
    fun `E2E test 1 - verify DI setup and service accessibility`() = runTest {
        // Verify app graph
        assertNotNull(app, "App graph should be initialized")
        ...
        assertNotNull(holderKms, "Holder KMS should be available from session.graph")
        assertNotNull(readerKms, "Reader KMS should be available from session.graph")
        ...
        assertNotNull(holderEngagementManager, "MdocEngagementManager should be available from session.graph")
    }
     ...
     * (set up in AndroidUnitTestAppGraph).
     *
