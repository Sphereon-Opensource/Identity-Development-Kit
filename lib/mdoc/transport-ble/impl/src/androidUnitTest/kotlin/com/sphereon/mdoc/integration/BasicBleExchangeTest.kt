...
class BasicBleExchangeTest {
    ...
    // Access services
    private val holderKms = holderSession.graph.asKeyManagerServiceGraph().keyManagerService
    private val readerKms = (readerSession.graph.asKeyManagerServiceGraph()).keyManagerService
    private val holderEngagementManager = (holderSession.graph as MdocEngagementManagerImpl.Graph).mdocEngagementManager
    ...
    fun setup() {
        ...
        val mdocSignService = MdocSignServiceImpl(
            ...
            mobileSecurityObjectCborCodec = com.sphereon.mdoc.data.mso.MobileSecurityObjectCborCodecImpl(),
    ...
    fun `1-1-3 Reverse engagement - reader peripheral server mode`() = kotlinx.coroutines.runBlocking {
        ...
        val qrData = com.sphereon.mdoc.transfer.reader.ReaderEngagementCborCodecImpl().decodeUri(qrCodeUri).getOrThrow().value
    ...
    fun `1-2-1 Simple mDL request - 3 elements`() = kotlinx.coroutines.runBlocking {
        ...
        val holderResponseJob = this.launch {
            try {
                ...
                println("Holder: Exception while sending response: ${expected.message}")
                e.printStackTrace()
            }
        }
    ...
    fun `1-2-3 Simple mDL request - reverse engagement reader peripheral`() = kotlinx.coroutines.runBlocking {
        ...
        val holderDeviceEngagementBytes = com.sphereon.mdoc.engagement.DeviceEngagementCborCodecImpl().encode(holderDeviceEngagement).getOrThrow()
        ...
        val receivedDeviceEngagement = com.sphereon.mdoc.engagement.DeviceEngagementCborCodecImpl()
            .decode(receivedDeviceEngagementBytes)
            .getOrThrow()
            .value
        ...
        val holderResponseJob = this.launch {
            try {
                ...
                println("Holder error: ${expected.message}")
                e.printStackTrace()
            }
        }
