...
class TransferManagerImpl(
    ...
    MdocRetrievalEvent.Dispatcher {
    // Debug logger for ISO 18013-5 debugging - wraps the session log service
    private val debugLogger: IMdocDebugLogger = MdocDebugLoggerImpl(execution.log, coseKeyCborCodec)
    ...
    internal fun setDataChannels(
        ...
        if (_incomingDataChannel != null || _outgoingDataChannel != null) {
            ...
            error("Data channels already set to different instances")
        }
    ...
    private val earlyListeners = mutableSetOf<MdocRetrievalEvent.Listener>()
    ...
    init {
        ...
        earlyListeners.add(eventListenerAdapter)
    }
    ...
    private val resolvedListeners: Set<MdocRetrievalEvent.Listener>
        // fixme: This makes no sense. Just register the earlyListeners on the session when creating it
        get() =
            if (!isClosed) {
                _instance.getRetrievalEventListeners()
            } else {
                earlyListeners.toSet()
            }
    ...
    private val readerAuthenticationValidator =
    ...
    override suspend fun validateReaderAuthentication(
        ...
        return readerAuthenticationValidator.validate(
            docRequest = docRequest,
            expectedSessionTranscript = sessionTranscript.value,
            requireReaderAuthentication = requireReaderAuthentication,
        )
    }
     ...
     */
    private fun validateDocumentRequest(
        request: DocRequest,
        document: Document,
    ) {
    ...
    override fun close() {
        ...
        try {
            ...
            log.error("$logId: Error during close", exception = expected)
        }
    }
    ...
    override suspend fun createDeviceResponse(
        ...
        val requestDocumentsSelector =
            customDocumentsSelector ?: if (customSingleDocSelector != null) {
                SimpleRequestDocumentsSelector(
                    docRequestSingleDocSelect = singleDocumentSelector,
                    log = selectorLog,
                    globalDocumentsProvider = documentProvider,
                )
            } else {
                defaultRequestDocumentsSelector
            }

        val requestResponseProcesser =
            customResponseProcesser ?: if (customDocumentsSelector != null) {
                SimpleRequestResponseProcessor(
                    requestDocumentsSelector,
                    documentProvider,
                )
            } else {
                defaultRequestResponseProcesser
            }
    ...
    private fun assertedInstance(
        minState: MdocRetrievalState? = null,
        maxState: MdocRetrievalState? = null,
    ): TransferInstanceImpl {
        val instance = this._instance
        if (minState != null && instance.getCurrentState().order < minState.order) {
            error("Engagement is already in progress with a state ${instance.getCurrentState()} smaller than $minState")
        } else if (maxState != null && instance.getCurrentState().order > maxState.order) {
            error("Engagement is already in progress with a state ${instance.getCurrentState()} bigger than $maxState")
        }
        if (earlyListeners.isNotEmpty()) {
            // Make sure we register any already listeners with the engagement
            // Since the instance is using a set, we simply add them always, so the earlyListeners can act as shared listeners across instances
            instance.addRetrievalEventListener(*earlyListeners.toTypedArray())
        }
        return instance
    }
    ...
    private class DocumentSigningBuilderImpl(
        ...
        override fun add(signingRequest: DocumentSigningRequest): DocumentSigningBuilder =
            apply {
                signingRequests.add(signingRequest)
            }

        override fun addAll(signingRequests: List<DocumentSigningRequest>): DocumentSigningBuilder =
            apply {
                this.signingRequests.addAll(signingRequests)
            }

        override suspend fun signAll(): List<Document> = transferManager.signDocuments(signingRequests)
    ...
    override suspend fun signDocuments(signingRequests: List<DocumentSigningRequest>): List<Document> {
    ...
    override fun documentsBuilder(): DocumentSigningBuilder = DocumentSigningBuilderImpl(this)
    ...
    override fun tryOps(): TransferManager.Try =
        object : TransferManager.Try {
            override suspend fun start(): IdkResult<TransferManager, IdkErrorType> =
                try {
                    ...
                    log.error("$logId: Error starting transfer session", exception = expected)
                    IdkError.UNKNOWN_ERROR(exception = expected).asErrorResult()
                }

            override suspend fun receiveDeviceRequest(): IdkResult<DeviceRequest, IdkErrorType> = receiveDeviceRequestInternal()

            override suspend fun sendDeviceResponse(deviceResponse: DeviceResponse): IdkResult<Int, IdkErrorType> = sendDeviceResponseInternal(deviceResponse)

            override suspend fun createResponse(
                deviceRequest: DeviceRequest,
                documentProvider: DocumentProvider?,
            ): IdkResult<DeviceResponse, IdkErrorType> = createDeviceResponse(deviceRequest, documentProvider)
        }
    ...
    private suspend fun processSessionEstablishmentToDeviceRequest(readerData: ByteArray): IdkResult<DeviceRequest, IdkErrorType> {
        ...
        return if (result.isOk) {
            ...
            dispatch(MdocRetrievalEvent.DocumentsSelectionProcessStart(engagement.id, encodeDeviceRequest(deviceRequest)))
    ...
    private suspend fun receiveDeviceRequestInternal(): IdkResult<DeviceRequest, IdkErrorType> {
        ...
        return try {
            ...
            log.info(
                "$logId: Transport type: ${if (isChannelBasedTransport) {
                    "channel-based (BLE/NFC)"
                } else {
                    "direct (OID4VP/REST)"
                }}"
            )
    ...
    private suspend fun receiveViaDirectTransport(): DeviceRequest {
        val transfer =
            _instance.transfer
                ?: error("Transfer not initialized. Call start() first.")
    ...
    private suspend fun setupDirectTransportSessionContext(transfer: IMdocTransfer<*>) {
        ...
        if (sessionTranscript.value == null) {
            val encodedTranscript =
                sessionTranscriptCborCodec
                    .encodeItem(transcript)
                    .getOrElse { throw it.toException() }
    ...
    private suspend fun setupOid4vpSessionContext(transfer: IMdocTransfer<*>) {
        ...
        try {
            ...
            val transcript =
                transfer.getContext("sessionTranscript") as? SessionTranscript
                    ?: error("OID4VP transport did not provide SessionTranscript")
            ...
            log.error("$logId: Failed to setup OID4VP session context", exception = expected)
            throw expected
        }
    }
    ...
    private suspend fun sendDeviceResponseInternal(deviceResponse: DeviceResponse): IdkResult<Int, IdkErrorType> {
        ...
        return try {
            ...
            if (hasChannels) {
                ...
                val currentEncryption = sessionEncryption.value ?: error("Session encryption not initialized. Call receiveDeviceRequest() first.")
            ...
            if (isClosed) {
                log.debug("Exception but we were already closed: ${expected.message}")
                return 0.asOkResult()
            }
            log.error("$logId: Error sending device response: ${expected.message}", exception = expected)
            dispatch(MdocRetrievalEvent.Error(engagement.id, data = byteArrayOf(), error = expected))
            IdkError.UNKNOWN_ERROR(exception = expected).asErrorResult().also { execution.log.error(it.error.message.defaultMessage) }
        }
    }

    private fun assertNotClosed() = require(!isClosed) { "Transfer manager is closed" }

    private fun encodeDeviceRequest(deviceRequest: DeviceRequest): ByteArray =
        deviceRequestCborCodec
