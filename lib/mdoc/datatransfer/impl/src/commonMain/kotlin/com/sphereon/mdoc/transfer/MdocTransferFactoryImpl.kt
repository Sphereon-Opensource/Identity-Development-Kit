...
class MdocTransferFactoryImpl(
    ...
    override fun setupTransfer(
        ...
        val factory =
            transportRegistry.getFactory(connectionMethod.transportType)
                ?: error("No transport factory for ${connectionMethod.transportType}")
...
internal class TransferAdapter(
            ...
            when (delegate) {
                is MdocTransport<*> -> {
                    ...
                    if (connectionMethod.transportType == TransportType.BLE) {
                        ...
                        val baseMethod =
                            connectionMethod as? ConnectionMethodBase<*>
                                ?: error("BLE transport but not ConnectionMethodBase: ${connectionMethod::class.simpleName}")

                        val bleOptions =
                            baseMethod.options as? BleOptions
                                ?: error("BLE transport but options are not BleOptions: ${baseMethod.options::class.simpleName}")
                            ...
                            when {
                                bleOptions.peripheralServerMode -> {
                                    bleOptions.peripheralServerModeUuid
                                        ?: error("Peripheral server mode enabled but no UUID provided")
                                }

                                bleOptions.centralClientMode -> {
                                    bleOptions.centralClientModeUuid
                                        ?: error("Central client mode enabled but no UUID provided")
                                }

                                else -> {
                                    error("BLE connection method has no mode enabled")
                                }
                            }
                        ...
                        val restApiOptions =
                            transfer.engagement.data
                                .getRetrievalMethods()
                                .first { it.type == DeviceRetrievalMethodType.WEBSITE }
     ...
     */
    fun getIncomingDataChannel(): IncomingDataChannel? = delegate.getIncomingDataChannel() as? IncomingDataChannel
     ...
     */
    fun getOutgoingDataChannel(): OutgoingDataChannel? = delegate.getOutgoingDataChannel() as? OutgoingDataChannel
     ...
     */
    override fun getContext(key: String): Any? = delegate.getContext(key)
}
