...
class TransferInstanceImpl(
    ...
    override var transfer: IMdocTransfer<*>
        get() = _transfer.value ?: error("Transfer has not been initialized. Call start() first.")
        set(value) {
    ...
    val sessionGraph = execution.sessionContextManager.getActive().graph

    override fun getCurrentState(): MdocRetrievalStateType = _states.value
    ...
    override fun tryOps(): TransferInstance.Try =
        object : TransferInstance.Try {
                ...
                return try {
                    ...
                    val selectedTransfer =
                        if (transfers.size > 1) {
                            ...
                            if (openResult.isErr) {
                                ...
                                error("Failed to open transfer: $error")
                            }
                    ...
                    if (losingTransfers.isNotEmpty()) {
                        ...
                        losingTransfers.forEach { transfer ->
                            try {
                                ...
                                log.warn("$id: Error closing losing transfer: ${expected.message}")
                            }
                        }
                    }
                    ...
                    if (incomingChannel != null && outgoingChannel != null) {
                        ...
                        if (deviceRequestResult.isOk) {
                            ...
                            error("Failed to receive device request from ${selectedTransfer.connectionMethod.transportType} transport")
                        }
                    }
                    ...
                    log.error("$id: Error starting transfer instance", exception = expected)
     ...
     */
    private suspend fun raceConnectionAttempts(transfers: List<IMdocTransfer<*>>): IMdocTransfer<*> =
        coroutineScope {
                ...
                transfers.mapIndexed { index, transfer ->
                    async {
                        try {
                            ...
                            log.debug("$id: [Attempt ${index + 1}/${transfers.size}] EXCEPTION: ${transfer.connectionMethod.transportType} - ${expected.message}")
                            null
                        }
                    }
                }
            ...
            val winner =
                select<IMdocTransfer<*>?> {
                    ...
                    error("All connection attempts failed. Check BLE permissions and device availability.")
                }
    ...
    override fun close() {
        ...
        try {
            ...
            log.error("$id: Error closing transfer instance", exception = expected)
        } finally {
            // TODO:
    ...
    override fun toString(): String =
        "TransferInstance(engagement=${engagement.id}, type=$type, id=$id, transmissionTypesSupported=${stringify(transmissionTypesSupported)}, connectionMethods=${
            stringify(
                connectionMethods,
            )
    ...
    fun dispatch(
        event: MdocRetrievalEvent,
        engagementEvent: com.sphereon.mdoc.engagement.MdocEngagementEvent?,
    ) {
        instanceCoroutineScope.launch {
            ...
            log.debug(
                "$id: Dispatching mdoc retrieval event with state: ${event.state} [isActive=$isActive, engagementState=${contextEvent?.state}, usingProvided=${engagementEvent != null}, usingStored=${engagementEvent == null && latestEngagementEvent.value != null}]",
            )

            // Wrap the event to include engagement event context if available
            val eventWithContext =
...
fun DataRetrievalTransmissionType.Companion.fromMdocTransfer(transfer: IMdocTransfer<*>): DataRetrievalTransmissionType =
    when (transfer.connectionMethod.transportType) {
