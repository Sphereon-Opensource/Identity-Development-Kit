...
class EngagementInstanceImpl(
    ...
    handoverBytes: ByteArray? = null,
) : EngagementInstance,
    SuspendableEngagement,
    MdocEngagementEvent.Dispatcher {
    @OptIn(ExperimentalUuidApi::class)
    override val id: Uuid = Uuid.random()
    override var handover: ByteArray? = handoverBytes
    ...
    private lateinit var _transferInstance: TransferInstance
    ...
    private suspend fun startInternal(): TransferManager {
        ...
        _transferInstance = transferManager.instance
                        ...
                        MdocEngagementState.CONNECTED -> {
                            ...
                            MdocEngagementEvent.Connected(
                                ...
                                deviceRetrievalMethod =
                                    data.getRetrievalMethods().firstOrNull()
                                        ?: error("No retrieval method available"),
                            )
                        }
    ...
    override suspend fun start(): TransferManager {
        ...
        return if (res.isOk) {
            ...
            throw res.error.toException().also { log.error("$logId: Error starting engagement: ${it.message}", exception = it) }
        }
    }
    ...
    override fun getConnectionMethods(supportedOnly: Boolean): Set<ConnectionMethod> {
        ...
        val transportRegistry =
            (transferFactory as? com.sphereon.mdoc.transfer.MdocTransferFactoryImpl)?.transportRegistry
                ?: error("Transfer factory must be MdocTransferFactoryImpl to access transport registry")
    ...
    override fun getReaderEngagement(): ReaderEngagement? = data.getReaderEngagement()

    override fun getDeviceEngagement(): CborEncodedItem<DeviceEngagement> = data.getDeviceEngagement()

    override fun tryOps(): EngagementInstance.Try =
        object : EngagementInstance.Try {
                ...
                try {
                    ...
                    IdkErrorResult(
                        IdkError.UNKNOWN_ERROR(
                            message = "Failed to start engagement: ${expected.message ?: expected::class.simpleName}",
                            exception = expected,
                        ),
                    )
                }
            ...
            override suspend fun getEngagementUri(): IdkResult<String, IdkErrorType> =
                try {
                    ...
                    IdkErrorResult(
                        IdkError.UNKNOWN_ERROR(
                            message = "Failed to generate engagement URI: ${expected.message ?: expected::class.simpleName}",
                            exception = expected,
                        ),
                    )
                }
            ...
            override suspend fun getEphemeralKey(): IdkResult<CoseKeyType, IdkErrorType> =
                try {
                    ...
                    IdkErrorResult(
                        IdkError.UNKNOWN_ERROR(
                            message = "Failed to get ephemeral key: ${expected.message ?: expected::class.simpleName}",
                            exception = expected,
                        ),
                    )
                }
    ...
    override fun isTransferInitialized(): Boolean = ::_transferInstance.isInitialized

    override val transferInstance: TransferInstance
        get() {
            check(::_transferInstance.isInitialized) {
    ...
    override fun close() {
        ...
        try {
            ...
            if (::_transferInstance.isInitialized) {
                _transferInstance.close()
            }
        } catch (expected: Throwable) {
            log.error("$logId: Error closing engagement instance: ${expected.message}", exception = expected)
        }
...
//            _events.emit(MdocEngagementEvent.Canceled(MdocRole.MDOC, engagementId = id))
            ...
            try {
                ...
                log.error("$logId: Error deleting ephemeral key with alias ${keyInfo.alias}: ${expected.message}", exception = expected)
            }
        }
    ...
    override fun getEngagementEventListeners(): Set<MdocEngagementEvent.Listener> = eventListeners.toSet()
    ...
    override fun toString(): String = "EngagementInstance(id=$id, currentState=$currentState)"
}
