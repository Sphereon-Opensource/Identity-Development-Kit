...
internal class SessionEstablishmentProcessor(
    ...
    suspend fun process(
        ...
        try {
            ...
            log.error("$logId: Error processing session establishment: ${expected.message}", exception = expected)
            IdkError
                .UNKNOWN_ERROR(
                    message = "Failed to process session establishment: ${expected.message}",
                    exception = expected,
                ).asErrorResult()
        }
    ...
    private fun logEReaderKey(sessionEstablishment: SessionEstablishment) {
        val eReaderKeyBytes = cborEncoder.encode(sessionEstablishment.encodedReaderKey)
        debugLogger?.logEReaderKeyBytes("EReaderKeyBytes from SessionEstablishment (Tag 24 wrapped)", eReaderKeyBytes)

        val eReaderKey = decodeReaderKey(sessionEstablishment)
        val eReaderKeyContent = coseKeyCborCodec.encode(eReaderKey).getOrElse { throw it.toException() }
        debugLogger?.logEReaderKey("EReaderKey content (COSE_Key)", eReaderKeyContent)
    }
    ...
    private fun buildSessionTranscript(
        ...
        val deviceEngagementBytes = cborEncoder.encode(deviceEngagementEncoded)
        ...
        val eDeviceKeyFromEngagement =
            deviceEngagementEncoded
                .data()
                .security.eDeviceKeyBytes
                .data()
        val eDeviceKeyBytes = coseKeyCborCodec.encode(CoseKey.fromDTO(eDeviceKeyFromEngagement)).getOrElse { throw it.toException() }
        ...
        val transcript =
            SessionTranscript(
                ...
                handover =
                    engagement.handover?.let { encodedHandover ->
                        handoverCborCodec.decode(encodedHandover).getOrElse { throw it.toException() }
                    } ?: QrHandover() as Handover<*, CborItem<*>>,
                original = null,
            )

        // Log raw SessionTranscript
        val rawTranscriptBytes = sessionTranscriptCborCodec.encode(transcript).getOrElse { throw it.toException() }
        debugLogger?.logSessionTranscript("Raw SessionTranscript (CBOR array, starts with 0x83)", rawTranscriptBytes)

        val encodedTranscript = sessionTranscriptCborCodec.encodeItem(transcript).getOrElse { throw it.toException() }

        // Per ISO 18013-5: SessionTranscriptBytes = #6.24(bstr .cbor SessionTranscript)
        val transcriptBytes = sessionTranscriptCborCodec.encodeTag24(transcript).getOrElse { throw it.toException() }
    ...
    private fun logDeviceEngagementBytesVerification(deviceEngagementEncoded: CborEncodedItem<*>) {
        ...
        val reEncodedDeviceEngagement = cborEncoder.encode(data.toCborItem())
    ...
    private fun logIosInteropDebugInfo(
        ...
        val eReaderKeyBytesForLog = cborEncoder.encode(sessionEstablishment.encodedReaderKey)
        debugLogger?.logText(
            "IOS_INTEROP_DEBUG",
            """
            ...
            |  - Handover: ${if (engagement.handover == null) {
                "null (QR)"
            } else {
                "${engagement.handover!!.size} bytes"
            }}
    ...
    private suspend fun resolveEphemeralKeyAndEcdh(
        ...
        val readerKey = decodeReaderKey(sessionEstablishment)
        ...
        return if (ephemeralKey.d == null) {
            ...
            if (alias != null) {
                ...
                val readerJwk =
                    com.sphereon.crypto.core.CoseJoseKeyMappingService
                        .toResolvedJwkKeyInfo(readerKeyInfo)
                        .key
                log.info("$logId: Attempting native keychain ECDH with alias: $alias")

                val result =
                    com.sphereon.crypto.kms
                        .performNativeKeychainEcdh(alias, readerJwk)
                if (result != null) {
    ...
    private suspend fun decryptDeviceRequest(
        ...
        return deviceRequestCborCodec
            .decode(deviceRequestData)
            .getOrElse { throw it.toException() }
            .value
    }

    private fun decodeReaderKey(sessionEstablishment: SessionEstablishment): CoseKey =
        coseKeyCborCodec
            .decode(sessionEstablishment.encodedReaderKey.value.taggedItem.value)
            .getOrElse { throw it.toException() }
            .value
}
