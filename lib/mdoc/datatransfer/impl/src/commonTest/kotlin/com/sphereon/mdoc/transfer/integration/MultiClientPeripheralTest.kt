...
class MultiClientPeripheralTest {
    ...
    @Test
    fun `session ID uniqueness for different devices`() = runTest {
    ...
    @Test
    fun `session ID persistence across operations for same device`() = runTest {
    ...
    @Test
    fun `concurrent clients can send different data`() = runTest {
    ...
    @Test
    fun `client disconnection removes session`() = runTest {
    ...
    @Test
    fun `one client disconnect does not affect others`() = runTest {
    ...
    @Test
    fun `session timeout cleanup`() = runTest {
        ...
        val staleSessions = sessions.filter { (_, lastActivity) ->
    ...
    @Test
    fun `activity update keeps session alive`() = runTest {
    ...
    @Test
    fun `maximum concurrent clients simulation`() = runTest {
        // Test with many concurrent clients to verify scalability
        val clientCount = 10
        val clients = (1..clientCount).map { i ->
    ...
    @Test
    fun `client reconnection gets new session ID`() = runTest {
    ...
    @Test
    fun `interleaved messages from multiple clients`() = runTest {
    ...
    @Test
    fun `peripheral notifications go to all subscribed clients`() = runTest {
        // Given - Multiple clients subscribed to notifications
        val subscribedClients = setOf(
            "device-01",
            "device-02",
            "device-03"
        )
    ...
    @Test
    fun `session ID not shared between different peripherals`() = runTest {
        // Document that session IDs are peripheral-specific

        // Peripheral 1
        val peripheral1Sessions = mutableMapOf(
            "device-A" to Uuid.random(),
            "device-B" to Uuid.random()
        )

        // Peripheral 2 (different physical device)
        val peripheral2Sessions = mutableMapOf(
    ...
    @Test
    fun `rapid connect-disconnect cycles`() = runTest {
    ...
    @Test
    fun `session cleanup does not affect active sessions`() = runTest {
    ...
    @Test
    fun `device address validation`() = runTest {
    ...
    @Test
    fun `session ID generation is random`() = runTest {
    ...
    @Test
    fun `concurrent client data does not intermix`() = runTest {
