...
val deviceEngagement = DeviceEngagement.fromEngagementUri(scannedQrUri)
...
1. **Scan Engagement:**
   ```kotlin
   val deviceEngagement = DeviceEngagement.fromEngagementUri(qrCodeData)
   ```
...
Decode with:
```kotlin
val de = DeviceEngagement.fromEngagementUri(uri)
```
