...
class EngagementConfiguration {
    ...
    fun getRetrievalMethodsWithSharedUuids(
        ...
        return _retrievalMethods.map { method ->
            if (method.retrievalOptions is BleOptions) {
                ...
                if (needsCentralUuid || needsPeripheralUuid) {
                    val updatedBleOptions = bleOptions.copy(
                        centralClientModeUuid = bleOptions.centralClientModeUuid ?: if (bleOptions.centralClientMode) sharedCentralClientUuid else null,
                        peripheralServerModeUuid = bleOptions.peripheralServerModeUuid ?: if (bleOptions.peripheralServerMode) sharedPeripheralServerUuid else null
                    )
...
class EngagementBuilder {
    private val _methods = mutableSetOf<MdocEngagementMethod>()
    ...
    fun build(): Set<MdocEngagementMethod> = _methods.toSet()
}

// Retrieval builder for grouping retrieval methods
@OptIn(ExperimentalObjCName::class)
@ObjCName("RetrievalBuilder", exact = true)
class RetrievalBuilder(private val _engagementMethods: Set<MdocEngagementMethod>) {
    private val _methods = mutableSetOf<DeviceRetrievalMethod>()
    ...
    fun website(init: WebsiteRetrievalBuilder.() -> Unit = {}) {
        val builder = WebsiteRetrievalBuilder(_engagementMethods)
        builder.init()
        _methods.add(builder.build())
    }

    fun oid4vp(init: Oid4vpRetrievalBuilder.() -> Unit = {}) {
        val builder = Oid4vpRetrievalBuilder(_engagementMethods)
        builder.init()
        _methods.add(builder.build())
    }

    fun build(): Set<DeviceRetrievalMethod> = _methods.toSet()
}
...
class ReaderEngagementBuilder {
    ...
    private var _readerEngagement: ReaderEngagement? = null

    fun withReaderEngagement(readerEngagement: ReaderEngagement) = apply {
        val scheme = if (readerEngagement.hasWebsiteRetrievalMethod) "mdoc://" else "mdoc:"
        withUri(readerEngagement.toEngagementUri(scheme))
        _readerEngagement = readerEngagement
    }
    ...
    fun build(): ReaderEngagementMethod {
        _readerEngagement?.let { return ReaderEngagementMethod(it) }

        if (dataUri.trim().isEmpty()) throw IllegalStateException("uri is not initialized. Cannot build reader engagement method")
        require(dataUri.startsWith("mdoc://") || (dataUri.startsWith("mdoc:") && !dataUri.startsWith("mdoc://"))) {
            "uri must start with 'mdoc://' (website) or 'mdoc:' (classic reverse engagement)"
        }
        return ReaderEngagementMethod(dataUri)
    }
}
...
class BleRetrievalBuilder {
    ...
    fun withVersion(version: DeviceRetrievalMethodVersion) = apply {
        this.version = version
    }

    fun withVersion1() = apply {
    ...
    fun build(): DeviceRetrievalMethod {
        ...
        return DeviceRetrievalMethod(
            type = DeviceRetrievalMethodType.BLE, version = version, retrievalOptions = BleOptions(
...
class NfcRetrievalBuilder {
    ...
    fun build(): DeviceRetrievalMethod = DeviceRetrievalMethod(
...
class WebsiteRetrievalBuilder(private val _engagementMethods: Set<MdocEngagementMethod>) {
    var uri: String?  = null
    ...
    fun build(): DeviceRetrievalMethod {
        if (uri.isNullOrEmpty()) {
            val engagementMethod =  _engagementMethods.first { it.type == EngagementType.TO_APP &&  (it as? ReaderEngagementMethod != null && it.readerEngagement.hasWebsiteRetrievalMethod)  } as? ReaderEngagementMethod
...
class Oid4vpRetrievalBuilder(private val _engagementMethods: Set<MdocEngagementMethod>) {
    /**
     ...
     */
    fun fromAuthorizationRequestUri(uri: String) = apply {
        require(uri.startsWith("mdoc-openid4vp://")) {
     ...
     */
    fun fromOid4vpEngagement() = apply {
        val engagementMethod = _engagementMethods
            .filterIsInstance<Oid4vpEngagementMethod>()
            .firstOrNull()
            ?: throw IllegalStateException("No OID4VP engagement method found")
    ...
    fun build(): DeviceRetrievalMethod {
        ...
        return DeviceRetrievalMethod(
            ...
            version = version,
            retrievalOptions = Oid4vpOptions(
                clientId = clientId,
