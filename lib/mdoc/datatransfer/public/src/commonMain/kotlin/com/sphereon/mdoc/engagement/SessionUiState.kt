...
data class SessionUiState(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (substateLabel != other.substateLabel) return false
        if (nfcMode != other.nfcMode) return false
        if (qrMode != other.qrMode) return false
        if (isSuspended != other.isSuspended) return false
        if (progressHint != other.progressHint) return false
        if (terminalOutcome != other.terminalOutcome) return false
        if (terminalMessage != other.terminalMessage) return false
        if (userInteractionRequired != other.userInteractionRequired) return false
        if (deviceRequest != null) {
            if (other.deviceRequest == null) return false
            if (!deviceRequest.contentEquals(other.deviceRequest)) return false
        } else if (other.deviceRequest != null) return false
...
data class ParsedDeviceRequest(
    ...
    companion object {
         ...
         */
        fun parse(deviceRequestBytes: ByteArray): ParsedDeviceRequest? {
            return try {
                val deviceRequest = com.sphereon.mdoc.data.device.DeviceRequest.Decoder.decodeCbor(deviceRequestBytes)

                val documents = deviceRequest.effectiveDocRequests().map { docReq ->
                    ParsedDocumentRequest(
                        docType = docReq.itemsRequest.docType.toString(),
                        nameSpaces = docReq.itemsRequest.nameSpaces.mapKeys { it.key.toString() }.mapValues { entry ->
                            entry.value.keys.map { it.toString() }.toSet()
                        },
 ...
 */
fun SessionUiState.parseDeviceRequest(): ParsedDeviceRequest? {
    return deviceRequest?.let { ParsedDeviceRequest.parse(it) }
}

/**
 * Alias for parseDeviceRequest() for backward compatibility.
 * @see parseDeviceRequest
 */
fun SessionUiState.getParsedDeviceRequest(): ParsedDeviceRequest? = parseDeviceRequest()
...
//    data object TransferConnectionSelected : SessionEvent
    ...
    data class UserInteractionRequired(val deviceRequest: ByteArray) : SessionEvent {
        override fun equals(other: Any?): Boolean {
            if (this === other) return true
            if (other == null || this::class != other::class) return false
        ...
        override fun hashCode(): Int {
            return deviceRequest.contentHashCode()
        }

        override fun toString(): String {
            return "UserInteractionRequired(deviceRequest=${deviceRequest.encodeToHex()})"
        }
    ...
    data class TransferProgress(val fraction: Float) : SessionEvent
