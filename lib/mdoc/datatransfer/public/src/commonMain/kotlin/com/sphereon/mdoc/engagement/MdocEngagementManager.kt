...
interface MdocEngagementManager : MdocEngagementFactory.Holder {
     ...
     */
    suspend fun enableAutoRestart(
        backgroundNfcConfig: (EngagementConfiguration.() -> Unit)? = null
     ...
     */
    suspend fun toApp(mdocUri: String, autoStart: Boolean = true): IdkResult<EngagementInstance, IdkError> {
        // Detect protocol by URI scheme
        val engagementResult =  when {
            // OID4VP protocol (ISO 18013-7 Annex B)
            mdocUri.startsWith("mdoc-openid4vp://") -> {
                val engagementResult = createEngagement {
                    engagement {
                        oid4vp {
                            fromUri(mdocUri)
                        }
            ...
            mdocUri.startsWith("mdoc:") && !mdocUri.startsWith("mdoc://") -> {
                ...
                val readerEngagement = com.sphereon.mdoc.transfer.reader.ReaderEngagement.fromEngagementUri(mdocUri)
                ...
                val engagementResult = createEngagement {
                    engagement {
                        reader {
                            // Use the URI - the ReaderEngagementMethod constructor will parse it
                            // and store the ReaderEngagement object
                            withUri(mdocUri)
                        }
                    }
            ...
            // Unknown protocol
                ...
                com.sphereon.core.api.IdkErrorResult(
                    com.sphereon.core.api.error.IdkError.ILLEGAL_ARGUMENT_ERROR(
                        message = "Unsupported URI scheme for toApp. Expected:\n" +
