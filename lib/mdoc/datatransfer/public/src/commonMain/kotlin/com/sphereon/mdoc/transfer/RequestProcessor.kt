...
interface RequestResponseProcessor {
     ...
     */
    suspend fun createDeviceResponse(deviceRequest: DeviceRequest, documentProvider: DocumentProvider? = null): IdkResult<DeviceResponse, IdkErrorType>
}


@OptIn(ExperimentalObjCName::class)
@ObjCName("SimpleDocumentRequestSingleDocumentSelector", exact = true)
class SimpleDocumentRequestSingleDocumentSelector(val log: LogService, val globalCustomSelectorData: Any? = null, val globalDocumentsSupplier: DocumentProvider? = null) :
    ...
    override suspend fun select(
        ...
        if (matches.isEmpty()) {
            return IdkError.NOT_FOUND_ERROR(resource = docType.toString()).asErrorResult().also { log.error("No mdoc found for request ${it.error}") }
...
data class MapDrivenDocRequestSelector(
...
) : DocumentRequestSingleDocumentSelector, RequestDocumentsSelector, DocumentProvider, RequestResponseProcessor {
    ...
    override suspend fun select(
        ...
        return selections[docRequest]?.let { docRequest to it }.asOkResult()
    }
    ...
    override suspend fun getDocuments(selectorData: Any?): Set<DocumentWithKeyAlias> {
        return selections.values.toSet()
    }
    ...
    override suspend fun createDeviceResponse(
        ...
        if (documentProvider != null && documentProvider != this) return IdkError.ILLEGAL_ARGUMENT_ERROR(
            arg = "Documents supplier provided not supported by MapDrivenDocRequestSingleDocumentSelector"
        ).asErrorResult()
...
class SimpleRequestDocumentsSelector(
    ...
    override suspend fun selectDocuments(
        ...
        val finalDocumentsSupplier =
            documentProvider ?: globalDocumentsProvider ?: return IdkError.ILLEGAL_ARGUMENT_ERROR(arg = "No documents supplier provided")
                .asErrorResult()
        ...
        if (docRequests.isEmpty() && minDocRequests > 0) {
            return IdkError.ILLEGAL_ARGUMENT_ERROR(arg = "Unsupported device request: empty or null doc requests").asErrorResult()
                .also { log.error(it.error.message.defaultMessage) }
        }
        val selected =
            docRequests.map { req ->
...
open class SimpleRequestResponseProcessor(
    ...
    override suspend fun createDeviceResponse(deviceRequest: DeviceRequest, documentProvider: DocumentProvider?): IdkResult<DeviceResponse, IdkErrorType> {
        ...
        val requestsToDocuments =
            documentsSelector.selectDocuments(
                deviceRequest = deviceRequest,
                minDocRequests = this.minDocRequests ?: effectiveDocRequests.size, finalDocumentsSupplier
            )
                .onFailure { return it.asErrorResult() }.value

        val documents: Array<Document> = if (mdocDeviceSignService == null) {
            ...
            for ((req: DocRequest, docWithKeyAlias: DocumentWithKeyAlias) in requestsToDocuments) {
                ...
                val signedDoc = mdocDeviceSignService.deviceSignDocument(
                    request = req,
                    document = doc,
        ...
        return DeviceResponse.Builder().withDocuments(documents).build().asOkResult()
    }
