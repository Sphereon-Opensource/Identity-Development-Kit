...
class SessionUiProjector(
     ...
     */
    fun sessionEvents(): Flow<SessionEvent> = merge(
        engagementEvents
            .filter { event ->
                ...
                val isTerminalEvent = event is MdocEngagementEvent.Disconnected ||
            ...
            val sessionEvent = when (engagementEvent) {
                is MdocEngagementEvent.QrShow -> SessionEvent.QrShow
                is MdocEngagementEvent.NfcEngagement -> SessionEvent.NfcPromptShown
...
//                    }
                ...
                is MdocEngagementEvent.Error -> SessionEvent.Terminal(
                    outcome = TerminalOutcome.ERROR,
                    message = engagementEvent.reason
                )
                is MdocEngagementEvent.Canceled -> SessionEvent.Terminal(
        ...
        retrievalEvents
            .filter { event ->
                ...
                log.debug("Retrieval event filter: ${event::class.simpleName} [engagementId=${event.engagementId}, activeEng=${activeEng?.id}, idMatches=$idMatches, eventIsActive=$eventIsActive, isFinal=$isFinalEvent, willPass=$willPass, state=${event.state}]")
            ...
            val sessionEvent = when (actualEvent) {
                ...
                is MdocRetrievalEvent.Error -> SessionEvent.Terminal(
                    outcome = TerminalOutcome.ERROR,
                    message = actualEvent.error.message ?: "Transfer error"
                )
                is MdocRetrievalEvent.Terminated -> SessionEvent.Terminal(
                    outcome = TerminalOutcome.SUCCESS,
            ...
            .runningFold(initial) { accumulator, event ->
                ...
                fun computeQrMode(): QrMode {
                    ...
                    return when {
                        ...
                        else -> QrMode.NONE  // TO_APP doesn't set SCAN - that's UI-managed before engagement
                    }
                }
                ...
                log.debug("Computed derived state: nfcState=$nfcMode, qrMode=$qrMode, suspended=$suspended [nfcEng=${nfcEngagement != null}, qrEng=${engagementsByType.value[EngagementType.QR] != null}, active=${activeEngagement.value?.id}]")
                ...
                when (event) {
                    SessionEvent.QrShow -> {
                        ...
                        val label = if (qrMode == QrMode.SCAN) {
                    ...
                    is SessionEvent.TransferProgress -> {
                        ...
                        val label = if (progress >= 0.9f) "Data sent" else "Transferring…"
                        ui = ui.copy(
                            phase = UiPhase.TRANSFER,
                            substateLabel = label,
                            nfcMode = nfcMode,
