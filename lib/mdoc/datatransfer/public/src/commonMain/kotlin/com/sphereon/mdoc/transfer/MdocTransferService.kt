...
data class QREngagementMethod(val scheme: String = "mdoc:") : MdocEngagementMethod {
...
data class Oid4vpEngagementMethod(val authorizationRequestUri: String) : MdocEngagementMethod {
...
data class ReaderEngagementMethod(val readerEngagement: ReaderEngagement) : MdocEngagementMethod {
    override val type: EngagementType = EngagementType.TO_APP
    ...
    /**
     * Parse reader engagement from app-to-app URI.
     *
     * @param engagementUri The URI starting with `mdoc:` or `mdoc://` depending on retrieval type
     */
    constructor(engagementUri: String) : this(ReaderEngagement.fromEngagementUri(engagementUri))
    ...
    fun getEngagementDataUri(): String {
        ...
        return readerEngagement.toEngagementUri(scheme)
    }
}
...
//        holderCentral: MdocHolderCentralClientTransfer,
    ) : this(keyManager, retrievalMethods, listeners, context/*, holderCentral*/) {
    ...
    protected suspend fun startQrEngagementImpl(): String {
        ...
        val engagementDataService =
            this.engagementDataService ?: EngagementData.HolderBuilder().withRetrievalMethods(retrievalMethods.toSet()).withEphemeralKey(ephemeralDeviceKey).build()
        this.engagementDataService = engagementDataService
    ...
    override fun addEngagementEventListener(vararg listener: MdocEngagementEvent.Listener) = apply {
        listeners.addAll(listener)
    }

    override fun removeEngagementEventListener(listener: MdocEngagementEvent.Listener) = apply {
        listeners.remove(listener)
    }

    override fun clearEngagementEventListeners() = apply {
        listeners.clear()
    }
