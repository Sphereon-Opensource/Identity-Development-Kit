...
fun MdocEventHub.observeQrLifecycle(
    ...
    return scope.launch {
        engagementEvents.collect { event ->
            when (event) {
                is MdocEngagementEvent.QrShow -> onShow(event.qrCodeData)
                is MdocEngagementEvent.QrHide -> onHide()
...
fun MdocEventHub.observeConnectionStatus(
    ...
    return scope.launch {
        engagementEvents.collect { event ->
            when (event) {
                is MdocEngagementEvent.Connecting -> onConnecting()
                is MdocEngagementEvent.Connected -> onConnected()
                is MdocEngagementEvent.Disconnected -> onDisconnected(event.reason)
                else -> { /* ignore other events */ }
            }
        }
    }
...
fun MdocEventHub.observeErrors(
    ...
    return scope.launch {
        allEvents.collect { event ->
            when (event) {
                is MdocEngagementEvent.Error -> onError(event.reason, event.error)
                is MdocRetrievalEvent.Error -> onError("Transfer error: ${event.data.decodeToString()}", event.error)
                else -> { /* ignore other events */ }
            }
        }
    }
