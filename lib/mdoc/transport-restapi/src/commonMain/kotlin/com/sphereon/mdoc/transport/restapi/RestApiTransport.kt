...
import com.sphereon.mdoc.engagement.toDeviceEngagementMessage
                ...
                MdocRole.MDOC -> {
                    ...
                    try {
                        ...
                        val data = engagementData
                            ?: return@withLock Err(
                                RestApiError.MissingEngagementData(
                        ...
                        val readerEngagement = data.getReaderEngagement()
                            ?: return@withLock Err(
                                RestApiError.MissingEngagementData(
                        ...
                        val originalBytes = deviceEngagementEncoded.encodeCbor()
...
//                        log.debug("ORIGINAL eDeviceKey has kid: ${originalEDeviceKey.kid != null}, alg: ${originalEDeviceKey.alg != null}, key_ops: ${originalEDeviceKey.key_ops != null}")
                        ...
                        val deviceEngagementMessage = deviceEngagementEncoded.toDeviceEngagementMessage()
...
//                        val sessionData : SessionData = SessionData.decodeCbor(sessionEstablishmentBytes)
//                        val sessionEstablishment = SessionEstablishment.Decoder.decodeCbor(sessionEstablishmentBytes)
                        ...
                        val readerEngagementOriginal = readerEngagement.original
                            ?: throw IllegalStateException("ReaderEngagement original bytes not available")

                        // Check if bytes are already Tag 24 wrapped (0xD8 0x18)
                        val isTag24Wrapped = readerEngagementOriginal.size >= 2 &&
                                readerEngagementOriginal[0] == 0xD8.toByte() &&
                                readerEngagementOriginal[1] == 0x18.toByte()

                        val readerEngagementBytesForHash = if (isTag24Wrapped) {
                            // Already wrapped - use as-is
                            log.debug("ReaderEngagement is Tag 24 wrapped (${readerEngagementOriginal.size} bytes)")
                            readerEngagementOriginal
                        } else {
                            // NOT wrapped - manually wrap with Tag 24
                            // Create CborEncodedItem with the unwrapped bytes and encode it (adds Tag 24)
                            log.debug("ReaderEngagement is NOT Tag 24 wrapped, wrapping manually")
                            val wrappedItem = CborEncodedItem(
                                value = readerEngagementOriginal,
                                data = readerEngagement
                            )
                            val wrappedBytes = wrappedItem.encodeCbor()
                            log.debug("Wrapped ReaderEngagement (${wrappedBytes.size} bytes)")
                            wrappedBytes
                        }
                        ...
                        log.debug(restApiHandover.encodeCbor().encodeToHex())
                        ...
                        @Suppress("UNCHECKED_CAST") val sessionTranscript = SessionTranscript(
                        ...
                        val sessionTranscriptBytes = sessionTranscript.toCborEncodedItem().encodeCbor()
                        ...
                        val sessionEncryptionInstance = SessionEncryption.Builder()
                        ...
                        val deviceRequestData = decryptResult.sessionData.data?.value
                        ...
                        val parsedDeviceRequest = DeviceRequest.decodeCbor(deviceRequestData)
                        ...
                        log.error("Failed to open REST API connection", exception = e)
                        Err(RestApiError.ConnectionFailed(uri, e).toIdkError())
                    }
                }
    ...
    private fun verifyKeyTypesAndCurves(deviceEngagement: DeviceEngagement, readerEngagement: ReaderEngagement): IdkResult<Unit, IdkErrorType> {
        ...
        mutex.withLock {
            ...
            when (role) {
                MdocRole.MDOC -> {
                    ...
                    try {
                        val encryption = sessionEncryption
                        ...
                        log.error("Failed to send DeviceResponse", exception = e)
                        throw RestApiError.NetworkError(uri, e)
                    }
                }
    ...
    private fun verifyCipherSuite(deviceEngagement: DeviceEngagement, readerEngagement: ReaderEngagement): IdkResult<Unit, IdkErrorType> {
        ...
        if (deviceEngagement.security.cipherSuite != 1u) {
            ...
            return Err(
                RestApiError.ConnectionFailed(
                    uri = uri,
                    throwable = IllegalStateException("DeviceEngagement cipher suite not supported: device=${deviceEngagement.security.cipherSuite}, reader=${readerEngagement.security.cipherSuite}")
                ).toIdkError()
            )
            ...
            return Err(
                RestApiError.ConnectionFailed(
                    uri = uri,
                    throwable = IllegalStateException("ReaderEngagement cipher suite not supported: device=${deviceEngagement.security.cipherSuite}, reader=${readerEngagement.security.cipherSuite}")
                ).toIdkError()
            )
