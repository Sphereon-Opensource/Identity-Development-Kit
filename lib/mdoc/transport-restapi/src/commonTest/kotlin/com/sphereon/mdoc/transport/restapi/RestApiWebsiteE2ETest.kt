...
class RestApiWebsiteE2ETest {
    ...
    /**
     * **TEST 1: Low-Level REST API Transport (Direct Component Testing)**
     *
     ...
     */
    @Test
    fun test_rest_api_transport_low_level_flow() = runTest {
        // ===== READER SETUP =====
        ...
        val readerEngagement = ReaderEngagement.V1_0(
            security = ReaderEngagementSecurity(
                cipherSuite = 1u, // ECDH-ES + HKDF-256 + AES-256-GCM
                eReaderKeyBytes = CborEncodedItem.fromData(readerKey)
            ),
        ...
        val mdocUri = readerEngagement.toEngagementUri()
        log.info("Generated mdoc:// URI for Reader Engagement: $mdocUri")

        // ===== MOCK READER SERVER =====
        val mockClient = createMockReaderServer(readerUri) { request ->
        ...
        val parsedEngagement = ReaderEngagement.fromEngagementUri(mdocUri)
     ...
     */
    @Test
    fun test_toApp_engagement_with_transfer_manager_full_flow() = runTest {
        ...
        val readerEngagement = ReaderEngagement.V1_0(
            security = ReaderEngagementSecurity(
                cipherSuite = 1u,
                eReaderKeyBytes = com.sphereon.cbor.CborEncodedItem.fromData(readerKey)
            ),
            deviceRetrievalMethods = arrayOf(
                DeviceRetrievalMethod(
                    type = DeviceRetrievalMethodType.WEBSITE,
     ...
     */
    @Test
    fun test_toApp_handles_server_error_gracefully() = runTest {
    ...
    fun test_parse_reader_engagement_with_multiple_methods() {
        ...
        val readerEngagement = ReaderEngagement.V1_0(
            ...
            deviceRetrievalMethods = arrayOf(
                ...
                DeviceRetrievalMethod(
                    ...
                    retrievalOptions = com.sphereon.mdoc.transfer.device.BleOptions(
        ...
        val mdocUri = readerEngagement.toEngagementUri()
        val parsed = ReaderEngagement.fromEngagementUri(mdocUri)
    ...
    private fun createSessionExecution(): SessionExecution {
        val sessionId = kotlin.uuid.Uuid.random().toString()

        // Create mock TenantContextData
        val tenantContextData = object : com.sphereon.di.context.TenantContextData {
            override val tenantId: String = "test-tenant"
        }

        // Create mock UserContext
        val userContext = object : com.sphereon.di.context.UserContext {
        ...
        val sessionContext = object : com.sphereon.di.session.SessionContext {
        ...
        val sessionContextManager = object : com.sphereon.di.session.SessionContextManager {
            ...
            override fun getById(sessionId: String, makeActive: Boolean) = null
            ...
            override fun createOrGetFromId(sessionId: String, makeActive: Boolean) = throw NotImplementedError()
        ...
        val contextConfig = object : com.sphereon.core.api.context.ContextConfig {
        ...
        val sessionLogManager = object : SessionLogManager {
            override suspend fun setGlobalConfig(config: LoggerConfig) = this
            override suspend fun getGlobalConfig() = LoggerConfig.Default
            override fun withTagAsync(tag: String, config: LoggerConfig?) = throw NotImplementedError()
            override fun withTag(tag: String, config: LoggerConfig?) = throw NotImplementedError()
        }

        // Create SessionLogService wrapper around the test log
        val sessionLogService = object : SessionLogService {
