...
class Tag24WrappingTest {
    ...
    @Test
    fun testManualTag24WrappingMatchesExpected() {
        ...
        // Manually wrap with Tag 24 using CborEncodedItem
        val wrappedItem = CborEncodedItem(
            value = unwrappedBytes,
            data = readerEngagement
        )
        val wrappedBytes = cborSerializer.encode(wrappedItem)
    ...
    fun testHashMatchesLoggedValue() {
        ...
        // Now wrap and hash (this is what we SHOULD do)
        val wrappedItem = CborEncodedItem(
            value = unwrappedBytes,
            data = readerEngagement
        )
