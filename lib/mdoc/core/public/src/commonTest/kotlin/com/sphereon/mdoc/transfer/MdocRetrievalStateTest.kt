...
class MdocRetrievalStateTest {
    ...
    fun testInitState() {
        ...
        assertEquals(MdocRetrievalState.INIT, state.asEngagementState)
    }
    ...
    fun testTransmissionTypeSelectedState() {
        ...
        assertEquals(MdocRetrievalState.TRANSMISSION_TYPE_SELECTED, state.asEngagementState)
    }
    ...
    @Test
    fun testStateOrderProgression() {
        // Verify that states have increasing order values for the happy path
    ...
    @Test
    fun testSessionDataStatesOrdering() {
        assertTrue(MdocRetrievalState.SESSION_DATA_SEND.order < MdocRetrievalState.SESSION_DATA_RECEIVED.order)
    }
        ...
        for (terminal in terminalStates) {
            for (nonTerminal in nonTerminalStates) {
                assertTrue(terminal.order > nonTerminal.order,
                    "Terminal state ${terminal.state} should have higher order than ${nonTerminal.state}")
            }
        }
    }
