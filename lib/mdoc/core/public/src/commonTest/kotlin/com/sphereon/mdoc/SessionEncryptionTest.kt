...
import com.sphereon.mdoc.testutil.coseCurveAsCborUInt
import com.sphereon.mdoc.testutil.coseKeyTypeAsCborUInt
...
class SessionEncryptionTest {
    ...
    private fun createSessionTranscriptBytes(
        holderKey: CoseKey,
        readerKey: CoseKey,
    ): ByteArray {
        val eReaderKeyEncoded = CborEncodedItem(encodeCoseKey(readerKey), readerKey)
        ...
        return sessionTranscriptCborCodec.encodeTag24(sessionTranscript).getOrThrow()
    }

    private fun newBuilder(): SessionEncryption.Builder =
    ...
    @Test
    fun testBuildSessionEncryptionAsMdoc() =
        runTest {
    ...
    @Test
    fun testBuildSessionEncryptionAsMdocReaderWithPreComputedSecret() =
        runTest {
    ...
    @Test
    fun testBuildWithPreComputedSharedSecret() =
        runTest {
            val holderPublicKey = createHolderPublicKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderPublicKey, readerPublicKey)
    ...
    @Test
    fun testSessionEncryptionEncrypt() =
        runTest {
            val holderPublicKey = createHolderPublicKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderPublicKey, readerPublicKey)
    ...
    @Test
    fun testSessionEncryptionEncryptWithStatus() =
        runTest {
            val holderKey = createHolderKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderKey, readerPublicKey)
    ...
    @Test
    fun testSessionEncryptionEncryptAsSessionData() =
        runTest {
            val holderKey = createHolderKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderKey, readerPublicKey)
    ...
    @Test
    fun testSessionEncryptionEncryptAsSessionDataWithTermination() =
        runTest {
            val holderKey = createHolderKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderKey, readerPublicKey)
    ...
    @Test
    fun testMdocReaderEncryptAsSessionEstablishment() =
        runTest {
            val holderPublicKey = createHolderPublicKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderPublicKey, readerPublicKey)
    ...
    @Test
    fun testEncryptAsSessionEstablishmentFailsForMdoc() =
        runTest {
            val holderKey = createHolderKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderKey, readerPublicKey)
    ...
    fun testRoundTripEncryptDecrypt() =
        runTest {
            ...
            // Holder decrypts
            val decryptedAtHolder = holderSession.decryptSessionEstablishment(sessionEstablishmentCborCodec.encode(encryptedFromReader).getOrThrow())
    ...
    @Test
    fun testSessionEncryptionProperties() =
        runTest {
    ...
    @Test
    fun testSessionEncryptionCryptoProvider() =
        runTest {
            val holderKey = createHolderKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderKey, readerPublicKey)
    ...
    fun testDecryptSessionData() =
        runTest {
            ...
            val decrypted = holderSession.decrypt(sessionEstablishmentCborCodec.encode(establishment).getOrThrow())
            assertNotNull(decrypted.sessionData)
        }

    @Test
    fun testEncryptStatusOnlyMessage() =
        runTest {
            val holderKey = createHolderKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderKey, readerPublicKey)
            ...
            assertNotNull(encrypted)
            val decoded = sessionDataCborCodec.decode(encrypted).getOrThrow().value
            assertNull(decoded.data)
            assertEquals(SessionDataStatus.SESSION_TERMINATION, decoded.getStatus())
        }

    @Test
    fun testDecryptSessionDataThrowsForSessionEstablishment() =
        runTest {
            val holderKey = createHolderKey()
            val holderPublicKey = createHolderPublicKey()
            val readerPublicKey = createReaderPublicKey()
            ...
            val result = holderSession.decryptSessionData(sessionEstablishmentCborCodec.encode(establishment).getOrThrow())
            assertNotNull(result)
        }

    @Test
    fun testDecryptSessionEstablishmentThrowsWhenNoEstablishment() =
        runTest {
            val holderPublicKey = createHolderPublicKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderPublicKey, readerPublicKey)
            ...
            assertFailsWith<IllegalStateException> {
                readerSession.decryptSessionEstablishment(sessionDataCborCodec.encode(sessionData).getOrThrow())
            }
        }

    @Test
    fun testDecryptStatusOnlyMessage() =
        runTest {
            val holderKey = createHolderKey()
            val holderPublicKey = createHolderPublicKey()
            val readerPublicKey = createReaderPublicKey()
            ...
            holderSession.decrypt(sessionEstablishmentCborCodec.encode(establishment).getOrThrow())
    ...
    @Test
    fun testSessionEncryptionToStringAfterEstablishment() =
        runTest {
            val holderPublicKey = createHolderPublicKey()
            val readerPublicKey = createReaderPublicKey()
            val sessionTranscriptBytes = createSessionTranscriptBytes(holderPublicKey, readerPublicKey)
            ...
            val debugLogger =
                object : IMdocDebugLogger {
                    ...
                    override fun logDeviceEngagement(
                        ...
                        loggedCalls.add("logDeviceEngagement")
                    }
                    ...
                    override fun logDeviceEngagementBytes(
                        ...
                        loggedCalls.add("logDeviceEngagementBytes")
                    }
                    ...
                    override fun logEDeviceKeyForEcdh(
                        ...
                        loggedCalls.add("logEDeviceKeyForEcdh")
                    }
                    ...
                    override fun logSessionEstablishment(
                        ...
                        loggedCalls.add("logSessionEstablishment")
                    }
                    ...
                    override fun logEReaderKeyBytes(
                        ...
                        loggedCalls.add("logEReaderKeyBytes")
                    }
                    ...
                    override fun logSessionTranscript(
                        ...
                        loggedCalls.add("logSessionTranscript")
                    }
                    ...
                    override fun logSessionTranscriptBytes(
                        ...
                        loggedCalls.add("logSessionTranscriptBytes")
                    }
                    ...
                    override fun logDeviceRequest(
                        ...
                        loggedCalls.add("logDeviceRequest")
                    }
                    ...
                    override fun logDeviceResponse(
                        ...
                        loggedCalls.add("logDeviceResponse")
                    }
                    ...
                    override fun logSessionData(
                        ...
                        loggedCalls.add("logSessionData")
                    }
