...
class IssuerSignedTest {
    ...
    private fun createTestMso(valueDigests: Map<NameSpace, Map<DigestID, ByteArray>> = emptyMap()): MobileSecurityObject =
    ...
    private fun createTestIssuerAuth(mso: MobileSecurityObject = createTestMso()) = createIssuerAuthForTest(mso)
    ...
    private fun createIssuerSignedItem(
        ...
        return encodeIssuerSignedItemAsEncoded(item as IssuerSignedItem<Any>)
    }
    ...
    fun testIssuerSignedCreationWithNamespaces() {
        ...
        assertNotNull(issuerSigned.issuerAuth.payload)
    }
    ...
    @Test
    fun testIssuerSignedEncodeWithOriginal() {
        val nameSpaces = createNameSpaces()
        val issuerAuth = createTestIssuerAuth()
        ...
        val encoded = encodeIssuerSignedWithCodec(issuerSigned)
        assertTrue(originalBytes.contentEquals(encoded))
    }

    @Test
    fun testIssuerSignedEncodeWithoutOriginal() {
        val nameSpaces = createNameSpaces()
        val issuerAuth = createTestIssuerAuth()
    ...
    @Test
    fun testIssuerSignedToCborStructure() {
        val nameSpaces = createNameSpaces()
        val issuerAuth = createTestIssuerAuth()
    ...
    fun testIssuerSignedToDocument() {
        ...
        val document = Document(docType = mdlDocType, issuerSigned = issuerSigned, deviceSigned = null, original = null)
    ...
    fun testRandomValueToCborStructure() {
        val bytes = ByteArray(24) { 0x42.toByte() }
        val randomValue = RandomValue(bytes)
        val cbor = randomValue.toCborItem()
        assertNotNull(cbor)
    ...
    fun testRandomValueFromCborStructure() {
        ...
        val randomValue = RandomValue.fromCborItem(cborByteString)
        assertTrue(bytes.contentEquals(randomValue.value))
    }
    ...
    fun testMsoBuilderWithDocType() {
        val builder =
            IssuerSigned
                .MsoBuilder(issuerSignedItemCborCodec = issuerSignedItemCodec())
    ...
    fun testMsoBuilderWithSigned() {
        val builder = IssuerSigned.MsoBuilder(issuerSignedItemCborCodec = issuerSignedItemCodec())
        val signedTime =
            com.sphereon.core.compat.DateTimeUtils.DEFAULTS
                .dateTimeLocal()
    ...
    fun testMsoBuilderWithSigningKeyInfo() {
        ...
        val coseKey = createTestCoseKey()
        val keyInfo =
            com.sphereon.crypto.core.ManagedKeyInfo<CoseKey>(
                alias = "test-key",
