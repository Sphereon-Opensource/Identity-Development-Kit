...
class DeviceRequestTest {
    ...
    @Test
    fun testDeviceRequestCodecEncode() {
        val request = DeviceRequest(original = null)
        val encoded = encodeDeviceRequestWithCodec(request)
    ...
    fun testDeviceRequestEncodeDecode() {
        val request = DeviceRequest(original = null)
        val encoded = encodeDeviceRequestWithCodec(request)
        val decoded = decodeDeviceRequestWithCodec(encoded)
    ...
    @Test
    fun testDeviceRequestCodecEncodeWithDocRequests() {
        val mdlNamespace = NameSpace("org.iso.18013.5.1")
        val mdlDocType = DocType("org.iso.18013.5.1.mDL")
        ...
        assertNotNull(encoded)
        assertEquals(1, decoded.effectiveDocRequests().size)
        assertEquals(mdlDocType, decoded.effectiveDocRequests().single().getDocType())
    }
    ...
    @Test
    fun testDeviceRequestWithOid4vpRequestCodecRoundTrip() {
        val oid4vpRequest = createOid4vpPresentationDefinition()
        val request =
            DeviceRequest(
                oid4vpRequest = oid4vpRequest,
        ...
        assertNotNull(encoded)
        assertNotNull(decoded.oid4vpRequest)
    }
