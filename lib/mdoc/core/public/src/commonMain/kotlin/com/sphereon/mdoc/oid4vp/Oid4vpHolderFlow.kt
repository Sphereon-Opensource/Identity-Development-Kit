...
class Oid4vpHolderFlow(
    ...
    suspend fun processAuthorizationRequest(
        ...
        val matchedDocuments = oid4vpService.matchDocumentsAndDescriptors(
            mdocNonce = mdocNonce,
        ...
        val deviceResponse = oid4vpService.createDeviceResponse(
 ...
 *
 * @property deviceResponse The signed device response (as base64url-encoded string for vp_token)
...
data class Oid4vpHolderResult(
    ...
    /**
     * Get the vp_token value for the Authorization Response.
     * Per ISO 18013-7 B.4.3.2, this is the base64url-encoded DeviceResponse.
     */
    fun getVpToken(): String {
        return deviceResponse.encodeToBase64UrlNoPadding()
    }
}
...
class Oid4vpHolderFlowBuilder {
    ...
    fun withOid4vpService(service: MdocOid4vpService) = apply {
        this.oid4vpService = service
    }

    fun withLogService(service: LogService) = apply {
...
/**
 * Extension to encode DeviceResponse as base64url without padding for vp_token.
 */
