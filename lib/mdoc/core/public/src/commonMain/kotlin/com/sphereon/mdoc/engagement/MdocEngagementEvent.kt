...
sealed interface MdocEngagementEvent : MdocEvent {
    ...
    abstract class AbstractEngagementEvent : MdocEngagementEvent {
        ...
        override fun toString(): String {
            return "${this::class.simpleName}(role=$role, engagementId=$engagementId, state=$state, time=$time, isActive=$isActive)"
        }
    }
    ...
    data class Initializing(override val role: MdocRole, override val engagementId: Uuid) : AbstractEngagementEvent() {
        /**
    ...
    data class Start(override val role: MdocRole, override val engagementId: Uuid) : AbstractEngagementEvent() {
        /**
         * Represents the initial state of an engagement within the `Initializing` phase.
         * This state is set to `MdocTransferSessionState.INIT`, indicating that the engagement
         * process has just started and no further steps have been executed yet.
    ...
    data class QrShow(
        override val role: MdocRole, val qrCodeData: String, val engagement: DeviceEngagement,
...
// TODO: Does not depend on role. Should depend on forward/reverse engagement
        override val state = if (ble) (if (role == MdocRole.MDOC_READER) MdocEngagementState.BLE_ADVERTISING else MdocEngagementState.BLE_SCANNING) else MdocEngagementState.INIT
    ...
    data class NfcEngagement(
        override val role: MdocRole, val engagement: DeviceEngagement,
        override val engagementId: Uuid,
    ) : AbstractEngagementEvent() {
        /**
...
// TODO: Does not depend on role. Should depend on forward/reverse engagement
    ...
    data class RestApiEngagement(
        override val role: MdocRole, val readerEngagement: ReaderEngagement? = null, val uri: String,
        override val engagementId: Uuid,
    ) : AbstractEngagementEvent() {
        override val state = MdocEngagementState.INIT
     ...
     */
    data class Debug(override val role: MdocRole, val message: String, override val engagementId: Uuid, override val state: MdocEngagementState) : AbstractEngagementEvent() {
        /**
    ...
    data class Connecting(override val role: MdocRole, override val engagementId: Uuid, val deviceRetrievalMethods: Array<DeviceRetrievalMethod>) :
        AbstractEngagementEvent() {
        /**
     ...
     */
    data class Connected(override val role: MdocRole, override val engagementId: Uuid, val deviceRetrievalMethod: DeviceRetrievalMethod) : AbstractEngagementEvent() {
        /**
    ...
    data class Canceled(override val role: MdocRole, val reason: String? = null, override val engagementId: Uuid) : AbstractEngagementEvent() {
        /**
    ...
    data class Disconnected(override val role: MdocRole, val reason: String, override val engagementId: Uuid) : AbstractEngagementEvent() {
        /**
    ...
    data class Error(override val role: MdocRole, val reason: String, val error: Throwable? = null, override val engagementId: Uuid) : AbstractEngagementEvent() {
        /**
    ...
    data class Data(override val role: MdocRole, override val engagementId: Uuid, val data: ByteArray, val direction: Direction) : AbstractEngagementEvent() {
        /**
        ...
        override fun toString(): String {
            return "Data(role=$role, engagementId=$engagementId, data=${data.encodeTo(Encoding.HEX).chunked(2000).joinToString(",")}, direction=$direction, state=$state, time=$time)"
        }
    ...
    data class Transfer(override val role: MdocRole, override val engagementId: Uuid, val transferId: Uuid): AbstractEngagementEvent() {
        /**
         * Represents the current state of the engagement event.
         * This specific state indicates that the engagement is established and connected.
         */
...
//    @JsExportCompat
    ...
    interface Listener {
         ...
         */
        suspend fun onData(event: Data, direction: Data.Direction)
    ...
    abstract class ListenerAdapter : Listener {
         ...
         */
        override suspend fun onInitializing(event: Initializing) {}
         ...
         */
        override suspend fun onQrShow(event: QrShow) {}
         ...
         */
        override suspend fun onQrHide(event: QrHide) {}
         ...
         */
        override suspend fun onNfcEngagement(event: NfcEngagement) {}

        override suspend fun onRestApiEngagement(event: RestApiEngagement) {}
         ...
         */
        override suspend fun onDebug(event: Debug) {}
         ...
         */
        override suspend fun onConnecting(event: Connecting) {}
         ...
         */
        override suspend fun onConnected(event: Connected) {}
         ...
         */
        override suspend fun onData(event: Data, direction: Data.Direction) {}
         ...
         */
        override suspend fun onCanceled(event: Canceled) {}
         ...
         */
        override suspend fun onDisconnected(event: Disconnected) {}
         ...
         */
        override suspend fun onError(event: Error) {}
    }
...
suspend fun Collection<MdocEngagementEvent.Listener>.onEngagementEvent(event: MdocEngagementEvent) {
    ...
    toList().forEach {
        when (event) {
            is MdocEngagementEvent.Initializing -> it.onInitializing(event)
            is MdocEngagementEvent.Start -> it.onStart(event)
            is MdocEngagementEvent.QrShow -> it.onQrShow(event)
            is MdocEngagementEvent.QrHide -> it.onQrHide(event)
            is MdocEngagementEvent.NfcEngagement -> it.onNfcEngagement(event)
            is MdocEngagementEvent.RestApiEngagement -> it.onRestApiEngagement(event)
            is MdocEngagementEvent.Debug -> it.onDebug(event)
            is MdocEngagementEvent.Connecting -> it.onConnecting(event)
            is MdocEngagementEvent.Connected -> it.onConnected(event)
            is MdocEngagementEvent.Data -> it.onData(event, event.direction)
            is MdocEngagementEvent.Transfer -> it.onTransfer(event)
            is MdocEngagementEvent.Canceled -> it.onCanceled(event)
            is MdocEngagementEvent.Disconnected -> it.onDisconnected(event)
            is MdocEngagementEvent.Error -> it.onError(event)
            is MdocEngagementEvent.AbstractEngagementEvent -> println("AbstractEngagementEvent should not be handled in EngagementEventHandlers")
        }
    }
}
