...
class EngagementData private constructor(
    ...
    fun getUuid(required: Boolean = true): Uuid? {
        ...
        if (uuid == null && required) {
            throw IllegalArgumentException("No UUID found for retrieval method, but required was true")
        }
    ...
    fun retrievalTransmissionTypesSupported(): Set<DataRetrievalTransmissionType> = setOfNotNull(
    ...
    fun getReaderEngagement(): ReaderEngagement? {
        if (readerEngagement === null) {
            this.readerEngagement = ReaderEngagement.fromEngagementUri(engagementUri ?: throw IllegalStateException("Engagement URI is not set. Cannot generate ReaderEngagement"))
        }
    ...
    fun getBleRetrievalMethod(): DeviceRetrievalMethod? =
        retrievalMethods.firstOrNull { method -> method.retrievalOptions is BleOptions }

    fun getBleRetrievalMethodOptions(): BleOptions? =
        getBleRetrievalMethod()?.retrievalOptions as? BleOptions
    ...
    private fun createDeviceEngagement(): DeviceEngagement {
        ...
        if (website != null) {
            ...
            val restApiOptions = readerEngagement!!.deviceRetrievalMethods?.firstOrNull { it.type == DeviceRetrievalMethodType.WEBSITE }?.retrievalOptions as? RestApiOptions
            ...
            val coseKey = CoseKey.fromDTO(ephemeralKey.key).toPublicKey()

            return DeviceEngagement.V1_1(
                security = DeviceEngagementSecurity(
                    cipherSuite = curve.cose.value.toUInt(),
                    eDeviceKeyBytes = CborEncodedItem.fromData(coseKey)
                ),
//                deviceRetrievalMethods = retrievalMethods.toTypedArray(),
        ...
        val coseKey = CoseKey.fromDTO(ephemeralKey.key).toPublicKey()
        val eDeviceKeyBytes = coseKey.encodeCbor()
        ...
        return DeviceEngagement.V1_0(
            ...
            val engagementBytes = engagement.encodeCbor()
    ...
    private fun combineBleMethods(methods: Set<DeviceRetrievalMethod>): Set<DeviceRetrievalMethod> {
        ...
        if (bleMethods.size <= 1) return methods
        ...
        val combinedBle = DeviceRetrievalMethod(
            ...
            retrievalOptions = BleOptions(
                ...
                peripheralServerModeUuid = if (supportsPeripheralServerMode) peripheralServerUuid else null,
                centralClientModeUuid = if (supportsCentralClientMode) centralClientUuid else null,
    ...
    fun generateEngagementUri(scheme: String? = null): String {
        if (engagementUri == null) {
            ...
            if (readerEngagement != null) {
                ...
                val actualScheme = scheme ?: if (readerEngagement!!.hasWebsiteRetrievalMethod) "mdoc://" else "mdoc:"
                engagementUri = readerEngagement!!.toEngagementUri(actualScheme)
            ...
            if (!isQrEngagementSupported()) {
                throw IllegalArgumentException("QR Engagement is not enabled. Cannot generate QR code")
            }
            check(retrievalMethods.isNotEmpty()) { "Device retrieval methods are not present. Cannot generate QR code" }

            // Get all BLE methods
            val bleMethods = (decoded.deviceRetrievalMethods as Array<out DeviceRetrievalMethod>)
            ...
            val encodedEngagementBytes = decoded.encodeCbor()
            debugLogger?.logDeviceEngagement(
    ...
    override fun toString(): String {
        return "MdocEngagementData(ephemeralKey=$ephemeralKey, retrievalMethods=${stringify(retrievalMethods)}, curve=$curve, role=$role, engagementMethods=${
            stringify(
                engagementMethods
            )
    ...
    companion object {
                    ...
                    if (bleOptions.centralClientMode && bleOptions.peripheralServerMode) {
                        ...
                        result.add(
                            DeviceRetrievalMethod(
                                type = DeviceRetrievalMethodType.BLE,
                                retrievalOptions = bleOptions.copy(
         ...
         */
        fun readerBuilder() = HolderBuilder(role = MdocRole.MDOC_READER)

        fun holderFromDeviceKey(ephemeralDeviceKey: ResolvedKeyInfoType<*>, retrievalMethods: Set<DeviceRetrievalMethod>) =
            holderBuilder()
                .withEphemeralKey(ephemeralDeviceKey = ephemeralDeviceKey)
                .withRetrievalMethods(retrievalMethods)
                .build()

        fun verifierFromDeviceEngagement(encodedEngagement: CborEncodedItem<DeviceEngagement>, engagementMethods: Set<MdocEngagementMethod> = setOf(QREngagementMethod())): EngagementData {
            ...
            if (retrievalMethods.isEmpty()) {
                throw IllegalArgumentException("Device engagement does not contain a single retrieval method")
            }
    ...
    class HolderBuilder(
        ...
        fun withEngagementMethods(vararg engagementMethods: MdocEngagementMethod) = apply {
        ...
        fun withNfcEngagement() = apply {
            this.engagementMethods.add(NfcEngagementMethod())
            throw IllegalArgumentException("NFC not supported yet. QR only")
        }

        fun withReaderEngagement(readerEngagement: ReaderEngagement) = apply {
            val engagementMethod = ReaderEngagementMethod(readerEngagement = readerEngagement)
        ...
        fun withReaderEngagementUri(dataUri: String) = apply {
            val engagementMethod = ReaderEngagementMethod(engagementUri = dataUri)
            withReaderEngagement(engagementMethod.readerEngagement)
            this.engagementUri = dataUri  // Store the URI for getReaderEngagement()
        }

        suspend fun withEphemeralDeviceKeyFromKeyManager(keyManager: KeyManagerService, signatureAlgorithm: SignatureAlgorithm, kmsProviderId: String? = null) =
            apply {
                this.ephemeralDeviceKey =
                    keyManager.generateKeyAsync(providerId = kmsProviderId, alg = signatureAlgorithm)
        ...
        fun addRetrievalMethod(retrievalMethod: DeviceRetrievalMethod) = apply {
            ...
            if (options is BleOptions) {
                ...
                if (options.centralClientMode && options.peripheralServerMode) {
                    ...
                    val centralMethod = DeviceRetrievalMethod(
                        type = DeviceRetrievalMethodType.BLE,
                        retrievalOptions = options.copy(
                            peripheralServerMode = false,
                            peripheralServerModeUuid = null
                        )
                    )
                    ...
                    val peripheralMethod = DeviceRetrievalMethod(
                        type = DeviceRetrievalMethodType.BLE,
                        retrievalOptions = options.copy(
                            centralClientMode = false,
                            centralClientModeUuid = null
