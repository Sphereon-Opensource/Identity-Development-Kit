...
private fun cborNullBytes(): ByteArray = byteArrayOf(0xF6.toByte())
...
sealed interface MdocRetrievalEvent : MdocEvent {
     ...
     * This byte array is retained as raw event data.
     */
    val data: ByteArray
     ...
     *
     * This parses the raw payload into a generic CBOR item. Typed protocol decoding belongs at explicit
     * codec-backed boundaries, not in the event surface.
     *
    ...
    abstract class AbstractRetrievalEvent : MdocRetrievalEvent {
        ...
        override fun toString(): String = "${this::class.simpleName}(engagementId=$engagementId, time=$time, state=$state, isActive=$isActive, isFinal=$isFinal, data=size(${data.size}))"
    }
    ...
    class TransmissionTypeSelected(
        override val engagementId: Uuid,
        val deviceRetrievalMethod: DeviceRetrievalMethod,
    ) : AbstractRetrievalEvent() {
        override val data: ByteArray = cborNullBytes()
    ...
    data class SessionEstablishmentReceived(
        override val engagementId: Uuid,
        override val data: ByteArray,
    ) : AbstractRetrievalEvent() {
        /**
         * This method parses the raw byte array data provided in the session establishment event
         * into a generic CBOR item.
         *
         * @return The parsed CBOR item decoded from the byte array data.
         */
    ...
    data class DeviceRequestReady(
        override val engagementId: Uuid,
        override val data: ByteArray,
    ) : AbstractRetrievalEvent() {
        /**
         * Parses the `data` property of the instance into a raw CBOR item.
         *
         * @return The parsed CBOR item derived from the `data` property.
         */
        override val time: LocalDateTimeKMP = DateTimeUtils.Companion.DEFAULTS.dateTimeLocal()
     ...
     */
    data class DocumentsSelectionProcessStart(
        override val engagementId: Uuid,
        override val data: ByteArray,
    ) : AbstractRetrievalEvent() {
        /**
         * Parses the associated `data` property of the class into a raw CBOR item.
         *
         * @return The parsed CBOR item which represents the structured representation of the `data`
         * in CBOR format.
         */
        override val time: LocalDateTimeKMP = DateTimeUtils.Companion.DEFAULTS.dateTimeLocal()
    ...
    data class DocumentsSelectionProcessAccepted(
        override val engagementId: Uuid,
        override val data: ByteArray,
    ) : AbstractRetrievalEvent() {
        /**
         * Parses the `data` property of the implementing class into a raw CBOR item.
         *
         * @return A parsed CBOR item representing the decoded CBOR data.
         */
        override val time: LocalDateTimeKMP = DateTimeUtils.Companion.DEFAULTS.dateTimeLocal()
    ...
    data class DocumentsSelectionProcessDeclined(
        override val engagementId: Uuid,
        override val data: ByteArray,
    ) : AbstractRetrievalEvent() {
        /**
     ...
     */
    data class SessionDataSend(
        override val engagementId: Uuid,
        override val data: ByteArray,
        val deviceResponse: DeviceResponse? = null,
    ) : AbstractRetrievalEvent() {
        /**
        ...
        override fun equals(other: Any?): Boolean {
            ...
            if (deviceResponse != other.deviceResponse) {
                return false
            }
            if (time != other.time) {
                return false
     ...
     * Inherits from the TransferSessionEvent interface.
     */
    data class SessionDataReceived(
        override val engagementId: Uuid,
        override val data: ByteArray,
        ...
        override fun equals(other: Any?): Boolean {
            ...
            if (deviceRequest != other.deviceRequest) {
                return false
            }
            if (time != other.time) {
                return false
    ...
    data class SessionTerminationSend(
        override val engagementId: Uuid,
        override val data: ByteArray = cborNullBytes(),
    ) : AbstractRetrievalEvent() {
        /**
    ...
    data class SessionTerminationReceived(
        override val engagementId: Uuid,
        override val data: ByteArray = cborNullBytes(),
    ) : AbstractRetrievalEvent() {
        /**
...
suspend fun Collection<MdocRetrievalEvent.Listener>.onRetrievalEvent(event: MdocRetrievalEvent) {
    ...
    toList().forEach {
        when (event) {
            is MdocRetrievalEvent.Initializing -> {
                it.onInitializing(event)
            }

            is MdocRetrievalEvent.TransmissionTypeSelected -> {
                it.onTransmissionTypeSelected(event)
            }

            is MdocRetrievalEvent.SessionEstablishmentReceived -> {
                it.onSessionEstablishmentReceived(event)
            }

            is MdocRetrievalEvent.DeviceRequestReady -> {
                it.onDeviceRequestReady(event)
            }

            is MdocRetrievalEvent.DocumentsSelectionProcessStart -> {
                it.onDocumentsSelectionProcessStart(event)
            }

            is MdocRetrievalEvent.DocumentsSelectionProcessAccepted -> {
                it.onDocumentsSelectionProcessAccepted(event)
            }

            is MdocRetrievalEvent.DocumentsSelectionProcessDeclined -> {
                it.onDocumentsSelectionProcessDeclined(event)
            }

            is MdocRetrievalEvent.SessionDataSend -> {
                it.onSessionDataSend(event)
            }

            is MdocRetrievalEvent.SessionDataReceived -> {
                it.onSessionDataReceived(event)
            }

            is MdocRetrievalEvent.SessionTerminationSend -> {
                it.onSessionTerminationSend(event)
            }

            is MdocRetrievalEvent.SessionTerminationReceived -> {
                it.onSessionTerminationReceived(event)
            }

            is MdocRetrievalEvent.Terminated -> {
                it.onTerminated(event)
            }
            ...
            is MdocRetrievalEventWithEngagement -> {
                ...
                print("AbstractTransferSessionEvent not handled in dispatcher")
            }
        }
    }
}
...
data class MdocRetrievalEventWithEngagement(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (engagementEvent != other.engagementEvent) {
            return false
        }
    ...
    override fun toString(): String = "MdocRetrievalEventWithEngagement(delegate=$delegate, engagementEvent=$engagementEvent)"
}
