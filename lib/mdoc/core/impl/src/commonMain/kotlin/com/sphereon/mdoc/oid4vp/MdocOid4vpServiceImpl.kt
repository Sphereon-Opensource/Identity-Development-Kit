...
class MdocOid4vpServiceImpl(val signService: MdocSignService, private val logService: SessionLogService) : MdocOid4vpService {
    ...
    override suspend fun createDeviceResponse(
        ...
        var documentErrors = arrayOf<DeviceResponseDocumentErrorCborAlias>()
        ...
        return DeviceResponse(documents = if (documents.isEmpty()) null else documents, documentErrors = if (documentErrors.isEmpty()) null else documentErrors, original = null)
    }
    ...
    override suspend fun signDocument(
        ...
        var resolvedKeyInfo = MdocSignServiceImpl.Utils.getSuppliedOrMSODerivedCborKeyInfo(keyInfo = deviceKeyInfo, mso = document?.MSO)
        ...
        if (resolvedKeyInfo.signatureAlgorithm == null) {
            ...
            val supportedSigAlgs = inputDescriptorAlg.flatMap { descAlg ->
                SignatureAlgorithm.Companion.asList.map { enumValue ->
        ...
        val documentError: DeviceResponseDocumentErrorCborAlias? = if (document != null) null else mapOf(Pair(docType, DocumentError(0)))
    ...
    override fun filterApplicableDocumentsPerInputDescriptor(
        allDocument: Array<Document>, inputDescriptor: IOid4VPInputDescriptor
    ): Array<Document> {
        /**
         * From 18013-7:
    ...
    override fun matchDocumentsAndDescriptors(
        ...
        applicableDocuments: Array<Document>, presentationDefinition: IOid4VPPresentationDefinition
        ...
        return presentationDefinition.input_descriptors.map { inputDescriptor ->
            ...
            if (matchingDocuments.isEmpty()) {
                ...
                val deviceKeyInfo = MdocSignServiceImpl.Utils.getSuppliedOrMSODerivedCborKeyInfo(mso = document.MSO)
                DocumentDescriptorMatchResult(
