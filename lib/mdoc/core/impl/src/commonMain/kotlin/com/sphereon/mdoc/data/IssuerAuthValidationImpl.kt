...
class IssuerAuthValidationImpl(val x509VerifyService: X509VerifyService, val coseCryptoService: CoseCryptoService) : IssuerAuthValidation {
...
//        deviceResponse: DeviceResponse
    ...
    override suspend fun verifyValidityInfo(
        ...
        val mso = MobileSecurityObject.Decoder.decodeCoseSign1(issuerAuth)

        val now =
            dateTimeUtils.epochSeconds().toLong() // toLong as the date time utils uses ints for these (valid till 2038)
        val signed = mso.validityInfo.signed.tDateToEpochSeconds(dateTimeUtils, timeZoneId).toLong()
        val validFrom = mso.validityInfo.validFrom.tDateToEpochSeconds(dateTimeUtils, timeZoneId).toLong()
        val validUntil = mso.validityInfo.validUntil.tDateToEpochSeconds(dateTimeUtils, timeZoneId).toLong()
        ...
        if (validUntil < (now - clockSkewAllowedInSec)) {
            ...
            return VerifyResult(
                ...
                message = "The document is not valid anymore. Current date/time: $verificationAt ${if (datesMatch) "(${now})" else ""} and valid Until $validUntilStr ${if (datesMatch) "(${validUntil})" else ""}",
                name = MdocConst.MDOC_LITERAL
            )
        }
