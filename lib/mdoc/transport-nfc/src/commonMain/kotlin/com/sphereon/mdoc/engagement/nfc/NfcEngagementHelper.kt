...
import com.sphereon.cbor.buildCborArray
 ...
 * @param sessionComponent Session component for dependency injection and logging
 *
...
class MdocNfcEngagementHelper(
    ...
    private val log = sessionComponent.logManager.withTag(TAG)
    ...
    private val handoverSelectBuilder = HandoverSelectMessageBuilder(
    ...
    init {
        ...
        val hasStaticHandover = instance.getEngagementMethods()
    ...
    private fun generateCapabilityContainerFile(): ByteString {
        val fileWriteAccessCondition = if (negotiatedHandoverPicker != null) {
        ...
        return ByteString(
            byteArrayOf(
                0x00, 0x0f, 0x20, 0x7f, 0xff.toByte(), 0x7f, 0xff.toByte(), 0x04,
                0x06, 0xe1.toByte(), 0x04, 0x7f, 0xff.toByte(),
            ...
            MdocNfcConstants.NDEF_DATA_FILE_ID -> {
                if (negotiatedHandoverPicker != null) {
                    ...
                    stateHolder.update { current ->
                        current.withSelectedFile(fileId, payload)
                    ...
                    val handover = buildCborArray {
                        ...
                        add(CborSimple.NULL)                // Handover Request message
                    }

                    instance.handover = handover.encodeCbor()
    ...
    private suspend fun processReadBinary(command: CommandApdu): ResponseApdu {
        ...
        if (currentState.selectedFileId == MdocNfcConstants.NDEF_DATA_FILE_ID &&
            ...
            if (newBytesRead >= currentState.expectedReadSize && !currentState.handoverCompleteCalled) {
                ...
                val (oldState, newState) = stateHolder.getAndUpdate { s ->
                    s.withBytesRead(newBytesRead)
                ...
                if (!oldState.handoverCompleteCalled) {
                    try {
                        ...
                        raiseError("Error invoking handover complete callback: ${e.message}", e)
                    }
                }
            } else {
    ...
    private suspend fun ndefTransactNegotiatedHandleHandoverRequest(message: NdefMessage): NdefMessage {
        ...
        for (record in message.records.subList(1, message.records.size)) {
            val result = connectionMethodFromNdef(
                record = record,
        ...
        val disambiguatedConnectionMethods = disambiguateConnectionMethods(
        ...
        //
        val encodedDeviceEngagement = DeviceEngagement.Decoder.builderFromKey(instance.getEphemeralKey(), {
            version = "1.0"
            retrievalMethods {
//FIXME
            }
        ...
        stateHolder.update { current ->
            current.withNegotiatedHandoverState(NegotiatedHandoverState.HANDOVER_COMPLETE)
                .withHandoverComplete()
        }
    ...
    private fun generateHandoverSelectMessage(
        ...
        log.info(engagementInstance.getDeviceEngagement().data().encodeCbor().encodeTo(Encoding.HEX))
        log.info("==============================================================")
    ...
    suspend fun processApdu(command: CommandApdu): ResponseApdu {
        ...
        return try {
            when (command.ins) {
                NfcConst.INS_SELECT -> {
                    when (command.p1) {
                        NfcConst.INS_SELECT_P1_FILE -> processSelectFile(command)
                        NfcConst.INS_SELECT_P1_APPLICATION -> processSelectApplication(command)
                        else -> {
                ...
                NfcConst.INS_READ_BINARY -> processReadBinary(command)
                NfcConst.INS_UPDATE_BINARY -> processUpdateBinaryCommand(command)
                else -> {
    ...
    fun getStateInfo(): Map<String, Any> {
        val currentState = state
        return currentState.toDebugMap() + mapOf(
    ...
    internal fun captureState(): NfcSessionState? {
        ...
        val sessionState = NfcSessionState(
    ...
    fun attemptRecovery(): Boolean {
        val currentState = state
        if (!currentState.inError) return true
