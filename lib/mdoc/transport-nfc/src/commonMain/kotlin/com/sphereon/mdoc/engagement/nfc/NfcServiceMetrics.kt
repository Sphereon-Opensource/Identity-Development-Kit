...
data class NfcServiceMetrics(
    ...
    fun nfcDuration(): Duration? =
        if (nfcStartTime != null && handoverTime != null) {
            handoverTime - nfcStartTime
        } else {
            null
        }

    fun bleDuration(): Duration? =
        if (bleStartTime != null && completionTime != null) {
            completionTime - bleStartTime
        } else {
            null
        }

    fun totalDuration(): Duration? =
        if (engagementStartTime != null && completionTime != null) {
            completionTime - engagementStartTime
        } else {
            null
        }
    ...
    fun successRate(): Double =
        if (totalApduCount > 0) {
            successfulApduCount.toDouble() / totalApduCount.toDouble()
        } else {
            1.0
        }
    ...
    fun errorRate(): Double =
        if (totalApduCount > 0) {
            errorCount.toDouble() / totalApduCount.toDouble()
        } else {
            0.0
        }
