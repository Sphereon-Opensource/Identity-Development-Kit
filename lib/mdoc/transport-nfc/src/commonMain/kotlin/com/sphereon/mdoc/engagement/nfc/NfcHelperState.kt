...
internal data class NfcHelperState(
     ...
     */
    fun withSelectedFile(fileId: Int, payload: ByteString): NfcHelperState =
        copy(selectedFileId = fileId, selectedFilePayload = payload)

    /**
     * Update negotiated handover state.
     */
    fun withNegotiatedHandoverState(state: NegotiatedHandoverState): NfcHelperState =
        copy(negotiatedHandoverState = state)

    /**
     * Update static handover state.
     */
    fun withStaticHandoverState(state: StaticHandoverState): NfcHelperState =
        copy(staticHandoverState = state)
     ...
     */
    fun withHandoverComplete(): NfcHelperState = copy(
        handoverCompleteCalled = true,
        staticHandoverState = if (staticHandoverState == StaticHandoverState.HANDOVER_READ)
            StaticHandoverState.HANDOVER_COMPLETE else staticHandoverState
    )

    /**
     * Mark static handover as read (data sent to reader).
     */
    fun withStaticHandoverRead(): NfcHelperState = copy(
     ...
     */
    fun withStaticHandoverError(): NfcHelperState = copy(
    ...
    fun reset(clearErrorState: Boolean = false): NfcHelperState =
        if (clearErrorState) INITIAL else INITIAL.copy(inError = inError)
...
internal class NfcHelperStateHolder {
    private val _state = atomic(NfcHelperState.INITIAL)
    ...
    inline fun update(transform: (NfcHelperState) -> NfcHelperState): NfcHelperState {
        while (true) {
            ...
            if (_state.compareAndSet(current, new)) {
                return new
            }
        }
    }
...
internal data class NfcSessionState(
    ...
    fun isValid(
        ...
        return currentTimeMs - captureTimeMs < validityMs
    }
    ...
    fun isRecoverable(
        ...
        return isValid(currentTimeMs, validityMs) && !helperState.isHandoverComplete
    }
}
