 ...
 * - **SessionTranscript**: Uses OID4VPHandover (clientIdHash, responseUriHash, nonce) vs RestApiHandover
 *
 * ## Security
 *
 * Per ISO 18013-7 B.5:
...
class Oid4VpTransport(
        ...
        return when (key) {
            "sessionTranscript" -> {
                ...
                try {
                    ...
                    log.error("Failed to get SessionTranscript", exception = e)
                    null
                }
            }
            else -> null
    ...
    override suspend fun open(
        ...
        return mutex.withLock {
            ...
            try {
                ...
                _engagementState.value = com.sphereon.mdoc.engagement.MdocEngagementState.CONNECTING
                ...
                val parsedRequest = oid4vpHolder.parseAuthorizationRequest(authorizationRequestUri)
                ...
                val resolvedRequest = oid4vpHolder.resolveAuthorizationRequest(parsedRequest)
                    .getOrElse { return@withLock Err(it) }

                val presentationDefinition = if (usePresentationExchange) {
                ...
                log.error("Failed to open OID4VP connection", exception = e)
                Err(Oid4vpError.ConnectionFailed(responseUri ?: options.requestUri ?: "unknown", e).toIdkError())
            }
        }
    }
    ...
    override suspend fun messageReceiveBlocking(): ByteArray {
        ...
        return mutex.withLock {
            ...
            val resolvedRequest = resolvedRequest
                ?: throw IllegalStateException("Authorization Request not resolved")

            val presentationDef = presentationDefinition

            val deviceRequest = if (presentationDef != null) {
                ...
                val dcqlQuery = resolvedRequest.dcqlQuery
                    ?: throw IllegalStateException("No Presentation Definition or DCQL query resolved")
            ...
            deviceRequest.encodeCbor()
        }
    }

    /**
     * Get the OID4VP session transcript.
     *
     * For OID4VP, the session transcript uses OID4VPHandover which contains:
     * - clientIdHash: SHA-256 hash of [clientId, mdocGeneratedNonce]
     * - responseUriHash: SHA-256 hash of [responseUri, mdocGeneratedNonce]
     * - nonce: The nonce from the Authorization Request
     *
     * This must be called after open() has completed successfully.
     *
     * @param mdocGeneratedNonce The nonce generated by the mdoc (wallet)
     * @return The SessionTranscript object
     */
    fun getSessionTranscript(mdocGeneratedNonce: String = Uuid.v4String()): com.sphereon.mdoc.transfer.reader.SessionTranscript {
        requireOpen()

        val clientId = options.clientId
        val responseUri = this.responseUri
            ?: throw IllegalStateException("response_uri not set - Authorization Request not fetched")
        val nonce = this.authorizationRequestNonce
            ?: throw IllegalStateException("nonce not set - Authorization Request not fetched")

        log.info("Creating OID4VP session transcript")
        log.debug("clientId: $clientId")
        log.debug("responseUri: $responseUri")
        log.debug("authzRequestNonce: ${nonce.take(16)}...")
        log.debug("mdocGeneratedNonce: ${mdocGeneratedNonce.take(16)}...")

        val handover = com.sphereon.mdoc.oid4vp.oid4vpHandoverFromClientIdAndResponseUri(
            clientId = clientId,
        ...
        val sessionTranscript = com.sphereon.mdoc.transfer.reader.SessionTranscript.fromOid4vpHandover(handover)
    ...
    override suspend fun messageSendBlocking(message: ByteArray) {
        ...
        mutex.withLock {
            val responseUri = this.responseUri
                ?: throw IllegalStateException("response_uri not set - Authorization Request not fetched")
            val resolvedRequest = this.resolvedRequest
                ?: throw IllegalStateException("Authorization Request not resolved")
            ...
            try {
                ...
                if (holderFlowResult != null) {
                    ...
                    deviceResponse = DeviceResponse.decodeCbor(message)
                }

                val submissionResult = if (usePresentationExchange) {
                    val authorizationResponse = buildPresentationAuthorizationResponse(deviceResponse, resolvedRequest)
                    oid4vpHolder.submitAuthorizationResponse(
                        resolvedRequest,
                    ...
                    val authorizationResponse = oid4vpHolder.createAuthorizationResponse(resolvedRequest, selectedCredentials)
                        .getOrElse { throw it.toException() }
                    oid4vpHolder.submitAuthorizationResponse(resolvedRequest, authorizationResponse, null)
                        .getOrElse { throw it.toException() }
                }
                ...
                log.error("Failed to send Authorization Response", exception = e)
                throw Oid4vpError.NetworkError(responseUri, e)
            }
        }
    }
    ...
    suspend fun executeHolderFlow(
        ...
        return mutex.withLock {
            try {
                ...
                val nonce = authorizationRequestNonce
                    ?: return@withLock Err(
                ...
                val result = holderFlow.processAuthorizationRequest(
                ...
                log.error("Failed to execute OID4VP holder flow", exception = e)
                Err(Oid4vpError.SigningFailed(e).toIdkError())
            }
        }
    }

    private fun buildAuthorizationRequestUri(): String {
        val requestUri = options.requestUri
            ?: throw IllegalStateException("request_uri must be provided in mdoc-openid4vp:// URI")
        val params = buildMap {
    ...
    private fun parsePresentationDefinition(definition: JsonElement): Oid4VPPresentationDefinition {
        return oid4vpJsonSerializer.decodeFromJsonElement(
    ...
    private fun buildPresentationAuthorizationResponse(
        ...
        val presentationDef = presentationDefinition
            ?: throw IllegalStateException("Presentation Definition not resolved")
        val presentationSubmission = holderFlowResult?.presentationSubmission
            ?: Oid4VPPresentationSubmission.fromPresentationDefinition(presentationDef)
        val presentationSubmissionElement = oid4vpJsonSerializer.parseToJsonElement(
    ...
    private fun buildSelectedCredentials(
        ...
        val ids = resolvedRequest.dcqlQuery?.credentials?.map { it.id }.orEmpty()
            .ifEmpty { listOf("mdoc") }
