...
class ForwardReaderConnection(
    ...
    override suspend fun establish(): IdkResult<Unit, IdkError> {
        ...
        try {
            ...
            val openResult = typedTransport.open(
            ...
            log.error("Failed to open transport", exception = e)
            return IdkError.UNKNOWN_ERROR(
                message = "Failed to open transport: ${e.message}",
                exception = e
            ).asErrorResult()
        }

        // 3. Extract data channels
        val channels = transportHelper.extractChannels(transport!!)
            ?: return IdkError.UNKNOWN_ERROR(
