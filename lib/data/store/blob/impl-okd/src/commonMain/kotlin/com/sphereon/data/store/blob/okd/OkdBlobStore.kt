...
class OkdBlobStore(
    ...
    override val capabilities: BlobStoreCapabilities = BlobStoreCapabilities(
        supportsTempUrls = true,
    )

    private fun documentUrl(documentId: String): String =
    ...
    override suspend fun put(
        ...
        val path = target.path ?: kotlin.uuid.Uuid.random().toString()
        return try {
            val response = http.patch(documentUrl(path)) {
            ...
            Err(IdkError.fromString(message = "OKD put failed for $path: ${e.message}", exception = e, code = "BLOB_OKD_PUT_FAILED"))
        }
    }
    ...
    override suspend fun get(info: BlobInfo): IdkResult<ResolvedBlobInfo, IdkError> {
        val path = info.path!!
        return try {
            val response = http.get(documentUrl(path)) {
                applyAuth()
            }
            if (!response.status.isSuccess()) {
                return Err(mapHttpError(response.status, path))
            ...
            Err(IdkError.fromString(message = "OKD get failed for $path: ${e.message}", exception = e, code = "BLOB_OKD_GET_FAILED"))
        }
    }
    ...
    override suspend fun delete(info: BlobInfo): IdkResult<Boolean, IdkError> {
        val path = info.path!!
        return try {
            val response = http.delete(documentUrl(path)) {
                applyAuth()
            }
            ...
            Err(IdkError.fromString(message = "OKD delete failed for $path: ${e.message}", exception = e, code = "BLOB_OKD_DELETE_FAILED"))
        }
    }
    ...
    override suspend fun stat(info: BlobInfo): IdkResult<BlobDescriptor, IdkError> {
        val path = info.path!!
        return try {
            val response = http.get("${documentUrl(path)}/metadata") {
                applyAuth()
            }
            if (response.status == HttpStatusCode.NotFound) {
            ...
            Err(IdkError.fromString(message = "OKD stat failed for $path: ${e.message}", exception = e, code = "BLOB_OKD_STAT_FAILED"))
        }
    }
    ...
    private fun mapHttpError(status: HttpStatusCode, path: String): IdkError {
        return when (status) {
