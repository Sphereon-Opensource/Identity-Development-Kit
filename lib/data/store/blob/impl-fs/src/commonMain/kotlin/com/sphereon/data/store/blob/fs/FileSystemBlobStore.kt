...
class FileSystemBlobStore(
    ...
    override suspend fun putIfAbsent(target: BlobInfo, data: ByteArray): IdkResult<BlobDescriptor, IdkError> =
        put(target, data, PutOptions.NO_OVERWRITE)

    @OptIn(ExperimentalUuidApi::class)
    override suspend fun put(target: BlobInfo, data: ByteArray, options: PutOptions): IdkResult<BlobDescriptor, IdkError> = mutex.withLock {
        try {
            val effectiveInfo = if (target.path == null) {
    ...
    override suspend fun get(info: BlobInfo): IdkResult<ResolvedBlobInfo, IdkError> = mutex.withLock {
        try {
            ...
            val data = fileSystem.source(path).buffer().use { source ->
    ...
    override suspend fun delete(info: BlobInfo): IdkResult<Boolean, IdkError> = mutex.withLock {
        try {
            val path = resolvePath(info)
            if (!fileSystem.exists(path)) {
    ...
    override suspend fun exists(info: BlobInfo): IdkResult<Boolean, IdkError> = mutex.withLock {
        try {
    ...
    override suspend fun stat(info: BlobInfo): IdkResult<BlobDescriptor, IdkError> = mutex.withLock {
        try {
            val path = resolvePath(info)
            if (!fileSystem.exists(path)) {
                return@withLock Err(BlobStoreError.NotFound(info.path!!).toIdkError())
    ...
    override suspend fun list(info: BlobInfo, options: ListOptions): IdkResult<ListResult, IdkError> = mutex.withLock {
        try {
            val basePath = rootDir.toPath()
            val optPrefix = options.prefix ?: info.path ?: ""
            val prefixPath = if (optPrefix.isNotEmpty()) basePath / optPrefix else basePath

            if (!fileSystem.exists(prefixPath) && optPrefix.isNotEmpty()) {
                return@withLock Ok(ListResult(descriptors = emptyList()))
            }

            val searchDir = if (fileSystem.metadata(prefixPath).isDirectory) prefixPath else prefixPath.parent ?: basePath

            val entries = if (options.recursive) {
    ...
    override suspend fun copy(source: BlobInfo, destination: BlobInfo): IdkResult<BlobDescriptor, IdkError> = mutex.withLock {
        try {
    ...
    override suspend fun move(source: BlobInfo, destination: BlobInfo): IdkResult<BlobDescriptor, IdkError> = mutex.withLock {
        try {
            val srcPath = resolvePath(source)
            val dstPath = resolvePath(destination)
    ...
    override suspend fun deletePrefix(info: BlobInfo): IdkResult<Int, IdkError> = mutex.withLock {
        try {
            ...
            var count = 0
            fileSystem.listRecursively(prefixPath).toList()
    ...
    private fun toDescriptor(path: String, fsMetadata: okio.FileMetadata, sidecar: SidecarMetadata?, metadata: BlobMetadata? = null): BlobDescriptor {
    ...
    private fun relativePathString(path: Path, base: Path): String {
