...
interface BlobStoreConfigBinder {
    ...
    fun getBlobStoreConfig(configService: ConfigService, storeId: String): BlobStoreConfigBase
 ...
 */
private val BlobStorePropertyNameAliases = mapOf(
...
class BlobStoreConfigBinderImpl(appLogManager: AppLogManager) : BlobStoreConfigBinder {
    ...
    private fun buildPrefixes(configService: ConfigService) =
        setOf("${configService.getNamespace()}.${BlobStoreConfigBinder.BLOB_STORES_PREFIX}", BlobStoreConfigBinder.BLOB_STORES_PREFIX)
    ...
    override fun getBlobStoreConfig(configService: ConfigService, storeId: String): BlobStoreConfigBase {
        val prefixes = buildPrefixes(configService)
...
interface BlobStoreManager : BlobStoreRegistry {
    ...
    fun createFromBlobStoreConfig(config: BlobStoreConfigBase, execution: SessionExecution?): BlobStore
    fun createFromProperties(configService: ConfigService, execution: SessionExecution?): Set<BlobStore>
}
...
class BlobStoreManagerImpl(
    ...
    override fun createFromBlobStoreConfig(config: BlobStoreConfigBase): BlobStore {
        return createFromBlobStoreConfig(config, null)
    }

    override fun createFromBlobStoreConfig(config: BlobStoreConfigBase, execution: SessionExecution?): BlobStore {
        if (factories.isEmpty()) {
            throw IllegalArgumentException("No BlobStoreFactory implementations found. Add at least one blob backend module (memory, filesystem, etc).")
        }
        val factory = factories.firstOrNull { it.backendId.equals(config.backendId, ignoreCase = true) }
            ?: throw IllegalArgumentException(
    ...
    override fun createFromProperties(configService: ConfigService, execution: SessionExecution?): Set<BlobStore> {
        return binder.getBlobStoreConfigs(configService).map { createFromBlobStoreConfig(it, execution) }.toSet()
    }
...
interface BlobStoreService {
    ...
    @OptIn(ExperimentalObjCName::class)
    @ObjCName("BlobStoreServiceComponent", exact = true)
...
class NoOpBlobStoreFactoryImpl : BlobStoreFactory {
    ...
    override fun create(config: BlobStoreConfigBase, execution: SessionExecution?): BlobStore {
        throw IllegalArgumentException("No BlobStoreFactory found. Add a blob backend module (memory, filesystem, etc) to your app.")
    }
