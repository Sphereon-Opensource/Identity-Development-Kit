...
class KvBlobMetadataIndex(
    ...
    private fun matchesQuery(
        ...
        if (query.contentType != null && descriptor.contentType != query.contentType) {
            return false
        }
        val prefix = query.pathPrefix
        if (prefix != null && !descriptor.path.startsWith(prefix)) {
            return false
        }
        if (query.customMetadata.isNotEmpty()) {
            for ((key, value) in query.customMetadata) {
                if (descriptor.metadata.custom[key] != value) {
                    return false
                }
            }
        }
