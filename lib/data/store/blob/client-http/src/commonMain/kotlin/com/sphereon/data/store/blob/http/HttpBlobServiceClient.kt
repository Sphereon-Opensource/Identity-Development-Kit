...
class HttpBlobServiceClient(
    ...
    private fun resolveStoreId(storeId: String?): String =
        storeId?.takeIf { it.isNotBlank() && it != "default" } ?: config.defaultStoreId

    private fun blobsUrl(storeId: String?, path: String? = null): String {
        val store = resolveStoreId(storeId)
        val base = "${config.baseUrl}/api/blob-stores/$store/blobs"
        return if (path != null) "$base/$path" else base
    }
    ...
    private fun io.ktor.client.request.HttpRequestBuilder.applyAuth() {
        ...
        if (tenantHeader != null && execution != null) {
            ...
            } catch (_: Exception) { /* no context available */ }
        }
    }
    ...
    override suspend fun storeBlob(
        ...
        return try {
            val response = http.put(blobsUrl(target.storeId, path)) {
                contentType(ContentType.Application.Json)
                setBody(BlobApiPutBody(
            ...
            Err(IdkError.fromString(message = "HTTP blob store failed for $path: ${e.message}", exception = e, code = "BLOB_HTTP_PUT_FAILED"))
        }
    }
    ...
    override suspend fun getBlob(
        ...
        return try {
            val response = http.get("${blobsUrl(blobInfo.storeId, path)}/content") {
                applyAuth()
            }
            if (!response.status.isSuccess()) {
                return Err(mapHttpError(response, path))
            ...
            Err(IdkError.fromString(message = "HTTP blob get failed for $path: ${e.message}", exception = e, code = "BLOB_HTTP_GET_FAILED"))
        }
    }
    ...
    override suspend fun getBlobInfo(
        ...
        return try {
            val response = http.get("${blobsUrl(blobInfo.storeId, path)}/stat") {
                applyAuth()
            }
            if (!response.status.isSuccess()) {
                return Err(mapHttpError(response, path))
            ...
            Err(IdkError.fromString(message = "HTTP blob stat failed for $path: ${e.message}", exception = e, code = "BLOB_HTTP_STAT_FAILED"))
        }
    }
    ...
    override suspend fun deleteBlob(
        ...
        return try {
            val response = http.delete(blobsUrl(blobInfo.storeId, path)) {
                applyAuth()
            }
            ...
            Err(IdkError.fromString(message = "HTTP blob delete failed for $path: ${e.message}", exception = e, code = "BLOB_HTTP_DELETE_FAILED"))
        }
    }
    ...
    override suspend fun listBlobs(
        ...
        return try {
            val response = http.get(blobsUrl(info.storeId)) {
                ...
                if (options.recursive) parameter("recursive", true)
                applyAuth()
            }
            if (!response.status.isSuccess()) {
            ...
            Err(IdkError.fromString(message = "HTTP blob list failed: ${e.message}", exception = e, code = "BLOB_HTTP_LIST_FAILED"))
        }
    }
    ...
    override suspend fun copyBlob(
        ...
        return try {
            val response = http.post("${blobsUrl(sourceInfo.storeId, sourcePath)}/copy") {
                contentType(ContentType.Application.Json)
            ...
            Err(IdkError.fromString(message = "HTTP blob copy failed for $sourcePath: ${e.message}", exception = e, code = "BLOB_HTTP_COPY_FAILED"))
        }
    }
    ...
    override suspend fun moveBlob(
        ...
        return try {
            val response = http.post("${blobsUrl(sourceInfo.storeId, sourcePath)}/move") {
                contentType(ContentType.Application.Json)
                setBody(BlobCopyMoveBody(destination = destinationPath))
                applyAuth()
            }
            ...
            Err(IdkError.fromString(message = "HTTP blob move failed for $sourcePath: ${e.message}", exception = e, code = "BLOB_HTTP_MOVE_FAILED"))
        }
    }
    ...
    override suspend fun casStore(
        ...
        return try {
            val response = http.post("${config.baseUrl}/api/commands/blob.cas.store") {
                contentType(ContentType.Application.Json)
                setBody(CasStoreBody(
            ...
            Err(IdkError.fromString(message = "HTTP CAS store failed: ${e.message}", exception = e, code = "BLOB_HTTP_CAS_STORE_FAILED"))
        }
    }
    ...
    override suspend fun casGet(
        ...
        return try {
            val response = http.post("${config.baseUrl}/api/commands/blob.cas.get") {
                contentType(ContentType.Application.Json)
                setBody(CasGetBody(
                    tenantId = info.tenantId ?: "default",
            ...
            Err(IdkError.fromString(message = "HTTP CAS get failed: ${e.message}", exception = e, code = "BLOB_HTTP_CAS_GET_FAILED"))
        }
    }
    ...
    override suspend fun casVerify(
        ...
        return try {
            val response = http.post("${config.baseUrl}/api/commands/blob.cas.verify") {
                contentType(ContentType.Application.Json)
                setBody(CasGetBody(
                    tenantId = info.tenantId ?: "default",
                    addressMultibase = address.toMultibaseString(),
            ...
            Err(IdkError.fromString(message = "HTTP CAS verify failed: ${e.message}", exception = e, code = "BLOB_HTTP_CAS_VERIFY_FAILED"))
        }
    }
    ...
    override suspend fun findByMetadata(
        ...
        return try {
            val response = http.post("${config.baseUrl}/api/commands/blob.metadata.search") {
                contentType(ContentType.Application.Json)
            ...
            Err(IdkError.fromString(message = "HTTP metadata search failed: ${e.message}", exception = e, code = "BLOB_HTTP_SEARCH_FAILED"))
        }
    }
    ...
    override suspend fun createTempUrl(
        ...
        return try {
            val response = http.post("${blobsUrl(blobInfo.storeId, path)}/temp-url") {
                contentType(ContentType.Application.Json)
            ...
            Err(IdkError.fromString(message = "HTTP createTempUrl failed for $path: ${e.message}", exception = e, code = "BLOB_HTTP_TEMP_URL_FAILED"))
        }
    }

    // ── Error mapping ──────────────────────────────────────────────────

    private suspend fun mapHttpError(response: HttpResponse, context: String): IdkError {
