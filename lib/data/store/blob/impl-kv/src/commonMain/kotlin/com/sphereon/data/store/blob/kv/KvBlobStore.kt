...
class KvBlobStore(
    ...
    override val capabilities: BlobStoreCapabilities = BlobStoreCapabilities(
        ...
        maxBlobSizeBytes = if (maxBlobSizeBytes > 0) maxBlobSizeBytes else Long.MAX_VALUE,
    )
    ...
    override suspend fun put(
        ...
        val entry = KvBlobEntry(
            data = kotlin.io.encoding.Base64.encode(data),
    ...
    override suspend fun get(info: BlobInfo): IdkResult<ResolvedBlobInfo, IdkError> {
        ...
        val data = try {
            kotlin.io.encoding.Base64.decode(entry.data)
        } catch (e: Exception) {
            return Err(IdkError.fromString(
                message = "Failed to decode blob data for '$path': ${e.message}",
                exception = e,
    ...
    override suspend fun exists(info: BlobInfo): IdkResult<Boolean, IdkError> {
        return kvStore.exists(namespace, info.path!!)
    }
    ...
    override suspend fun list(info: BlobInfo, options: ListOptions): IdkResult<ListResult, IdkError> {
        ...
        val descriptors = allResult.value.entries
        ...
        val allMatching = allResult.value.keys.filter { it.startsWith(prefix) }.sorted()
        val nextPageToken = if (allMatching.size > options.maxResults) allMatching[options.maxResults] else null
    ...
    override suspend fun deletePrefix(info: BlobInfo): IdkResult<Int, IdkError> {
        ...
        for (key in allResult.value) {
            if (key.startsWith(prefix)) {
                ...
                if (delResult.isOk && delResult.value) count++
            }
        }
        return Ok(count)
    }
    ...
    override suspend fun copy(source: BlobInfo, destination: BlobInfo): IdkResult<BlobDescriptor, IdkError> {
        ...
        val destPath = destination.path ?: kotlin.uuid.Uuid.random().toString()
    ...
    private fun toDescriptor(path: String, entry: KvBlobEntry): BlobDescriptor {
