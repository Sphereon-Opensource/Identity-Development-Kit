...
data class PageRequest(
    ...
    companion object {
        ...
        fun of(limit: Int, offset: Int = 0) = PageRequest(limit = limit, offset = offset)

        /** Create a page request sorted by a single field */
        fun sorted(field: String, direction: SortDirection = SortDirection.ASC) =
...
data class Page<T>(
    ...
    val pageNumber: Int get() = if (limit > 0) offset / limit else 0

    /** Total number of pages */
    @SerialName("total_pages")
    val totalPages: Int get() = if (limit > 0) ((totalCount + limit - 1) / limit).toInt() else 1

    companion object {
        /** Create an empty page */
        fun <T> empty(limit: Int = 50) = Page<T>(
            items = emptyList(),
            totalCount = 0,
            limit = limit,
            offset = 0
