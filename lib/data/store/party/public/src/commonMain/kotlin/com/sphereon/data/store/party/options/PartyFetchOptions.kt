...
data class PartyFetchOptions(
    ...
    companion object {
        ...
        val WITH_FULL_IDENTITIES = PartyFetchOptions(
            includeIdentities = true,
            identityOptions = IdentityFetchOptions.WITH_IDENTIFIERS
        )

        /** Load everything */
        val FULL = PartyFetchOptions(
            includeOwner = true,
            includeIdentities = true,
    ...
    fun withIdentities(options: IdentityFetchOptions = IdentityFetchOptions.MINIMAL) =
        copy(includeIdentities = true, identityOptions = options)

    /** Builder method to include identities with correlation identifiers */
    fun withFullIdentities() = copy(
        includeIdentities = true,
        identityOptions = IdentityFetchOptions.WITH_IDENTIFIERS
    )
}
