...
class InMemoryKvStoreDiTest {
    private lateinit var app: KvMemoryTestAppComponent
    ...
    @BeforeTest
    fun setup() {
        app = createKvMemoryTestAppComponent()
    ...
    @Test
    fun `SESSION scope - store instance can be recreated without losing data (session-per-request safe)`() = runTest {
    ...
    @Test
    fun `TENANT vs PRINCIPAL_TENANT - tenant scoped store is shared across principals while principal_tenant is isolated`() = runTest {
        val tenant = tenantData("tenant-a")
    ...
    @Test
    fun `TTL and cleanup - expired entries are not returned and cleanup removes them`() = runTest {
        val tenant = tenantData("tenant-a")
        val user = app.userContextManager.createOrGetFromData(tenantData = tenant, principalValue = "principal-a")
        val session = user.createSession(sessionId = "session-a", makeActive = true)
        val execution = session.sessionExecution
...
abstract class KvMemoryTestAppComponent : AbstractAppComponent() {
    ...
    fun interface Factory {
        ...
        ): KvMemoryTestAppComponent
    }
}

fun createKvMemoryTestAppComponent(
    application: Any = "KvMemoryTest",
    appId: String = "com.sphereon.kv.memory.test",
    profile: String = "test",
    version: String = "1.0.0",
): KvMemoryTestAppComponent {
    val component = createGraphFactory<KvMemoryTestAppComponent.Factory>().create(
