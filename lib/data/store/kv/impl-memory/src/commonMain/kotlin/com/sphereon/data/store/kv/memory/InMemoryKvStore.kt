    ...
    override suspend fun <V : Any> put(
        ...
        partition.mutex.withLock {
            try {
                ...
                partition.entries[InMemoryKvEntryKey(namespace = namespace.name, key = key)] =
                    InMemoryKvStoredBytes(
                        value = bytes,
                ...
                Err(IdkError.fromString(message = "Failed to put KV entry '${namespace.name}:$key': ${expected.message}", exception = expected, code = "KV_PUT_FAILED"))
            }
        }
    ...
    override suspend fun <V : Any> getEntry(
        ...
        partition.mutex.withLock {
            try {
                ...
                Err(IdkError.fromString(message = "Failed to get KV entry '${namespace.name}:$key': ${expected.message}", exception = expected, code = "KV_GET_FAILED"))
            }
        }
    ...
    override suspend fun delete(
        ...
        partition.mutex.withLock {
            try {
                ...
                Err(IdkError.fromString(message = "Failed to delete KV entry '${namespace.name}:$key': ${expected.message}", exception = expected, code = "KV_DELETE_FAILED"))
            }
        }
    ...
    override suspend fun exists(
        ...
        partition.mutex.withLock {
            try {
                ...
                Err(IdkError.fromString(message = "Failed to check existence of KV entry '${namespace.name}:$key': ${expected.message}", exception = expected, code = "KV_EXISTS_FAILED"))
            }
        }
    ...
    override suspend fun touch(
        ...
        partition.mutex.withLock {
            try {
                ...
                Err(IdkError.fromString(message = "Failed to touch KV entry '${namespace.name}:$key': ${expected.message}", exception = expected, code = "KV_TOUCH_FAILED"))
            }
        }
        ...
        partition.mutex.withLock {
            try {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (entryKey.namespace != namespace.name) {
                        continue
                    }
                    if (stored.expiresAtEpochMillis <= now) {
                        iterator.remove()
