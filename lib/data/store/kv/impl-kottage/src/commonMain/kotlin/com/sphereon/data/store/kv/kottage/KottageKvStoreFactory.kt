...
class KottageKvStoreFactory(
    ...
    override fun create(config: KvStoreConfigBase, execution: SessionExecution?): KvStore {
        ...
        val partitionKey = when (config.scopeBinding) {
            KvStoreScopeBinding.APP -> KvPartitionKey(storeId = config.id)
            KvStoreScopeBinding.TENANT -> {
                val tenantId = execution?.sessionContext?.context?.tenant?.tenantId
                    ?: throw IllegalStateException("Cannot create TENANT-scoped KV store without execution context with tenant information")
            ...
            KvStoreScopeBinding.PRINCIPAL_TENANT -> {
                val context = execution?.sessionContext?.context
                    ?: throw IllegalStateException("Cannot create PRINCIPAL_TENANT-scoped KV store without execution context")
                val tenantId = context.tenant.tenantId
                val principalId = context.principal?.toString()
                    ?: throw IllegalStateException("Cannot create PRINCIPAL_TENANT-scoped KV store without principal information")
            ...
            KvStoreScopeBinding.SESSION -> {
                val sessionContext = execution?.sessionContext
                    ?: throw IllegalStateException("Cannot create SESSION-scoped KV store without execution context with session information")
                val tenantId = sessionContext.context.tenant.tenantId
                val principalId = sessionContext.context.principal?.toString()
                    ?: throw IllegalStateException("Cannot create SESSION-scoped KV store without principal information")
