...
class KvStoreConfigSupportJvmTest {
    private var app: KvImplTestAppGraph? = null
    ...
    @Test
    fun `KvStoreConfigBinder can parse kv dot stores and defaultconfigs`() =
        runTest {
            ...
            val localApp = createKvImplTestAppGraph().also { app = it }
    ...
    @Test
    fun `KvStoreService resolves store by id and creates a working KvStore instance`() =
        runTest {
            ...
            val kvStoreService = (session.graph as KvStoreService.Graph).kvStoreService
    ...
    @Test
    fun `KvStoreManager createFromProperties returns only enabled stores`() =
        runTest {
            DefaultAppMapPropertySource.addProperties(
                mapOf(
...
abstract class KvImplTestAppGraph : AbstractAppGraph() {
    abstract override val kvStoreConfigBinder: KvStoreConfigBinder
    abstract override val kvStoreManager: KvStoreManager
...
fun createKvImplTestAppGraph(
    ...
    val graph =
        createGraphFactory<KvImplTestAppGraph.Factory>().create(
