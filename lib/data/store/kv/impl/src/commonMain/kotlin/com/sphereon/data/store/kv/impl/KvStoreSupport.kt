 ...
 */
private val KvStorePropertyNameAliases = mapOf(
...
interface KvStoreConfigBinder {
    ...
    fun getKvStoreConfig(configService: ConfigService, storeId: String): KvStoreConfigBase
...
class KvStoreConfigBinderImpl(appLogManager: AppLogManager) : KvStoreConfigBinder {
    ...
    private fun buildPrefixes(configService: ConfigService) =
        setOf("${configService.getNamespace()}.${KvStoreConfigBinder.KV_STORES_PREFIX}", KvStoreConfigBinder.KV_STORES_PREFIX)
    ...
    override fun getKvStoreConfig(configService: ConfigService, storeId: String): KvStoreConfigBase {
        val prefixes = buildPrefixes(configService)
...
interface KvStoreManager {
    ...
    fun createFromKvStoreConfig(config: KvStoreConfigBase, execution: SessionExecution?): KvStore
    fun createFromProperties(configService: ConfigService, execution: SessionExecution?): Set<KvStore>
}
...
class KvStoreManagerImpl(
    ...
    override fun createFromKvStoreConfig(config: KvStoreConfigBase): KvStore {
        return createFromKvStoreConfig(config, null)
    }

    override fun createFromKvStoreConfig(config: KvStoreConfigBase, execution: SessionExecution?): KvStore {
    ...
    override fun createFromProperties(configService: ConfigService, execution: SessionExecution?): Set<KvStore> {
        return binder.getKvStoreConfigs(configService).map { createFromKvStoreConfig(it, execution) }.toSet()
    }
...
interface KvStoreService {
    ...
    @OptIn(ExperimentalObjCName::class)
    @ObjCName("KvStoreServiceComponent", exact = true)
...
class NoOpKvStoreFactoryImpl : KvStoreFactory {
    ...
    override fun create(config: KvStoreConfigBase, execution: SessionExecution?): KvStore {
        throw IllegalArgumentException("No KvStoreFactory found. Add a KV backend module (memory, kottage, etc) to your app.")
    }
