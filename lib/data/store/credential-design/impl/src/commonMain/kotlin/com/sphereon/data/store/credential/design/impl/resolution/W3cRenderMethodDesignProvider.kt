...
import com.sphereon.data.store.credential.design.impl.mapper.W3cRenderMethodDesignMapper
...
class W3cRenderMethodDesignProvider(
    private val mapper: W3cRenderMethodDesignMapper = W3cRenderMethodDesignMapper(),
) : DesignLayerProvider {
    override val sourceType = DesignSourceType.W3C_VC_RENDER_METHOD
    ...
    override suspend fun resolveCredentialLayer(
        ...
        val renderMethodJson = input.externalMetadata?.w3cRenderMethod ?: return null

        val methods = renderMethodJson["renderMethod"]?.jsonArray ?: return null
        val variants = mapper.parseRenderMethods(methods)

        return CredentialDesignLayerResult(
            renderVariants = variants,
            providedFields = variants.map { "variant:${it.kind}" }.toSet(),
        )
    }
}
