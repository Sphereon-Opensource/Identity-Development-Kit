...
/**
 * Blob-store-backed implementation of [CredentialDesignRepository].
 *
 * Stores credential design metadata as JSON documents in the blob store under a `_meta/` prefix.
 * EDK's SQL persistence modules replace this via `@ContributesBinding(replaces=[...])`.
 */
@Inject
@SingleIn(SessionScope::class)
@ContributesBinding(SessionScope::class, binding = binding<CredentialDesignRepository>())
class BlobStoreCredentialDesignRepository(
    private val blobService: BlobService,
) : CredentialDesignRepository {
    private val json = BlobStoreJson

    private fun metaPath(id: Uuid): String = "vc-designs/_meta/credentials/$id/record.json"

    private fun metaPathById(id: Uuid): String = "vc-designs/_meta/credentials/_by-id/$id/record.json"
    ...
    override suspend fun findById(
        ...
        return json.decodeFromString<CredentialDesignRecord>(result.value.data.decodeToString())
    }
    ...
    override suspend fun findAll(
        tenantId: String,
        filter: DesignFilter,
    ): List<CredentialDesignRecord> = findAllRecords(tenantId).filter { matchesFilter(it, filter) }

    override suspend fun create(record: CredentialDesignRecord): CredentialDesignRecord {
    ...
    override suspend fun update(record: CredentialDesignRecord): CredentialDesignRecord {
    ...
    private suspend fun findAllRecords(tenantId: String): List<CredentialDesignRecord> {
        val listResult =
            blobService.listBlobs(
                BlobInfo(path = "vc-designs/_meta/credentials/", tenantId = tenantId),
                ListOptions(prefix = "vc-designs/_meta/credentials/", recursive = true, maxResults = 1000),
            )
        if (listResult.isErr) {
            return emptyList()
        }

        val records = mutableListOf<CredentialDesignRecord>()
        for (descriptor in listResult.value.descriptors) {
            ...
            records.add(json.decodeFromString<CredentialDesignRecord>(blob.value.data.decodeToString()))
        }
        return records
    }

    private fun matchesFilter(
        record: CredentialDesignRecord,
        filter: DesignFilter,
    ): Boolean {
