...
/**
 * Blob-store-backed implementation of [IssuerDesignRepository].
 *
 * Stores issuer design metadata as JSON documents in the blob store under a `_meta/` prefix.
 * EDK's SQL persistence modules replace this via `@ContributesBinding(replaces=[...])`.
 */
@Inject
@SingleIn(SessionScope::class)
@ContributesBinding(SessionScope::class, binding = binding<IssuerDesignRepository>())
class BlobStoreIssuerDesignRepository(
    private val blobService: BlobService,
) : IssuerDesignRepository {
    private val json = BlobStoreJson

    private fun metaPath(id: Uuid): String = "vc-designs/_meta/issuers/$id/record.json"

    private fun metaPathById(id: Uuid): String = "vc-designs/_meta/issuers/_by-id/$id/record.json"
    ...
    override suspend fun findById(
        ...
        return json.decodeFromString<IssuerDesignRecord>(result.value.data.decodeToString())
    }
    ...
    override suspend fun findAll(
        tenantId: String,
        filter: DesignFilter,
    ): List<IssuerDesignRecord> = findAllRecords(tenantId).filter { matchesFilter(it, filter) }

    override suspend fun create(record: IssuerDesignRecord): IssuerDesignRecord {
    ...
    override suspend fun update(record: IssuerDesignRecord): IssuerDesignRecord {
    ...
    private suspend fun findAllRecords(tenantId: String): List<IssuerDesignRecord> {
        val listResult =
            blobService.listBlobs(
                BlobInfo(path = "vc-designs/_meta/issuers/", tenantId = tenantId),
                ListOptions(prefix = "vc-designs/_meta/issuers/", recursive = true, maxResults = 1000),
            )
        if (listResult.isErr) {
            return emptyList()
        }

        val records = mutableListOf<IssuerDesignRecord>()
        for (descriptor in listResult.value.descriptors) {
            ...
            records.add(json.decodeFromString<IssuerDesignRecord>(blob.value.data.decodeToString()))
        }
        return records
    }

    private fun matchesFilter(
        record: IssuerDesignRecord,
        filter: DesignFilter,
    ): Boolean {
