...
/**
 * Result of fetching an external schema.
 */
...
@ContributesBinding(SessionScope::class, binding = binding<SchemaExternalFetcher>())
class DefaultSchemaExternalFetcher(
    private val httpClientFactory: HttpClientFactory,
) : SchemaExternalFetcher {
    ...
    override suspend fun fetch(
        ...
        return try {
            val response =
                httpClient.get(url) {
                    header("User-Agent", "Sphereon-SchemaRegistry/1.0")
                    if (ifNoneMatch != null) {
            ...
            when (response.status) {
                HttpStatusCode.NotModified -> {
                    Ok(FetchResult(data = ByteArray(0), contentType = null, etag = ifNoneMatch, notModified = true))
                }
                    ...
                    if (data.size > maxSizeBytes) {
                        return Err(
                            IdkError.ILLEGAL_ARGUMENT_ERROR(
                                message = "External schema exceeds max size: ${data.size} bytes > $maxSizeBytes bytes",
                            ),
                        )
                    }
                    val contentType = response.contentType()?.toString()
                    val etag = response.headers["ETag"]?.trim('"')
                    Ok(FetchResult(data = data, contentType = contentType, etag = etag))
                }

                else -> {
                    Err(
                        IdkError.UNKNOWN_ERROR(
                            message = "Failed to fetch external schema from $url: HTTP ${response.status.value}",
                        ),
                    )
                }
            }
            ...
            Err(IdkError.UNKNOWN_ERROR(message = "Failed to fetch external schema from $url: ${expected.message}", exception = expected))
        } finally {
            try {
                httpClient.close()
            } catch (_: Exception) {
