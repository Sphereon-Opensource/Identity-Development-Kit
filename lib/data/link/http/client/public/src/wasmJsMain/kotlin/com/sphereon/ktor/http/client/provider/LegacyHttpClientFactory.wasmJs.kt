...
import com.sphereon.ktor.http.client.config.WasmJsSslProviderImpl
...
actual class LegacyHttpClientFactory {
    ...
    actual fun createClient(options: LegacyHttpClientOptions): HttpClient {
        with(options) {
            val engine = when (engine) {
                HttpClientEngineType.CIO -> createCioEngine(sslConfig as WasmJsSslProviderImpl?)
                HttpClientEngineType.OKHTTP -> TODO("OKHTTP is not supported on wasmJs")
                HttpClientEngineType.DARWIN -> TODO("Darwin is not supported on wasmJs")
                HttpClientEngineType.JS -> TODO("JS engine is not supported on wasmJs, use CIO")
            }
    ...
    /**
     * Create a CIO engine with the specified [WasmJsSslProviderImpl] options.
     *
     * @param config The SSL provider configuration
     * @return The configured HTTP client engine
     */
    private fun createCioEngine(config: WasmJsSslProviderImpl?): HttpClientEngine =
        CIO.create {
            https {
                // No SSL customization support on wasmJs CIO yet
            }
        }
}
