...
actual class LegacyHttpClientFactory() {
    ...
    actual fun getSupportedEngineTypes(): List<HttpClientEngineType> {
        return listOf(HttpClientEngineType.CIO, HttpClientEngineType.OKHTTP)
    }
    ...
    private fun createOkHttpEngine(sslOptions: LegacyJvmSslProviderImpl?): HttpClientEngine {
        return OkHttp.create {
            sslOptions?.let { clientSslOptions ->
                this.config {
                    val (clientCertificatesAndKeys, trustManager) = runBlocking {
                        clientSslOptions.getCertificates() to clientSslOptions.getTrustManager()
                    }

                    val keyManagers = when {
...
val passwordChars = (33..126).map { it.toChar() }.toCharArray()
