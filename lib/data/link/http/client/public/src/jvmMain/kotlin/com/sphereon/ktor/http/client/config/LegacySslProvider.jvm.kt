 ...
 */
class LegacyJvmSslProviderImpl(private val opts: LegacySslConfig) : LegacySslProvider {
    /**
    ...
    override suspend fun getCertificates(): List<LegacyCertificateAndKeyJvm> {
        with(opts) {
            return certificateAliases.map { alias ->
                ...
                val certificateAndKey = LegacyCertificateAndKeyJvm(
...
*/
    ...
    override suspend fun getTrustManager(): X509TrustManager {
        ...
        val allTrustManagers = trustManagerFactory.trustManagers
            .filterIsInstance<X509TrustManager>().ifEmpty { throw IllegalStateException("No default X509TrustManager found in TrustManagerFactory.") }
...
actual fun createLegacySslProvider(opts: LegacySslConfig): LegacySslProvider {
    return LegacyJvmSslProviderImpl(opts)
}
