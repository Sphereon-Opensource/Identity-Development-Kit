...
actual fun startServer(opts: ServerOpts): TestServer {
    ...
    val trustStore = opts.trustStoreOpts?.let { ts ->
    ...
    val server = HttpsServer.create(InetSocketAddress(opts.port), 0).apply {
    ...
    return object : TestServer {
        override fun actualPort(): Int =
            (server.address as InetSocketAddress).port
...
private fun loadKeyStore(path: String, password: String, type: PredefinedKeyStoreTypes): KeyStore {
    require(
