...
class HttpClientProviderIntegrationTest {
    ...
    val app = createJvmHttpClientTestAppComponent(application = this)
    ...
    fun beforeEach() {
        ...
        rootCaCert = TestCertificateGenerator.createSelfSignedCertificate(
    ...
    fun `full mTLS handshake using KMS key should succeed`() = runTest {
        // --- server setup ---
        val serverKeyPair = TestCertificateGenerator.generateKeyPair(curveNameOrKeySize = "secp256r1")
        val serverCert = TestCertificateGenerator.createSignedCertificate(
        ...
        val serverJks = KeyStore.getInstance("JKS").apply {
            load(null, null)
            setKeyEntry("serverKey", serverKeyPair.private, privateKeyPassword.toCharArray(), arrayOf(serverCert, rootCaCert))
        }
        val serverKeystoreFile = File.createTempFile("srv-keystore", ".jks").apply {
            deleteOnExit()
        ...
        val serverTrustJks = KeyStore.getInstance("JKS").apply {
            load(null, null)
            setCertificateEntry("rootCa", rootCaCert)
        }
        val serverTrustFile = File.createTempFile("srv-trust", ".jks").apply {
            deleteOnExit()
            FileOutputStream(this).use { os ->
        ...
        app as JvmHttpClientTestAppComponent
        ...
        val clientCert = TestCertificateGenerator.createSignedCertificate(
            subjectDNStr = "CN=TestClient-${Uuid.v4String()}",
            subjectPublicKey = run {
                // reconstruct EC public key from JWK
                val curveName = when (jwk.crv.toString()) {
                    "P-256" -> "secp256r1"
                    "P-384" -> "secp384r1"
                    "P-521" -> "secp521r1"
                ...
                val params = AlgorithmParameters
                    .getInstance("EC").apply { init(ECGenParameterSpec(curveName)) }
        ...
        // --- client trust store ---
        val clientTrustJks = KeyStore.getInstance("JKS").apply {
            load(null, null)
            setCertificateEntry("rootCa", rootCaCert)
        }
        val clientTrustFile = File.createTempFile("cli-trust", ".jks").apply {
            deleteOnExit()
            FileOutputStream(this).use { os ->
        ...
        client = LegacyHttpClientFactory().createClient(
    ...
    @Test
    fun `mTLS handshake should fail when client certificate is missing`() = runTest {
        val serverKeyPair = TestCertificateGenerator.generateKeyPair(curveNameOrKeySize = "secp256r1")
        val serverCert = TestCertificateGenerator.createSignedCertificate(
            subjectDNStr = "CN=TestServer-${Uuid.v4String()}",
            subjectPublicKey = serverKeyPair.public,
