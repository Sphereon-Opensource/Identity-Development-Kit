...
class HttpClientFactoryIosImpl(
    ...
    init {
        ...
        kms.getProviderIds().forEach { providerId ->
            ...
            (keyStoreService as? KeyStore)?.let { keyStores.add(it); log.debug("Added KeyStore $it from provider $providerId") }
        }
    }
...
//                        loggingConfig?.invoke(this)
                ...
                val identityMap: Map<String, SecIdentityRef> = if (options.sslConfig.client.isMtls()) {
                ...
                if (identityMap.isNotEmpty() || serverTrustAnchors != null) {
                    engine {
                        handleChallenge(({ session: NSURLSession,
    ...
    private suspend fun buildIdentityMap(sslConfig: ClientSslConfig): Map<String, SecIdentityRef> {
        ...
        sslConfig.defaultCertificate?.let { certOpts ->
            try {
                ...
                log.error("Failed to create identity for default certificate ${certOpts.certificateAlias}: ${e.message}", e)
                throw e
            }
        }
        ...
        sslConfig.perHostCertificate.forEach { (host, certOpts) ->
            try {
                ...
                log.error("Failed to create identity for host $host with certificate ${certOpts.certificateAlias}: ${e.message}", e)
                throw e
            }
        }
    ...
    private suspend fun createIdentityFromCertOpts(certOpts: KeystoreCertificateOpts): SecIdentityRef? {
        val keyStore = keyStores.find { it.id == certOpts.keyStoreId }
        ...
        val secKey = com.sphereon.crypto.kms.keystore.software.getNativeKeychainKey(certOpts.certificateAlias)
    ...
    private fun convertCertificatesToSecCerts(certChain: Array<Certificate>): List<SecCertificateRef> {
        return certChain.map { cert ->
            val cfData: CFDataRef? = cert.der.usePinned { pinned ->
    ...
    /**
     * Converts a JWK to a native SecKeyRef using the Signum library.
     * Returns both the SecKeyRef and the DER-encoded key data.
     */
    @OptIn(ExperimentalForeignApi::class)
    private fun convertJwkToSecKey(jwk: Jwk): Pair<SecKeyRef, ByteArray> {
        val signumKey = jwk.toSignumPrivateKey()
        return when (signumKey) {
            is CryptoPrivateKey.EC.WithPublicKey -> {
                val ecKey = signumKey as CryptoPrivateKey.EC.WithPublicKey
                val ownedSecKey = ecKey.toSecKey().getOrThrow()
                val derData = ecKey.encodeToDer()
                val secKey: SecKeyRef = ownedSecKey.giveToCF()
                secKey to derData
            }
            is CryptoPrivateKey.RSA -> {
                val rsaKey = signumKey as CryptoPrivateKey.RSA
                val withPubKey = rsaKey as CryptoPrivateKey.WithPublicKey<*>
                val ownedSecKey = withPubKey.toSecKey().getOrThrow()
                val derData = rsaKey.encodeToDer()
                val secKey: SecKeyRef = ownedSecKey.giveToCF()
                secKey to derData
            }

            else -> throw IllegalStateException("Unsupported key type: ${signumKey::class.simpleName}")
        }
    }
    ...
    private fun createIdentityViaKeychainNative(
        ...
        key: SecKeyRef
    ): SecIdentityRef = memScoped {
        val label = "com.sphereon.ktor.mtls.$alias".encodeToByteArray()
        val labelData = label.usePinned { pinned ->
        ...
        val certPubKey = SecCertificateCopyPublicKey(cert)
        ...
        val certAppLabel = pubKeyAttrs?.let {
            CFDictionaryGetValue(it, kSecAttrApplicationLabel) as? CFDataRef
        ...
        val certDict = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, kCFTypeDictionaryKeyCallBacks.ptr, kCFTypeDictionaryValueCallBacks.ptr)
        ...
        if (certStatus != 0 && certStatus != -25299) {
            throw IllegalStateException("Failed to add certificate to keychain: status=$certStatus")
        }
        ...
        val plainAliasLabel = alias.encodeToByteArray().usePinned { pinned ->
        ...
        val privKeyQuery = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, kCFTypeDictionaryKeyCallBacks.ptr, kCFTypeDictionaryValueCallBacks.ptr)
        ...
        if (privKeyStatus != 0) {
            throw IllegalStateException("Failed to find private key by application label: status=$privKeyStatus")
        }

        val privateKeyForIdentity = privKeyOut.value as? SecKeyRef
        ...
        val certQuery = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, kCFTypeDictionaryKeyCallBacks.ptr, kCFTypeDictionaryValueCallBacks.ptr)
        ...
        val idQuery = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, kCFTypeDictionaryKeyCallBacks.ptr, kCFTypeDictionaryValueCallBacks.ptr)
    ...
    private fun createIdentityViaKeychain(
        ...
        val certPubKey = SecCertificateCopyPublicKey(cert) ?: run {
        ...
        val keyData = keyDer.usePinned { pinned ->
    ...
    private fun createIdentitySimple(
        ...
        val deleteCertQuery = CFDictionaryCreateMutable(
            kCFAllocatorDefault,
            0,
        ...
        // Add certificate
        val certDict = CFDictionaryCreateMutable(
            kCFAllocatorDefault,
            0,
            kCFTypeDictionaryKeyCallBacks.ptr,
            kCFTypeDictionaryValueCallBacks.ptr
        ...
        val searchQuery = CFDictionaryCreateMutable(
            kCFAllocatorDefault,
            0,
            kCFTypeDictionaryKeyCallBacks.ptr,
            kCFTypeDictionaryValueCallBacks.ptr
    ...
    private fun createIdentityOld(
        ...
        // Delete key
        val deleteKeyQuery = CFDictionaryCreateMutable(
            kCFAllocatorDefault,
            0,
            kCFTypeDictionaryKeyCallBacks.ptr,
            kCFTypeDictionaryValueCallBacks.ptr
        ...
        val deleteIdentityQuery = CFDictionaryCreateMutable(
            kCFAllocatorDefault,
            0,
            kCFTypeDictionaryKeyCallBacks.ptr,
            kCFTypeDictionaryValueCallBacks.ptr
        ...
        val certSearchQuery = CFDictionaryCreateMutable(
            kCFAllocatorDefault,
            0,
            kCFTypeDictionaryKeyCallBacks.ptr,
            kCFTypeDictionaryValueCallBacks.ptr
    ...
    private suspend fun buildServerTrustAnchors(caOpts: CaOpts): List<SecCertificateRef> {
        ...
        caOpts.additionalCAs.forEach { ca ->
            ...
            try {
                ...
                val cfData: CFDataRef? = caCert.der.usePinned { pinned ->
                    val ptr: CPointer<UByteVar> = pinned.addressOf(0).reinterpret()
                ...
                log.warn("Failed to load CA certificate ${ca.certificateAlias}: ${e.message}")
            }
        }
