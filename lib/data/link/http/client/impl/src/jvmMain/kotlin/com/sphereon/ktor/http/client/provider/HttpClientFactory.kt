...
class HttpClientFactoryJvmImpl(private val execution: SessionExecution, val kms: KeyManagerService, private val keyStoreManager: KeyStoreManager) : HttpClientFactory {
    ...
    override fun createClient(options: HttpClientOptions): HttpClient {
        ...
        with(options) {
            val engine = when (engine ?: getEngineTypeDefault()) {
...
//                        loggingConfig?.invoke(this)
    ...
    private fun createCioEngine(sslOptions: LegacyJvmSslProviderImpl? = null): HttpClientEngine {
        return CIO.create {
            sslOptions?.let { clientSslOptions ->
                https {
                    val (certs: List<LegacyCertificateAndKeyJvm>, trustMgr) = runBlocking {
                    ...
                    certificates += certs.map {
                        ...
                        ktorCert.certificateChain.forEach { cert ->
                            if (cert.publicKey.algorithm != "RSA" || cert.publicKey.algorithm != "DSS") throw UnsupportedOperationException(
    ...
    private suspend fun buildClientSslContext(sslConfig: SslConfig, trustManagers: Set<X509TrustManager>? = null): SSLContext {
