...
class ParseUriQueryCommandImpl(
    ...
    ParseUriQueryCommandService {
    override suspend fun parseUriQuery(uri: String): IdkResult<ParsedUri, IdkError> = execute(uri)
    ...
    override suspend fun doExecute(
        ...
        val baseUri =
            if (queryIndex >= 0) {
                processedUri.substring(0, queryIndex)
            } else {
                processedUri
            }
        val queryString =
            if (queryIndex >= 0 && queryIndex < processedUri.length - 1) {
        ...
        val queryParameters: Parameters =
            if (queryString != null && queryString.isNotBlank()) {
