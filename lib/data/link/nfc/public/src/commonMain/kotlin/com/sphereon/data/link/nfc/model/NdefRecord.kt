...
data class NdefRecord(
    ...
    enum class Tnf(
         ...
         */
        ABSOLUTE_URI(TNF_ABSOLUTE_URI),

        /**
         * Indicates the type field contains an external type name.
         */
        EXTERNAL_TYPE(TNF_EXTERNAL_TYPE),
         ...
         * Reserved TNF type.
         */
        RESERVED(TNF_RESERVED),
    }

    /**
     * Encodes the record into a [kotlinx.io.bytestring.ByteStringBuilder].
     *
     * @param bsb the [kotlinx.io.bytestring.ByteStringBuilder] to append the encoding of the record to other records.
    ...
    internal fun encode(
        ...
        val isShortRecord = payload.size < SHORT_RECORD_MAX_SIZE
        val idLengthPresent =
            if (tnf == Tnf.EMPTY) {
                true
            } else {
        ...
        var flags = tnf.value
        if (isMessageBegin) {
            flags = flags or FLAG_MB
        }
        if (isMessageEnd) {
            flags = flags or FLAG_ME
        }
        if (isShortRecord) {
            flags = flags or FLAG_SR
        }
        if (idLengthPresent) {
            flags = flags or FLAG_IL
        }
     ...
     * - [Tnf.WELL_KNOWN] with type [com.sphereon.data.link.nfc.model.NfcConst.RTD_URI]
     * - [Tnf.WELL_KNOWN] with type [com.sphereon.data.link.nfc.model.NfcConst.RTD_SMART_POSTER] and containing a URI record in the NDEF message nested in the
     * payload.
     *
        ...
        return when (tnf) {
            Tnf.WELL_KNOWN -> {
                when (type) {
                    ...
                    NfcConst.RTD_URI -> {
                        return URI_PREFIX_MAP[payload[0].toInt()] + payload.toByteArray(1).decodeToString()
                    }
    ...
    companion object {
        // Defined in NFC Forum NFC Data Exchange Format Technical Specification section 2.6
        //
        ...
        private const val TNF_ABSOLUTE_URI = 0x03
        private const val TNF_EXTERNAL_TYPE = 0x04
        private const val TNF_UNKNOWN = 0x05
        private const val TNF_UNCHANGED = 0x06
        private const val TNF_RESERVED = 0x07
        private const val TNF_MASK = 0x07
        private const val SHORT_RECORD_MAX_SIZE = 256
        ...
        internal fun fromEncoded(encoded: ByteArray): List<NdefRecord> {
            try {
                ...
                while (!reader.exhausted()) {
                    ...
                    var tnf = Tnf.entries[flags.and(TNF_MASK)]
                    ...
                    require(tnf != Tnf.EMPTY || id.size == 0) { "TNF_EMPTY must have empty id" }

                    require(chunkBsb == null || tnf == Tnf.UNCHANGED) { "In-chunk but TNF isn't UNCHANGED" }
                    require(chunkBsb != null || tnf != Tnf.UNCHANGED) { "Not in-chunk with TNF UNCHANGED" }
                ...
                require(records.size != 0) { "No records in message" }
                return records
            } catch (expected: Throwable) {
                throw IllegalArgumentException("Parsing NdefMessage failed", expected)
            }
        }
         ...
         *
         * This is a mapping of "URI IdentifierAlias Codes" to URI string prefixes,
         * per section 3.2.2 of the NFC Forum URI Record Type Definition document.
         */
        private val URI_PREFIX_MAP =
            listOf(
                "", // 0x00
                "http://www.", // 0x01
                "https://www.", // 0x02
         ...
         * Uses the well known URI type representation [Tnf.WELL_KNOWN] and [com.sphereon.data.link.nfc.model.NfcConst.RTD_URI].
