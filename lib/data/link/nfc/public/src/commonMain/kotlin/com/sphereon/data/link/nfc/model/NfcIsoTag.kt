...
abstract class NfcIsoTag {
    ...
    suspend fun ndefTransact(
        ...
        if (!NDEF_TRANSACT_BYPASS_UPDATE_BINARY_OPTIMIZATION && encodedNdefMessage.size < 256 - 2) {
            updateBinary(
                0, buildByteString { appendUInt16(encodedNdefMessage.size).append(encodedNdefMessage) }.toByteArray()
            )
        } else {
            ...
            updateBinary(
                0, buildByteString { appendUInt16(encodedNdefMessage.size) }.toByteArray()
            )
        }
        val tWait = Duration.fromWtInt(wtInt)
        delay(tWait)
