...
class NdefMessageTest {
    ...
    fun testInvalidParsing() {
        ...
        for (encodedHex in invalidMessages) {
            assertFailsWith(
                ...
                val messsage = NdefMessage.fromEncoded(encodedHex.decodeFromHex())
            }
        }
    }
    ...
    fun testValidParsing() {
        ...
        val testData =
            listOf<Pair<String, NdefMessage>>(
                // short record
                Pair(
                    "d0 00 00",
                ...
                Pair(
                    ...
                    NdefMessage(
                        listOf(
                            NdefRecord(
                                NdefRecord.Tnf.WELL_KNOWN,
                                type = NfcConst.RTD_SMART_POSTER,
                                payload =
                                    ByteString(
                                        NdefMessage(
                                            listOf(
        ...
        for ((hexEncoding, message) in testData) {
            assertEquals(
                message,
                NdefMessage.fromEncoded(hexEncoding.replace(" ", "").decodeFromHex()),
    ...
    fun testEncodeDecode() {
        ...
        val testData =
            listOf<Pair<String, NdefMessage>>(
                ...
                Pair(
                    "d1 01 08 55 01" + "nfc.com".encodeToByteArray().encodeToHex(),
                    NdefMessage(
                        listOf(
                ...
                Pair(
                    "d1 01 0d 55 05" + "+35891234567".encodeToByteArray().encodeToHex(),
                    NdefMessage(
                        listOf(
                ...
                Pair(
                    "d1 01 04 55 00" + "foo".encodeToByteArray().encodeToHex(),
                    NdefMessage(
                        listOf(
                    ...
                    NdefRecord(
                        ...
                        payload =
                            ByteString(
                                NdefMessage(
                                    listOf(
                                        NdefRecord(NdefRecord.Tnf.WELL_KNOWN, NfcConst.RTD_TEXT, ByteString(), ByteString()),
                                        NdefRecord.createUri("http://www.android.com"),
                                    ),
                                ).encode(),
                            ),
    ...
    fun testCreateMimeType() {
        val testData =
            listOf<Triple<String?, ByteString, NdefRecord>>(
                Triple(
                    "text/plain",
