...
class CommandApduTest {
    ...
    fun encodeDecode() {
        ...
        val pairs: List<Pair<String, CommandApdu>> =
            listOf(
        ...
        for ((hexEncoding, commandApdu) in pairs) {
            ...
            assertEquals(
                commandApdu,
                CommandApdu.decode(hexEncoding.decodeFrom(Encoding.HEX)),
                "Decoding of $hexEncoding is ${CommandApdu.decode(hexEncoding.decodeFrom(Encoding.HEX))} which wasn't expected",
            )
        }
    }
}
