...
//    val app: App,
    ...
    private val centralManagerDelegate = object : NSObject(), CBCentralManagerDelegateProtocol {
        ...
        override fun centralManagerDidUpdateState(central: CBCentralManager) {
            val stateDescription = when (central.state) {
        ...
        override fun centralManager(
            ...
            val services = serviceUUIDs?.mapNotNull {
    ...
    init {
        ...
        internalBleEvents.filter { it is BleEvent.CharacteristicChanged }
            .onEach { event ->
    ...
    override suspend fun scan(args: ScanDevicesArgs): IdkResult<List<BleDevice>, ScanError> {
        ...
        return try {
            ...
            if (currentState != CBCentralManagerStatePoweredOn) {
                ...
                try {
                    withTimeout(10000) { // 10 second timeout for power on
                        powerOnDeferred.await()
                    }
                } catch (e: kotlinx.coroutines.TimeoutCancellationException) {
                    val finalState = manager.state
                    val finalStateDesc = when (finalState) {
                        CBCentralManagerStateUnknown -> "Unknown"
                        CBCentralManagerStateResetting -> "Resetting"
                        CBCentralManagerStateUnsupported -> "Unsupported"
                        CBCentralManagerStateUnauthorized -> "Unauthorized"
            ...
            val serviceUUIDs = args.filters?.firstOrNull()?.let { filter ->
            ...
            log.error("scan() failed: ${e.message}", exception = e)
            scanCompletionDeferred = null
            val error = BleErrors.scanFailed("Scan failed", e)
            externalDispatch(
                BleEvent.Error(
                    requestId = requestId,
                    deviceAddress = "<scanning>",
    ...
    override suspend fun connect(device: IHasAddress): IdkResult<BleDevice, ConnectionFailedError> {
        ...
        return try {
            ...
            val peripheralToConnect = discoveredDevices[device.address]?.let {
            ...
            log.error("connect() failed: ${e.message}", exception = e)
    ...
    override suspend fun disconnect(): IdkResult<Unit, BleError> {
        ...
        return try {
            ...
            log.error("disconnect() failed: ${e.message}", exception = e)
            BleErrors.unknown("Disconnect failed", e).asErrorResult()
        }
    }
    ...
    override suspend fun setMtu(mtu: Int): IdkResult<Int, MtuChangeError> {
        ...
        val maximumWriteValueLength = device.maximumWriteValueLengthForType(
    ...
    override suspend fun discoverServices(): IdkResult<List<GattService>, BleError> {
        ...
        return try {
            ...
            log.error("discoverServices() failed: ${e.message}", exception = e)
    ...
    override suspend fun discoverServiceCharacteristics(
        ...
        return try {
            ...
            val cbService = device.services?.firstOrNull {
            ...
            val discoveredChars = cbService.characteristics?.mapNotNull { char ->
                (char as? CBCharacteristic)?.toGattCharacteristic()
            } ?: emptyList()

            // If specific characteristics requested, filter them
            val result = if (characteristics.isNullOrEmpty()) {
                discoveredChars
            } else {
            ...
            log.error("discoverServiceCharacteristics() failed: ${e.message}", exception = e)
            BleErrors.readCharacteristicFailed("Characteristic discovery failed", e).asErrorResult()
        }
    }
    ...
    override suspend fun readCharacteristic(
        ...
        return try {
            ...
            val cbCharacteristic = findCharacteristic(device, service, characteristic)
            ...
            log.error("readCharacteristic() failed: ${e.message}", exception = e)
            BleErrors.readCharacteristicFailed("Read characteristic failed", e).asErrorResult()
        }
    }
    ...
    override suspend fun writeCharacteristic(
        ...
        return try {
            ...
            log.error("writeCharacteristic() failed: ${e.message}", exception = e)
            BleErrors.writeCharacteristicFailed("Write characteristic failed", e).asErrorResult()
        }
    }

    override suspend fun getDevice(): IdkResult<BleDevice, ConnectionFailedError> {
        return connectedDevice?.asOkResult()
    ...
    override suspend fun enableNotifications(
        ...
        return try {
            ...
            try {
                withTimeout(10000) { // 10 second timeout for notification subscription
                    deferred.await()
            ...
            log.error("enableNotifications() failed: ${e.message}", exception = e)
            BleErrors.notificationFailed("Enable notifications failed", e).asErrorResult()
        }
    }
    ...
    private fun internalDispatch(event: BleEvent) {
        ...
        val emitted = _internalBleEvents.tryEmit(event)
        if (event is BleEvent.CharacteristicChanged) {
            log.info("internalDispatch: tryEmit result=$emitted, subscriptionCount=${_internalBleEvents.subscriptionCount.value}")
        }
    }
    ...
    private fun externalDispatch(event: BleEvent) {
        if (event is BleEvent.Error) {
            ...
            log.info("externalDispatch(): CharacteristicChanged for ${event.characteristic.id}, ${event.value.size} bytes, listeners=${externalListeners.size}")
        } else {
        ...
        for (listener in externalListeners.toList()) {
            when (event) {
                ...
                is BleEvent.AbstractBleEvent -> listener.onBleError(
                    BleEvent.Error.from(event, message = "Default BLE event should not be used directly")
                )
            }
        }
    }
    ...
    override fun close() {
        scope.launch {
            try {
                ...
                log.error("close() failed: ${e.message}", exception = e)
            }
        }
    }
}
...
fun CBService.toGattService(): GattService {
    return GattService(
        id = this.UUID.UUIDString.toFullBluetoothUuid(),
        characteristics = this.characteristics?.mapNotNull { char ->
