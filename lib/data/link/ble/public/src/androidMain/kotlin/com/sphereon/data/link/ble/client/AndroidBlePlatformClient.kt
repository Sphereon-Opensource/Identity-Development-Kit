...
val sureRequiredBlePermissions = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
    arrayOf(
...
class AndroidBlePlatformClient(
    ...
    private val internalBleEvents: SharedFlow<BleEvent> = _internalBleEvents.shareIn(scope, SharingStarted.Eagerly, replay = 0)
    ...
    private val externalListeners = mutableSetOf<BleEvent.Listener>()
    ...
    init {
        ...
        internalBleEvents.filter { event ->
    ...
    override suspend fun scan(args: ScanDevicesArgs): IdkResult<List<BleDevice>, ScanError> = suspendCancellableCoroutine { cont ->
        ...
        val callback = object : ScanCallback() {
            ...
            override fun onScanResult(callbackType: Int, result: ScanResult) {
                ...
                val bleDevice = BleDevice(
                    address = device.address, name = name, rssi = result.rssi, services = uuids.map { it.toUuidId() })
                ...
                if (devices.size >= args.maxResults) {
                    ...
                    try {
                        ...
                        log.warn("[id: $shortId] Failed to stop scan in onScanResult: ${e.message}")
                    }
                    externalDispatch(
                        BleEvent.ScanResult(
                            requestId = requestId,
            ...
            override fun onScanFailed(errorCode: Int) {
                ...
                externalDispatch(
                    BleEvent.Error(
                        requestId = requestId, error = error, deviceAddress = bluetoothGatt?.device?.address ?: "<unknown>", originalOperationId = "ScanStarted"
                    )
                )
        ...
        try {
            ...
            log.debug("[id: $shortId] Stop scan failed, on new scan start, ${e.message}")
        ...
        if (existingGatt != null) {
            ...
            try {
                ...
                log.warn("[id: $shortId] Failed to cleanup existing GATT: ${e.message}")
            }
        }
        ...
        cont.invokeOnCancellation {
            ...
            try {
                ...
                log.warn("[id: $shortId] Failed to stop scan on cancellation: ${e.message}")
            }
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            try {
                ...
                if (e.message?.contains("Already resumed") != true) {
                    log.warn("[id: $shortId] Scan timeout handling failed: ${e.message}")
                }
            }
        ...
        return try {
            ...
            if (existingGatt != null) {
                ...
                try {
                    ...
                    log.warn("[id: $shortId] Failed to cleanup existing connection: ${e.message}")
                }
                this.bluetoothGatt = null
                this.connectedDevice = null
            ...
            this.bluetoothGatt = bluetoothDevice.connectGatt(
                context,
                false,
                AndroidBleClientCallback(
                    requestId = requestId,
                    log = log,
                    internalBleEvents = _internalBleEvents
                ),
                TRANSPORT_LE
            )

            val event = internalBleEvents.filter { it.requestId == requestId }
                .filter { it is BleEvent.ConnectionStateChanged || (it is BleEvent.Error && it.originalEvent is BleEvent.ConnectionStateChanged) }.first()
            ...
            log.error("[id: $shortId] connect() failed: ${e.message}", exception = e)
            val error = BleErrors.connectionFailed("Connection failed", e)
            externalDispatch(
                BleEvent.Error(
                    requestId = requestId,
                    deviceAddress = bluetoothGatt?.device?.address ?: "<unknown>",
    ...
    override suspend fun setMtu(mtu: Int): IdkResult<Int, MtuChangeError> {
        ...
        return gattOperationMutex.withLock {
            try {
                ...
                val event =
                    internalBleEvents.filter { it.requestId == requestId }
                        .filter { it is BleEvent.MtuChanged || (it is BleEvent.Error && it.originalEvent is BleEvent.MtuChanged) }.first()
                ...
                log.error("[id: $shortId] setMtu(): failed: ${e.message}", exception = e)
                val error = BleErrors.mtuChangeFailed("Mtu change failed", e)
                externalDispatch(
                    BleEvent.Error(
                        requestId = requestId,
                        deviceAddress = bluetoothGatt?.device?.address ?: "<unknown>",
    ...
    override suspend fun discoverServices(): IdkResult<List<GattService>, ServiceDiscoveryError> {
        ...
        return gattOperationMutex.withLock {
            try {
                ...
                val event = internalBleEvents.filter { it.requestId == requestId }
                    .filter { it is BleEvent.ServicesDiscovered || (it is BleEvent.Error && it.originalEvent is BleEvent.ServicesDiscovered) }.first()
                ...
                log.error("[id: $shortId] discoverServices(): failed: ${e.message}", exception = e)
                val error = BleErrors.serviceDiscoveryFailed("Service discovery failed", e)
                externalDispatch(
                    BleEvent.Error(
                        requestId = requestId,
                        deviceAddress = bluetoothGatt?.device?.address ?: "<unknown>",
    ...
    @OptIn(ExperimentalUuidApi::class)
    override suspend fun discoverServiceCharacteristics(service: HasUuidId, characteristics: List<HasUuidId>?): IdkResult<List<GattCharacteristic>, CharacteristicReadError> {
        val requestId = scanRequestId
        ...
        if (characteristics.isNullOrEmpty()) {
            return gattService.characteristics.map { it.toGattCharacteristic() }.toList().asOkResult()
        }
    ...
    @OptIn(ExperimentalUuidApi::class)
    @RequiresPermission(Manifest.permission.BLUETOOTH_CONNECT)
    override suspend fun readCharacteristic(service: HasUuidId, characteristic: HasUuidId): IdkResult<GattCharacteristic, CharacteristicReadError> {
        val requestId = scanRequestId
        ...
        fun createErrorEvent(bluetoothGattCharacteristic: BluetoothGattCharacteristic): IdkResult<BleEvent.CharacteristicRead, CharacteristicReadError> {
            val gattService = getGattService(
                "CharacteristicRead", service, { BleErrors.readCharacteristicFailed("Service not found: ${service.id}") }, requestId = requestId
            ).onFailure { return it.asErrorResult() }.value
        ...
        val bluetoothGattCharacteristic = getCharacteristicFromService("CharacteristicRead", service = service, characteristic = characteristic, {
            BleErrors.readCharacteristicFailed("Read characteristic failed at request call. Characteristic $characteristic not found")
        }, requestId = requestId).onFailure {
            return it.asErrorResult().also {
                externalDispatch(
                    BleEvent.Error(
                        requestId = requestId, deviceAddress = bluetoothGatt?.device?.address ?: "<unknown>", error = it.error, originalOperationId = "ReadCharacteristic"
                    )
                )
            }
        }.value
        ...
        return gattOperationMutex.withLock {
            try {
                ...
                val event = internalBleEvents.filter { it.requestId == requestId }
                    .filter { it is BleEvent.CharacteristicRead || (it is BleEvent.Error && it.originalEvent is BleEvent.CharacteristicRead) }.first()
                ...
                log.error("[id: $shortId] readCharacteristic(): failed: ${e.message}", exception = e)
                val error = BleErrors.readCharacteristicFailed("Read characteristic failed", e)
                externalDispatch(
                    BleEvent.Error(
                        requestId = requestId,
                        deviceAddress = bluetoothGatt?.device?.address ?: "<unknown>",
    ...
    override suspend fun writeCharacteristic(
        ...
        val char = getCharacteristicFromService("WriteCharacteristic", service = service, characteristic = characteristic, {
                ...
                try {
                    ...
                    val job = scope.launch {
                        try {
                            ...
                            val result = if (event is BleEvent.Error && event.originalEvent is BleEvent.CharacteristicWrite) {
                            ...
                            if (continuation.isActive) {
                                log.error("[id: $shortId] writeCharacteristic(): event handler failed: ${e.message}", exception = e)
                                val error = BleErrors.writeCharacteristicFailed("Event handler failed: ${e.message}", e)
                    ...
                    val writeStatus = if (Build.VERSION.SDK_INT > Build.VERSION_CODES.TIRAMISU) {
                        ...
                        @Suppress("DEPRECATION") char.setValue(value)
                        char.writeType = writeType.value
                        @Suppress("DEPRECATION") gatt.writeCharacteristic(char).let { if (it) GATT_SUCCESS else GATT_FAILURE }
                    }
                    ...
                    if (writeStatus > GATT_SUCCESS) {
                        ...
                        val event = BleEvent.CharacteristicWrite(
                            requestId = requestId, deviceAddress = gatt.device.address, service = service, characteristic = characteristic, status = GATT_FAILURE
                        )
                    ...
                    log.error("[id: $shortId] writeCharacteristic(): failed: ${e.message}", exception = e)
                    val error = BleErrors.writeCharacteristicFailed("Write characteristic failed for ${characteristic.id}", e)
                    val event = BleEvent.CharacteristicWrite(
                        requestId = requestId, deviceAddress = gatt.device.address, service = service, characteristic = characteristic, status = GATT_FAILURE
                    )
                    if (continuation.isActive) {
    ...
    override suspend fun enableNotifications(
        ...
        val bluetoothGattCharacteristic = getCharacteristicFromService("EnableNotification", service = service, characteristic = characteristic, {
        ...
        return gattOperationMutex.withLock {
            try {
                val writeSuccess = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    ...
                    @Suppress("DEPRECATION") descriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE)
                ...
                val event = internalBleEvents.filter { it.requestId == requestId }
                    .filter { it is BleEvent.DescriptorWrite || (it is BleEvent.Error && it.originalEvent is BleEvent.DescriptorWrite) }.first()
                ...
                log.error("[id: $shortId] enableNotifications(): failed: ${e.message}", exception = e)
                val error = BleErrors.notificationFailed("Enabling notifications failed", e)
                externalDispatch(
                    BleEvent.Error(
                        requestId = requestId,
                        deviceAddress = gatt.device.address,
    ...
    override fun getBleEventListeners(): Set<BleEvent.Listener> = externalListeners
    override fun addBleEventListener(vararg listener: BleEvent.Listener) = apply { externalListeners += listener }
    override fun removeBleEventListener(listener: BleEvent.Listener) = apply { externalListeners -= listener }
    override fun clearBleEventListeners() = apply { externalListeners.clear() }
    ...
    private fun externalDispatch(event: BleEvent) {
        ...
        for (listener in externalListeners.toList()) {
            when (event) {
                ...
                is BleEvent.AbstractBleEvent -> listener.onBleError(BleEvent.Error.from(event, message = "Default BLE event should not be used directly"))
    ...
    private fun <ErrorType : BleErrors> getGattService(
        ...
        val gatt = getBluetoothGatt(errorOnNoGattCallback = errorOnNoServiceFoundCallback, operationId = operationId, requestId = requestId).onFailure {
    ...
    fun <ErrorType : BleErrors> getCharacteristicFromService(
        ...
        val gattService = getGattService(
            operationId = operationId, service = service, errorOnNoCharacteristicFoundCallback, requestId = requestId
        ).onFailure { return it.asErrorResult() }.value
        ...
        if (result == null) {
            ...
            val deviceAddress = getBluetoothDevice(
                operationId = "CharacteristicRead",
                errorOnNoCharacteristicFoundCallback,
                requestId = requestId
            ).onFailure { return it.asErrorResult() }.value.address
            val event = BleEvent.Error(
                requestId = requestId, deviceAddress = deviceAddress, error = error, message = error.message.defaultMessage, originalOperationId = "CharacteristicRead"
            )
            return error.asErrorResult().also { externalDispatch(BleEvent.Error.from(event = event, error = error)) }
        }
    ...
    private fun <ErrorType : BleErrors> getBluetoothDevice(
        ...
        val gatt = getBluetoothGatt(errorOnNoGattCallback = errorOnNoGattCallback, operationId = operationId, requestId = requestId).onFailure {
    ...
    override fun close() {
        ...
        if (!granted) {
            ...
            _internalBleEvents.resetReplayCache()
...
fun BluetoothGattCharacteristic.toGattCharacteristic(service: GattService? = null): GattCharacteristic {
    ...
    return GattCharacteristic(
        id = uuid.toKotlinUuid(), properties = props, service = service
    )
}
...
fun BluetoothGattService.toGattService(): GattService {
    return GattService(
        ...
        id = this.uuid.toKotlinUuid(), characteristics = this.characteristics.map { it.toGattCharacteristic(service = null) })
}
...
fun BluetoothGatt.toMinimalBleDevice(): BleDevice {
    ...
    return BleDevice(
        address = device.address, name = "unknown"
    )
}
...
fun BluetoothGatt.toBleDevice(): BleDevice {
    ...
    return BleDevice(
        ...
        name = device.name,
        rssi = null /*TODO*/,
        services = /*TODO: Is this correct?*/device.uuids?.map { it.uuid.toKotlinUuid().toUuidId() }?.toList() ?: emptyList()
    )
}

@OptIn(ExperimentalUuidApi::class)
fun BluetoothGattDescriptor.toGattDescriptor(
    characteristic: GattCharacteristic? = null,
): GattDescriptor {
    val permissionsResult = mutableSetOf<GattPermission>().apply {
    ...
    return GattDescriptor(
        id = uuid.toKotlinUuid(), permissions = permissionsResult, characteristic = characteristic
    )
}
