...
) : BaseBleCommand<Unit, BleResponse.Services>() {

    override val id: String = COMMAND_ID

    override suspend fun doExecute(args: Unit): IdkResult<BleResponse.Services, BleError> {
        return client.discoverServices().map { services -> BleResponse.Services(services) }
    }

    companion object {
        const val COMMAND_ID = "ble.services.discover"
    }
}
