...
) : IdkError(code = code, severity=severity, message = message, causes = causes, meta = meta, exception = exception), BleError {
...
//    fun asErrorResult() : IdkResult<Nothing, INfcError> = Err(this) as IdkResult<Nothing, INfcError>
    ...
    companion object {
        fun unknown(
            reason: String = "An unknown BLE error occurred", throwable: Throwable? = null, causes: List<IdkErrorType> = emptyList(),
        ) = UnknownBleError(
            message = Message(
                i18nKey = "ble.error.unknown", defaultMessage = reason
        ...
        fun scanFailed(
            ...
            message = Message(
                i18nKey = "ble.error.scan-failed", defaultMessage = reason
            ), exception = throwable, causes = causes
        )

        fun deviceNotFound(deviceAddress: String) = DeviceNotFoundError(
            message = Message(
                i18nKey = "ble.error.device-not-found", defaultMessage = "Device with id $deviceAddress not found"
            )
        )

        fun notificationFailed(reason: String, throwable: Throwable? = null) = NotificationFailedError(
            message = Message(
                i18nKey = "ble.error.notification-failed", defaultMessage = reason
            ), exception = throwable
        )

        fun serviceDiscoveryFailed(reason: String, throwable: Throwable? = null) = ServiceDiscoveryError(
            message = Message(
                i18nKey = "ble.error.service-discovery-failed", defaultMessage = reason
            ), exception = throwable
        )
         ...
         */
        fun cancelled(reason: String = "Operation was cancelled", throwable: Throwable? = null) = CancelledError(
            message = Message(
                i18nKey = "ble.cancelled", defaultMessage = reason
            ), exception = throwable
        )

        fun mtuChangeFailed(reason: String, throwable: Throwable? = null) = MtuChangeError(
            message = Message(
                i18nKey = "ble.error.mtu-change", defaultMessage = reason
            ), exception = throwable
        )
    }
