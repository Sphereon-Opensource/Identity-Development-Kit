...
sealed class BleResponse {
    ...
    data class Characteristic(val characteristic: GattCharacteristic) : BleResponse()
    @OptIn(ExperimentalObjCName::class)
    @ObjCName("Data", exact = true)
    data class Data(val value: ByteArray) : BleResponse()
    @OptIn(ExperimentalObjCName::class)
    @ObjCName("Services", exact = true)
    data class Services(val services: List<GattService>) : BleResponse()
    @OptIn(ExperimentalObjCName::class)
    @ObjCName("Devices", exact = true)
    data class Devices(val devices: List<BleDevice>) : BleResponse()
    @OptIn(ExperimentalObjCName::class)
    @ObjCName("Connect", exact = true)
    data class Connect(val device: BleDevice) : BleResponse()
}