...
/**
 * BLE scan filter support.
 * Filter API design inspired by Kable (https://github.com/JuulLabs/kable).
 * Only Service filter is supported for now.
 *
...
internal fun Filter.Name.matches(name: String?): Boolean {
    ...
    return when (this) {
        is Filter.Name.Exact -> name == exact
        is Filter.Name.Prefix -> name.startsWith(prefix)
    }
}

internal fun Filter.ManufacturerData.matches(
    id: Int?,
    data: ByteArray?,
): Boolean {
    if (this.id != id) {
...
internal fun Filter.ManufacturerData.matches(data: ByteArray?): Boolean = this.data?.matches(data, dataMask) ?: true

internal fun Filter.ServiceData.matches(data: ByteArray?): Boolean = this.data?.matches(data, dataMask) ?: true
...
private fun ByteArray.matches(
    ...
    if (lastMaskIndex > data.lastIndex) {
        return false
    }
...
private fun requireDataAndMaskHaveSameLength(
    data: ByteArray,
    dataMask: ByteArray,
