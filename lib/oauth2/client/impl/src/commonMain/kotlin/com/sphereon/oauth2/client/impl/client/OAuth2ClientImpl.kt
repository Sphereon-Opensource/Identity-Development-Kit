...
class OAuth2ClientImpl(
    ...
    override suspend fun fetchAuthorizationServerMetadata(
        issuer: String
    ): IdkResult<AuthorizationServerMetadata, IdkError> {
        return fetchMetadataCommand.execute(FetchServerMetadataArgs(issuer))
    }

    override fun isDpopSupported(
        authorizationServerMetadata: AuthorizationServerMetadata
    ): Boolean {
        return !authorizationServerMetadata.dpopSigningAlgValuesSupported.isNullOrEmpty()
    }
    ...
    override suspend fun initiateAuthorization(
        ...
        val pkceData = if (supportedMethods != null && supportedMethods.isNotEmpty()) {
            ...
            if (allowedMethods.isNotEmpty()) {
                val pkceResult = createPkceCommand.execute(
                    CreatePkceArgs(
                        codeVerifier = null,
        ...
        val urlResult = createAuthorizationRequestUrlCommand.execute(
    ...
    override suspend fun introspectToken(
        ...
        return introspectTokenCommand.execute(
            IntrospectTokenArgs(
                authorizationServerMetadata = authorizationServerMetadata,
                token = token,
                clientAuthentication = clientAuthentication,
    ...
    private suspend fun exchangeTokenWithDpopRetry(
        ...
        val initialDpopProof = dpopContext?.let { context ->
            val proofOptions = CreateDpopProofOptions(
        ...
        return if (result.isOk) {
            ...
            if (error.code == "use_dpop_nonce" && dpopNonce != null && dpopContext != null) {
                ...
                val retryProofOptions = CreateDpopProofOptions(
                ...
                val retryProof = retryProofResult.getOrElse { retryError ->
                    return Err(IdkError(
                        code = "server_error",
                        message = IdkError.Message(
                            i18nKey = "oauth2.error.server_error",
    ...
    override suspend fun validateIdToken(
        ...
        return validateIdTokenCommand.execute(ValidateIdTokenArgs(idToken, options))
    }
