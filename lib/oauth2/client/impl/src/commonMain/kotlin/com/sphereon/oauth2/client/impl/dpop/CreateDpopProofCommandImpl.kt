...
class CreateDpopProofCommandImpl(
    ...
    private suspend fun createDpopProofInternal(
        ...
        try {
            ...
            val jwtResult = jwtService.createJwsCompact(jwsArgs).getOrElse {error ->
                return Err(DpopError.GenerationFailed(
            ...
            return Err(DpopError.GenerationFailed(
                reason = "DPoP proof generation failed: ${e.message}",
                exception = e
            ))
        }
    }
    ...
    private fun determineAlgorithm(jwk: Jwk): String {
        ...
        return when (jwk.kty.value) {
            "RSA" -> "RS256" // Default RSA algorithm
            "EC" -> when (jwk.crv?.value) {
                "P-256" -> "ES256"
                "P-384" -> "ES384"
                "P-521" -> "ES512"
                "secp256k1" -> "ES256K"
