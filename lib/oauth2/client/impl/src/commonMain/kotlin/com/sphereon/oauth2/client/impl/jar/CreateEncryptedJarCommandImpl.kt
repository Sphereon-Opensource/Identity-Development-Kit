...
class CreateEncryptedJarCommandImpl(
    ...
    private suspend fun createEncryptedJarInternal(args: CreateEncryptedJarArgs): IdkResult<String, Oauth2Error> {
        try {
            ...
            val prepareResult = jweService.prepareJwe(prepareArgs).getOrElse { error ->
            ...
            // Create compact JWE
            val createArgs = CreateJweCompactArgs(
            ...
            return Err(Oauth2Error.JarCreationFailed(
                failureMessage = "Encrypted JAR creation failed: ${e.message}",
                cause = e
            ))
        }
    }
