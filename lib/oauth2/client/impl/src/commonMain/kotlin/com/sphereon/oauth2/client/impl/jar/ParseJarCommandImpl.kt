...
class ParseJarCommandImpl(
    ...
    private suspend fun parseJarInternal(args: ParseJarArgs): IdkResult<ParsedJarResult, Oauth2Error> {
        try {
            ...
            // Parse the payload as JSON - get payload from the JWS result
            val jwtParts = signedJwt.split(".")
            val payloadBytes = jwtParts[1].decodeFromBase64Url()
            val payloadString = payloadBytes.decodeToString()
            val payloadJson = Json.parseToJsonElement(payloadString) as? JsonObject
                ?: return Err(Oauth2Error.JarParsingFailed(
                    failureMessage = "JAR payload is not a valid JSON object"
                ))
            ...
            val authorizationRequest = try {
                ...
                return Err(Oauth2Error.JarParsingFailed(
                    failureMessage = "Failed to parse authorization request from JAR: ${e.message}",
                    cause = e
                ))
            }

            // Convert JsonObject to Map for additional claims
            val claims = payloadJson.mapValues { (_, value) ->
                when {
            ...
            return Err(Oauth2Error.JarParsingFailed(
                failureMessage = "JAR parsing failed: ${e.message}",
                cause = e
            ))
        }
    }
     ...
     */
    private fun validateJwtClaims(payload: JsonObject, args: ParseJarArgs): Oauth2Error? {
