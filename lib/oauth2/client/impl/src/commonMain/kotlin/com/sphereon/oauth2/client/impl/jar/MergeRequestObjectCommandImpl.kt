...
class MergeRequestObjectCommandImpl(
    ...
    companion object {
        ...
        private val ALLOWED_DUPLICATES = setOf("client_id", "response_type")
    ...
    private suspend fun mergeRequestObjectInternal(args: MergeRequestObjectArgs): IdkResult<MergedRequestObjectResult, Oauth2Error> {
        try {
            ...
            val mergedParameters = mergeParameters(requestObjectParams, args.queryParameters)
            ...
            return Ok(MergedRequestObjectResult(
    ...
    private fun parseJwtPayload(signedJwt: String): IdkResult<JsonObject, Oauth2Error> {
        return try {
            ...
            Err(Oauth2Error.JarParsingFailed(
     ...
     */
    private fun validateJwtClaims(payload: JsonObject, args: MergeRequestObjectArgs): Oauth2Error? {
     ...
     */
    private fun extractAuthorizationParameters(payload: JsonObject): Map<String, JsonElement> {
        return payload.filterKeys { key -> key !in JWT_CLAIMS }
    }
