...
class ClientAuthenticationCommandTest {
    ...
    val app = createOAuth2ClientTestAppComponent(this)
    ...
    fun testBasicAuthentication() = runTest {
        ...
        val config = ClientAuthenticationConfig.Basic(
    ...
    @Test
    fun testPostAuthentication() = runTest {
        val command = ApplyClientAuthenticationCommandImpl(execution)
        val config = ClientAuthenticationConfig.Post(
            credentials = ClientCredentials(
                clientId = "test-client-id", clientSecret = "test-client-secret"
            )
        )
    ...
    @Test
    fun testSecretJwtAuthentication() = runTest {
        val command = ApplyClientAuthenticationCommandImpl(execution)
        val config = ClientAuthenticationConfig.SecretJwt(
        ...
        // Verify body parameters
        assertEquals(
            "urn:ietf:params:oauth:client-assertion-type:jwt-bearer", authResult.bodyParameters["client_assertion_type"]
        )
        assertEquals(
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.test.signature", authResult.bodyParameters["client_assertion"]
        )
        assertEquals(2, authResult.bodyParameters.size)
    }

    @Test
    fun testPrivateKeyJwtAuthentication() = runTest {
        val command = ApplyClientAuthenticationCommandImpl(execution)
        val config = ClientAuthenticationConfig.PrivateKeyJwt(
            assertion = ClientAssertion(
                clientId = "test-client-id",
                assertionType = "urn:ietf:params:oauth:client-assertion-type:jwt-bearer",
        ...
        assertEquals(
            "eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9.test.signature", authResult.bodyParameters["client_assertion"]
        )
        assertEquals(2, authResult.bodyParameters.size)
    }

    @Test
    fun testNoneAuthentication() = runTest {
        val command = ApplyClientAuthenticationCommandImpl(execution)
    ...
    @Test
    fun testAttestationJwtAuthentication() = runTest {
        val command = ApplyClientAuthenticationCommandImpl(execution)
        val config = ClientAuthenticationConfig.AttestationJwt(
            attestation = ClientAttestation(
                clientAttestationJwt = "attestation.jwt.here", clientAttestationPopJwt = "pop.jwt.here"
            )
        )
    ...
    @Test
    fun testAnonymousAuthentication() = runTest {
        val command = ApplyClientAuthenticationCommandImpl(execution)
    ...
    @Test
    fun testBasicAuthWithSpecialCharacters() = runTest {
        val command = ApplyClientAuthenticationCommandImpl(execution)
        val config = ClientAuthenticationConfig.Basic(
            credentials = ClientCredentials(
                clientId = "client@example.com", clientSecret = "p@ssw0rd:with:colons"
            )
        )
