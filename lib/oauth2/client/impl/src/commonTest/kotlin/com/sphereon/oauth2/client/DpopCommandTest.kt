...
class DpopCommandTest {
    ...
    @Test
    fun testCreateDpopProofBasic() = runTest {
        // Generate key pair
        ...
        val jwtService = (session.component as JwtServiceImpl.Component).jwtService
    ...
    @Test
    fun testCreateAndVerifyDpopProof() = runTest {
        // Setup
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
        ...
        val verifyResult = verifyCommand.execute(
    ...
    @Test
    fun testVerifyFailsWithWrongHttpMethod() = runTest {
        // Setup
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
    ...
    @Test
    fun testDpopProofWithAccessToken() = runTest {
        // Setup
        val managedKeyPair = keyManagerService.generateKeyAsync(alg = SignatureAlgorithm.ECDSA_SHA256)
        val keyInfo: ManagedKeyInfoType<*> = managedKeyPair.joseToManagedKeyInfo(KeyVisibility.PRIVATE)
