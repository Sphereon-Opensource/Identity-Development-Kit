...
class OAuth2ClientE2ETest {
    ...
    @Test
    fun `test fetchAuthorizationServerMetadata with synthetic metadata`() =
        runTest {
            // Create synthetic metadata (without making actual HTTP call)
            val syntheticMetadata =
    ...
    @Test
    fun `test DPoP support detection from metadata`() =
        runTest {
            val metadataWithDPoP =
                AuthorizationServerMetadata(
                    issuer = "https://as.example.com",
                    authorizationEndpoint = "https://as.example.com/authorize",
                    tokenEndpoint = "https://as.example.com/token",
    ...
    @Test
    fun `test token exchange parameters are constructed correctly`() =
        runTest {
