...
import com.sphereon.oauth2.client.command.*
...
class JarServiceImpl(
    ...
    override suspend fun createSignedJar(args: CreateSignedJarArgs): IdkResult<StringResult, IdkError> =
        createSignedJarCommand.execute(args)

    override suspend fun createEncryptedJar(args: CreateEncryptedJarArgs): IdkResult<StringResult, IdkError> =
        createEncryptedJarCommand.execute(args)

    override suspend fun parseJar(args: ParseJarArgs): IdkResult<ParsedJarResult, IdkError> =
        parseJarCommand.execute(args)

    override suspend fun mergeRequestObject(args: MergeRequestObjectArgs): IdkResult<MergedRequestObjectResult, IdkError> =
        mergeRequestObjectCommand.execute(args)
}
