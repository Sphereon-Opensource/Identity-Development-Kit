...
class DefaultJwtValidationService(
    ...
    override suspend fun validateAccessToken(
        ...
        if (idpResult is Err) return idpResult
        ...
        val verifyResult = verifyJwtCommand.execute(
            VerifyJwtArgs(
                jwt = token,
    ...
    override suspend fun validateIdToken(
        ...
        // Find the IdP configuration to use
        val accessOptions = AccessTokenValidationOptions(
            expectedAudience = options.expectedAudience,
            idpId = options.idpId,
            tenantHint = options.tenantHint
        )
        val idpResult = resolveIdpConfig(token, accessOptions)
        if (idpResult is Err) return Err(idpResult.error)
        ...
        return verifyResult.fold(
            success = { jwtPayload ->
                ...
                if (options.expectedNonce != null) {
                    ...
                    if (tokenNonce != options.expectedNonce) {
                        return Err(JwtValidationError.validationError(
            ...
            failure = { error ->
                Err(JwtValidationError.validationError(
    ...
    override suspend fun extractClaims(token: String): IdkResult<TokenClaims, JwtValidationError> {
        ...
        return try {
            ...
            val audiences = if (audRaw != null) listOf(audRaw) else emptyList()
            ...
            Err(JwtValidationError.invalidFormat("Failed to parse JWT: ${e.message}"))
        }
    }
    ...
    private suspend fun resolveIdpConfig(
        ...
        if (tenantHint != null) {
            ...
            if (tenantIdp is Ok) return tenantIdp
        }
    ...
    private fun extractTenantId(jwtPayload: TokenPayload.Jwt, idpConfig: IdpConfig): String? {
    ...
    // Simple base64url decode - in production use proper library
    private fun decodeBase64Url(input: String): String {
        ...
        // In real code, use proper Base64 decoding
        return try {
    ...
    private fun String.decodeBase64Bytes(): ByteArray {
        // Simplified Base64 decoding - in production use kotlinx or platform-specific
        val table = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
        val input = this.filter { it != '=' }
        ...
        for (char in input) {
            ...
            if (bufferBits >= 8) {
                bufferBits -= 8
                output[outputIndex++] = ((buffer shr bufferBits) and 0xFF).toByte()
            }
        }
    ...
    private fun extractJsonNumber(json: String, key: String): Long? {
        ...
        val regex = Regex(pattern)
        return regex.find(json)?.groupValues?.get(1)?.toDoubleOrNull()?.toLong()
    }
}
