...
class ValidateAccessTokenCommandImpl(
    ...
    private suspend fun executeInternal(
        ...
        val authHeader =
            request.headers.entries
                ...
                ?.value
                ?: return Err(
                    ResourceServerError.MissingAuthorizationHeader(
        ...
        val (scheme, token) =
            when {
                ...
                authHeader.lowercase().startsWith(DPOP_PREFIX) -> {
                    ...
                    return Err(
                        ResourceServerError.MalformedAuthorizationHeader(
                            header = authHeader,
                        ),
                    )
                }
        ...
        val authorizationServer =
            extractAuthorizationServerFromToken(token)
                ?: return Err(
    ...
    private suspend fun validateAndBuildResult(
        ...
        if (requiredScope != null) {
            ...
            val hasAllScopes =
                requiredScopes.all { required ->
                    tokenScopes.any { it.trim() == required.trim() }
                }

            if (!hasAllScopes) {
                return Err(
                    ResourceServerError.InsufficientScope(
