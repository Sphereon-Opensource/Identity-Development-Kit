...
class ResourceServerIntrospectTokenCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        val result = executeInternal(applied.token, applied.authorizationServer).mapError { IdkError.fromDTO(it) }
    ...
    private suspend fun emitOutcome(
        args: IntrospectTokenArgs,
        result: IdkResult<TokenPayload.Introspection, IdkError>,
    ) {
        ...
        val payload =
            buildJsonObject {
                put("authorizationServer", args.authorizationServer)
                if (!result.isOk) put("operation", "introspect")
            }
    ...
    private suspend fun executeInternal(
        ...
        return try {
            ...
            Err(
                ResourceServerError.InvalidToken(
                    "Token introspection failed: ${expected.message}",
                ),
            )
        }
    }
