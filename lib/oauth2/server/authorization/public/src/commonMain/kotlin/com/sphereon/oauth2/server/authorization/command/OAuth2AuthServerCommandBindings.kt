...
interface OAuth2AuthServerCommandBindings {
    ...
    @Provides
    fun handleIaeInitialRequest(registry: SessionScopedCommandRegistry): HandleIaeInitialRequestCommand =
        registry.get(HandleIaeInitialRequestCommand.COMMAND_ID) as? HandleIaeInitialRequestCommand
            ?: error("No binding for ${HandleIaeInitialRequestCommand.COMMAND_ID}")

    @Provides
    fun handleIaeFollowUp(registry: SessionScopedCommandRegistry): HandleIaeFollowUpCommand =
        registry.get(HandleIaeFollowUpCommand.COMMAND_ID) as? HandleIaeFollowUpCommand
            ?: error("No binding for ${HandleIaeFollowUpCommand.COMMAND_ID}")
}
