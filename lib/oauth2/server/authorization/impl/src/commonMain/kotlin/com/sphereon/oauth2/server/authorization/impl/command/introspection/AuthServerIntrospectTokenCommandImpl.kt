...
class AuthServerIntrospectTokenCommandImpl(
    ...
    override suspend fun doExecute(
        ...
        return executeInternal(applied.token, applied.tokenTypeHint, applied.clientId)
    ...
    private suspend fun executeInternal(
        ...
        val tokenData = when (tokenTypeHint) {
            "access_token" -> {
                // Try access token first
                val accessToken = tokenStorage.getAccessToken(token)
                    .mapError { error ->
                ...
                if (accessToken != null) {
                    ...
                    tokenStorage.getRefreshToken(token)
                        .mapError { error ->
                            AuthorizationServerError.ServerError(
            ...
            "refresh_token" -> {
                ...
                val refreshToken = tokenStorage.getRefreshToken(token)
                    .mapError { error ->
                        AuthorizationServerError.ServerError(
                            details = "Failed to retrieve refresh token: $error",
                            exception = null
                ...
                if (refreshToken != null) {
                    ...
                    tokenStorage.getAccessToken(token)
                        .mapError { error ->
                            AuthorizationServerError.ServerError(
                                details = "Failed to retrieve access token: $error", exception = null
                            )
                        }
                        .getOrElse { return Ok(TokenIntrospectionResponse(active = false)) }
                }
                ...
                if (accessToken != null) {
                    ...
                    tokenStorage.getRefreshToken(token)
                        .mapError { error ->
                            AuthorizationServerError.ServerError(
                                details = "Failed to retrieve refresh token: $error", exception = null
                            )
                        }
                        .getOrElse { return Ok(TokenIntrospectionResponse(active = false)) }
                }
        ...
        return when (tokenData) {
            ...
            is RefreshTokenData -> {
                ...
                Ok(TokenIntrospectionResponse(
                    ...
                    clientId = tokenData.clientId,
                    username = tokenData.subject,
                    tokenType = if (tokenData.dpopJkt != null) "DPoP" else "Bearer",
                    exp = tokenData.expiresAt?.epochSeconds,
                    iat = tokenData.issuedAt.epochSeconds,
                    nbf = tokenData.issuedAt.epochSeconds,
                    sub = tokenData.subject,
