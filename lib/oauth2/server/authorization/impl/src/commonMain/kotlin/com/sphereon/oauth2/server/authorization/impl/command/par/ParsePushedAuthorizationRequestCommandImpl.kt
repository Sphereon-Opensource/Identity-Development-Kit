...
class ParsePushedAuthorizationRequestCommandImpl(
    ...
    private suspend fun executeInternal(
        ...
        val responseType = try {
            ...
            return Err(
                AuthorizationServerError.InvalidRequest(
                    details = "Invalid response_type format: ${e.message}"
                )
            )
        }

        // Extract client_id from authentication config
        val clientId = when (clientAuthentication) {
            is ClientAuthenticationConfig.Basic -> clientAuthentication.credentials.clientId
            is ClientAuthenticationConfig.Post -> clientAuthentication.credentials.clientId
            is ClientAuthenticationConfig.SecretJwt -> clientAuthentication.assertion.clientId
            is ClientAuthenticationConfig.PrivateKeyJwt -> clientAuthentication.assertion.clientId
            is ClientAuthenticationConfig.None -> clientAuthentication.clientId
        ...
        val codeChallengeMethod = if (codeChallengeMethodStr != null) {
        ...
        if (codeChallenge != null) {
            ...
            if (codeChallenge.length !in 43..128) {
                return Err(
                    AuthorizationServerError.InvalidRequest(
