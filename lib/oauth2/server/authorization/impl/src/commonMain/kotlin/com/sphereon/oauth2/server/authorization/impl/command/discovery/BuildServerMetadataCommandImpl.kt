...
class BuildServerMetadataCommandImpl(
    ...
    private fun executeInternal(
        ...
        val baseUrl = (config.issuer ?: baseUrlOverride ?: config.baseUrl).trimEnd('/')
        ...
        val metadata =
            AuthorizationServerMetadata(
                ...
                codeChallengeMethodsSupported =
                    if (config.pkce.isEnabled) {
                        config.pkceMethodsSupported.toList()
                    } else {
                        null
                    },
                dpopSigningAlgValuesSupported =
                    if (config.dpop.isEnabled) {
                        config.dpopSigningAlgValuesSupported?.toList()
                    } else {
                        null
                    },
                requirePushedAuthorizationRequests =
                    if (config.par.isRequired) {
                        true
                    } else if (config.par.isEnabled) {
                        false
                    } else {
                        null
                    },
                ...
                introspectionEndpointAuthMethodsSupported =
                    if (config.introspection.isEnabled) {
                        config.introspectionEndpointAuthMethodsSupported.toList()
                    } else {
                        null
                    },
                revocationEndpoint =
                ...
                revocationEndpointAuthMethodsSupported =
                    if (config.revocation.isEnabled) {
                        config.revocationEndpointAuthMethodsSupported.toList()
                    } else {
                        null
                    },
                // Attestation-based client auth metadata
                clientAttestationPopNonceRequired =
                    if (config.attestation.isEnabled) {
                        config.attestationChallengeRequired
                    } else {
                        null
                    },
                challengeEndpoint =
                    if (config.attestation.isEnabled && config.attestationChallengeRequired) {
                        "$baseUrl/attestation-challenge"
                    } else {
                        null
                    },
                clientAttestationSigningAlgValuesSupported =
                    if (config.attestation.isEnabled) {
                        config.clientAttestationSigningAlgValuesSupported?.toList()
                    } else {
                        null
                    },
                clientAttestationPopSigningAlgValuesSupported =
                    if (config.attestation.isEnabled) {
                        config.clientAttestationPopSigningAlgValuesSupported?.toList()
                    } else {
                        null
                    },
                ...
                subjectTypesSupported =
                    if (config.oidc.isEnabled) {
                        config.subjectTypesSupported
                    } else {
                        null
                    },
                idTokenSigningAlgValuesSupported =
                    if (config.oidc.isEnabled) {
                        config.idTokenSigningAlgValuesSupported?.toList() ?: listOf("ES256")
                    } else {
                        null
                    },
                claimsSupported =
                    if (config.oidc.isEnabled) {
                        config.claimsSupported ?: listOf("sub", "name", "preferred_username", "email", "email_verified", "phone_number", "phone_number_verified")
                    } else {
                        null
                    },
                // OID4VCI 1.1 Section 13.3 — Interactive Authorization Endpoint
                interactiveAuthorizationEndpoint =
                    if (config.iae.isEnabled) {
                        "$baseUrl/iae"
                    } else {
                        null
                    },
