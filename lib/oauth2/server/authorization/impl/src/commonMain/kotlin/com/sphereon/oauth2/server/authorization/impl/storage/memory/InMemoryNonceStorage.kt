...
class InMemoryNonceStorageImpl(
    ...
    override suspend fun generateNonce(
        ...
        return try {
            ...
            val nonce = bytes.joinToString("") { it.toUByte().toString(16).padStart(2, '0') }
    ...
    override suspend fun nonceExists(
        nonce: String
    ...
    override suspend fun wasNonceUsed(
        nonce: String
    ): IdkResult<Boolean, AuthorizationServerError.StorageError> {
        return try {
            val data = partition.nonces[nonce] as? NonceData
    ...
    override suspend fun markNonceAsUsed(
        nonce: String
    ): IdkResult<Unit, AuthorizationServerError> {
        return try {
            val data = partition.nonces[nonce] as? NonceData
            if (data == null) {
                return Err(AuthorizationServerError.InvalidDpopProof(
    ...
    override suspend fun verifyAndConsumeNonce(
        nonce: String
    ): IdkResult<Unit, AuthorizationServerError> {
        return try {
            val data = partition.nonces[nonce] as? NonceData
    ...
    override suspend fun revokeNonce(
        nonce: String
    ): IdkResult<Unit, AuthorizationServerError.StorageError> {
        return try {
    ...
    override suspend fun cleanupExpiredNonces(): IdkResult<Int, AuthorizationServerError.StorageError> {
        return try {
            val now = Clock.System.now()
            val expired = partition.nonces.filter { (_, value) ->
    ...
    override suspend fun getNonceExpiration(
        nonce: String
