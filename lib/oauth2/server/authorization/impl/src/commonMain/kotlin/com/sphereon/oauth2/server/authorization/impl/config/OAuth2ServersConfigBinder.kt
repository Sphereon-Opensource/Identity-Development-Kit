...
class OAuth2ServersConfigBinder(
    ...
    private val prefix: String
        get() = "${configService.getNamespace()}.${OAuth2ServerInstanceConfig.CONFIG_PREFIX}"
    ...
    private fun loadConfig(): OAuth2ServersConfig {
        val defaultServer = configService.getPropertyAsString(
        ...
        val servers = if (serverIds.isEmpty()) {
    ...
    private fun loadServerConfig(id: String): OAuth2ServerInstanceConfig {
        ...
        return OAuth2ServerInstanceConfig(
            mode = configService.getPropertyAsString("$serverPrefix.mode", null)
            ...
            baseUrl = configService.getPropertyAsString("$serverPrefix.base-url", null)
                ?: defaults.baseUrl,
            accessTokenLifetimeSeconds = configService.getProperty(
                "$serverPrefix.access-token-lifetime-seconds", Int::class, defaults.accessTokenLifetimeSeconds
            ) ?: defaults.accessTokenLifetimeSeconds,
            refreshTokenLifetimeSeconds = configService.getProperty(
                "$serverPrefix.refresh-token-lifetime-seconds", Int::class, defaults.refreshTokenLifetimeSeconds
            ) ?: defaults.refreshTokenLifetimeSeconds,
            authorizationCodeLifetimeSeconds = configService.getProperty(
                "$serverPrefix.authorization-code-lifetime-seconds", Int::class, defaults.authorizationCodeLifetimeSeconds
            ) ?: defaults.authorizationCodeLifetimeSeconds,
            tokenFormat = configService.getPropertyAsString("$serverPrefix.token-format", null)
                ?.let { runCatching { TokenFormat.valueOf(it.uppercase()) }.getOrNull() }
                ?: defaults.tokenFormat,
            refreshTokenRotation = configService.getProperty(
                "$serverPrefix.refresh-token-rotation", Boolean::class, defaults.refreshTokenRotation
            ) ?: defaults.refreshTokenRotation,
            grantTypesEnabled = configService.getPropertyAsString("$serverPrefix.grant-types-enabled", null)
                ?.split(",")?.map { it.trim() }?.toSet()
                ?: defaults.grantTypesEnabled,
            responseTypesSupported = configService.getPropertyAsString("$serverPrefix.response-types-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet()
                ?: defaults.responseTypesSupported,
            scopesSupported = configService.getPropertyAsString("$serverPrefix.scopes-supported", null)
                ?.split(",")?.map { it.trim() },
            // OpenID Connect
            oidc = readFeaturePolicy("$serverPrefix.oidc", defaults.oidc),
            idTokenLifetimeSeconds = configService.getProperty(
                "$serverPrefix.id-token-lifetime-seconds", Int::class, defaults.idTokenLifetimeSeconds
            ) ?: defaults.idTokenLifetimeSeconds,
            subjectTypesSupported = configService.getPropertyAsString("$serverPrefix.subject-types-supported", null)
                ?.split(",")?.map { it.trim() }
                ?: defaults.subjectTypesSupported,
            claimsSupported = configService.getPropertyAsString("$serverPrefix.claims-supported", null)
                ?.split(",")?.map { it.trim() },
            userinfoSigningAlgValuesSupported = configService.getPropertyAsString("$serverPrefix.userinfo-signing-alg-values-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet(),
            ...
            pkceMethodsSupported = configService.getPropertyAsString("$serverPrefix.pkce-methods-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet()
                ?: defaults.pkceMethodsSupported,
            tokenEndpointAuthMethodsSupported = configService.getPropertyAsString("$serverPrefix.token-endpoint-auth-methods-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet()
                ?: defaults.tokenEndpointAuthMethodsSupported,
            introspectionEndpointAuthMethodsSupported = configService.getPropertyAsString("$serverPrefix.introspection-endpoint-auth-methods-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet()
                ?: defaults.introspectionEndpointAuthMethodsSupported,
            revocationEndpointAuthMethodsSupported = configService.getPropertyAsString("$serverPrefix.revocation-endpoint-auth-methods-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet()
                ?: defaults.revocationEndpointAuthMethodsSupported,
            dpopSigningAlgValuesSupported = configService.getPropertyAsString("$serverPrefix.dpop-signing-alg-values-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet(),
            signingKeyAlias = configService.getPropertyAsString("$serverPrefix.signing-key-alias", null),
            signingAlgorithmsSupported = configService.getPropertyAsString("$serverPrefix.signing-algorithms-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet(),
            idTokenSigningAlgValuesSupported = configService.getPropertyAsString("$serverPrefix.id-token-signing-alg-values-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet(),
            requestObjectSigningAlgValuesSupported = configService.getPropertyAsString("$serverPrefix.request-object-signing-alg-values-supported", null)
                ?.split(",")?.map { it.trim() }?.toSet(),
    ...
    private fun readFeaturePolicy(key: String, default: FeaturePolicy): FeaturePolicy {
        return configService.getPropertyAsString(key, null)
