...
class ParseTokenExchangeRequestTest {
    ...
    private val TOKEN_EXCHANGE_GRANT_TYPE = "urn:ietf:params:oauth:grant-type:token-exchange"
    private val ACCESS_TOKEN_TYPE = "urn:ietf:params:oauth:token-type:access_token"
    private val JWT_TOKEN_TYPE = "urn:ietf:params:oauth:token-type:jwt"

    @Test
    fun parseValidTokenExchangeImpersonationRequest() = runTest {
        // Given - impersonation: subject_token only, no actor
        val requestBody = mapOf(
            "grant_type" to listOf(TOKEN_EXCHANGE_GRANT_TYPE),
            "subject_token" to listOf("eyJhbGciOiJSUzI1NiJ9.subject-token-content"),
            "subject_token_type" to listOf(ACCESS_TOKEN_TYPE),
        ...
        assertEquals(ACCESS_TOKEN_TYPE, grantParams.subjectTokenType)
    ...
    @Test
    fun parseValidTokenExchangeDelegationRequest() = runTest {
        ...
        val requestBody = mapOf(
            ...
            "actor_token_type" to listOf(ACCESS_TOKEN_TYPE),
            "client_id" to listOf("client123")
        )
        ...
        assertEquals(ACCESS_TOKEN_TYPE, grantParams.actorTokenType)
    }

    @Test
    fun parseTokenExchangeWithMultiValueResourceParameters() = runTest {
    ...
    @Test
    fun parseTokenExchangeWithMultiValueAudienceParameters() = runTest {
    ...
    @Test
    fun parseTokenExchangeWithRequestedTokenType() = runTest {
        // Given
        val requestBody = mapOf(
            "grant_type" to listOf(TOKEN_EXCHANGE_GRANT_TYPE),
            "subject_token" to listOf("eyJhbGciOiJSUzI1NiJ9.subject-token"),
            "subject_token_type" to listOf(ACCESS_TOKEN_TYPE),
            "requested_token_type" to listOf(JWT_TOKEN_TYPE),
        ...
        assertTrue(request.grantParameters is GrantParameters.TokenExchange)
        val grantParams = request.grantParameters as GrantParameters.TokenExchange
        assertEquals(JWT_TOKEN_TYPE, grantParams.requestedTokenType)
        assertEquals("openid profile", grantParams.scope)
    }

    @Test
    fun parseMissingSubjectTokenStillParses() = runTest {
    ...
    @Test
    fun parseMissingSubjectTokenTypeStillParses() = runTest {
