...
import com.sphereon.crypto.jose.jws.CreateJwsJsonArgs
...
class VerifyClientAuthenticationCommandImplTest {
    ...
    private fun createCommand(
        ...
        config: OAuth2ServerInstanceConfig = OAuth2ServerInstanceConfig(
        ...
        return VerifyClientAuthenticationCommandImpl(
            ctx.execution, clientRegistry, jwtService, configProvider, challengeStorage
        )
    }
    ...
    @Test
    fun testBasicAuthSuccess() = runTest {
        ...
        val result = command.execute(
            VerifyClientAuthenticationArgs(
                clientAuthentication = ClientAuthenticationConfig.Basic(
    ...
    @Test
    fun testBasicAuthInvalidCredentials() = runTest {
    ...
    @Test
    fun testPostAuthSuccess() = runTest {
        val registry = StubClientRegistry(verifyResult = true)
        val command = createCommand(clientRegistry = registry)

        val result = command.execute(
            VerifyClientAuthenticationArgs(
                clientAuthentication = ClientAuthenticationConfig.Post(
                    ClientCredentials("client1", "secret1")
                ),
                clientId = "client1",
                tokenEndpointUrl = "https://auth.example.com/token"
    ...
    @Test
    fun testPostAuthInvalidCredentials() = runTest {
        val registry = StubClientRegistry(verifyResult = false)
        val command = createCommand(clientRegistry = registry)
    ...
    @Test
    fun testNoneAuthPassesThrough() = runTest {
    ...
    @Test
    fun testAnonymousAuthPassesThrough() = runTest {
        val command = createCommand()
    ...
    @Test
    fun testAttestationRejectedWhenDisabled() = runTest {
        val command = createCommand(
        ...
        val result = command.execute(
            VerifyClientAuthenticationArgs(
                clientAuthentication = ClientAuthenticationConfig.AttestationJwt(
                    com.sphereon.oauth2.common.model.ClientAttestation("att.jwt", "pop.jwt")
                ),
                clientId = "client1",
                tokenEndpointUrl = "https://auth.example.com/token"
            )
    ...
    private class StubClientRegistry(
        ...
        override suspend fun updateClient(clientId: String, registration: ClientRegistration): IdkResult<ClientRegistration, AuthorizationServerError> =
            Ok(registration)

        override suspend fun deleteClient(clientId: String): IdkResult<Unit, AuthorizationServerError> =
            Ok(Unit)

        override suspend fun listClients(limit: Int, offset: Int): IdkResult<List<ClientRegistration>, AuthorizationServerError.StorageError> =
            Ok(emptyList())
        ...
        override suspend fun verifyClientCredentials(clientId: String, clientSecret: String): IdkResult<Boolean, AuthorizationServerError.StorageError> =
            Ok(verifyResult)
    }
