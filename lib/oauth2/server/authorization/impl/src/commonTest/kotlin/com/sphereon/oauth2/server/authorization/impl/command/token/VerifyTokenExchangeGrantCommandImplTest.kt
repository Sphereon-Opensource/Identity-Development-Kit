...
class VerifyTokenExchangeGrantCommandImplTest {
    ...
    private val jwtService: JwtService = (ctx.session.component as JwtServiceImpl.Component).jwtService

    // Test client authorized for TOKEN_EXCHANGE
    private val tokenExchangeClient = TestFixtures.confidentialClient.copy(
    ...
    private val unauthorizedClient = TestFixtures.confidentialClient.copy(
     ...
     */
    private val testPolicy = object : TokenExchangePolicy {
        private val delegate = DefaultTokenExchangePolicy()

        override suspend fun evaluate(
            request: TokenExchangePolicyRequest
        ): IdkResult<TokenExchangePolicyDecision, AuthorizationServerError> {
    ...
    private fun createCommand(
        ...
        return VerifyTokenExchangeGrantCommandImpl(
            execution = execution,
    ...
    @Test
    fun testDelegationFlowWithActorToken() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
        ...
        val actorJwt = createTestJwt(mapOf(
            "sub" to "service-a",
    ...
    @Test
    fun testMissingSubjectTokenReturnsError() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testMissingSubjectTokenTypeReturnsError() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testActorTokenWithoutActorTokenTypeReturnsError() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testClientNotAuthorizedForTokenExchangeReturnsError() = runTest {
    ...
    @Test
    fun testUnknownClientReturnsError() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testInvalidJwtFormatReturnsError() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testSamlTokenTypeReturnsError() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testUnsupportedTokenTypeReturnsError() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testRefreshTokenTypeReturnsMinimalClaims() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testResourcesAndAudiencesPassedToResult() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)
    ...
    @Test
    fun testIdTokenTypeAccepted() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
        val command = createCommand(registry)

        val idToken = createTestJwt(mapOf(
            "sub" to "user456",
            "iss" to "https://idp.example.com",
            "aud" to "client-id",
            "nonce" to "test-nonce"
    ...
    @Test
    fun testDefaultPolicyRejectsUnverifiedSubjectToken() = runTest {
        val registry = setupClientRegistry(tokenExchangeClient)
