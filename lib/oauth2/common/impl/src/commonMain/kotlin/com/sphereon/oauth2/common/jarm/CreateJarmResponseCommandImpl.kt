...
class CreateJarmResponseCommandImpl(
     ...
     */
    private suspend fun createSignedResponse(
        args: CreateJarmResponseArgs
    ): IdkResult<CreateJarmResponseResult, IdkError> {
        ...
        val jwsResult = jwtService.createJwsCompact(jwsArgs).getOrElse { error ->
    ...
    private suspend fun createEncryptedResponse(
        ...
        val encAlg = args.jarmConfig.encryptionAlgorithm
            ?: return Err(IdkError.ILLEGAL_ARGUMENT_ERROR(
                message = "Encryption algorithm is required for ENCRYPTED mode"
            ))

        val contentEncAlg = args.jarmConfig.contentEncryptionAlgorithm
        ...
        val preparedJwe = jweService.prepareJwe(prepareArgs).getOrElse { error ->
        ...
        val jweResult = jweService.createJweCompact(createArgs).getOrElse { error ->
     ...
     */
    private suspend fun createSignedEncryptedResponse(
        args: CreateJarmResponseArgs
    ): IdkResult<CreateJarmResponseResult, IdkError> {
