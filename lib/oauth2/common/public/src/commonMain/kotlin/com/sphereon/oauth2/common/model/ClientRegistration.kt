...
internal object ClientRegistrationSerializer : KSerializer<ClientRegistration> {
    ...
    private val knownJsonKeys = setOf(
        ...
        "client_type", "grant_types", "response_types", "redirect_uris",
        "allowed_scopes", "scope", "token_endpoint_auth_method",
        "jwks", "jwks_uri", "contacts", "tos_uri", "policy_uri",
        "software_id", "software_version", "software_statement",
        "require_pkce", "require_pushed_authorization_requests",
        "dpop_bound_access_tokens", "access_token_lifetime",
        "refresh_token_lifetime", "authorization_code_lifetime"
    )

    override fun serialize(encoder: Encoder, value: ClientRegistration) {
    ...
    override fun deserialize(decoder: Decoder): ClientRegistration {
        ...
        return ClientRegistration(
            clientId = jsonObject["client_id"]?.let { decoder.json.decodeFromJsonElement(it) }
            ...
            clientType = jsonObject["client_type"]?.let { decoder.json.decodeFromJsonElement<ClientType>(it) }
                ?: ClientType.CONFIDENTIAL,
            grantTypes = jsonObject["grant_types"]?.let { decoder.json.decodeFromJsonElement(it) }
                ?: throw IllegalArgumentException("grant_types is required"),
            responseTypes = jsonObject["response_types"]?.let { decoder.json.decodeFromJsonElement(it) }
                ?: emptyList(),
            redirectUris = jsonObject["redirect_uris"]?.let { decoder.json.decodeFromJsonElement(it) }
                ?: emptyList(),
            ...
            tokenEndpointAuthMethod = jsonObject["token_endpoint_auth_method"]?.let {
            ...
            requirePkce = jsonObject["require_pkce"]?.let { decoder.json.decodeFromJsonElement<Boolean>(it) }
                ?: ((jsonObject["client_type"]?.let { decoder.json.decodeFromJsonElement<ClientType>(it) }
                    ?: ClientType.CONFIDENTIAL) == ClientType.PUBLIC),
            requirePushedAuthorizationRequests = jsonObject["require_pushed_authorization_requests"]?.let {
                decoder.json.decodeFromJsonElement<Boolean>(it)
            } ?: false,
            dpopBoundAccessTokens = jsonObject["dpop_bound_access_tokens"]?.let {
                decoder.json.decodeFromJsonElement<Boolean>(it)
            } ?: false,
            accessTokenLifetime = jsonObject["access_token_lifetime"]?.let { decoder.json.decodeFromJsonElement(it) }
                ?: 3600,
            refreshTokenLifetime = jsonObject["refresh_token_lifetime"]?.let { decoder.json.decodeFromJsonElement(it) },
            authorizationCodeLifetime = jsonObject["authorization_code_lifetime"]?.let {
...
val validateClientRegistration = Validation<ClientRegistration> {
    ...
    run {
        constrain("redirect_uris required for authorization_code grants") { registration ->
            val requiresRedirectUri = registration.grantTypes.any {
