...
data class OAuth2ServersConfig(
    ...
    val servers: Map<String, OAuth2ServerInstanceConfig> = mapOf(
        "default" to OAuth2ServerInstanceConfig()
    )
) {
    fun getServer(id: String): OAuth2ServerInstanceConfig? = servers[id]

    fun getDefaultServer(): OAuth2ServerInstanceConfig = servers[defaultServer]
        ?: error("Default server '$defaultServer' not found in oauth2.servers config")

    fun hostedServers(): Map<String, OAuth2ServerInstanceConfig> =
        servers.filterValues { it.mode == AuthorizationServerMode.HOSTED }

    fun externalServers(): Map<String, OAuth2ServerInstanceConfig> =
        servers.filterValues { it.mode == AuthorizationServerMode.EXTERNAL }
}
