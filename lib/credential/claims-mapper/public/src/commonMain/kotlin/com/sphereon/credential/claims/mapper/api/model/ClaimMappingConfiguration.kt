...
data class ClaimMappingConfiguration(
     ...
     */
    fun allCredentialIds(): Set<String> {
        return credentialMappings.map { it.credentialId }.toSet()
    }

    /**
     * Returns all target claim paths across all credential mappings as dot-separated strings.
     */
    fun allTargetClaimPaths(): Set<String> {
        return credentialMappings.flatMap { it.targetClaimPaths() }.toSet()
    }

    /**
     * Finds the credential mapping for a given credential ID.
     */
    fun findCredentialMapping(credentialId: String): CredentialMapping? {
        return credentialMappings.find { it.credentialId == credentialId }
    }
