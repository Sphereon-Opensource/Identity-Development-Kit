...
data class CredentialMapping(
     ...
     */
    fun requiredMappings(): List<ClaimMapping> {
        return claimMappings.filter { it.required }
    }

    /**
     * Returns all target claim paths in this mapping as dot-separated strings.
     */
    fun targetClaimPaths(): Set<String> {
        return claimMappings.map { it.targetPathAsString() }.toSet()
    }
    ...
    companion object {
        ...
        fun simple(
            ...
            return CredentialMapping(
                ...
                optional = optional,
                claimMappings = claims.map { (source, target) ->
