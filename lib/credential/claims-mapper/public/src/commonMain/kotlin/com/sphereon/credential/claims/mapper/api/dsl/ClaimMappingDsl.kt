...
): ClaimMappingConfiguration = ClaimMappingConfigBuilder(id, name).apply(block).build()
...
class CredentialMappingBuilder(
     ...
     * ```
     */
    fun path(vararg segments: String): SourcePath = SourcePath(segments.toList())
    ...
    ): SourcePath = SourcePath(listOf(namespace, elementIdentifier))
     ...
     * ```
     */
    fun target(vararg segments: String): TargetPath = TargetPath(segments.toList())
...
class TransformationBuilder {
    ...
    ): ClaimTransformation = ClaimTransformation.Substring(start, end)

    /** Creates a regex replace transformation. */
    fun regexReplace(
        pattern: String,
        replacement: String,
    ): ClaimTransformation = ClaimTransformation.RegexReplace(pattern, replacement)

    /** Creates a toIsoDate transformation. */
    fun toIsoDate(inputFormat: DateInputFormat = DateInputFormat.AUTO): ClaimTransformation = ClaimTransformation.ToIsoDate(inputFormat)

    /** Creates a concatenate transformation. */
    fun concatenate(
        separator: String = " ",
        block: ConcatenateBuilder.() -> Unit = {},
    ): ClaimTransformation = ConcatenateBuilder(separator).apply(block).build()
}
