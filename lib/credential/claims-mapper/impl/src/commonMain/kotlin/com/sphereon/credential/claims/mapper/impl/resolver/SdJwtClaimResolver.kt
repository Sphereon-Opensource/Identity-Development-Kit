...
class SdJwtClaimResolver : CredentialClaimResolver {
    ...
    override val supportedFormats: Set<CredentialFormat> = setOf(
    ...
    override suspend fun extractAllClaims(
        ...
        val payload = disclosedClaims ?: run {
    ...
    override suspend fun extractClaim(
        ...
        return when (val result = ClaimPathUtils.lookup(payload, path)) {
            is ClaimPathLookupResult.Found -> Ok(result.value).asResult()
            is ClaimPathLookupResult.NotFound -> Ok(null).asResult()
            is ClaimPathLookupResult.Error -> Err(
     ...
     */
    private fun claimPathToString(path: ClaimPath): String {
        return path.segments.filterNotNull().joinToString(".")
    }
    ...
    companion object {
         ...
         * - "address.street" -> ["address", "street"]
         */
        fun parseClaimPath(pathString: String): List<String> {
