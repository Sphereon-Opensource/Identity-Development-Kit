 ...
 */
internal fun parseColor(value: String, fallback: Color = Color.Unspecified): Color {
    val trimmed = value.trim()
    if (trimmed == "transparent") return Color.Transparent
    return try {
        when {
...
private fun parseRgbColor(value: String, fallback: Color): Color {
    ...
    val a = match.groupValues[4].let {
        if (it.isEmpty()) 1f else it.toFloatOrNull() ?: 1f
    }
    return Color(
 ...
 */
internal fun parseDp(value: String, fallback: Dp = 0.dp): Dp {
    return try {
        val trimmed = value.trim()
