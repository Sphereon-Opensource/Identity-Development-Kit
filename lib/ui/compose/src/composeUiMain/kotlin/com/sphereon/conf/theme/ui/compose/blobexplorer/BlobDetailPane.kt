...
fun BlobDetailPane(
    ...
    Column(
        ...
        if (blob.contentType != null) DetailRow("Type", blob.contentType!!, fgSecondary, fg)
        if (blob.etag != null) DetailRow("ETag", blob.etag!!, fgSecondary, fg)
        ...
        FlowRow(
            ...
            if (!config.readOnly) {
                ...
                if (state.capabilities.supportsDelete) {
                    OutlinedButton(
                        onClick = {
                            if (onRequestDelete != null) onRequestDelete(blob)
                            else state.deleteBlob(blob)
                        },
