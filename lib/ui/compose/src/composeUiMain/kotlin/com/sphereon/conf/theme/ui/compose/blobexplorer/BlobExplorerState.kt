...
class BlobExplorerState(
    ...
    fun uploadBlob(path: String, data: ByteArray, contentType: String?) {
        scope.launch {
            try {
    ...
    fun copyBlob(source: String, destination: String) {
        scope.launch {
            try {
    ...
    fun moveBlob(source: String, destination: String) {
        scope.launch {
            try {
                ...
                items.add(moved)
                sortItems()
                if (selectedBlob?.path == source) selectedBlob = moved
            } catch (e: Exception) {
                e.printStackTrace()
    ...
    private fun sortItems() {
        val sorted = items.sortedWith(compareBy<BlobItem> { !it.isFolder }.then(
            when (sortField) {
        ...
        val finalSorted = if (sortDirection == SortDirection.Descending) sorted.reversed() else sorted
        items.clear()
