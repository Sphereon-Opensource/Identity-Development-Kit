...
class BlobExplorerTest {
    ...
    private fun testDataSource(
    ...
    ): BlobExplorerDataSource = object : BlobExplorerDataSource {
        ...
        override suspend fun copyBlob(source: String, destination: String) =
            BlobItem(path = destination)
        override suspend fun moveBlob(source: String, destination: String) =
            BlobItem(path = destination)
        override suspend fun uploadBlob(path: String, data: ByteArray, contentType: String?) =
    ...
    @Test
    fun emptyExplorerShowsEmptyState() = runComposeUiTest {
        setContent {
            ComponentTheme {
                MaterialTheme {
                    Box(Modifier.size(1200.dp, 800.dp)) { BlobExplorer(
    ...
    @Test
    fun explorerRendersBreadcrumb() = runComposeUiTest {
        setContent {
            ComponentTheme {
                MaterialTheme {
                    Box(Modifier.size(1200.dp, 800.dp)) { BlobExplorer(
    ...
    @Test
    fun explorerRendersWithCustomTokens() = runComposeUiTest {
    ...
    @Test
    fun explorerFallsBackToDefaults() = runComposeUiTest {
        setContent {
            CompositionLocalProvider(
    ...
    fun dismissErrorClearsError() {
        val scope = TestScope()
        val ds = object : BlobExplorerDataSource {
            override suspend fun listBlobs(prefix: String?, pageToken: String?): BlobListResult {
                throw RuntimeException("Test error")
            }
            override suspend fun deleteBlob(path: String) = false
            override suspend fun copyBlob(source: String, destination: String) = BlobItem(path = destination)
            override suspend fun moveBlob(source: String, destination: String) = BlobItem(path = destination)
            override suspend fun uploadBlob(path: String, data: ByteArray, contentType: String?) = BlobItem(path = path)
    ...
    @Test
    fun fileSelectionOnClick() = runComposeUiTest {
        val items = listOf(
    ...
    @Test
    fun folderNavigationOnClick() = runComposeUiTest {
    ...
    @Test
    fun sortColumnClick() = runComposeUiTest {
        val items = listOf(
