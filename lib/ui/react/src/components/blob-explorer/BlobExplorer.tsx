...
import {extractFilename, formatFileSize, useBlobExplorer, type UseBlobExplorerProps} from './use-blob-explorer'
...
export interface BlobExplorerLabels {
  allFiles?: string
  name?: string
  size?: string
  modified?: string
...
const defaultLabels: Required<BlobExplorerLabels> = {
  allFiles: 'All files',
  name: 'Name',
  size: 'Size',
  modified: 'Modified',
...
export interface BlobExplorerProps extends UseBlobExplorerProps {
  ...
  /** Override UI labels for i18n. Only specified keys are replaced; others use English defaults. */
  labels?: BlobExplorerLabels
  /**
...
export function BlobExplorer({ className, initialPrefix = null, onUploadClick, sidebar, labels: labelsProp, formatDate: formatDateProp, ...hookProps }: BlobExplorerProps) {
        ...
        {!sidebarHidden && (
          <nav className={`${styles.sidebar} ${!sidebarOpen ? styles.sidebarCollapsed : ''}`} aria-label="Folder tree">
            ...
            <div className={styles.sidebarContent} role="tree">
              <TreeNode label={L.allFiles} prefix={null} depth={0} explorer={explorer}
          ...
          {explorer.items.length === 0 && explorer.folders.length === 0 && !explorer.isLoading ? (
            <>
              <div className={styles.listHeader}>
                ...
                <SortHeader field="name" label={L.name} explorer={explorer} className={styles.headerName} />
                <div className={styles.headerActions} />
              </div>
              <div className={styles.empty}>
                <span className={styles.emptyIcon}><IconEmptyFolder /></span>
                <span className={styles.emptyText}>{L.noFiles}</span>
              </div>
            </>
          ) : (
            <>
              ...
              <div className={styles.listHeader}>
                ...
                <SortHeader field="name" label={L.name} explorer={explorer} className={styles.headerName} />
                <SortHeader field="size" label={L.size} explorer={explorer} className={styles.headerSize} />
                <SortHeader field="lastModified" label={L.modified} explorer={explorer} className={styles.headerDate} />
                <div className={`${styles.sortHeader} ${styles.headerCreated}`}>{L.created}</div>
                <SortHeader field="contentType" label={L.type} explorer={explorer} className={styles.headerType} />
                <div className={styles.headerActions} />
              </div>
              ...
              <div className={styles.fileListContent} onKeyDown={(e) => {
                ...
                {explorer.isLoading && <div className={styles.loading}>{L.loading}</div>}
                {explorer.hasMore && !explorer.isLoading && (
                  <div className={styles.loading}>
                    <button type="button" className={styles.detailBtn} onClick={explorer.loadMore}>{L.loadMore}</button>
                  </div>
                )}
              </div>
            </>
        ...
        <div className={styles.confirmOverlay}>
          <div className={styles.confirmDialog} role="alertdialog" aria-labelledby="delete-confirm-msg">
            ...
            <div className={styles.confirmActions}>
              <button type="button" className={styles.detailBtn} onClick={closeConfirm} disabled={explorer.isDeleting}>{L.cancel}</button>
              <button type="button" className={`${styles.detailBtn} ${styles.detailBtnDanger}`}
                disabled={explorer.isDeleting}
                onClick={() => { explorer.deleteBlob(confirmDelete.blob); closeConfirm() }}>{explorer.isDeleting ? L.deleting : L.delete}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {copyMoveDialog && (
        <div className={styles.confirmOverlay}>
          <div className={styles.confirmDialog}>
            <p>{copyMoveDialog.mode === 'copy' ? L.copy : L.move} &ldquo;{extractFilename(copyMoveDialog.blob.path)}&rdquo; to:</p>
            <input
              type="text"
